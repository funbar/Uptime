
kernel:     file format elf32-i386


Disassembly of section .text:

80100000 <multiboot_header>:
80100000:	02 b0 ad 1b 00 00    	add    0x1bad(%eax),%dh
80100006:	00 00                	add    %al,(%eax)
80100008:	fe 4f 52             	decb   0x52(%edi)
8010000b:	e4 0f                	in     $0xf,%al

8010000c <entry>:

# Entering xv6 on boot processor, with paging off.
.globl entry
entry:
  # Turn on page size extension for 4Mbyte pages
  movl    %cr4, %eax
8010000c:	0f 20 e0             	mov    %cr4,%eax
  orl     $(CR4_PSE), %eax
8010000f:	83 c8 10             	or     $0x10,%eax
  movl    %eax, %cr4
80100012:	0f 22 e0             	mov    %eax,%cr4
  # Set page directory
  movl    $(V2P_WO(entrypgdir)), %eax
80100015:	b8 00 a0 10 00       	mov    $0x10a000,%eax
  movl    %eax, %cr3
8010001a:	0f 22 d8             	mov    %eax,%cr3
  # Turn on paging.
  movl    %cr0, %eax
8010001d:	0f 20 c0             	mov    %cr0,%eax
  orl     $(CR0_PG|CR0_WP), %eax
80100020:	0d 00 00 01 80       	or     $0x80010000,%eax
  movl    %eax, %cr0
80100025:	0f 22 c0             	mov    %eax,%cr0

  # Set up the stack pointer.
  movl $(stack + KSTACKSIZE), %esp
80100028:	bc 50 c6 10 80       	mov    $0x8010c650,%esp

  # Jump to main(), and switch to executing at
  # high addresses. The indirect call is needed because
  # the assembler produces a PC-relative instruction
  # for a direct jump.
  mov $main, %eax
8010002d:	b8 c5 33 10 80       	mov    $0x801033c5,%eax
  jmp *%eax
80100032:	ff e0                	jmp    *%eax

80100034 <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
80100034:	55                   	push   %ebp
80100035:	89 e5                	mov    %esp,%ebp
80100037:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  initlock(&bcache.lock, "bcache");
8010003a:	c7 44 24 04 60 80 10 	movl   $0x80108060,0x4(%esp)
80100041:	80 
80100042:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
80100049:	e8 29 4a 00 00       	call   80104a77 <initlock>

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
8010004e:	c7 05 90 db 10 80 84 	movl   $0x8010db84,0x8010db90
80100055:	db 10 80 
  bcache.head.next = &bcache.head;
80100058:	c7 05 94 db 10 80 84 	movl   $0x8010db84,0x8010db94
8010005f:	db 10 80 
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
80100062:	c7 45 f4 94 c6 10 80 	movl   $0x8010c694,-0xc(%ebp)
80100069:	eb 3a                	jmp    801000a5 <binit+0x71>
    b->next = bcache.head.next;
8010006b:	8b 15 94 db 10 80    	mov    0x8010db94,%edx
80100071:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100074:	89 50 10             	mov    %edx,0x10(%eax)
    b->prev = &bcache.head;
80100077:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010007a:	c7 40 0c 84 db 10 80 	movl   $0x8010db84,0xc(%eax)
    b->dev = -1;
80100081:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100084:	c7 40 04 ff ff ff ff 	movl   $0xffffffff,0x4(%eax)
    bcache.head.next->prev = b;
8010008b:	a1 94 db 10 80       	mov    0x8010db94,%eax
80100090:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100093:	89 50 0c             	mov    %edx,0xc(%eax)
    bcache.head.next = b;
80100096:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100099:	a3 94 db 10 80       	mov    %eax,0x8010db94

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
  bcache.head.next = &bcache.head;
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
8010009e:	81 45 f4 18 02 00 00 	addl   $0x218,-0xc(%ebp)
801000a5:	81 7d f4 84 db 10 80 	cmpl   $0x8010db84,-0xc(%ebp)
801000ac:	72 bd                	jb     8010006b <binit+0x37>
    b->prev = &bcache.head;
    b->dev = -1;
    bcache.head.next->prev = b;
    bcache.head.next = b;
  }
}
801000ae:	c9                   	leave  
801000af:	c3                   	ret    

801000b0 <bget>:
// Look through buffer cache for sector on device dev.
// If not found, allocate fresh block.
// In either case, return B_BUSY buffer.
static struct buf*
bget(uint dev, uint sector)
{
801000b0:	55                   	push   %ebp
801000b1:	89 e5                	mov    %esp,%ebp
801000b3:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  acquire(&bcache.lock);
801000b6:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
801000bd:	e8 d6 49 00 00       	call   80104a98 <acquire>

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
801000c2:	a1 94 db 10 80       	mov    0x8010db94,%eax
801000c7:	89 45 f4             	mov    %eax,-0xc(%ebp)
801000ca:	eb 63                	jmp    8010012f <bget+0x7f>
    if(b->dev == dev && b->sector == sector){
801000cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000cf:	8b 40 04             	mov    0x4(%eax),%eax
801000d2:	3b 45 08             	cmp    0x8(%ebp),%eax
801000d5:	75 4f                	jne    80100126 <bget+0x76>
801000d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000da:	8b 40 08             	mov    0x8(%eax),%eax
801000dd:	3b 45 0c             	cmp    0xc(%ebp),%eax
801000e0:	75 44                	jne    80100126 <bget+0x76>
      if(!(b->flags & B_BUSY)){
801000e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000e5:	8b 00                	mov    (%eax),%eax
801000e7:	83 e0 01             	and    $0x1,%eax
801000ea:	85 c0                	test   %eax,%eax
801000ec:	75 23                	jne    80100111 <bget+0x61>
        b->flags |= B_BUSY;
801000ee:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000f1:	8b 00                	mov    (%eax),%eax
801000f3:	83 c8 01             	or     $0x1,%eax
801000f6:	89 c2                	mov    %eax,%edx
801000f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000fb:	89 10                	mov    %edx,(%eax)
        release(&bcache.lock);
801000fd:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
80100104:	e8 f1 49 00 00       	call   80104afa <release>
        return b;
80100109:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010010c:	e9 93 00 00 00       	jmp    801001a4 <bget+0xf4>
      }
      sleep(b, &bcache.lock);
80100111:	c7 44 24 04 60 c6 10 	movl   $0x8010c660,0x4(%esp)
80100118:	80 
80100119:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010011c:	89 04 24             	mov    %eax,(%esp)
8010011f:	e8 aa 46 00 00       	call   801047ce <sleep>
      goto loop;
80100124:	eb 9c                	jmp    801000c2 <bget+0x12>

  acquire(&bcache.lock);

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
80100126:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100129:	8b 40 10             	mov    0x10(%eax),%eax
8010012c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010012f:	81 7d f4 84 db 10 80 	cmpl   $0x8010db84,-0xc(%ebp)
80100136:	75 94                	jne    801000cc <bget+0x1c>
      goto loop;
    }
  }

  // Not cached; recycle some non-busy and clean buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100138:	a1 90 db 10 80       	mov    0x8010db90,%eax
8010013d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100140:	eb 4d                	jmp    8010018f <bget+0xdf>
    if((b->flags & B_BUSY) == 0 && (b->flags & B_DIRTY) == 0){
80100142:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100145:	8b 00                	mov    (%eax),%eax
80100147:	83 e0 01             	and    $0x1,%eax
8010014a:	85 c0                	test   %eax,%eax
8010014c:	75 38                	jne    80100186 <bget+0xd6>
8010014e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100151:	8b 00                	mov    (%eax),%eax
80100153:	83 e0 04             	and    $0x4,%eax
80100156:	85 c0                	test   %eax,%eax
80100158:	75 2c                	jne    80100186 <bget+0xd6>
      b->dev = dev;
8010015a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010015d:	8b 55 08             	mov    0x8(%ebp),%edx
80100160:	89 50 04             	mov    %edx,0x4(%eax)
      b->sector = sector;
80100163:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100166:	8b 55 0c             	mov    0xc(%ebp),%edx
80100169:	89 50 08             	mov    %edx,0x8(%eax)
      b->flags = B_BUSY;
8010016c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010016f:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
      release(&bcache.lock);
80100175:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
8010017c:	e8 79 49 00 00       	call   80104afa <release>
      return b;
80100181:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100184:	eb 1e                	jmp    801001a4 <bget+0xf4>
      goto loop;
    }
  }

  // Not cached; recycle some non-busy and clean buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100186:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100189:	8b 40 0c             	mov    0xc(%eax),%eax
8010018c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010018f:	81 7d f4 84 db 10 80 	cmpl   $0x8010db84,-0xc(%ebp)
80100196:	75 aa                	jne    80100142 <bget+0x92>
      b->flags = B_BUSY;
      release(&bcache.lock);
      return b;
    }
  }
  panic("bget: no buffers");
80100198:	c7 04 24 67 80 10 80 	movl   $0x80108067,(%esp)
8010019f:	e8 96 03 00 00       	call   8010053a <panic>
}
801001a4:	c9                   	leave  
801001a5:	c3                   	ret    

801001a6 <bread>:

// Return a B_BUSY buf with the contents of the indicated disk sector.
struct buf*
bread(uint dev, uint sector)
{
801001a6:	55                   	push   %ebp
801001a7:	89 e5                	mov    %esp,%ebp
801001a9:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  b = bget(dev, sector);
801001ac:	8b 45 0c             	mov    0xc(%ebp),%eax
801001af:	89 44 24 04          	mov    %eax,0x4(%esp)
801001b3:	8b 45 08             	mov    0x8(%ebp),%eax
801001b6:	89 04 24             	mov    %eax,(%esp)
801001b9:	e8 f2 fe ff ff       	call   801000b0 <bget>
801001be:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(!(b->flags & B_VALID))
801001c1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001c4:	8b 00                	mov    (%eax),%eax
801001c6:	83 e0 02             	and    $0x2,%eax
801001c9:	85 c0                	test   %eax,%eax
801001cb:	75 0b                	jne    801001d8 <bread+0x32>
    iderw(b);
801001cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001d0:	89 04 24             	mov    %eax,(%esp)
801001d3:	e8 c9 25 00 00       	call   801027a1 <iderw>
  return b;
801001d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801001db:	c9                   	leave  
801001dc:	c3                   	ret    

801001dd <bwrite>:

// Write b's contents to disk.  Must be B_BUSY.
void
bwrite(struct buf *b)
{
801001dd:	55                   	push   %ebp
801001de:	89 e5                	mov    %esp,%ebp
801001e0:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
801001e3:	8b 45 08             	mov    0x8(%ebp),%eax
801001e6:	8b 00                	mov    (%eax),%eax
801001e8:	83 e0 01             	and    $0x1,%eax
801001eb:	85 c0                	test   %eax,%eax
801001ed:	75 0c                	jne    801001fb <bwrite+0x1e>
    panic("bwrite");
801001ef:	c7 04 24 78 80 10 80 	movl   $0x80108078,(%esp)
801001f6:	e8 3f 03 00 00       	call   8010053a <panic>
  b->flags |= B_DIRTY;
801001fb:	8b 45 08             	mov    0x8(%ebp),%eax
801001fe:	8b 00                	mov    (%eax),%eax
80100200:	83 c8 04             	or     $0x4,%eax
80100203:	89 c2                	mov    %eax,%edx
80100205:	8b 45 08             	mov    0x8(%ebp),%eax
80100208:	89 10                	mov    %edx,(%eax)
  iderw(b);
8010020a:	8b 45 08             	mov    0x8(%ebp),%eax
8010020d:	89 04 24             	mov    %eax,(%esp)
80100210:	e8 8c 25 00 00       	call   801027a1 <iderw>
}
80100215:	c9                   	leave  
80100216:	c3                   	ret    

80100217 <brelse>:

// Release a B_BUSY buffer.
// Move to the head of the MRU list.
void
brelse(struct buf *b)
{
80100217:	55                   	push   %ebp
80100218:	89 e5                	mov    %esp,%ebp
8010021a:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
8010021d:	8b 45 08             	mov    0x8(%ebp),%eax
80100220:	8b 00                	mov    (%eax),%eax
80100222:	83 e0 01             	and    $0x1,%eax
80100225:	85 c0                	test   %eax,%eax
80100227:	75 0c                	jne    80100235 <brelse+0x1e>
    panic("brelse");
80100229:	c7 04 24 7f 80 10 80 	movl   $0x8010807f,(%esp)
80100230:	e8 05 03 00 00       	call   8010053a <panic>

  acquire(&bcache.lock);
80100235:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
8010023c:	e8 57 48 00 00       	call   80104a98 <acquire>

  b->next->prev = b->prev;
80100241:	8b 45 08             	mov    0x8(%ebp),%eax
80100244:	8b 40 10             	mov    0x10(%eax),%eax
80100247:	8b 55 08             	mov    0x8(%ebp),%edx
8010024a:	8b 52 0c             	mov    0xc(%edx),%edx
8010024d:	89 50 0c             	mov    %edx,0xc(%eax)
  b->prev->next = b->next;
80100250:	8b 45 08             	mov    0x8(%ebp),%eax
80100253:	8b 40 0c             	mov    0xc(%eax),%eax
80100256:	8b 55 08             	mov    0x8(%ebp),%edx
80100259:	8b 52 10             	mov    0x10(%edx),%edx
8010025c:	89 50 10             	mov    %edx,0x10(%eax)
  b->next = bcache.head.next;
8010025f:	8b 15 94 db 10 80    	mov    0x8010db94,%edx
80100265:	8b 45 08             	mov    0x8(%ebp),%eax
80100268:	89 50 10             	mov    %edx,0x10(%eax)
  b->prev = &bcache.head;
8010026b:	8b 45 08             	mov    0x8(%ebp),%eax
8010026e:	c7 40 0c 84 db 10 80 	movl   $0x8010db84,0xc(%eax)
  bcache.head.next->prev = b;
80100275:	a1 94 db 10 80       	mov    0x8010db94,%eax
8010027a:	8b 55 08             	mov    0x8(%ebp),%edx
8010027d:	89 50 0c             	mov    %edx,0xc(%eax)
  bcache.head.next = b;
80100280:	8b 45 08             	mov    0x8(%ebp),%eax
80100283:	a3 94 db 10 80       	mov    %eax,0x8010db94

  b->flags &= ~B_BUSY;
80100288:	8b 45 08             	mov    0x8(%ebp),%eax
8010028b:	8b 00                	mov    (%eax),%eax
8010028d:	83 e0 fe             	and    $0xfffffffe,%eax
80100290:	89 c2                	mov    %eax,%edx
80100292:	8b 45 08             	mov    0x8(%ebp),%eax
80100295:	89 10                	mov    %edx,(%eax)
  wakeup(b);
80100297:	8b 45 08             	mov    0x8(%ebp),%eax
8010029a:	89 04 24             	mov    %eax,(%esp)
8010029d:	e8 05 46 00 00       	call   801048a7 <wakeup>

  release(&bcache.lock);
801002a2:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
801002a9:	e8 4c 48 00 00       	call   80104afa <release>
}
801002ae:	c9                   	leave  
801002af:	c3                   	ret    

801002b0 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
801002b0:	55                   	push   %ebp
801002b1:	89 e5                	mov    %esp,%ebp
801002b3:	83 ec 14             	sub    $0x14,%esp
801002b6:	8b 45 08             	mov    0x8(%ebp),%eax
801002b9:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801002bd:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
801002c1:	89 c2                	mov    %eax,%edx
801002c3:	ec                   	in     (%dx),%al
801002c4:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
801002c7:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
801002cb:	c9                   	leave  
801002cc:	c3                   	ret    

801002cd <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801002cd:	55                   	push   %ebp
801002ce:	89 e5                	mov    %esp,%ebp
801002d0:	83 ec 08             	sub    $0x8,%esp
801002d3:	8b 55 08             	mov    0x8(%ebp),%edx
801002d6:	8b 45 0c             	mov    0xc(%ebp),%eax
801002d9:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801002dd:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801002e0:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801002e4:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801002e8:	ee                   	out    %al,(%dx)
}
801002e9:	c9                   	leave  
801002ea:	c3                   	ret    

801002eb <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
801002eb:	55                   	push   %ebp
801002ec:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
801002ee:	fa                   	cli    
}
801002ef:	5d                   	pop    %ebp
801002f0:	c3                   	ret    

801002f1 <printint>:
  int locking;
} cons;

static void
printint(int xx, int base, int sign)
{
801002f1:	55                   	push   %ebp
801002f2:	89 e5                	mov    %esp,%ebp
801002f4:	56                   	push   %esi
801002f5:	53                   	push   %ebx
801002f6:	83 ec 30             	sub    $0x30,%esp
  static char digits[] = "0123456789abcdef";
  char buf[16];
  int i;
  uint x;

  if(sign && (sign = xx < 0))
801002f9:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801002fd:	74 1c                	je     8010031b <printint+0x2a>
801002ff:	8b 45 08             	mov    0x8(%ebp),%eax
80100302:	c1 e8 1f             	shr    $0x1f,%eax
80100305:	0f b6 c0             	movzbl %al,%eax
80100308:	89 45 10             	mov    %eax,0x10(%ebp)
8010030b:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010030f:	74 0a                	je     8010031b <printint+0x2a>
    x = -xx;
80100311:	8b 45 08             	mov    0x8(%ebp),%eax
80100314:	f7 d8                	neg    %eax
80100316:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100319:	eb 06                	jmp    80100321 <printint+0x30>
  else
    x = xx;
8010031b:	8b 45 08             	mov    0x8(%ebp),%eax
8010031e:	89 45 f0             	mov    %eax,-0x10(%ebp)

  i = 0;
80100321:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  do{
    buf[i++] = digits[x % base];
80100328:	8b 4d f4             	mov    -0xc(%ebp),%ecx
8010032b:	8d 41 01             	lea    0x1(%ecx),%eax
8010032e:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100331:	8b 5d 0c             	mov    0xc(%ebp),%ebx
80100334:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100337:	ba 00 00 00 00       	mov    $0x0,%edx
8010033c:	f7 f3                	div    %ebx
8010033e:	89 d0                	mov    %edx,%eax
80100340:	0f b6 80 04 90 10 80 	movzbl -0x7fef6ffc(%eax),%eax
80100347:	88 44 0d e0          	mov    %al,-0x20(%ebp,%ecx,1)
  }while((x /= base) != 0);
8010034b:	8b 75 0c             	mov    0xc(%ebp),%esi
8010034e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100351:	ba 00 00 00 00       	mov    $0x0,%edx
80100356:	f7 f6                	div    %esi
80100358:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010035b:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010035f:	75 c7                	jne    80100328 <printint+0x37>

  if(sign)
80100361:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100365:	74 10                	je     80100377 <printint+0x86>
    buf[i++] = '-';
80100367:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010036a:	8d 50 01             	lea    0x1(%eax),%edx
8010036d:	89 55 f4             	mov    %edx,-0xc(%ebp)
80100370:	c6 44 05 e0 2d       	movb   $0x2d,-0x20(%ebp,%eax,1)

  while(--i >= 0)
80100375:	eb 18                	jmp    8010038f <printint+0x9e>
80100377:	eb 16                	jmp    8010038f <printint+0x9e>
    consputc(buf[i]);
80100379:	8d 55 e0             	lea    -0x20(%ebp),%edx
8010037c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010037f:	01 d0                	add    %edx,%eax
80100381:	0f b6 00             	movzbl (%eax),%eax
80100384:	0f be c0             	movsbl %al,%eax
80100387:	89 04 24             	mov    %eax,(%esp)
8010038a:	e8 c1 03 00 00       	call   80100750 <consputc>
  }while((x /= base) != 0);

  if(sign)
    buf[i++] = '-';

  while(--i >= 0)
8010038f:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
80100393:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100397:	79 e0                	jns    80100379 <printint+0x88>
    consputc(buf[i]);
}
80100399:	83 c4 30             	add    $0x30,%esp
8010039c:	5b                   	pop    %ebx
8010039d:	5e                   	pop    %esi
8010039e:	5d                   	pop    %ebp
8010039f:	c3                   	ret    

801003a0 <cprintf>:
//PAGEBREAK: 50

// Print to the console. only understands %d, %x, %p, %s.
void
cprintf(char *fmt, ...)
{
801003a0:	55                   	push   %ebp
801003a1:	89 e5                	mov    %esp,%ebp
801003a3:	83 ec 38             	sub    $0x38,%esp
  int i, c, locking;
  uint *argp;
  char *s;

  locking = cons.locking;
801003a6:	a1 f4 b5 10 80       	mov    0x8010b5f4,%eax
801003ab:	89 45 e8             	mov    %eax,-0x18(%ebp)
  if(locking)
801003ae:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801003b2:	74 0c                	je     801003c0 <cprintf+0x20>
    acquire(&cons.lock);
801003b4:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
801003bb:	e8 d8 46 00 00       	call   80104a98 <acquire>

  if (fmt == 0)
801003c0:	8b 45 08             	mov    0x8(%ebp),%eax
801003c3:	85 c0                	test   %eax,%eax
801003c5:	75 0c                	jne    801003d3 <cprintf+0x33>
    panic("null fmt");
801003c7:	c7 04 24 86 80 10 80 	movl   $0x80108086,(%esp)
801003ce:	e8 67 01 00 00       	call   8010053a <panic>

  argp = (uint*)(void*)(&fmt + 1);
801003d3:	8d 45 0c             	lea    0xc(%ebp),%eax
801003d6:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801003d9:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801003e0:	e9 21 01 00 00       	jmp    80100506 <cprintf+0x166>
    if(c != '%'){
801003e5:	83 7d e4 25          	cmpl   $0x25,-0x1c(%ebp)
801003e9:	74 10                	je     801003fb <cprintf+0x5b>
      consputc(c);
801003eb:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801003ee:	89 04 24             	mov    %eax,(%esp)
801003f1:	e8 5a 03 00 00       	call   80100750 <consputc>
      continue;
801003f6:	e9 07 01 00 00       	jmp    80100502 <cprintf+0x162>
    }
    c = fmt[++i] & 0xff;
801003fb:	8b 55 08             	mov    0x8(%ebp),%edx
801003fe:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100402:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100405:	01 d0                	add    %edx,%eax
80100407:	0f b6 00             	movzbl (%eax),%eax
8010040a:	0f be c0             	movsbl %al,%eax
8010040d:	25 ff 00 00 00       	and    $0xff,%eax
80100412:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(c == 0)
80100415:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80100419:	75 05                	jne    80100420 <cprintf+0x80>
      break;
8010041b:	e9 06 01 00 00       	jmp    80100526 <cprintf+0x186>
    switch(c){
80100420:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100423:	83 f8 70             	cmp    $0x70,%eax
80100426:	74 4f                	je     80100477 <cprintf+0xd7>
80100428:	83 f8 70             	cmp    $0x70,%eax
8010042b:	7f 13                	jg     80100440 <cprintf+0xa0>
8010042d:	83 f8 25             	cmp    $0x25,%eax
80100430:	0f 84 a6 00 00 00    	je     801004dc <cprintf+0x13c>
80100436:	83 f8 64             	cmp    $0x64,%eax
80100439:	74 14                	je     8010044f <cprintf+0xaf>
8010043b:	e9 aa 00 00 00       	jmp    801004ea <cprintf+0x14a>
80100440:	83 f8 73             	cmp    $0x73,%eax
80100443:	74 57                	je     8010049c <cprintf+0xfc>
80100445:	83 f8 78             	cmp    $0x78,%eax
80100448:	74 2d                	je     80100477 <cprintf+0xd7>
8010044a:	e9 9b 00 00 00       	jmp    801004ea <cprintf+0x14a>
    case 'd':
      printint(*argp++, 10, 1);
8010044f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100452:	8d 50 04             	lea    0x4(%eax),%edx
80100455:	89 55 f0             	mov    %edx,-0x10(%ebp)
80100458:	8b 00                	mov    (%eax),%eax
8010045a:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
80100461:	00 
80100462:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80100469:	00 
8010046a:	89 04 24             	mov    %eax,(%esp)
8010046d:	e8 7f fe ff ff       	call   801002f1 <printint>
      break;
80100472:	e9 8b 00 00 00       	jmp    80100502 <cprintf+0x162>
    case 'x':
    case 'p':
      printint(*argp++, 16, 0);
80100477:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010047a:	8d 50 04             	lea    0x4(%eax),%edx
8010047d:	89 55 f0             	mov    %edx,-0x10(%ebp)
80100480:	8b 00                	mov    (%eax),%eax
80100482:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100489:	00 
8010048a:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
80100491:	00 
80100492:	89 04 24             	mov    %eax,(%esp)
80100495:	e8 57 fe ff ff       	call   801002f1 <printint>
      break;
8010049a:	eb 66                	jmp    80100502 <cprintf+0x162>
    case 's':
      if((s = (char*)*argp++) == 0)
8010049c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010049f:	8d 50 04             	lea    0x4(%eax),%edx
801004a2:	89 55 f0             	mov    %edx,-0x10(%ebp)
801004a5:	8b 00                	mov    (%eax),%eax
801004a7:	89 45 ec             	mov    %eax,-0x14(%ebp)
801004aa:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801004ae:	75 09                	jne    801004b9 <cprintf+0x119>
        s = "(null)";
801004b0:	c7 45 ec 8f 80 10 80 	movl   $0x8010808f,-0x14(%ebp)
      for(; *s; s++)
801004b7:	eb 17                	jmp    801004d0 <cprintf+0x130>
801004b9:	eb 15                	jmp    801004d0 <cprintf+0x130>
        consputc(*s);
801004bb:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004be:	0f b6 00             	movzbl (%eax),%eax
801004c1:	0f be c0             	movsbl %al,%eax
801004c4:	89 04 24             	mov    %eax,(%esp)
801004c7:	e8 84 02 00 00       	call   80100750 <consputc>
      printint(*argp++, 16, 0);
      break;
    case 's':
      if((s = (char*)*argp++) == 0)
        s = "(null)";
      for(; *s; s++)
801004cc:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
801004d0:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004d3:	0f b6 00             	movzbl (%eax),%eax
801004d6:	84 c0                	test   %al,%al
801004d8:	75 e1                	jne    801004bb <cprintf+0x11b>
        consputc(*s);
      break;
801004da:	eb 26                	jmp    80100502 <cprintf+0x162>
    case '%':
      consputc('%');
801004dc:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004e3:	e8 68 02 00 00       	call   80100750 <consputc>
      break;
801004e8:	eb 18                	jmp    80100502 <cprintf+0x162>
    default:
      // Print unknown % sequence to draw attention.
      consputc('%');
801004ea:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004f1:	e8 5a 02 00 00       	call   80100750 <consputc>
      consputc(c);
801004f6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801004f9:	89 04 24             	mov    %eax,(%esp)
801004fc:	e8 4f 02 00 00       	call   80100750 <consputc>
      break;
80100501:	90                   	nop

  if (fmt == 0)
    panic("null fmt");

  argp = (uint*)(void*)(&fmt + 1);
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
80100502:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100506:	8b 55 08             	mov    0x8(%ebp),%edx
80100509:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010050c:	01 d0                	add    %edx,%eax
8010050e:	0f b6 00             	movzbl (%eax),%eax
80100511:	0f be c0             	movsbl %al,%eax
80100514:	25 ff 00 00 00       	and    $0xff,%eax
80100519:	89 45 e4             	mov    %eax,-0x1c(%ebp)
8010051c:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80100520:	0f 85 bf fe ff ff    	jne    801003e5 <cprintf+0x45>
      consputc(c);
      break;
    }
  }

  if(locking)
80100526:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
8010052a:	74 0c                	je     80100538 <cprintf+0x198>
    release(&cons.lock);
8010052c:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
80100533:	e8 c2 45 00 00       	call   80104afa <release>
}
80100538:	c9                   	leave  
80100539:	c3                   	ret    

8010053a <panic>:

void
panic(char *s)
{
8010053a:	55                   	push   %ebp
8010053b:	89 e5                	mov    %esp,%ebp
8010053d:	83 ec 48             	sub    $0x48,%esp
  int i;
  uint pcs[10];
  
  cli();
80100540:	e8 a6 fd ff ff       	call   801002eb <cli>
  cons.locking = 0;
80100545:	c7 05 f4 b5 10 80 00 	movl   $0x0,0x8010b5f4
8010054c:	00 00 00 
  cprintf("cpu%d: panic: ", cpu->id);
8010054f:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80100555:	0f b6 00             	movzbl (%eax),%eax
80100558:	0f b6 c0             	movzbl %al,%eax
8010055b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010055f:	c7 04 24 96 80 10 80 	movl   $0x80108096,(%esp)
80100566:	e8 35 fe ff ff       	call   801003a0 <cprintf>
  cprintf(s);
8010056b:	8b 45 08             	mov    0x8(%ebp),%eax
8010056e:	89 04 24             	mov    %eax,(%esp)
80100571:	e8 2a fe ff ff       	call   801003a0 <cprintf>
  cprintf("\n");
80100576:	c7 04 24 a5 80 10 80 	movl   $0x801080a5,(%esp)
8010057d:	e8 1e fe ff ff       	call   801003a0 <cprintf>
  getcallerpcs(&s, pcs);
80100582:	8d 45 cc             	lea    -0x34(%ebp),%eax
80100585:	89 44 24 04          	mov    %eax,0x4(%esp)
80100589:	8d 45 08             	lea    0x8(%ebp),%eax
8010058c:	89 04 24             	mov    %eax,(%esp)
8010058f:	e8 b5 45 00 00       	call   80104b49 <getcallerpcs>
  for(i=0; i<10; i++)
80100594:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010059b:	eb 1b                	jmp    801005b8 <panic+0x7e>
    cprintf(" %p", pcs[i]);
8010059d:	8b 45 f4             	mov    -0xc(%ebp),%eax
801005a0:	8b 44 85 cc          	mov    -0x34(%ebp,%eax,4),%eax
801005a4:	89 44 24 04          	mov    %eax,0x4(%esp)
801005a8:	c7 04 24 a7 80 10 80 	movl   $0x801080a7,(%esp)
801005af:	e8 ec fd ff ff       	call   801003a0 <cprintf>
  cons.locking = 0;
  cprintf("cpu%d: panic: ", cpu->id);
  cprintf(s);
  cprintf("\n");
  getcallerpcs(&s, pcs);
  for(i=0; i<10; i++)
801005b4:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801005b8:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
801005bc:	7e df                	jle    8010059d <panic+0x63>
    cprintf(" %p", pcs[i]);
  panicked = 1; // freeze other CPU
801005be:	c7 05 a0 b5 10 80 01 	movl   $0x1,0x8010b5a0
801005c5:	00 00 00 
  for(;;)
    ;
801005c8:	eb fe                	jmp    801005c8 <panic+0x8e>

801005ca <cgaputc>:
#define CRTPORT 0x3d4
static ushort *crt = (ushort*)P2V(0xb8000);  // CGA memory

static void
cgaputc(int c)
{
801005ca:	55                   	push   %ebp
801005cb:	89 e5                	mov    %esp,%ebp
801005cd:	83 ec 28             	sub    $0x28,%esp
  int pos;
  
  // Cursor position: col + 80*row.
  outb(CRTPORT, 14);
801005d0:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801005d7:	00 
801005d8:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801005df:	e8 e9 fc ff ff       	call   801002cd <outb>
  pos = inb(CRTPORT+1) << 8;
801005e4:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
801005eb:	e8 c0 fc ff ff       	call   801002b0 <inb>
801005f0:	0f b6 c0             	movzbl %al,%eax
801005f3:	c1 e0 08             	shl    $0x8,%eax
801005f6:	89 45 f4             	mov    %eax,-0xc(%ebp)
  outb(CRTPORT, 15);
801005f9:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80100600:	00 
80100601:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
80100608:	e8 c0 fc ff ff       	call   801002cd <outb>
  pos |= inb(CRTPORT+1);
8010060d:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100614:	e8 97 fc ff ff       	call   801002b0 <inb>
80100619:	0f b6 c0             	movzbl %al,%eax
8010061c:	09 45 f4             	or     %eax,-0xc(%ebp)

  if(c == '\n')
8010061f:	83 7d 08 0a          	cmpl   $0xa,0x8(%ebp)
80100623:	75 30                	jne    80100655 <cgaputc+0x8b>
    pos += 80 - pos%80;
80100625:	8b 4d f4             	mov    -0xc(%ebp),%ecx
80100628:	ba 67 66 66 66       	mov    $0x66666667,%edx
8010062d:	89 c8                	mov    %ecx,%eax
8010062f:	f7 ea                	imul   %edx
80100631:	c1 fa 05             	sar    $0x5,%edx
80100634:	89 c8                	mov    %ecx,%eax
80100636:	c1 f8 1f             	sar    $0x1f,%eax
80100639:	29 c2                	sub    %eax,%edx
8010063b:	89 d0                	mov    %edx,%eax
8010063d:	c1 e0 02             	shl    $0x2,%eax
80100640:	01 d0                	add    %edx,%eax
80100642:	c1 e0 04             	shl    $0x4,%eax
80100645:	29 c1                	sub    %eax,%ecx
80100647:	89 ca                	mov    %ecx,%edx
80100649:	b8 50 00 00 00       	mov    $0x50,%eax
8010064e:	29 d0                	sub    %edx,%eax
80100650:	01 45 f4             	add    %eax,-0xc(%ebp)
80100653:	eb 35                	jmp    8010068a <cgaputc+0xc0>
  else if(c == BACKSPACE){
80100655:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010065c:	75 0c                	jne    8010066a <cgaputc+0xa0>
    if(pos > 0) --pos;
8010065e:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100662:	7e 26                	jle    8010068a <cgaputc+0xc0>
80100664:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
80100668:	eb 20                	jmp    8010068a <cgaputc+0xc0>
  } else
    crt[pos++] = (c&0xff) | 0x0700;  // black on white
8010066a:	8b 0d 00 90 10 80    	mov    0x80109000,%ecx
80100670:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100673:	8d 50 01             	lea    0x1(%eax),%edx
80100676:	89 55 f4             	mov    %edx,-0xc(%ebp)
80100679:	01 c0                	add    %eax,%eax
8010067b:	8d 14 01             	lea    (%ecx,%eax,1),%edx
8010067e:	8b 45 08             	mov    0x8(%ebp),%eax
80100681:	0f b6 c0             	movzbl %al,%eax
80100684:	80 cc 07             	or     $0x7,%ah
80100687:	66 89 02             	mov    %ax,(%edx)
  
  if((pos/80) >= 24){  // Scroll up.
8010068a:	81 7d f4 7f 07 00 00 	cmpl   $0x77f,-0xc(%ebp)
80100691:	7e 53                	jle    801006e6 <cgaputc+0x11c>
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
80100693:	a1 00 90 10 80       	mov    0x80109000,%eax
80100698:	8d 90 a0 00 00 00    	lea    0xa0(%eax),%edx
8010069e:	a1 00 90 10 80       	mov    0x80109000,%eax
801006a3:	c7 44 24 08 60 0e 00 	movl   $0xe60,0x8(%esp)
801006aa:	00 
801006ab:	89 54 24 04          	mov    %edx,0x4(%esp)
801006af:	89 04 24             	mov    %eax,(%esp)
801006b2:	e8 04 47 00 00       	call   80104dbb <memmove>
    pos -= 80;
801006b7:	83 6d f4 50          	subl   $0x50,-0xc(%ebp)
    memset(crt+pos, 0, sizeof(crt[0])*(24*80 - pos));
801006bb:	b8 80 07 00 00       	mov    $0x780,%eax
801006c0:	2b 45 f4             	sub    -0xc(%ebp),%eax
801006c3:	8d 14 00             	lea    (%eax,%eax,1),%edx
801006c6:	a1 00 90 10 80       	mov    0x80109000,%eax
801006cb:	8b 4d f4             	mov    -0xc(%ebp),%ecx
801006ce:	01 c9                	add    %ecx,%ecx
801006d0:	01 c8                	add    %ecx,%eax
801006d2:	89 54 24 08          	mov    %edx,0x8(%esp)
801006d6:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801006dd:	00 
801006de:	89 04 24             	mov    %eax,(%esp)
801006e1:	e8 06 46 00 00       	call   80104cec <memset>
  }
  
  outb(CRTPORT, 14);
801006e6:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801006ed:	00 
801006ee:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801006f5:	e8 d3 fb ff ff       	call   801002cd <outb>
  outb(CRTPORT+1, pos>>8);
801006fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801006fd:	c1 f8 08             	sar    $0x8,%eax
80100700:	0f b6 c0             	movzbl %al,%eax
80100703:	89 44 24 04          	mov    %eax,0x4(%esp)
80100707:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
8010070e:	e8 ba fb ff ff       	call   801002cd <outb>
  outb(CRTPORT, 15);
80100713:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
8010071a:	00 
8010071b:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
80100722:	e8 a6 fb ff ff       	call   801002cd <outb>
  outb(CRTPORT+1, pos);
80100727:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010072a:	0f b6 c0             	movzbl %al,%eax
8010072d:	89 44 24 04          	mov    %eax,0x4(%esp)
80100731:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100738:	e8 90 fb ff ff       	call   801002cd <outb>
  crt[pos] = ' ' | 0x0700;
8010073d:	a1 00 90 10 80       	mov    0x80109000,%eax
80100742:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100745:	01 d2                	add    %edx,%edx
80100747:	01 d0                	add    %edx,%eax
80100749:	66 c7 00 20 07       	movw   $0x720,(%eax)
}
8010074e:	c9                   	leave  
8010074f:	c3                   	ret    

80100750 <consputc>:

void
consputc(int c)
{
80100750:	55                   	push   %ebp
80100751:	89 e5                	mov    %esp,%ebp
80100753:	83 ec 18             	sub    $0x18,%esp
  if(panicked){
80100756:	a1 a0 b5 10 80       	mov    0x8010b5a0,%eax
8010075b:	85 c0                	test   %eax,%eax
8010075d:	74 07                	je     80100766 <consputc+0x16>
    cli();
8010075f:	e8 87 fb ff ff       	call   801002eb <cli>
    for(;;)
      ;
80100764:	eb fe                	jmp    80100764 <consputc+0x14>
  }

  if(c == BACKSPACE){
80100766:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010076d:	75 26                	jne    80100795 <consputc+0x45>
    uartputc('\b'); uartputc(' '); uartputc('\b');
8010076f:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
80100776:	e8 28 5f 00 00       	call   801066a3 <uartputc>
8010077b:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80100782:	e8 1c 5f 00 00       	call   801066a3 <uartputc>
80100787:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
8010078e:	e8 10 5f 00 00       	call   801066a3 <uartputc>
80100793:	eb 0b                	jmp    801007a0 <consputc+0x50>
  } else
    uartputc(c);
80100795:	8b 45 08             	mov    0x8(%ebp),%eax
80100798:	89 04 24             	mov    %eax,(%esp)
8010079b:	e8 03 5f 00 00       	call   801066a3 <uartputc>
  cgaputc(c);
801007a0:	8b 45 08             	mov    0x8(%ebp),%eax
801007a3:	89 04 24             	mov    %eax,(%esp)
801007a6:	e8 1f fe ff ff       	call   801005ca <cgaputc>
}
801007ab:	c9                   	leave  
801007ac:	c3                   	ret    

801007ad <consoleintr>:

#define C(x)  ((x)-'@')  // Control-x

void
consoleintr(int (*getc)(void))
{
801007ad:	55                   	push   %ebp
801007ae:	89 e5                	mov    %esp,%ebp
801007b0:	83 ec 28             	sub    $0x28,%esp
  int c;

  acquire(&input.lock);
801007b3:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
801007ba:	e8 d9 42 00 00       	call   80104a98 <acquire>
  while((c = getc()) >= 0){
801007bf:	e9 37 01 00 00       	jmp    801008fb <consoleintr+0x14e>
    switch(c){
801007c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801007c7:	83 f8 10             	cmp    $0x10,%eax
801007ca:	74 1e                	je     801007ea <consoleintr+0x3d>
801007cc:	83 f8 10             	cmp    $0x10,%eax
801007cf:	7f 0a                	jg     801007db <consoleintr+0x2e>
801007d1:	83 f8 08             	cmp    $0x8,%eax
801007d4:	74 64                	je     8010083a <consoleintr+0x8d>
801007d6:	e9 91 00 00 00       	jmp    8010086c <consoleintr+0xbf>
801007db:	83 f8 15             	cmp    $0x15,%eax
801007de:	74 2f                	je     8010080f <consoleintr+0x62>
801007e0:	83 f8 7f             	cmp    $0x7f,%eax
801007e3:	74 55                	je     8010083a <consoleintr+0x8d>
801007e5:	e9 82 00 00 00       	jmp    8010086c <consoleintr+0xbf>
    case C('P'):  // Process listing.
      procdump();
801007ea:	e8 5b 41 00 00       	call   8010494a <procdump>
      break;
801007ef:	e9 07 01 00 00       	jmp    801008fb <consoleintr+0x14e>
    case C('U'):  // Kill line.
      while(input.e != input.w &&
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
801007f4:	a1 5c de 10 80       	mov    0x8010de5c,%eax
801007f9:	83 e8 01             	sub    $0x1,%eax
801007fc:	a3 5c de 10 80       	mov    %eax,0x8010de5c
        consputc(BACKSPACE);
80100801:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
80100808:	e8 43 ff ff ff       	call   80100750 <consputc>
8010080d:	eb 01                	jmp    80100810 <consoleintr+0x63>
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
8010080f:	90                   	nop
80100810:	8b 15 5c de 10 80    	mov    0x8010de5c,%edx
80100816:	a1 58 de 10 80       	mov    0x8010de58,%eax
8010081b:	39 c2                	cmp    %eax,%edx
8010081d:	74 16                	je     80100835 <consoleintr+0x88>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
8010081f:	a1 5c de 10 80       	mov    0x8010de5c,%eax
80100824:	83 e8 01             	sub    $0x1,%eax
80100827:	83 e0 7f             	and    $0x7f,%eax
8010082a:	0f b6 80 d4 dd 10 80 	movzbl -0x7fef222c(%eax),%eax
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
80100831:	3c 0a                	cmp    $0xa,%al
80100833:	75 bf                	jne    801007f4 <consoleintr+0x47>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
80100835:	e9 c1 00 00 00       	jmp    801008fb <consoleintr+0x14e>
    case C('H'): case '\x7f':  // Backspace
      if(input.e != input.w){
8010083a:	8b 15 5c de 10 80    	mov    0x8010de5c,%edx
80100840:	a1 58 de 10 80       	mov    0x8010de58,%eax
80100845:	39 c2                	cmp    %eax,%edx
80100847:	74 1e                	je     80100867 <consoleintr+0xba>
        input.e--;
80100849:	a1 5c de 10 80       	mov    0x8010de5c,%eax
8010084e:	83 e8 01             	sub    $0x1,%eax
80100851:	a3 5c de 10 80       	mov    %eax,0x8010de5c
        consputc(BACKSPACE);
80100856:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
8010085d:	e8 ee fe ff ff       	call   80100750 <consputc>
      }
      break;
80100862:	e9 94 00 00 00       	jmp    801008fb <consoleintr+0x14e>
80100867:	e9 8f 00 00 00       	jmp    801008fb <consoleintr+0x14e>
    default:
      if(c != 0 && input.e-input.r < INPUT_BUF){
8010086c:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100870:	0f 84 84 00 00 00    	je     801008fa <consoleintr+0x14d>
80100876:	8b 15 5c de 10 80    	mov    0x8010de5c,%edx
8010087c:	a1 54 de 10 80       	mov    0x8010de54,%eax
80100881:	29 c2                	sub    %eax,%edx
80100883:	89 d0                	mov    %edx,%eax
80100885:	83 f8 7f             	cmp    $0x7f,%eax
80100888:	77 70                	ja     801008fa <consoleintr+0x14d>
        c = (c == '\r') ? '\n' : c;
8010088a:	83 7d f4 0d          	cmpl   $0xd,-0xc(%ebp)
8010088e:	74 05                	je     80100895 <consoleintr+0xe8>
80100890:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100893:	eb 05                	jmp    8010089a <consoleintr+0xed>
80100895:	b8 0a 00 00 00       	mov    $0xa,%eax
8010089a:	89 45 f4             	mov    %eax,-0xc(%ebp)
        input.buf[input.e++ % INPUT_BUF] = c;
8010089d:	a1 5c de 10 80       	mov    0x8010de5c,%eax
801008a2:	8d 50 01             	lea    0x1(%eax),%edx
801008a5:	89 15 5c de 10 80    	mov    %edx,0x8010de5c
801008ab:	83 e0 7f             	and    $0x7f,%eax
801008ae:	89 c2                	mov    %eax,%edx
801008b0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801008b3:	88 82 d4 dd 10 80    	mov    %al,-0x7fef222c(%edx)
        consputc(c);
801008b9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801008bc:	89 04 24             	mov    %eax,(%esp)
801008bf:	e8 8c fe ff ff       	call   80100750 <consputc>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
801008c4:	83 7d f4 0a          	cmpl   $0xa,-0xc(%ebp)
801008c8:	74 18                	je     801008e2 <consoleintr+0x135>
801008ca:	83 7d f4 04          	cmpl   $0x4,-0xc(%ebp)
801008ce:	74 12                	je     801008e2 <consoleintr+0x135>
801008d0:	a1 5c de 10 80       	mov    0x8010de5c,%eax
801008d5:	8b 15 54 de 10 80    	mov    0x8010de54,%edx
801008db:	83 ea 80             	sub    $0xffffff80,%edx
801008de:	39 d0                	cmp    %edx,%eax
801008e0:	75 18                	jne    801008fa <consoleintr+0x14d>
          input.w = input.e;
801008e2:	a1 5c de 10 80       	mov    0x8010de5c,%eax
801008e7:	a3 58 de 10 80       	mov    %eax,0x8010de58
          wakeup(&input.r);
801008ec:	c7 04 24 54 de 10 80 	movl   $0x8010de54,(%esp)
801008f3:	e8 af 3f 00 00       	call   801048a7 <wakeup>
        }
      }
      break;
801008f8:	eb 00                	jmp    801008fa <consoleintr+0x14d>
801008fa:	90                   	nop
consoleintr(int (*getc)(void))
{
  int c;

  acquire(&input.lock);
  while((c = getc()) >= 0){
801008fb:	8b 45 08             	mov    0x8(%ebp),%eax
801008fe:	ff d0                	call   *%eax
80100900:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100903:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100907:	0f 89 b7 fe ff ff    	jns    801007c4 <consoleintr+0x17>
        }
      }
      break;
    }
  }
  release(&input.lock);
8010090d:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
80100914:	e8 e1 41 00 00       	call   80104afa <release>
}
80100919:	c9                   	leave  
8010091a:	c3                   	ret    

8010091b <consoleread>:

int
consoleread(struct inode *ip, char *dst, int n)
{
8010091b:	55                   	push   %ebp
8010091c:	89 e5                	mov    %esp,%ebp
8010091e:	83 ec 28             	sub    $0x28,%esp
  uint target;
  int c;

  iunlock(ip);
80100921:	8b 45 08             	mov    0x8(%ebp),%eax
80100924:	89 04 24             	mov    %eax,(%esp)
80100927:	e8 7d 10 00 00       	call   801019a9 <iunlock>
  target = n;
8010092c:	8b 45 10             	mov    0x10(%ebp),%eax
8010092f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  acquire(&input.lock);
80100932:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
80100939:	e8 5a 41 00 00       	call   80104a98 <acquire>
  while(n > 0){
8010093e:	e9 aa 00 00 00       	jmp    801009ed <consoleread+0xd2>
    while(input.r == input.w){
80100943:	eb 42                	jmp    80100987 <consoleread+0x6c>
      if(proc->killed){
80100945:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010094b:	8b 40 24             	mov    0x24(%eax),%eax
8010094e:	85 c0                	test   %eax,%eax
80100950:	74 21                	je     80100973 <consoleread+0x58>
        release(&input.lock);
80100952:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
80100959:	e8 9c 41 00 00       	call   80104afa <release>
        ilock(ip);
8010095e:	8b 45 08             	mov    0x8(%ebp),%eax
80100961:	89 04 24             	mov    %eax,(%esp)
80100964:	e8 f2 0e 00 00       	call   8010185b <ilock>
        return -1;
80100969:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010096e:	e9 a5 00 00 00       	jmp    80100a18 <consoleread+0xfd>
      }
      sleep(&input.r, &input.lock);
80100973:	c7 44 24 04 a0 dd 10 	movl   $0x8010dda0,0x4(%esp)
8010097a:	80 
8010097b:	c7 04 24 54 de 10 80 	movl   $0x8010de54,(%esp)
80100982:	e8 47 3e 00 00       	call   801047ce <sleep>

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
    while(input.r == input.w){
80100987:	8b 15 54 de 10 80    	mov    0x8010de54,%edx
8010098d:	a1 58 de 10 80       	mov    0x8010de58,%eax
80100992:	39 c2                	cmp    %eax,%edx
80100994:	74 af                	je     80100945 <consoleread+0x2a>
        ilock(ip);
        return -1;
      }
      sleep(&input.r, &input.lock);
    }
    c = input.buf[input.r++ % INPUT_BUF];
80100996:	a1 54 de 10 80       	mov    0x8010de54,%eax
8010099b:	8d 50 01             	lea    0x1(%eax),%edx
8010099e:	89 15 54 de 10 80    	mov    %edx,0x8010de54
801009a4:	83 e0 7f             	and    $0x7f,%eax
801009a7:	0f b6 80 d4 dd 10 80 	movzbl -0x7fef222c(%eax),%eax
801009ae:	0f be c0             	movsbl %al,%eax
801009b1:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(c == C('D')){  // EOF
801009b4:	83 7d f0 04          	cmpl   $0x4,-0x10(%ebp)
801009b8:	75 19                	jne    801009d3 <consoleread+0xb8>
      if(n < target){
801009ba:	8b 45 10             	mov    0x10(%ebp),%eax
801009bd:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801009c0:	73 0f                	jae    801009d1 <consoleread+0xb6>
        // Save ^D for next time, to make sure
        // caller gets a 0-byte result.
        input.r--;
801009c2:	a1 54 de 10 80       	mov    0x8010de54,%eax
801009c7:	83 e8 01             	sub    $0x1,%eax
801009ca:	a3 54 de 10 80       	mov    %eax,0x8010de54
      }
      break;
801009cf:	eb 26                	jmp    801009f7 <consoleread+0xdc>
801009d1:	eb 24                	jmp    801009f7 <consoleread+0xdc>
    }
    *dst++ = c;
801009d3:	8b 45 0c             	mov    0xc(%ebp),%eax
801009d6:	8d 50 01             	lea    0x1(%eax),%edx
801009d9:	89 55 0c             	mov    %edx,0xc(%ebp)
801009dc:	8b 55 f0             	mov    -0x10(%ebp),%edx
801009df:	88 10                	mov    %dl,(%eax)
    --n;
801009e1:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
    if(c == '\n')
801009e5:	83 7d f0 0a          	cmpl   $0xa,-0x10(%ebp)
801009e9:	75 02                	jne    801009ed <consoleread+0xd2>
      break;
801009eb:	eb 0a                	jmp    801009f7 <consoleread+0xdc>
  int c;

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
801009ed:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801009f1:	0f 8f 4c ff ff ff    	jg     80100943 <consoleread+0x28>
    *dst++ = c;
    --n;
    if(c == '\n')
      break;
  }
  release(&input.lock);
801009f7:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
801009fe:	e8 f7 40 00 00       	call   80104afa <release>
  ilock(ip);
80100a03:	8b 45 08             	mov    0x8(%ebp),%eax
80100a06:	89 04 24             	mov    %eax,(%esp)
80100a09:	e8 4d 0e 00 00       	call   8010185b <ilock>

  return target - n;
80100a0e:	8b 45 10             	mov    0x10(%ebp),%eax
80100a11:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100a14:	29 c2                	sub    %eax,%edx
80100a16:	89 d0                	mov    %edx,%eax
}
80100a18:	c9                   	leave  
80100a19:	c3                   	ret    

80100a1a <consolewrite>:

int
consolewrite(struct inode *ip, char *buf, int n)
{
80100a1a:	55                   	push   %ebp
80100a1b:	89 e5                	mov    %esp,%ebp
80100a1d:	83 ec 28             	sub    $0x28,%esp
  int i;

  iunlock(ip);
80100a20:	8b 45 08             	mov    0x8(%ebp),%eax
80100a23:	89 04 24             	mov    %eax,(%esp)
80100a26:	e8 7e 0f 00 00       	call   801019a9 <iunlock>
  acquire(&cons.lock);
80100a2b:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
80100a32:	e8 61 40 00 00       	call   80104a98 <acquire>
  for(i = 0; i < n; i++)
80100a37:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80100a3e:	eb 1d                	jmp    80100a5d <consolewrite+0x43>
    consputc(buf[i] & 0xff);
80100a40:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100a43:	8b 45 0c             	mov    0xc(%ebp),%eax
80100a46:	01 d0                	add    %edx,%eax
80100a48:	0f b6 00             	movzbl (%eax),%eax
80100a4b:	0f be c0             	movsbl %al,%eax
80100a4e:	0f b6 c0             	movzbl %al,%eax
80100a51:	89 04 24             	mov    %eax,(%esp)
80100a54:	e8 f7 fc ff ff       	call   80100750 <consputc>
{
  int i;

  iunlock(ip);
  acquire(&cons.lock);
  for(i = 0; i < n; i++)
80100a59:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100a5d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100a60:	3b 45 10             	cmp    0x10(%ebp),%eax
80100a63:	7c db                	jl     80100a40 <consolewrite+0x26>
    consputc(buf[i] & 0xff);
  release(&cons.lock);
80100a65:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
80100a6c:	e8 89 40 00 00       	call   80104afa <release>
  ilock(ip);
80100a71:	8b 45 08             	mov    0x8(%ebp),%eax
80100a74:	89 04 24             	mov    %eax,(%esp)
80100a77:	e8 df 0d 00 00       	call   8010185b <ilock>

  return n;
80100a7c:	8b 45 10             	mov    0x10(%ebp),%eax
}
80100a7f:	c9                   	leave  
80100a80:	c3                   	ret    

80100a81 <consoleinit>:

void
consoleinit(void)
{
80100a81:	55                   	push   %ebp
80100a82:	89 e5                	mov    %esp,%ebp
80100a84:	83 ec 18             	sub    $0x18,%esp
  initlock(&cons.lock, "console");
80100a87:	c7 44 24 04 ab 80 10 	movl   $0x801080ab,0x4(%esp)
80100a8e:	80 
80100a8f:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
80100a96:	e8 dc 3f 00 00       	call   80104a77 <initlock>
  initlock(&input.lock, "input");
80100a9b:	c7 44 24 04 b3 80 10 	movl   $0x801080b3,0x4(%esp)
80100aa2:	80 
80100aa3:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
80100aaa:	e8 c8 3f 00 00       	call   80104a77 <initlock>

  devsw[CONSOLE].write = consolewrite;
80100aaf:	c7 05 0c e8 10 80 1a 	movl   $0x80100a1a,0x8010e80c
80100ab6:	0a 10 80 
  devsw[CONSOLE].read = consoleread;
80100ab9:	c7 05 08 e8 10 80 1b 	movl   $0x8010091b,0x8010e808
80100ac0:	09 10 80 
  cons.locking = 1;
80100ac3:	c7 05 f4 b5 10 80 01 	movl   $0x1,0x8010b5f4
80100aca:	00 00 00 

  picenable(IRQ_KBD);
80100acd:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100ad4:	e8 89 2f 00 00       	call   80103a62 <picenable>
  ioapicenable(IRQ_KBD, 0);
80100ad9:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80100ae0:	00 
80100ae1:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100ae8:	e8 70 1e 00 00       	call   8010295d <ioapicenable>
}
80100aed:	c9                   	leave  
80100aee:	c3                   	ret    

80100aef <exec>:
#include "x86.h"
#include "elf.h"

int
exec(char *path, char **argv)
{
80100aef:	55                   	push   %ebp
80100af0:	89 e5                	mov    %esp,%ebp
80100af2:	81 ec 38 01 00 00    	sub    $0x138,%esp
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pde_t *pgdir, *oldpgdir;

  begin_trans();
80100af8:	e8 e8 26 00 00       	call   801031e5 <begin_trans>
  if((ip = namei(path)) == 0){
80100afd:	8b 45 08             	mov    0x8(%ebp),%eax
80100b00:	89 04 24             	mov    %eax,(%esp)
80100b03:	e8 fe 18 00 00       	call   80102406 <namei>
80100b08:	89 45 d8             	mov    %eax,-0x28(%ebp)
80100b0b:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100b0f:	75 0f                	jne    80100b20 <exec+0x31>
    commit_trans();
80100b11:	e8 18 27 00 00       	call   8010322e <commit_trans>
    return -1;
80100b16:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100b1b:	e9 e8 03 00 00       	jmp    80100f08 <exec+0x419>
  }
  ilock(ip);
80100b20:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b23:	89 04 24             	mov    %eax,(%esp)
80100b26:	e8 30 0d 00 00       	call   8010185b <ilock>
  pgdir = 0;
80100b2b:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) < sizeof(elf))
80100b32:	c7 44 24 0c 34 00 00 	movl   $0x34,0xc(%esp)
80100b39:	00 
80100b3a:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100b41:	00 
80100b42:	8d 85 0c ff ff ff    	lea    -0xf4(%ebp),%eax
80100b48:	89 44 24 04          	mov    %eax,0x4(%esp)
80100b4c:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b4f:	89 04 24             	mov    %eax,(%esp)
80100b52:	e8 11 12 00 00       	call   80101d68 <readi>
80100b57:	83 f8 33             	cmp    $0x33,%eax
80100b5a:	77 05                	ja     80100b61 <exec+0x72>
    goto bad;
80100b5c:	e9 7b 03 00 00       	jmp    80100edc <exec+0x3ed>
  if(elf.magic != ELF_MAGIC)
80100b61:	8b 85 0c ff ff ff    	mov    -0xf4(%ebp),%eax
80100b67:	3d 7f 45 4c 46       	cmp    $0x464c457f,%eax
80100b6c:	74 05                	je     80100b73 <exec+0x84>
    goto bad;
80100b6e:	e9 69 03 00 00       	jmp    80100edc <exec+0x3ed>

  if((pgdir = setupkvm()) == 0)
80100b73:	e8 7c 6c 00 00       	call   801077f4 <setupkvm>
80100b78:	89 45 d4             	mov    %eax,-0x2c(%ebp)
80100b7b:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100b7f:	75 05                	jne    80100b86 <exec+0x97>
    goto bad;
80100b81:	e9 56 03 00 00       	jmp    80100edc <exec+0x3ed>

  // Load program into memory.
  sz = 0;
80100b86:	c7 45 e0 00 00 00 00 	movl   $0x0,-0x20(%ebp)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100b8d:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
80100b94:	8b 85 28 ff ff ff    	mov    -0xd8(%ebp),%eax
80100b9a:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100b9d:	e9 cb 00 00 00       	jmp    80100c6d <exec+0x17e>
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
80100ba2:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100ba5:	c7 44 24 0c 20 00 00 	movl   $0x20,0xc(%esp)
80100bac:	00 
80100bad:	89 44 24 08          	mov    %eax,0x8(%esp)
80100bb1:	8d 85 ec fe ff ff    	lea    -0x114(%ebp),%eax
80100bb7:	89 44 24 04          	mov    %eax,0x4(%esp)
80100bbb:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100bbe:	89 04 24             	mov    %eax,(%esp)
80100bc1:	e8 a2 11 00 00       	call   80101d68 <readi>
80100bc6:	83 f8 20             	cmp    $0x20,%eax
80100bc9:	74 05                	je     80100bd0 <exec+0xe1>
      goto bad;
80100bcb:	e9 0c 03 00 00       	jmp    80100edc <exec+0x3ed>
    if(ph.type != ELF_PROG_LOAD)
80100bd0:	8b 85 ec fe ff ff    	mov    -0x114(%ebp),%eax
80100bd6:	83 f8 01             	cmp    $0x1,%eax
80100bd9:	74 05                	je     80100be0 <exec+0xf1>
      continue;
80100bdb:	e9 80 00 00 00       	jmp    80100c60 <exec+0x171>
    if(ph.memsz < ph.filesz)
80100be0:	8b 95 00 ff ff ff    	mov    -0x100(%ebp),%edx
80100be6:	8b 85 fc fe ff ff    	mov    -0x104(%ebp),%eax
80100bec:	39 c2                	cmp    %eax,%edx
80100bee:	73 05                	jae    80100bf5 <exec+0x106>
      goto bad;
80100bf0:	e9 e7 02 00 00       	jmp    80100edc <exec+0x3ed>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
80100bf5:	8b 95 f4 fe ff ff    	mov    -0x10c(%ebp),%edx
80100bfb:	8b 85 00 ff ff ff    	mov    -0x100(%ebp),%eax
80100c01:	01 d0                	add    %edx,%eax
80100c03:	89 44 24 08          	mov    %eax,0x8(%esp)
80100c07:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c0a:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c0e:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c11:	89 04 24             	mov    %eax,(%esp)
80100c14:	e8 a9 6f 00 00       	call   80107bc2 <allocuvm>
80100c19:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100c1c:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100c20:	75 05                	jne    80100c27 <exec+0x138>
      goto bad;
80100c22:	e9 b5 02 00 00       	jmp    80100edc <exec+0x3ed>
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
80100c27:	8b 8d fc fe ff ff    	mov    -0x104(%ebp),%ecx
80100c2d:	8b 95 f0 fe ff ff    	mov    -0x110(%ebp),%edx
80100c33:	8b 85 f4 fe ff ff    	mov    -0x10c(%ebp),%eax
80100c39:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80100c3d:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100c41:	8b 55 d8             	mov    -0x28(%ebp),%edx
80100c44:	89 54 24 08          	mov    %edx,0x8(%esp)
80100c48:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c4c:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c4f:	89 04 24             	mov    %eax,(%esp)
80100c52:	e8 80 6e 00 00       	call   80107ad7 <loaduvm>
80100c57:	85 c0                	test   %eax,%eax
80100c59:	79 05                	jns    80100c60 <exec+0x171>
      goto bad;
80100c5b:	e9 7c 02 00 00       	jmp    80100edc <exec+0x3ed>
  if((pgdir = setupkvm()) == 0)
    goto bad;

  // Load program into memory.
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100c60:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
80100c64:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100c67:	83 c0 20             	add    $0x20,%eax
80100c6a:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100c6d:	0f b7 85 38 ff ff ff 	movzwl -0xc8(%ebp),%eax
80100c74:	0f b7 c0             	movzwl %ax,%eax
80100c77:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80100c7a:	0f 8f 22 ff ff ff    	jg     80100ba2 <exec+0xb3>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
      goto bad;
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
      goto bad;
  }
  iunlockput(ip);
80100c80:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100c83:	89 04 24             	mov    %eax,(%esp)
80100c86:	e8 54 0e 00 00       	call   80101adf <iunlockput>
  commit_trans();
80100c8b:	e8 9e 25 00 00       	call   8010322e <commit_trans>
  ip = 0;
80100c90:	c7 45 d8 00 00 00 00 	movl   $0x0,-0x28(%ebp)

  // Allocate two pages at the next page boundary.
  // Make the first inaccessible.  Use the second as the user stack.
  sz = PGROUNDUP(sz);
80100c97:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c9a:	05 ff 0f 00 00       	add    $0xfff,%eax
80100c9f:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80100ca4:	89 45 e0             	mov    %eax,-0x20(%ebp)
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
80100ca7:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100caa:	05 00 20 00 00       	add    $0x2000,%eax
80100caf:	89 44 24 08          	mov    %eax,0x8(%esp)
80100cb3:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cb6:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cba:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100cbd:	89 04 24             	mov    %eax,(%esp)
80100cc0:	e8 fd 6e 00 00       	call   80107bc2 <allocuvm>
80100cc5:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100cc8:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100ccc:	75 05                	jne    80100cd3 <exec+0x1e4>
    goto bad;
80100cce:	e9 09 02 00 00       	jmp    80100edc <exec+0x3ed>
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100cd3:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cd6:	2d 00 20 00 00       	sub    $0x2000,%eax
80100cdb:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cdf:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100ce2:	89 04 24             	mov    %eax,(%esp)
80100ce5:	e8 08 71 00 00       	call   80107df2 <clearpteu>
  sp = sz;
80100cea:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100ced:	89 45 dc             	mov    %eax,-0x24(%ebp)

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100cf0:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
80100cf7:	e9 9a 00 00 00       	jmp    80100d96 <exec+0x2a7>
    if(argc >= MAXARG)
80100cfc:	83 7d e4 1f          	cmpl   $0x1f,-0x1c(%ebp)
80100d00:	76 05                	jbe    80100d07 <exec+0x218>
      goto bad;
80100d02:	e9 d5 01 00 00       	jmp    80100edc <exec+0x3ed>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
80100d07:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d0a:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100d11:	8b 45 0c             	mov    0xc(%ebp),%eax
80100d14:	01 d0                	add    %edx,%eax
80100d16:	8b 00                	mov    (%eax),%eax
80100d18:	89 04 24             	mov    %eax,(%esp)
80100d1b:	e8 36 42 00 00       	call   80104f56 <strlen>
80100d20:	8b 55 dc             	mov    -0x24(%ebp),%edx
80100d23:	29 c2                	sub    %eax,%edx
80100d25:	89 d0                	mov    %edx,%eax
80100d27:	83 e8 01             	sub    $0x1,%eax
80100d2a:	83 e0 fc             	and    $0xfffffffc,%eax
80100d2d:	89 45 dc             	mov    %eax,-0x24(%ebp)
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
80100d30:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d33:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100d3a:	8b 45 0c             	mov    0xc(%ebp),%eax
80100d3d:	01 d0                	add    %edx,%eax
80100d3f:	8b 00                	mov    (%eax),%eax
80100d41:	89 04 24             	mov    %eax,(%esp)
80100d44:	e8 0d 42 00 00       	call   80104f56 <strlen>
80100d49:	83 c0 01             	add    $0x1,%eax
80100d4c:	89 c2                	mov    %eax,%edx
80100d4e:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d51:	8d 0c 85 00 00 00 00 	lea    0x0(,%eax,4),%ecx
80100d58:	8b 45 0c             	mov    0xc(%ebp),%eax
80100d5b:	01 c8                	add    %ecx,%eax
80100d5d:	8b 00                	mov    (%eax),%eax
80100d5f:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100d63:	89 44 24 08          	mov    %eax,0x8(%esp)
80100d67:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d6a:	89 44 24 04          	mov    %eax,0x4(%esp)
80100d6e:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100d71:	89 04 24             	mov    %eax,(%esp)
80100d74:	e8 3e 72 00 00       	call   80107fb7 <copyout>
80100d79:	85 c0                	test   %eax,%eax
80100d7b:	79 05                	jns    80100d82 <exec+0x293>
      goto bad;
80100d7d:	e9 5a 01 00 00       	jmp    80100edc <exec+0x3ed>
    ustack[3+argc] = sp;
80100d82:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d85:	8d 50 03             	lea    0x3(%eax),%edx
80100d88:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d8b:	89 84 95 40 ff ff ff 	mov    %eax,-0xc0(%ebp,%edx,4)
    goto bad;
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
  sp = sz;

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100d92:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
80100d96:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d99:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100da0:	8b 45 0c             	mov    0xc(%ebp),%eax
80100da3:	01 d0                	add    %edx,%eax
80100da5:	8b 00                	mov    (%eax),%eax
80100da7:	85 c0                	test   %eax,%eax
80100da9:	0f 85 4d ff ff ff    	jne    80100cfc <exec+0x20d>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
      goto bad;
    ustack[3+argc] = sp;
  }
  ustack[3+argc] = 0;
80100daf:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100db2:	83 c0 03             	add    $0x3,%eax
80100db5:	c7 84 85 40 ff ff ff 	movl   $0x0,-0xc0(%ebp,%eax,4)
80100dbc:	00 00 00 00 

  ustack[0] = 0xffffffff;  // fake return PC
80100dc0:	c7 85 40 ff ff ff ff 	movl   $0xffffffff,-0xc0(%ebp)
80100dc7:	ff ff ff 
  ustack[1] = argc;
80100dca:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dcd:	89 85 44 ff ff ff    	mov    %eax,-0xbc(%ebp)
  ustack[2] = sp - (argc+1)*4;  // argv pointer
80100dd3:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dd6:	83 c0 01             	add    $0x1,%eax
80100dd9:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100de0:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100de3:	29 d0                	sub    %edx,%eax
80100de5:	89 85 48 ff ff ff    	mov    %eax,-0xb8(%ebp)

  sp -= (3+argc+1) * 4;
80100deb:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dee:	83 c0 04             	add    $0x4,%eax
80100df1:	c1 e0 02             	shl    $0x2,%eax
80100df4:	29 45 dc             	sub    %eax,-0x24(%ebp)
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100df7:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dfa:	83 c0 04             	add    $0x4,%eax
80100dfd:	c1 e0 02             	shl    $0x2,%eax
80100e00:	89 44 24 0c          	mov    %eax,0xc(%esp)
80100e04:	8d 85 40 ff ff ff    	lea    -0xc0(%ebp),%eax
80100e0a:	89 44 24 08          	mov    %eax,0x8(%esp)
80100e0e:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100e11:	89 44 24 04          	mov    %eax,0x4(%esp)
80100e15:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100e18:	89 04 24             	mov    %eax,(%esp)
80100e1b:	e8 97 71 00 00       	call   80107fb7 <copyout>
80100e20:	85 c0                	test   %eax,%eax
80100e22:	79 05                	jns    80100e29 <exec+0x33a>
    goto bad;
80100e24:	e9 b3 00 00 00       	jmp    80100edc <exec+0x3ed>

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100e29:	8b 45 08             	mov    0x8(%ebp),%eax
80100e2c:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100e2f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e32:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100e35:	eb 17                	jmp    80100e4e <exec+0x35f>
    if(*s == '/')
80100e37:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e3a:	0f b6 00             	movzbl (%eax),%eax
80100e3d:	3c 2f                	cmp    $0x2f,%al
80100e3f:	75 09                	jne    80100e4a <exec+0x35b>
      last = s+1;
80100e41:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e44:	83 c0 01             	add    $0x1,%eax
80100e47:	89 45 f0             	mov    %eax,-0x10(%ebp)
  sp -= (3+argc+1) * 4;
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
    goto bad;

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100e4a:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100e4e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e51:	0f b6 00             	movzbl (%eax),%eax
80100e54:	84 c0                	test   %al,%al
80100e56:	75 df                	jne    80100e37 <exec+0x348>
    if(*s == '/')
      last = s+1;
  safestrcpy(proc->name, last, sizeof(proc->name));
80100e58:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e5e:	8d 50 6c             	lea    0x6c(%eax),%edx
80100e61:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80100e68:	00 
80100e69:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100e6c:	89 44 24 04          	mov    %eax,0x4(%esp)
80100e70:	89 14 24             	mov    %edx,(%esp)
80100e73:	e8 94 40 00 00       	call   80104f0c <safestrcpy>

  // Commit to the user image.
  oldpgdir = proc->pgdir;
80100e78:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e7e:	8b 40 04             	mov    0x4(%eax),%eax
80100e81:	89 45 d0             	mov    %eax,-0x30(%ebp)
  proc->pgdir = pgdir;
80100e84:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e8a:	8b 55 d4             	mov    -0x2c(%ebp),%edx
80100e8d:	89 50 04             	mov    %edx,0x4(%eax)
  proc->sz = sz;
80100e90:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e96:	8b 55 e0             	mov    -0x20(%ebp),%edx
80100e99:	89 10                	mov    %edx,(%eax)
  proc->tf->eip = elf.entry;  // main
80100e9b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100ea1:	8b 40 18             	mov    0x18(%eax),%eax
80100ea4:	8b 95 24 ff ff ff    	mov    -0xdc(%ebp),%edx
80100eaa:	89 50 38             	mov    %edx,0x38(%eax)
  proc->tf->esp = sp;
80100ead:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100eb3:	8b 40 18             	mov    0x18(%eax),%eax
80100eb6:	8b 55 dc             	mov    -0x24(%ebp),%edx
80100eb9:	89 50 44             	mov    %edx,0x44(%eax)
  switchuvm(proc);
80100ebc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100ec2:	89 04 24             	mov    %eax,(%esp)
80100ec5:	e8 1b 6a 00 00       	call   801078e5 <switchuvm>
  freevm(oldpgdir);
80100eca:	8b 45 d0             	mov    -0x30(%ebp),%eax
80100ecd:	89 04 24             	mov    %eax,(%esp)
80100ed0:	e8 83 6e 00 00       	call   80107d58 <freevm>
  return 0;
80100ed5:	b8 00 00 00 00       	mov    $0x0,%eax
80100eda:	eb 2c                	jmp    80100f08 <exec+0x419>

 bad:
  if(pgdir)
80100edc:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100ee0:	74 0b                	je     80100eed <exec+0x3fe>
    freevm(pgdir);
80100ee2:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100ee5:	89 04 24             	mov    %eax,(%esp)
80100ee8:	e8 6b 6e 00 00       	call   80107d58 <freevm>
  if(ip){
80100eed:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100ef1:	74 10                	je     80100f03 <exec+0x414>
    iunlockput(ip);
80100ef3:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100ef6:	89 04 24             	mov    %eax,(%esp)
80100ef9:	e8 e1 0b 00 00       	call   80101adf <iunlockput>
    commit_trans();
80100efe:	e8 2b 23 00 00       	call   8010322e <commit_trans>
  }
  return -1;
80100f03:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80100f08:	c9                   	leave  
80100f09:	c3                   	ret    

80100f0a <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
80100f0a:	55                   	push   %ebp
80100f0b:	89 e5                	mov    %esp,%ebp
80100f0d:	83 ec 18             	sub    $0x18,%esp
  initlock(&ftable.lock, "ftable");
80100f10:	c7 44 24 04 b9 80 10 	movl   $0x801080b9,0x4(%esp)
80100f17:	80 
80100f18:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f1f:	e8 53 3b 00 00       	call   80104a77 <initlock>
}
80100f24:	c9                   	leave  
80100f25:	c3                   	ret    

80100f26 <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
80100f26:	55                   	push   %ebp
80100f27:	89 e5                	mov    %esp,%ebp
80100f29:	83 ec 28             	sub    $0x28,%esp
  struct file *f;

  acquire(&ftable.lock);
80100f2c:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f33:	e8 60 3b 00 00       	call   80104a98 <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f38:	c7 45 f4 94 de 10 80 	movl   $0x8010de94,-0xc(%ebp)
80100f3f:	eb 29                	jmp    80100f6a <filealloc+0x44>
    if(f->ref == 0){
80100f41:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f44:	8b 40 04             	mov    0x4(%eax),%eax
80100f47:	85 c0                	test   %eax,%eax
80100f49:	75 1b                	jne    80100f66 <filealloc+0x40>
      f->ref = 1;
80100f4b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f4e:	c7 40 04 01 00 00 00 	movl   $0x1,0x4(%eax)
      release(&ftable.lock);
80100f55:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f5c:	e8 99 3b 00 00       	call   80104afa <release>
      return f;
80100f61:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f64:	eb 1e                	jmp    80100f84 <filealloc+0x5e>
filealloc(void)
{
  struct file *f;

  acquire(&ftable.lock);
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f66:	83 45 f4 18          	addl   $0x18,-0xc(%ebp)
80100f6a:	81 7d f4 f4 e7 10 80 	cmpl   $0x8010e7f4,-0xc(%ebp)
80100f71:	72 ce                	jb     80100f41 <filealloc+0x1b>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
80100f73:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f7a:	e8 7b 3b 00 00       	call   80104afa <release>
  return 0;
80100f7f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80100f84:	c9                   	leave  
80100f85:	c3                   	ret    

80100f86 <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
80100f86:	55                   	push   %ebp
80100f87:	89 e5                	mov    %esp,%ebp
80100f89:	83 ec 18             	sub    $0x18,%esp
  acquire(&ftable.lock);
80100f8c:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f93:	e8 00 3b 00 00       	call   80104a98 <acquire>
  if(f->ref < 1)
80100f98:	8b 45 08             	mov    0x8(%ebp),%eax
80100f9b:	8b 40 04             	mov    0x4(%eax),%eax
80100f9e:	85 c0                	test   %eax,%eax
80100fa0:	7f 0c                	jg     80100fae <filedup+0x28>
    panic("filedup");
80100fa2:	c7 04 24 c0 80 10 80 	movl   $0x801080c0,(%esp)
80100fa9:	e8 8c f5 ff ff       	call   8010053a <panic>
  f->ref++;
80100fae:	8b 45 08             	mov    0x8(%ebp),%eax
80100fb1:	8b 40 04             	mov    0x4(%eax),%eax
80100fb4:	8d 50 01             	lea    0x1(%eax),%edx
80100fb7:	8b 45 08             	mov    0x8(%ebp),%eax
80100fba:	89 50 04             	mov    %edx,0x4(%eax)
  release(&ftable.lock);
80100fbd:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100fc4:	e8 31 3b 00 00       	call   80104afa <release>
  return f;
80100fc9:	8b 45 08             	mov    0x8(%ebp),%eax
}
80100fcc:	c9                   	leave  
80100fcd:	c3                   	ret    

80100fce <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
80100fce:	55                   	push   %ebp
80100fcf:	89 e5                	mov    %esp,%ebp
80100fd1:	83 ec 38             	sub    $0x38,%esp
  struct file ff;

  acquire(&ftable.lock);
80100fd4:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100fdb:	e8 b8 3a 00 00       	call   80104a98 <acquire>
  if(f->ref < 1)
80100fe0:	8b 45 08             	mov    0x8(%ebp),%eax
80100fe3:	8b 40 04             	mov    0x4(%eax),%eax
80100fe6:	85 c0                	test   %eax,%eax
80100fe8:	7f 0c                	jg     80100ff6 <fileclose+0x28>
    panic("fileclose");
80100fea:	c7 04 24 c8 80 10 80 	movl   $0x801080c8,(%esp)
80100ff1:	e8 44 f5 ff ff       	call   8010053a <panic>
  if(--f->ref > 0){
80100ff6:	8b 45 08             	mov    0x8(%ebp),%eax
80100ff9:	8b 40 04             	mov    0x4(%eax),%eax
80100ffc:	8d 50 ff             	lea    -0x1(%eax),%edx
80100fff:	8b 45 08             	mov    0x8(%ebp),%eax
80101002:	89 50 04             	mov    %edx,0x4(%eax)
80101005:	8b 45 08             	mov    0x8(%ebp),%eax
80101008:	8b 40 04             	mov    0x4(%eax),%eax
8010100b:	85 c0                	test   %eax,%eax
8010100d:	7e 11                	jle    80101020 <fileclose+0x52>
    release(&ftable.lock);
8010100f:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80101016:	e8 df 3a 00 00       	call   80104afa <release>
8010101b:	e9 82 00 00 00       	jmp    801010a2 <fileclose+0xd4>
    return;
  }
  ff = *f;
80101020:	8b 45 08             	mov    0x8(%ebp),%eax
80101023:	8b 10                	mov    (%eax),%edx
80101025:	89 55 e0             	mov    %edx,-0x20(%ebp)
80101028:	8b 50 04             	mov    0x4(%eax),%edx
8010102b:	89 55 e4             	mov    %edx,-0x1c(%ebp)
8010102e:	8b 50 08             	mov    0x8(%eax),%edx
80101031:	89 55 e8             	mov    %edx,-0x18(%ebp)
80101034:	8b 50 0c             	mov    0xc(%eax),%edx
80101037:	89 55 ec             	mov    %edx,-0x14(%ebp)
8010103a:	8b 50 10             	mov    0x10(%eax),%edx
8010103d:	89 55 f0             	mov    %edx,-0x10(%ebp)
80101040:	8b 40 14             	mov    0x14(%eax),%eax
80101043:	89 45 f4             	mov    %eax,-0xc(%ebp)
  f->ref = 0;
80101046:	8b 45 08             	mov    0x8(%ebp),%eax
80101049:	c7 40 04 00 00 00 00 	movl   $0x0,0x4(%eax)
  f->type = FD_NONE;
80101050:	8b 45 08             	mov    0x8(%ebp),%eax
80101053:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  release(&ftable.lock);
80101059:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80101060:	e8 95 3a 00 00       	call   80104afa <release>
  
  if(ff.type == FD_PIPE)
80101065:	8b 45 e0             	mov    -0x20(%ebp),%eax
80101068:	83 f8 01             	cmp    $0x1,%eax
8010106b:	75 18                	jne    80101085 <fileclose+0xb7>
    pipeclose(ff.pipe, ff.writable);
8010106d:	0f b6 45 e9          	movzbl -0x17(%ebp),%eax
80101071:	0f be d0             	movsbl %al,%edx
80101074:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101077:	89 54 24 04          	mov    %edx,0x4(%esp)
8010107b:	89 04 24             	mov    %eax,(%esp)
8010107e:	e8 8f 2c 00 00       	call   80103d12 <pipeclose>
80101083:	eb 1d                	jmp    801010a2 <fileclose+0xd4>
  else if(ff.type == FD_INODE){
80101085:	8b 45 e0             	mov    -0x20(%ebp),%eax
80101088:	83 f8 02             	cmp    $0x2,%eax
8010108b:	75 15                	jne    801010a2 <fileclose+0xd4>
    begin_trans();
8010108d:	e8 53 21 00 00       	call   801031e5 <begin_trans>
    iput(ff.ip);
80101092:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101095:	89 04 24             	mov    %eax,(%esp)
80101098:	e8 71 09 00 00       	call   80101a0e <iput>
    commit_trans();
8010109d:	e8 8c 21 00 00       	call   8010322e <commit_trans>
  }
}
801010a2:	c9                   	leave  
801010a3:	c3                   	ret    

801010a4 <filestat>:

// Get metadata about file f.
int
filestat(struct file *f, struct stat *st)
{
801010a4:	55                   	push   %ebp
801010a5:	89 e5                	mov    %esp,%ebp
801010a7:	83 ec 18             	sub    $0x18,%esp
  if(f->type == FD_INODE){
801010aa:	8b 45 08             	mov    0x8(%ebp),%eax
801010ad:	8b 00                	mov    (%eax),%eax
801010af:	83 f8 02             	cmp    $0x2,%eax
801010b2:	75 38                	jne    801010ec <filestat+0x48>
    ilock(f->ip);
801010b4:	8b 45 08             	mov    0x8(%ebp),%eax
801010b7:	8b 40 10             	mov    0x10(%eax),%eax
801010ba:	89 04 24             	mov    %eax,(%esp)
801010bd:	e8 99 07 00 00       	call   8010185b <ilock>
    stati(f->ip, st);
801010c2:	8b 45 08             	mov    0x8(%ebp),%eax
801010c5:	8b 40 10             	mov    0x10(%eax),%eax
801010c8:	8b 55 0c             	mov    0xc(%ebp),%edx
801010cb:	89 54 24 04          	mov    %edx,0x4(%esp)
801010cf:	89 04 24             	mov    %eax,(%esp)
801010d2:	e8 4c 0c 00 00       	call   80101d23 <stati>
    iunlock(f->ip);
801010d7:	8b 45 08             	mov    0x8(%ebp),%eax
801010da:	8b 40 10             	mov    0x10(%eax),%eax
801010dd:	89 04 24             	mov    %eax,(%esp)
801010e0:	e8 c4 08 00 00       	call   801019a9 <iunlock>
    return 0;
801010e5:	b8 00 00 00 00       	mov    $0x0,%eax
801010ea:	eb 05                	jmp    801010f1 <filestat+0x4d>
  }
  return -1;
801010ec:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801010f1:	c9                   	leave  
801010f2:	c3                   	ret    

801010f3 <fileread>:

// Read from file f.
int
fileread(struct file *f, char *addr, int n)
{
801010f3:	55                   	push   %ebp
801010f4:	89 e5                	mov    %esp,%ebp
801010f6:	83 ec 28             	sub    $0x28,%esp
  int r;

  if(f->readable == 0)
801010f9:	8b 45 08             	mov    0x8(%ebp),%eax
801010fc:	0f b6 40 08          	movzbl 0x8(%eax),%eax
80101100:	84 c0                	test   %al,%al
80101102:	75 0a                	jne    8010110e <fileread+0x1b>
    return -1;
80101104:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101109:	e9 9f 00 00 00       	jmp    801011ad <fileread+0xba>
  if(f->type == FD_PIPE)
8010110e:	8b 45 08             	mov    0x8(%ebp),%eax
80101111:	8b 00                	mov    (%eax),%eax
80101113:	83 f8 01             	cmp    $0x1,%eax
80101116:	75 1e                	jne    80101136 <fileread+0x43>
    return piperead(f->pipe, addr, n);
80101118:	8b 45 08             	mov    0x8(%ebp),%eax
8010111b:	8b 40 0c             	mov    0xc(%eax),%eax
8010111e:	8b 55 10             	mov    0x10(%ebp),%edx
80101121:	89 54 24 08          	mov    %edx,0x8(%esp)
80101125:	8b 55 0c             	mov    0xc(%ebp),%edx
80101128:	89 54 24 04          	mov    %edx,0x4(%esp)
8010112c:	89 04 24             	mov    %eax,(%esp)
8010112f:	e8 5f 2d 00 00       	call   80103e93 <piperead>
80101134:	eb 77                	jmp    801011ad <fileread+0xba>
  if(f->type == FD_INODE){
80101136:	8b 45 08             	mov    0x8(%ebp),%eax
80101139:	8b 00                	mov    (%eax),%eax
8010113b:	83 f8 02             	cmp    $0x2,%eax
8010113e:	75 61                	jne    801011a1 <fileread+0xae>
    ilock(f->ip);
80101140:	8b 45 08             	mov    0x8(%ebp),%eax
80101143:	8b 40 10             	mov    0x10(%eax),%eax
80101146:	89 04 24             	mov    %eax,(%esp)
80101149:	e8 0d 07 00 00       	call   8010185b <ilock>
    if((r = readi(f->ip, addr, f->off, n)) > 0)
8010114e:	8b 4d 10             	mov    0x10(%ebp),%ecx
80101151:	8b 45 08             	mov    0x8(%ebp),%eax
80101154:	8b 50 14             	mov    0x14(%eax),%edx
80101157:	8b 45 08             	mov    0x8(%ebp),%eax
8010115a:	8b 40 10             	mov    0x10(%eax),%eax
8010115d:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80101161:	89 54 24 08          	mov    %edx,0x8(%esp)
80101165:	8b 55 0c             	mov    0xc(%ebp),%edx
80101168:	89 54 24 04          	mov    %edx,0x4(%esp)
8010116c:	89 04 24             	mov    %eax,(%esp)
8010116f:	e8 f4 0b 00 00       	call   80101d68 <readi>
80101174:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101177:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010117b:	7e 11                	jle    8010118e <fileread+0x9b>
      f->off += r;
8010117d:	8b 45 08             	mov    0x8(%ebp),%eax
80101180:	8b 50 14             	mov    0x14(%eax),%edx
80101183:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101186:	01 c2                	add    %eax,%edx
80101188:	8b 45 08             	mov    0x8(%ebp),%eax
8010118b:	89 50 14             	mov    %edx,0x14(%eax)
    iunlock(f->ip);
8010118e:	8b 45 08             	mov    0x8(%ebp),%eax
80101191:	8b 40 10             	mov    0x10(%eax),%eax
80101194:	89 04 24             	mov    %eax,(%esp)
80101197:	e8 0d 08 00 00       	call   801019a9 <iunlock>
    return r;
8010119c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010119f:	eb 0c                	jmp    801011ad <fileread+0xba>
  }
  panic("fileread");
801011a1:	c7 04 24 d2 80 10 80 	movl   $0x801080d2,(%esp)
801011a8:	e8 8d f3 ff ff       	call   8010053a <panic>
}
801011ad:	c9                   	leave  
801011ae:	c3                   	ret    

801011af <filewrite>:

//PAGEBREAK!
// Write to file f.
int
filewrite(struct file *f, char *addr, int n)
{
801011af:	55                   	push   %ebp
801011b0:	89 e5                	mov    %esp,%ebp
801011b2:	53                   	push   %ebx
801011b3:	83 ec 24             	sub    $0x24,%esp
  int r;

  if(f->writable == 0)
801011b6:	8b 45 08             	mov    0x8(%ebp),%eax
801011b9:	0f b6 40 09          	movzbl 0x9(%eax),%eax
801011bd:	84 c0                	test   %al,%al
801011bf:	75 0a                	jne    801011cb <filewrite+0x1c>
    return -1;
801011c1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801011c6:	e9 20 01 00 00       	jmp    801012eb <filewrite+0x13c>
  if(f->type == FD_PIPE)
801011cb:	8b 45 08             	mov    0x8(%ebp),%eax
801011ce:	8b 00                	mov    (%eax),%eax
801011d0:	83 f8 01             	cmp    $0x1,%eax
801011d3:	75 21                	jne    801011f6 <filewrite+0x47>
    return pipewrite(f->pipe, addr, n);
801011d5:	8b 45 08             	mov    0x8(%ebp),%eax
801011d8:	8b 40 0c             	mov    0xc(%eax),%eax
801011db:	8b 55 10             	mov    0x10(%ebp),%edx
801011de:	89 54 24 08          	mov    %edx,0x8(%esp)
801011e2:	8b 55 0c             	mov    0xc(%ebp),%edx
801011e5:	89 54 24 04          	mov    %edx,0x4(%esp)
801011e9:	89 04 24             	mov    %eax,(%esp)
801011ec:	e8 b3 2b 00 00       	call   80103da4 <pipewrite>
801011f1:	e9 f5 00 00 00       	jmp    801012eb <filewrite+0x13c>
  if(f->type == FD_INODE){
801011f6:	8b 45 08             	mov    0x8(%ebp),%eax
801011f9:	8b 00                	mov    (%eax),%eax
801011fb:	83 f8 02             	cmp    $0x2,%eax
801011fe:	0f 85 db 00 00 00    	jne    801012df <filewrite+0x130>
    // the maximum log transaction size, including
    // i-node, indirect block, allocation blocks,
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
80101204:	c7 45 ec 00 06 00 00 	movl   $0x600,-0x14(%ebp)
    int i = 0;
8010120b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    while(i < n){
80101212:	e9 a8 00 00 00       	jmp    801012bf <filewrite+0x110>
      int n1 = n - i;
80101217:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010121a:	8b 55 10             	mov    0x10(%ebp),%edx
8010121d:	29 c2                	sub    %eax,%edx
8010121f:	89 d0                	mov    %edx,%eax
80101221:	89 45 f0             	mov    %eax,-0x10(%ebp)
      if(n1 > max)
80101224:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101227:	3b 45 ec             	cmp    -0x14(%ebp),%eax
8010122a:	7e 06                	jle    80101232 <filewrite+0x83>
        n1 = max;
8010122c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010122f:	89 45 f0             	mov    %eax,-0x10(%ebp)

      begin_trans();
80101232:	e8 ae 1f 00 00       	call   801031e5 <begin_trans>
      ilock(f->ip);
80101237:	8b 45 08             	mov    0x8(%ebp),%eax
8010123a:	8b 40 10             	mov    0x10(%eax),%eax
8010123d:	89 04 24             	mov    %eax,(%esp)
80101240:	e8 16 06 00 00       	call   8010185b <ilock>
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
80101245:	8b 4d f0             	mov    -0x10(%ebp),%ecx
80101248:	8b 45 08             	mov    0x8(%ebp),%eax
8010124b:	8b 50 14             	mov    0x14(%eax),%edx
8010124e:	8b 5d f4             	mov    -0xc(%ebp),%ebx
80101251:	8b 45 0c             	mov    0xc(%ebp),%eax
80101254:	01 c3                	add    %eax,%ebx
80101256:	8b 45 08             	mov    0x8(%ebp),%eax
80101259:	8b 40 10             	mov    0x10(%eax),%eax
8010125c:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80101260:	89 54 24 08          	mov    %edx,0x8(%esp)
80101264:	89 5c 24 04          	mov    %ebx,0x4(%esp)
80101268:	89 04 24             	mov    %eax,(%esp)
8010126b:	e8 5c 0c 00 00       	call   80101ecc <writei>
80101270:	89 45 e8             	mov    %eax,-0x18(%ebp)
80101273:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80101277:	7e 11                	jle    8010128a <filewrite+0xdb>
        f->off += r;
80101279:	8b 45 08             	mov    0x8(%ebp),%eax
8010127c:	8b 50 14             	mov    0x14(%eax),%edx
8010127f:	8b 45 e8             	mov    -0x18(%ebp),%eax
80101282:	01 c2                	add    %eax,%edx
80101284:	8b 45 08             	mov    0x8(%ebp),%eax
80101287:	89 50 14             	mov    %edx,0x14(%eax)
      iunlock(f->ip);
8010128a:	8b 45 08             	mov    0x8(%ebp),%eax
8010128d:	8b 40 10             	mov    0x10(%eax),%eax
80101290:	89 04 24             	mov    %eax,(%esp)
80101293:	e8 11 07 00 00       	call   801019a9 <iunlock>
      commit_trans();
80101298:	e8 91 1f 00 00       	call   8010322e <commit_trans>

      if(r < 0)
8010129d:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801012a1:	79 02                	jns    801012a5 <filewrite+0xf6>
        break;
801012a3:	eb 26                	jmp    801012cb <filewrite+0x11c>
      if(r != n1)
801012a5:	8b 45 e8             	mov    -0x18(%ebp),%eax
801012a8:	3b 45 f0             	cmp    -0x10(%ebp),%eax
801012ab:	74 0c                	je     801012b9 <filewrite+0x10a>
        panic("short filewrite");
801012ad:	c7 04 24 db 80 10 80 	movl   $0x801080db,(%esp)
801012b4:	e8 81 f2 ff ff       	call   8010053a <panic>
      i += r;
801012b9:	8b 45 e8             	mov    -0x18(%ebp),%eax
801012bc:	01 45 f4             	add    %eax,-0xc(%ebp)
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
    int i = 0;
    while(i < n){
801012bf:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012c2:	3b 45 10             	cmp    0x10(%ebp),%eax
801012c5:	0f 8c 4c ff ff ff    	jl     80101217 <filewrite+0x68>
        break;
      if(r != n1)
        panic("short filewrite");
      i += r;
    }
    return i == n ? n : -1;
801012cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012ce:	3b 45 10             	cmp    0x10(%ebp),%eax
801012d1:	75 05                	jne    801012d8 <filewrite+0x129>
801012d3:	8b 45 10             	mov    0x10(%ebp),%eax
801012d6:	eb 05                	jmp    801012dd <filewrite+0x12e>
801012d8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801012dd:	eb 0c                	jmp    801012eb <filewrite+0x13c>
  }
  panic("filewrite");
801012df:	c7 04 24 eb 80 10 80 	movl   $0x801080eb,(%esp)
801012e6:	e8 4f f2 ff ff       	call   8010053a <panic>
}
801012eb:	83 c4 24             	add    $0x24,%esp
801012ee:	5b                   	pop    %ebx
801012ef:	5d                   	pop    %ebp
801012f0:	c3                   	ret    

801012f1 <readsb>:
static void itrunc(struct inode*);

// Read the super block.
void
readsb(int dev, struct superblock *sb)
{
801012f1:	55                   	push   %ebp
801012f2:	89 e5                	mov    %esp,%ebp
801012f4:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, 1);
801012f7:	8b 45 08             	mov    0x8(%ebp),%eax
801012fa:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80101301:	00 
80101302:	89 04 24             	mov    %eax,(%esp)
80101305:	e8 9c ee ff ff       	call   801001a6 <bread>
8010130a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memmove(sb, bp->data, sizeof(*sb));
8010130d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101310:	83 c0 18             	add    $0x18,%eax
80101313:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
8010131a:	00 
8010131b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010131f:	8b 45 0c             	mov    0xc(%ebp),%eax
80101322:	89 04 24             	mov    %eax,(%esp)
80101325:	e8 91 3a 00 00       	call   80104dbb <memmove>
  brelse(bp);
8010132a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010132d:	89 04 24             	mov    %eax,(%esp)
80101330:	e8 e2 ee ff ff       	call   80100217 <brelse>
}
80101335:	c9                   	leave  
80101336:	c3                   	ret    

80101337 <bzero>:

// Zero a block.
static void
bzero(int dev, int bno)
{
80101337:	55                   	push   %ebp
80101338:	89 e5                	mov    %esp,%ebp
8010133a:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, bno);
8010133d:	8b 55 0c             	mov    0xc(%ebp),%edx
80101340:	8b 45 08             	mov    0x8(%ebp),%eax
80101343:	89 54 24 04          	mov    %edx,0x4(%esp)
80101347:	89 04 24             	mov    %eax,(%esp)
8010134a:	e8 57 ee ff ff       	call   801001a6 <bread>
8010134f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(bp->data, 0, BSIZE);
80101352:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101355:	83 c0 18             	add    $0x18,%eax
80101358:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
8010135f:	00 
80101360:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80101367:	00 
80101368:	89 04 24             	mov    %eax,(%esp)
8010136b:	e8 7c 39 00 00       	call   80104cec <memset>
  log_write(bp);
80101370:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101373:	89 04 24             	mov    %eax,(%esp)
80101376:	e8 0b 1f 00 00       	call   80103286 <log_write>
  brelse(bp);
8010137b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010137e:	89 04 24             	mov    %eax,(%esp)
80101381:	e8 91 ee ff ff       	call   80100217 <brelse>
}
80101386:	c9                   	leave  
80101387:	c3                   	ret    

80101388 <balloc>:
// Blocks. 

// Allocate a zeroed disk block.
static uint
balloc(uint dev)
{
80101388:	55                   	push   %ebp
80101389:	89 e5                	mov    %esp,%ebp
8010138b:	83 ec 38             	sub    $0x38,%esp
  int b, bi, m;
  struct buf *bp;
  struct superblock sb;

  bp = 0;
8010138e:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
  readsb(dev, &sb);
80101395:	8b 45 08             	mov    0x8(%ebp),%eax
80101398:	8d 55 d8             	lea    -0x28(%ebp),%edx
8010139b:	89 54 24 04          	mov    %edx,0x4(%esp)
8010139f:	89 04 24             	mov    %eax,(%esp)
801013a2:	e8 4a ff ff ff       	call   801012f1 <readsb>
  for(b = 0; b < sb.size; b += BPB){
801013a7:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801013ae:	e9 07 01 00 00       	jmp    801014ba <balloc+0x132>
    bp = bread(dev, BBLOCK(b, sb.ninodes));
801013b3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801013b6:	8d 90 ff 0f 00 00    	lea    0xfff(%eax),%edx
801013bc:	85 c0                	test   %eax,%eax
801013be:	0f 48 c2             	cmovs  %edx,%eax
801013c1:	c1 f8 0c             	sar    $0xc,%eax
801013c4:	8b 55 e0             	mov    -0x20(%ebp),%edx
801013c7:	c1 ea 03             	shr    $0x3,%edx
801013ca:	01 d0                	add    %edx,%eax
801013cc:	83 c0 03             	add    $0x3,%eax
801013cf:	89 44 24 04          	mov    %eax,0x4(%esp)
801013d3:	8b 45 08             	mov    0x8(%ebp),%eax
801013d6:	89 04 24             	mov    %eax,(%esp)
801013d9:	e8 c8 ed ff ff       	call   801001a6 <bread>
801013de:	89 45 ec             	mov    %eax,-0x14(%ebp)
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
801013e1:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
801013e8:	e9 9d 00 00 00       	jmp    8010148a <balloc+0x102>
      m = 1 << (bi % 8);
801013ed:	8b 45 f0             	mov    -0x10(%ebp),%eax
801013f0:	99                   	cltd   
801013f1:	c1 ea 1d             	shr    $0x1d,%edx
801013f4:	01 d0                	add    %edx,%eax
801013f6:	83 e0 07             	and    $0x7,%eax
801013f9:	29 d0                	sub    %edx,%eax
801013fb:	ba 01 00 00 00       	mov    $0x1,%edx
80101400:	89 c1                	mov    %eax,%ecx
80101402:	d3 e2                	shl    %cl,%edx
80101404:	89 d0                	mov    %edx,%eax
80101406:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if((bp->data[bi/8] & m) == 0){  // Is block free?
80101409:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010140c:	8d 50 07             	lea    0x7(%eax),%edx
8010140f:	85 c0                	test   %eax,%eax
80101411:	0f 48 c2             	cmovs  %edx,%eax
80101414:	c1 f8 03             	sar    $0x3,%eax
80101417:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010141a:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
8010141f:	0f b6 c0             	movzbl %al,%eax
80101422:	23 45 e8             	and    -0x18(%ebp),%eax
80101425:	85 c0                	test   %eax,%eax
80101427:	75 5d                	jne    80101486 <balloc+0xfe>
        bp->data[bi/8] |= m;  // Mark block in use.
80101429:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010142c:	8d 50 07             	lea    0x7(%eax),%edx
8010142f:	85 c0                	test   %eax,%eax
80101431:	0f 48 c2             	cmovs  %edx,%eax
80101434:	c1 f8 03             	sar    $0x3,%eax
80101437:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010143a:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
8010143f:	89 d1                	mov    %edx,%ecx
80101441:	8b 55 e8             	mov    -0x18(%ebp),%edx
80101444:	09 ca                	or     %ecx,%edx
80101446:	89 d1                	mov    %edx,%ecx
80101448:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010144b:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
        log_write(bp);
8010144f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101452:	89 04 24             	mov    %eax,(%esp)
80101455:	e8 2c 1e 00 00       	call   80103286 <log_write>
        brelse(bp);
8010145a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010145d:	89 04 24             	mov    %eax,(%esp)
80101460:	e8 b2 ed ff ff       	call   80100217 <brelse>
        bzero(dev, b + bi);
80101465:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101468:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010146b:	01 c2                	add    %eax,%edx
8010146d:	8b 45 08             	mov    0x8(%ebp),%eax
80101470:	89 54 24 04          	mov    %edx,0x4(%esp)
80101474:	89 04 24             	mov    %eax,(%esp)
80101477:	e8 bb fe ff ff       	call   80101337 <bzero>
        return b + bi;
8010147c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010147f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101482:	01 d0                	add    %edx,%eax
80101484:	eb 4e                	jmp    801014d4 <balloc+0x14c>

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
    bp = bread(dev, BBLOCK(b, sb.ninodes));
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
80101486:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
8010148a:	81 7d f0 ff 0f 00 00 	cmpl   $0xfff,-0x10(%ebp)
80101491:	7f 15                	jg     801014a8 <balloc+0x120>
80101493:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101496:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101499:	01 d0                	add    %edx,%eax
8010149b:	89 c2                	mov    %eax,%edx
8010149d:	8b 45 d8             	mov    -0x28(%ebp),%eax
801014a0:	39 c2                	cmp    %eax,%edx
801014a2:	0f 82 45 ff ff ff    	jb     801013ed <balloc+0x65>
        brelse(bp);
        bzero(dev, b + bi);
        return b + bi;
      }
    }
    brelse(bp);
801014a8:	8b 45 ec             	mov    -0x14(%ebp),%eax
801014ab:	89 04 24             	mov    %eax,(%esp)
801014ae:	e8 64 ed ff ff       	call   80100217 <brelse>
  struct buf *bp;
  struct superblock sb;

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
801014b3:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801014ba:	8b 55 f4             	mov    -0xc(%ebp),%edx
801014bd:	8b 45 d8             	mov    -0x28(%ebp),%eax
801014c0:	39 c2                	cmp    %eax,%edx
801014c2:	0f 82 eb fe ff ff    	jb     801013b3 <balloc+0x2b>
        return b + bi;
      }
    }
    brelse(bp);
  }
  panic("balloc: out of blocks");
801014c8:	c7 04 24 f5 80 10 80 	movl   $0x801080f5,(%esp)
801014cf:	e8 66 f0 ff ff       	call   8010053a <panic>
}
801014d4:	c9                   	leave  
801014d5:	c3                   	ret    

801014d6 <bfree>:

// Free a disk block.
static void
bfree(int dev, uint b)
{
801014d6:	55                   	push   %ebp
801014d7:	89 e5                	mov    %esp,%ebp
801014d9:	83 ec 38             	sub    $0x38,%esp
  struct buf *bp;
  struct superblock sb;
  int bi, m;

  readsb(dev, &sb);
801014dc:	8d 45 dc             	lea    -0x24(%ebp),%eax
801014df:	89 44 24 04          	mov    %eax,0x4(%esp)
801014e3:	8b 45 08             	mov    0x8(%ebp),%eax
801014e6:	89 04 24             	mov    %eax,(%esp)
801014e9:	e8 03 fe ff ff       	call   801012f1 <readsb>
  bp = bread(dev, BBLOCK(b, sb.ninodes));
801014ee:	8b 45 0c             	mov    0xc(%ebp),%eax
801014f1:	c1 e8 0c             	shr    $0xc,%eax
801014f4:	89 c2                	mov    %eax,%edx
801014f6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801014f9:	c1 e8 03             	shr    $0x3,%eax
801014fc:	01 d0                	add    %edx,%eax
801014fe:	8d 50 03             	lea    0x3(%eax),%edx
80101501:	8b 45 08             	mov    0x8(%ebp),%eax
80101504:	89 54 24 04          	mov    %edx,0x4(%esp)
80101508:	89 04 24             	mov    %eax,(%esp)
8010150b:	e8 96 ec ff ff       	call   801001a6 <bread>
80101510:	89 45 f4             	mov    %eax,-0xc(%ebp)
  bi = b % BPB;
80101513:	8b 45 0c             	mov    0xc(%ebp),%eax
80101516:	25 ff 0f 00 00       	and    $0xfff,%eax
8010151b:	89 45 f0             	mov    %eax,-0x10(%ebp)
  m = 1 << (bi % 8);
8010151e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101521:	99                   	cltd   
80101522:	c1 ea 1d             	shr    $0x1d,%edx
80101525:	01 d0                	add    %edx,%eax
80101527:	83 e0 07             	and    $0x7,%eax
8010152a:	29 d0                	sub    %edx,%eax
8010152c:	ba 01 00 00 00       	mov    $0x1,%edx
80101531:	89 c1                	mov    %eax,%ecx
80101533:	d3 e2                	shl    %cl,%edx
80101535:	89 d0                	mov    %edx,%eax
80101537:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if((bp->data[bi/8] & m) == 0)
8010153a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010153d:	8d 50 07             	lea    0x7(%eax),%edx
80101540:	85 c0                	test   %eax,%eax
80101542:	0f 48 c2             	cmovs  %edx,%eax
80101545:	c1 f8 03             	sar    $0x3,%eax
80101548:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010154b:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
80101550:	0f b6 c0             	movzbl %al,%eax
80101553:	23 45 ec             	and    -0x14(%ebp),%eax
80101556:	85 c0                	test   %eax,%eax
80101558:	75 0c                	jne    80101566 <bfree+0x90>
    panic("freeing free block");
8010155a:	c7 04 24 0b 81 10 80 	movl   $0x8010810b,(%esp)
80101561:	e8 d4 ef ff ff       	call   8010053a <panic>
  bp->data[bi/8] &= ~m;
80101566:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101569:	8d 50 07             	lea    0x7(%eax),%edx
8010156c:	85 c0                	test   %eax,%eax
8010156e:	0f 48 c2             	cmovs  %edx,%eax
80101571:	c1 f8 03             	sar    $0x3,%eax
80101574:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101577:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
8010157c:	8b 4d ec             	mov    -0x14(%ebp),%ecx
8010157f:	f7 d1                	not    %ecx
80101581:	21 ca                	and    %ecx,%edx
80101583:	89 d1                	mov    %edx,%ecx
80101585:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101588:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
  log_write(bp);
8010158c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010158f:	89 04 24             	mov    %eax,(%esp)
80101592:	e8 ef 1c 00 00       	call   80103286 <log_write>
  brelse(bp);
80101597:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010159a:	89 04 24             	mov    %eax,(%esp)
8010159d:	e8 75 ec ff ff       	call   80100217 <brelse>
}
801015a2:	c9                   	leave  
801015a3:	c3                   	ret    

801015a4 <iinit>:
  struct inode inode[NINODE];
} icache;

void
iinit(void)
{
801015a4:	55                   	push   %ebp
801015a5:	89 e5                	mov    %esp,%ebp
801015a7:	83 ec 18             	sub    $0x18,%esp
  initlock(&icache.lock, "icache");
801015aa:	c7 44 24 04 1e 81 10 	movl   $0x8010811e,0x4(%esp)
801015b1:	80 
801015b2:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
801015b9:	e8 b9 34 00 00       	call   80104a77 <initlock>
}
801015be:	c9                   	leave  
801015bf:	c3                   	ret    

801015c0 <ialloc>:
//PAGEBREAK!
// Allocate a new inode with the given type on device dev.
// A free inode has a type of zero.
struct inode*
ialloc(uint dev, short type)
{
801015c0:	55                   	push   %ebp
801015c1:	89 e5                	mov    %esp,%ebp
801015c3:	83 ec 38             	sub    $0x38,%esp
801015c6:	8b 45 0c             	mov    0xc(%ebp),%eax
801015c9:	66 89 45 d4          	mov    %ax,-0x2c(%ebp)
  int inum;
  struct buf *bp;
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);
801015cd:	8b 45 08             	mov    0x8(%ebp),%eax
801015d0:	8d 55 dc             	lea    -0x24(%ebp),%edx
801015d3:	89 54 24 04          	mov    %edx,0x4(%esp)
801015d7:	89 04 24             	mov    %eax,(%esp)
801015da:	e8 12 fd ff ff       	call   801012f1 <readsb>

  for(inum = 1; inum < sb.ninodes; inum++){
801015df:	c7 45 f4 01 00 00 00 	movl   $0x1,-0xc(%ebp)
801015e6:	e9 98 00 00 00       	jmp    80101683 <ialloc+0xc3>
    bp = bread(dev, IBLOCK(inum));
801015eb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801015ee:	c1 e8 03             	shr    $0x3,%eax
801015f1:	83 c0 02             	add    $0x2,%eax
801015f4:	89 44 24 04          	mov    %eax,0x4(%esp)
801015f8:	8b 45 08             	mov    0x8(%ebp),%eax
801015fb:	89 04 24             	mov    %eax,(%esp)
801015fe:	e8 a3 eb ff ff       	call   801001a6 <bread>
80101603:	89 45 f0             	mov    %eax,-0x10(%ebp)
    dip = (struct dinode*)bp->data + inum%IPB;
80101606:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101609:	8d 50 18             	lea    0x18(%eax),%edx
8010160c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010160f:	83 e0 07             	and    $0x7,%eax
80101612:	c1 e0 06             	shl    $0x6,%eax
80101615:	01 d0                	add    %edx,%eax
80101617:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if(dip->type == 0){  // a free inode
8010161a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010161d:	0f b7 00             	movzwl (%eax),%eax
80101620:	66 85 c0             	test   %ax,%ax
80101623:	75 4f                	jne    80101674 <ialloc+0xb4>
      memset(dip, 0, sizeof(*dip));
80101625:	c7 44 24 08 40 00 00 	movl   $0x40,0x8(%esp)
8010162c:	00 
8010162d:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80101634:	00 
80101635:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101638:	89 04 24             	mov    %eax,(%esp)
8010163b:	e8 ac 36 00 00       	call   80104cec <memset>
      dip->type = type;
80101640:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101643:	0f b7 55 d4          	movzwl -0x2c(%ebp),%edx
80101647:	66 89 10             	mov    %dx,(%eax)
      log_write(bp);   // mark it allocated on the disk
8010164a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010164d:	89 04 24             	mov    %eax,(%esp)
80101650:	e8 31 1c 00 00       	call   80103286 <log_write>
      brelse(bp);
80101655:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101658:	89 04 24             	mov    %eax,(%esp)
8010165b:	e8 b7 eb ff ff       	call   80100217 <brelse>
      return iget(dev, inum);
80101660:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101663:	89 44 24 04          	mov    %eax,0x4(%esp)
80101667:	8b 45 08             	mov    0x8(%ebp),%eax
8010166a:	89 04 24             	mov    %eax,(%esp)
8010166d:	e8 e5 00 00 00       	call   80101757 <iget>
80101672:	eb 29                	jmp    8010169d <ialloc+0xdd>
    }
    brelse(bp);
80101674:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101677:	89 04 24             	mov    %eax,(%esp)
8010167a:	e8 98 eb ff ff       	call   80100217 <brelse>
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);

  for(inum = 1; inum < sb.ninodes; inum++){
8010167f:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80101683:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101686:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80101689:	39 c2                	cmp    %eax,%edx
8010168b:	0f 82 5a ff ff ff    	jb     801015eb <ialloc+0x2b>
      brelse(bp);
      return iget(dev, inum);
    }
    brelse(bp);
  }
  panic("ialloc: no inodes");
80101691:	c7 04 24 25 81 10 80 	movl   $0x80108125,(%esp)
80101698:	e8 9d ee ff ff       	call   8010053a <panic>
}
8010169d:	c9                   	leave  
8010169e:	c3                   	ret    

8010169f <iupdate>:

// Copy a modified in-memory inode to disk.
void
iupdate(struct inode *ip)
{
8010169f:	55                   	push   %ebp
801016a0:	89 e5                	mov    %esp,%ebp
801016a2:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  bp = bread(ip->dev, IBLOCK(ip->inum));
801016a5:	8b 45 08             	mov    0x8(%ebp),%eax
801016a8:	8b 40 04             	mov    0x4(%eax),%eax
801016ab:	c1 e8 03             	shr    $0x3,%eax
801016ae:	8d 50 02             	lea    0x2(%eax),%edx
801016b1:	8b 45 08             	mov    0x8(%ebp),%eax
801016b4:	8b 00                	mov    (%eax),%eax
801016b6:	89 54 24 04          	mov    %edx,0x4(%esp)
801016ba:	89 04 24             	mov    %eax,(%esp)
801016bd:	e8 e4 ea ff ff       	call   801001a6 <bread>
801016c2:	89 45 f4             	mov    %eax,-0xc(%ebp)
  dip = (struct dinode*)bp->data + ip->inum%IPB;
801016c5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801016c8:	8d 50 18             	lea    0x18(%eax),%edx
801016cb:	8b 45 08             	mov    0x8(%ebp),%eax
801016ce:	8b 40 04             	mov    0x4(%eax),%eax
801016d1:	83 e0 07             	and    $0x7,%eax
801016d4:	c1 e0 06             	shl    $0x6,%eax
801016d7:	01 d0                	add    %edx,%eax
801016d9:	89 45 f0             	mov    %eax,-0x10(%ebp)
  dip->type = ip->type;
801016dc:	8b 45 08             	mov    0x8(%ebp),%eax
801016df:	0f b7 50 10          	movzwl 0x10(%eax),%edx
801016e3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801016e6:	66 89 10             	mov    %dx,(%eax)
  dip->major = ip->major;
801016e9:	8b 45 08             	mov    0x8(%ebp),%eax
801016ec:	0f b7 50 12          	movzwl 0x12(%eax),%edx
801016f0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801016f3:	66 89 50 02          	mov    %dx,0x2(%eax)
  dip->minor = ip->minor;
801016f7:	8b 45 08             	mov    0x8(%ebp),%eax
801016fa:	0f b7 50 14          	movzwl 0x14(%eax),%edx
801016fe:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101701:	66 89 50 04          	mov    %dx,0x4(%eax)
  dip->nlink = ip->nlink;
80101705:	8b 45 08             	mov    0x8(%ebp),%eax
80101708:	0f b7 50 16          	movzwl 0x16(%eax),%edx
8010170c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010170f:	66 89 50 06          	mov    %dx,0x6(%eax)
  dip->size = ip->size;
80101713:	8b 45 08             	mov    0x8(%ebp),%eax
80101716:	8b 50 18             	mov    0x18(%eax),%edx
80101719:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010171c:	89 50 08             	mov    %edx,0x8(%eax)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
8010171f:	8b 45 08             	mov    0x8(%ebp),%eax
80101722:	8d 50 1c             	lea    0x1c(%eax),%edx
80101725:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101728:	83 c0 0c             	add    $0xc,%eax
8010172b:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
80101732:	00 
80101733:	89 54 24 04          	mov    %edx,0x4(%esp)
80101737:	89 04 24             	mov    %eax,(%esp)
8010173a:	e8 7c 36 00 00       	call   80104dbb <memmove>
  log_write(bp);
8010173f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101742:	89 04 24             	mov    %eax,(%esp)
80101745:	e8 3c 1b 00 00       	call   80103286 <log_write>
  brelse(bp);
8010174a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010174d:	89 04 24             	mov    %eax,(%esp)
80101750:	e8 c2 ea ff ff       	call   80100217 <brelse>
}
80101755:	c9                   	leave  
80101756:	c3                   	ret    

80101757 <iget>:
// Find the inode with number inum on device dev
// and return the in-memory copy. Does not lock
// the inode and does not read it from disk.
static struct inode*
iget(uint dev, uint inum)
{
80101757:	55                   	push   %ebp
80101758:	89 e5                	mov    %esp,%ebp
8010175a:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *empty;

  acquire(&icache.lock);
8010175d:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101764:	e8 2f 33 00 00       	call   80104a98 <acquire>

  // Is the inode already cached?
  empty = 0;
80101769:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
80101770:	c7 45 f4 94 e8 10 80 	movl   $0x8010e894,-0xc(%ebp)
80101777:	eb 59                	jmp    801017d2 <iget+0x7b>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
80101779:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010177c:	8b 40 08             	mov    0x8(%eax),%eax
8010177f:	85 c0                	test   %eax,%eax
80101781:	7e 35                	jle    801017b8 <iget+0x61>
80101783:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101786:	8b 00                	mov    (%eax),%eax
80101788:	3b 45 08             	cmp    0x8(%ebp),%eax
8010178b:	75 2b                	jne    801017b8 <iget+0x61>
8010178d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101790:	8b 40 04             	mov    0x4(%eax),%eax
80101793:	3b 45 0c             	cmp    0xc(%ebp),%eax
80101796:	75 20                	jne    801017b8 <iget+0x61>
      ip->ref++;
80101798:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010179b:	8b 40 08             	mov    0x8(%eax),%eax
8010179e:	8d 50 01             	lea    0x1(%eax),%edx
801017a1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017a4:	89 50 08             	mov    %edx,0x8(%eax)
      release(&icache.lock);
801017a7:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
801017ae:	e8 47 33 00 00       	call   80104afa <release>
      return ip;
801017b3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017b6:	eb 6f                	jmp    80101827 <iget+0xd0>
    }
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
801017b8:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017bc:	75 10                	jne    801017ce <iget+0x77>
801017be:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017c1:	8b 40 08             	mov    0x8(%eax),%eax
801017c4:	85 c0                	test   %eax,%eax
801017c6:	75 06                	jne    801017ce <iget+0x77>
      empty = ip;
801017c8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017cb:	89 45 f0             	mov    %eax,-0x10(%ebp)

  acquire(&icache.lock);

  // Is the inode already cached?
  empty = 0;
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
801017ce:	83 45 f4 50          	addl   $0x50,-0xc(%ebp)
801017d2:	81 7d f4 34 f8 10 80 	cmpl   $0x8010f834,-0xc(%ebp)
801017d9:	72 9e                	jb     80101779 <iget+0x22>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
      empty = ip;
  }

  // Recycle an inode cache entry.
  if(empty == 0)
801017db:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017df:	75 0c                	jne    801017ed <iget+0x96>
    panic("iget: no inodes");
801017e1:	c7 04 24 37 81 10 80 	movl   $0x80108137,(%esp)
801017e8:	e8 4d ed ff ff       	call   8010053a <panic>

  ip = empty;
801017ed:	8b 45 f0             	mov    -0x10(%ebp),%eax
801017f0:	89 45 f4             	mov    %eax,-0xc(%ebp)
  ip->dev = dev;
801017f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017f6:	8b 55 08             	mov    0x8(%ebp),%edx
801017f9:	89 10                	mov    %edx,(%eax)
  ip->inum = inum;
801017fb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017fe:	8b 55 0c             	mov    0xc(%ebp),%edx
80101801:	89 50 04             	mov    %edx,0x4(%eax)
  ip->ref = 1;
80101804:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101807:	c7 40 08 01 00 00 00 	movl   $0x1,0x8(%eax)
  ip->flags = 0;
8010180e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101811:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  release(&icache.lock);
80101818:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
8010181f:	e8 d6 32 00 00       	call   80104afa <release>

  return ip;
80101824:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80101827:	c9                   	leave  
80101828:	c3                   	ret    

80101829 <idup>:

// Increment reference count for ip.
// Returns ip to enable ip = idup(ip1) idiom.
struct inode*
idup(struct inode *ip)
{
80101829:	55                   	push   %ebp
8010182a:	89 e5                	mov    %esp,%ebp
8010182c:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
8010182f:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101836:	e8 5d 32 00 00       	call   80104a98 <acquire>
  ip->ref++;
8010183b:	8b 45 08             	mov    0x8(%ebp),%eax
8010183e:	8b 40 08             	mov    0x8(%eax),%eax
80101841:	8d 50 01             	lea    0x1(%eax),%edx
80101844:	8b 45 08             	mov    0x8(%ebp),%eax
80101847:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
8010184a:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101851:	e8 a4 32 00 00       	call   80104afa <release>
  return ip;
80101856:	8b 45 08             	mov    0x8(%ebp),%eax
}
80101859:	c9                   	leave  
8010185a:	c3                   	ret    

8010185b <ilock>:

// Lock the given inode.
// Reads the inode from disk if necessary.
void
ilock(struct inode *ip)
{
8010185b:	55                   	push   %ebp
8010185c:	89 e5                	mov    %esp,%ebp
8010185e:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  if(ip == 0 || ip->ref < 1)
80101861:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80101865:	74 0a                	je     80101871 <ilock+0x16>
80101867:	8b 45 08             	mov    0x8(%ebp),%eax
8010186a:	8b 40 08             	mov    0x8(%eax),%eax
8010186d:	85 c0                	test   %eax,%eax
8010186f:	7f 0c                	jg     8010187d <ilock+0x22>
    panic("ilock");
80101871:	c7 04 24 47 81 10 80 	movl   $0x80108147,(%esp)
80101878:	e8 bd ec ff ff       	call   8010053a <panic>

  acquire(&icache.lock);
8010187d:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101884:	e8 0f 32 00 00       	call   80104a98 <acquire>
  while(ip->flags & I_BUSY)
80101889:	eb 13                	jmp    8010189e <ilock+0x43>
    sleep(ip, &icache.lock);
8010188b:	c7 44 24 04 60 e8 10 	movl   $0x8010e860,0x4(%esp)
80101892:	80 
80101893:	8b 45 08             	mov    0x8(%ebp),%eax
80101896:	89 04 24             	mov    %eax,(%esp)
80101899:	e8 30 2f 00 00       	call   801047ce <sleep>

  if(ip == 0 || ip->ref < 1)
    panic("ilock");

  acquire(&icache.lock);
  while(ip->flags & I_BUSY)
8010189e:	8b 45 08             	mov    0x8(%ebp),%eax
801018a1:	8b 40 0c             	mov    0xc(%eax),%eax
801018a4:	83 e0 01             	and    $0x1,%eax
801018a7:	85 c0                	test   %eax,%eax
801018a9:	75 e0                	jne    8010188b <ilock+0x30>
    sleep(ip, &icache.lock);
  ip->flags |= I_BUSY;
801018ab:	8b 45 08             	mov    0x8(%ebp),%eax
801018ae:	8b 40 0c             	mov    0xc(%eax),%eax
801018b1:	83 c8 01             	or     $0x1,%eax
801018b4:	89 c2                	mov    %eax,%edx
801018b6:	8b 45 08             	mov    0x8(%ebp),%eax
801018b9:	89 50 0c             	mov    %edx,0xc(%eax)
  release(&icache.lock);
801018bc:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
801018c3:	e8 32 32 00 00       	call   80104afa <release>

  if(!(ip->flags & I_VALID)){
801018c8:	8b 45 08             	mov    0x8(%ebp),%eax
801018cb:	8b 40 0c             	mov    0xc(%eax),%eax
801018ce:	83 e0 02             	and    $0x2,%eax
801018d1:	85 c0                	test   %eax,%eax
801018d3:	0f 85 ce 00 00 00    	jne    801019a7 <ilock+0x14c>
    bp = bread(ip->dev, IBLOCK(ip->inum));
801018d9:	8b 45 08             	mov    0x8(%ebp),%eax
801018dc:	8b 40 04             	mov    0x4(%eax),%eax
801018df:	c1 e8 03             	shr    $0x3,%eax
801018e2:	8d 50 02             	lea    0x2(%eax),%edx
801018e5:	8b 45 08             	mov    0x8(%ebp),%eax
801018e8:	8b 00                	mov    (%eax),%eax
801018ea:	89 54 24 04          	mov    %edx,0x4(%esp)
801018ee:	89 04 24             	mov    %eax,(%esp)
801018f1:	e8 b0 e8 ff ff       	call   801001a6 <bread>
801018f6:	89 45 f4             	mov    %eax,-0xc(%ebp)
    dip = (struct dinode*)bp->data + ip->inum%IPB;
801018f9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801018fc:	8d 50 18             	lea    0x18(%eax),%edx
801018ff:	8b 45 08             	mov    0x8(%ebp),%eax
80101902:	8b 40 04             	mov    0x4(%eax),%eax
80101905:	83 e0 07             	and    $0x7,%eax
80101908:	c1 e0 06             	shl    $0x6,%eax
8010190b:	01 d0                	add    %edx,%eax
8010190d:	89 45 f0             	mov    %eax,-0x10(%ebp)
    ip->type = dip->type;
80101910:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101913:	0f b7 10             	movzwl (%eax),%edx
80101916:	8b 45 08             	mov    0x8(%ebp),%eax
80101919:	66 89 50 10          	mov    %dx,0x10(%eax)
    ip->major = dip->major;
8010191d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101920:	0f b7 50 02          	movzwl 0x2(%eax),%edx
80101924:	8b 45 08             	mov    0x8(%ebp),%eax
80101927:	66 89 50 12          	mov    %dx,0x12(%eax)
    ip->minor = dip->minor;
8010192b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010192e:	0f b7 50 04          	movzwl 0x4(%eax),%edx
80101932:	8b 45 08             	mov    0x8(%ebp),%eax
80101935:	66 89 50 14          	mov    %dx,0x14(%eax)
    ip->nlink = dip->nlink;
80101939:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010193c:	0f b7 50 06          	movzwl 0x6(%eax),%edx
80101940:	8b 45 08             	mov    0x8(%ebp),%eax
80101943:	66 89 50 16          	mov    %dx,0x16(%eax)
    ip->size = dip->size;
80101947:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010194a:	8b 50 08             	mov    0x8(%eax),%edx
8010194d:	8b 45 08             	mov    0x8(%ebp),%eax
80101950:	89 50 18             	mov    %edx,0x18(%eax)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
80101953:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101956:	8d 50 0c             	lea    0xc(%eax),%edx
80101959:	8b 45 08             	mov    0x8(%ebp),%eax
8010195c:	83 c0 1c             	add    $0x1c,%eax
8010195f:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
80101966:	00 
80101967:	89 54 24 04          	mov    %edx,0x4(%esp)
8010196b:	89 04 24             	mov    %eax,(%esp)
8010196e:	e8 48 34 00 00       	call   80104dbb <memmove>
    brelse(bp);
80101973:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101976:	89 04 24             	mov    %eax,(%esp)
80101979:	e8 99 e8 ff ff       	call   80100217 <brelse>
    ip->flags |= I_VALID;
8010197e:	8b 45 08             	mov    0x8(%ebp),%eax
80101981:	8b 40 0c             	mov    0xc(%eax),%eax
80101984:	83 c8 02             	or     $0x2,%eax
80101987:	89 c2                	mov    %eax,%edx
80101989:	8b 45 08             	mov    0x8(%ebp),%eax
8010198c:	89 50 0c             	mov    %edx,0xc(%eax)
    if(ip->type == 0)
8010198f:	8b 45 08             	mov    0x8(%ebp),%eax
80101992:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101996:	66 85 c0             	test   %ax,%ax
80101999:	75 0c                	jne    801019a7 <ilock+0x14c>
      panic("ilock: no type");
8010199b:	c7 04 24 4d 81 10 80 	movl   $0x8010814d,(%esp)
801019a2:	e8 93 eb ff ff       	call   8010053a <panic>
  }
}
801019a7:	c9                   	leave  
801019a8:	c3                   	ret    

801019a9 <iunlock>:

// Unlock the given inode.
void
iunlock(struct inode *ip)
{
801019a9:	55                   	push   %ebp
801019aa:	89 e5                	mov    %esp,%ebp
801019ac:	83 ec 18             	sub    $0x18,%esp
  if(ip == 0 || !(ip->flags & I_BUSY) || ip->ref < 1)
801019af:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801019b3:	74 17                	je     801019cc <iunlock+0x23>
801019b5:	8b 45 08             	mov    0x8(%ebp),%eax
801019b8:	8b 40 0c             	mov    0xc(%eax),%eax
801019bb:	83 e0 01             	and    $0x1,%eax
801019be:	85 c0                	test   %eax,%eax
801019c0:	74 0a                	je     801019cc <iunlock+0x23>
801019c2:	8b 45 08             	mov    0x8(%ebp),%eax
801019c5:	8b 40 08             	mov    0x8(%eax),%eax
801019c8:	85 c0                	test   %eax,%eax
801019ca:	7f 0c                	jg     801019d8 <iunlock+0x2f>
    panic("iunlock");
801019cc:	c7 04 24 5c 81 10 80 	movl   $0x8010815c,(%esp)
801019d3:	e8 62 eb ff ff       	call   8010053a <panic>

  acquire(&icache.lock);
801019d8:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
801019df:	e8 b4 30 00 00       	call   80104a98 <acquire>
  ip->flags &= ~I_BUSY;
801019e4:	8b 45 08             	mov    0x8(%ebp),%eax
801019e7:	8b 40 0c             	mov    0xc(%eax),%eax
801019ea:	83 e0 fe             	and    $0xfffffffe,%eax
801019ed:	89 c2                	mov    %eax,%edx
801019ef:	8b 45 08             	mov    0x8(%ebp),%eax
801019f2:	89 50 0c             	mov    %edx,0xc(%eax)
  wakeup(ip);
801019f5:	8b 45 08             	mov    0x8(%ebp),%eax
801019f8:	89 04 24             	mov    %eax,(%esp)
801019fb:	e8 a7 2e 00 00       	call   801048a7 <wakeup>
  release(&icache.lock);
80101a00:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101a07:	e8 ee 30 00 00       	call   80104afa <release>
}
80101a0c:	c9                   	leave  
80101a0d:	c3                   	ret    

80101a0e <iput>:
// to it, free the inode (and its content) on disk.
// All calls to iput() must be inside a transaction in
// case it has to free the inode.
void
iput(struct inode *ip)
{
80101a0e:	55                   	push   %ebp
80101a0f:	89 e5                	mov    %esp,%ebp
80101a11:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
80101a14:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101a1b:	e8 78 30 00 00       	call   80104a98 <acquire>
  if(ip->ref == 1 && (ip->flags & I_VALID) && ip->nlink == 0){
80101a20:	8b 45 08             	mov    0x8(%ebp),%eax
80101a23:	8b 40 08             	mov    0x8(%eax),%eax
80101a26:	83 f8 01             	cmp    $0x1,%eax
80101a29:	0f 85 93 00 00 00    	jne    80101ac2 <iput+0xb4>
80101a2f:	8b 45 08             	mov    0x8(%ebp),%eax
80101a32:	8b 40 0c             	mov    0xc(%eax),%eax
80101a35:	83 e0 02             	and    $0x2,%eax
80101a38:	85 c0                	test   %eax,%eax
80101a3a:	0f 84 82 00 00 00    	je     80101ac2 <iput+0xb4>
80101a40:	8b 45 08             	mov    0x8(%ebp),%eax
80101a43:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80101a47:	66 85 c0             	test   %ax,%ax
80101a4a:	75 76                	jne    80101ac2 <iput+0xb4>
    // inode has no links: truncate and free inode.
    if(ip->flags & I_BUSY)
80101a4c:	8b 45 08             	mov    0x8(%ebp),%eax
80101a4f:	8b 40 0c             	mov    0xc(%eax),%eax
80101a52:	83 e0 01             	and    $0x1,%eax
80101a55:	85 c0                	test   %eax,%eax
80101a57:	74 0c                	je     80101a65 <iput+0x57>
      panic("iput busy");
80101a59:	c7 04 24 64 81 10 80 	movl   $0x80108164,(%esp)
80101a60:	e8 d5 ea ff ff       	call   8010053a <panic>
    ip->flags |= I_BUSY;
80101a65:	8b 45 08             	mov    0x8(%ebp),%eax
80101a68:	8b 40 0c             	mov    0xc(%eax),%eax
80101a6b:	83 c8 01             	or     $0x1,%eax
80101a6e:	89 c2                	mov    %eax,%edx
80101a70:	8b 45 08             	mov    0x8(%ebp),%eax
80101a73:	89 50 0c             	mov    %edx,0xc(%eax)
    release(&icache.lock);
80101a76:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101a7d:	e8 78 30 00 00       	call   80104afa <release>
    itrunc(ip);
80101a82:	8b 45 08             	mov    0x8(%ebp),%eax
80101a85:	89 04 24             	mov    %eax,(%esp)
80101a88:	e8 7d 01 00 00       	call   80101c0a <itrunc>
    ip->type = 0;
80101a8d:	8b 45 08             	mov    0x8(%ebp),%eax
80101a90:	66 c7 40 10 00 00    	movw   $0x0,0x10(%eax)
    iupdate(ip);
80101a96:	8b 45 08             	mov    0x8(%ebp),%eax
80101a99:	89 04 24             	mov    %eax,(%esp)
80101a9c:	e8 fe fb ff ff       	call   8010169f <iupdate>
    acquire(&icache.lock);
80101aa1:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101aa8:	e8 eb 2f 00 00       	call   80104a98 <acquire>
    ip->flags = 0;
80101aad:	8b 45 08             	mov    0x8(%ebp),%eax
80101ab0:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    wakeup(ip);
80101ab7:	8b 45 08             	mov    0x8(%ebp),%eax
80101aba:	89 04 24             	mov    %eax,(%esp)
80101abd:	e8 e5 2d 00 00       	call   801048a7 <wakeup>
  }
  ip->ref--;
80101ac2:	8b 45 08             	mov    0x8(%ebp),%eax
80101ac5:	8b 40 08             	mov    0x8(%eax),%eax
80101ac8:	8d 50 ff             	lea    -0x1(%eax),%edx
80101acb:	8b 45 08             	mov    0x8(%ebp),%eax
80101ace:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
80101ad1:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101ad8:	e8 1d 30 00 00       	call   80104afa <release>
}
80101add:	c9                   	leave  
80101ade:	c3                   	ret    

80101adf <iunlockput>:

// Common idiom: unlock, then put.
void
iunlockput(struct inode *ip)
{
80101adf:	55                   	push   %ebp
80101ae0:	89 e5                	mov    %esp,%ebp
80101ae2:	83 ec 18             	sub    $0x18,%esp
  iunlock(ip);
80101ae5:	8b 45 08             	mov    0x8(%ebp),%eax
80101ae8:	89 04 24             	mov    %eax,(%esp)
80101aeb:	e8 b9 fe ff ff       	call   801019a9 <iunlock>
  iput(ip);
80101af0:	8b 45 08             	mov    0x8(%ebp),%eax
80101af3:	89 04 24             	mov    %eax,(%esp)
80101af6:	e8 13 ff ff ff       	call   80101a0e <iput>
}
80101afb:	c9                   	leave  
80101afc:	c3                   	ret    

80101afd <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
80101afd:	55                   	push   %ebp
80101afe:	89 e5                	mov    %esp,%ebp
80101b00:	53                   	push   %ebx
80101b01:	83 ec 24             	sub    $0x24,%esp
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
80101b04:	83 7d 0c 0b          	cmpl   $0xb,0xc(%ebp)
80101b08:	77 3e                	ja     80101b48 <bmap+0x4b>
    if((addr = ip->addrs[bn]) == 0)
80101b0a:	8b 45 08             	mov    0x8(%ebp),%eax
80101b0d:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b10:	83 c2 04             	add    $0x4,%edx
80101b13:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101b17:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b1a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b1e:	75 20                	jne    80101b40 <bmap+0x43>
      ip->addrs[bn] = addr = balloc(ip->dev);
80101b20:	8b 45 08             	mov    0x8(%ebp),%eax
80101b23:	8b 00                	mov    (%eax),%eax
80101b25:	89 04 24             	mov    %eax,(%esp)
80101b28:	e8 5b f8 ff ff       	call   80101388 <balloc>
80101b2d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b30:	8b 45 08             	mov    0x8(%ebp),%eax
80101b33:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b36:	8d 4a 04             	lea    0x4(%edx),%ecx
80101b39:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b3c:	89 54 88 0c          	mov    %edx,0xc(%eax,%ecx,4)
    return addr;
80101b40:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101b43:	e9 bc 00 00 00       	jmp    80101c04 <bmap+0x107>
  }
  bn -= NDIRECT;
80101b48:	83 6d 0c 0c          	subl   $0xc,0xc(%ebp)

  if(bn < NINDIRECT){
80101b4c:	83 7d 0c 7f          	cmpl   $0x7f,0xc(%ebp)
80101b50:	0f 87 a2 00 00 00    	ja     80101bf8 <bmap+0xfb>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
80101b56:	8b 45 08             	mov    0x8(%ebp),%eax
80101b59:	8b 40 4c             	mov    0x4c(%eax),%eax
80101b5c:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b5f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b63:	75 19                	jne    80101b7e <bmap+0x81>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
80101b65:	8b 45 08             	mov    0x8(%ebp),%eax
80101b68:	8b 00                	mov    (%eax),%eax
80101b6a:	89 04 24             	mov    %eax,(%esp)
80101b6d:	e8 16 f8 ff ff       	call   80101388 <balloc>
80101b72:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b75:	8b 45 08             	mov    0x8(%ebp),%eax
80101b78:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b7b:	89 50 4c             	mov    %edx,0x4c(%eax)
    bp = bread(ip->dev, addr);
80101b7e:	8b 45 08             	mov    0x8(%ebp),%eax
80101b81:	8b 00                	mov    (%eax),%eax
80101b83:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b86:	89 54 24 04          	mov    %edx,0x4(%esp)
80101b8a:	89 04 24             	mov    %eax,(%esp)
80101b8d:	e8 14 e6 ff ff       	call   801001a6 <bread>
80101b92:	89 45 f0             	mov    %eax,-0x10(%ebp)
    a = (uint*)bp->data;
80101b95:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101b98:	83 c0 18             	add    $0x18,%eax
80101b9b:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if((addr = a[bn]) == 0){
80101b9e:	8b 45 0c             	mov    0xc(%ebp),%eax
80101ba1:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101ba8:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101bab:	01 d0                	add    %edx,%eax
80101bad:	8b 00                	mov    (%eax),%eax
80101baf:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bb2:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101bb6:	75 30                	jne    80101be8 <bmap+0xeb>
      a[bn] = addr = balloc(ip->dev);
80101bb8:	8b 45 0c             	mov    0xc(%ebp),%eax
80101bbb:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101bc2:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101bc5:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
80101bc8:	8b 45 08             	mov    0x8(%ebp),%eax
80101bcb:	8b 00                	mov    (%eax),%eax
80101bcd:	89 04 24             	mov    %eax,(%esp)
80101bd0:	e8 b3 f7 ff ff       	call   80101388 <balloc>
80101bd5:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bd8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bdb:	89 03                	mov    %eax,(%ebx)
      log_write(bp);
80101bdd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101be0:	89 04 24             	mov    %eax,(%esp)
80101be3:	e8 9e 16 00 00       	call   80103286 <log_write>
    }
    brelse(bp);
80101be8:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101beb:	89 04 24             	mov    %eax,(%esp)
80101bee:	e8 24 e6 ff ff       	call   80100217 <brelse>
    return addr;
80101bf3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bf6:	eb 0c                	jmp    80101c04 <bmap+0x107>
  }

  panic("bmap: out of range");
80101bf8:	c7 04 24 6e 81 10 80 	movl   $0x8010816e,(%esp)
80101bff:	e8 36 e9 ff ff       	call   8010053a <panic>
}
80101c04:	83 c4 24             	add    $0x24,%esp
80101c07:	5b                   	pop    %ebx
80101c08:	5d                   	pop    %ebp
80101c09:	c3                   	ret    

80101c0a <itrunc>:
// to it (no directory entries referring to it)
// and has no in-memory reference to it (is
// not an open file or current directory).
static void
itrunc(struct inode *ip)
{
80101c0a:	55                   	push   %ebp
80101c0b:	89 e5                	mov    %esp,%ebp
80101c0d:	83 ec 28             	sub    $0x28,%esp
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c10:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101c17:	eb 44                	jmp    80101c5d <itrunc+0x53>
    if(ip->addrs[i]){
80101c19:	8b 45 08             	mov    0x8(%ebp),%eax
80101c1c:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c1f:	83 c2 04             	add    $0x4,%edx
80101c22:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101c26:	85 c0                	test   %eax,%eax
80101c28:	74 2f                	je     80101c59 <itrunc+0x4f>
      bfree(ip->dev, ip->addrs[i]);
80101c2a:	8b 45 08             	mov    0x8(%ebp),%eax
80101c2d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c30:	83 c2 04             	add    $0x4,%edx
80101c33:	8b 54 90 0c          	mov    0xc(%eax,%edx,4),%edx
80101c37:	8b 45 08             	mov    0x8(%ebp),%eax
80101c3a:	8b 00                	mov    (%eax),%eax
80101c3c:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c40:	89 04 24             	mov    %eax,(%esp)
80101c43:	e8 8e f8 ff ff       	call   801014d6 <bfree>
      ip->addrs[i] = 0;
80101c48:	8b 45 08             	mov    0x8(%ebp),%eax
80101c4b:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c4e:	83 c2 04             	add    $0x4,%edx
80101c51:	c7 44 90 0c 00 00 00 	movl   $0x0,0xc(%eax,%edx,4)
80101c58:	00 
{
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c59:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80101c5d:	83 7d f4 0b          	cmpl   $0xb,-0xc(%ebp)
80101c61:	7e b6                	jle    80101c19 <itrunc+0xf>
      bfree(ip->dev, ip->addrs[i]);
      ip->addrs[i] = 0;
    }
  }
  
  if(ip->addrs[NDIRECT]){
80101c63:	8b 45 08             	mov    0x8(%ebp),%eax
80101c66:	8b 40 4c             	mov    0x4c(%eax),%eax
80101c69:	85 c0                	test   %eax,%eax
80101c6b:	0f 84 9b 00 00 00    	je     80101d0c <itrunc+0x102>
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
80101c71:	8b 45 08             	mov    0x8(%ebp),%eax
80101c74:	8b 50 4c             	mov    0x4c(%eax),%edx
80101c77:	8b 45 08             	mov    0x8(%ebp),%eax
80101c7a:	8b 00                	mov    (%eax),%eax
80101c7c:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c80:	89 04 24             	mov    %eax,(%esp)
80101c83:	e8 1e e5 ff ff       	call   801001a6 <bread>
80101c88:	89 45 ec             	mov    %eax,-0x14(%ebp)
    a = (uint*)bp->data;
80101c8b:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101c8e:	83 c0 18             	add    $0x18,%eax
80101c91:	89 45 e8             	mov    %eax,-0x18(%ebp)
    for(j = 0; j < NINDIRECT; j++){
80101c94:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80101c9b:	eb 3b                	jmp    80101cd8 <itrunc+0xce>
      if(a[j])
80101c9d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ca0:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101ca7:	8b 45 e8             	mov    -0x18(%ebp),%eax
80101caa:	01 d0                	add    %edx,%eax
80101cac:	8b 00                	mov    (%eax),%eax
80101cae:	85 c0                	test   %eax,%eax
80101cb0:	74 22                	je     80101cd4 <itrunc+0xca>
        bfree(ip->dev, a[j]);
80101cb2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cb5:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101cbc:	8b 45 e8             	mov    -0x18(%ebp),%eax
80101cbf:	01 d0                	add    %edx,%eax
80101cc1:	8b 10                	mov    (%eax),%edx
80101cc3:	8b 45 08             	mov    0x8(%ebp),%eax
80101cc6:	8b 00                	mov    (%eax),%eax
80101cc8:	89 54 24 04          	mov    %edx,0x4(%esp)
80101ccc:	89 04 24             	mov    %eax,(%esp)
80101ccf:	e8 02 f8 ff ff       	call   801014d6 <bfree>
  }
  
  if(ip->addrs[NDIRECT]){
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    a = (uint*)bp->data;
    for(j = 0; j < NINDIRECT; j++){
80101cd4:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80101cd8:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cdb:	83 f8 7f             	cmp    $0x7f,%eax
80101cde:	76 bd                	jbe    80101c9d <itrunc+0x93>
      if(a[j])
        bfree(ip->dev, a[j]);
    }
    brelse(bp);
80101ce0:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101ce3:	89 04 24             	mov    %eax,(%esp)
80101ce6:	e8 2c e5 ff ff       	call   80100217 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
80101ceb:	8b 45 08             	mov    0x8(%ebp),%eax
80101cee:	8b 50 4c             	mov    0x4c(%eax),%edx
80101cf1:	8b 45 08             	mov    0x8(%ebp),%eax
80101cf4:	8b 00                	mov    (%eax),%eax
80101cf6:	89 54 24 04          	mov    %edx,0x4(%esp)
80101cfa:	89 04 24             	mov    %eax,(%esp)
80101cfd:	e8 d4 f7 ff ff       	call   801014d6 <bfree>
    ip->addrs[NDIRECT] = 0;
80101d02:	8b 45 08             	mov    0x8(%ebp),%eax
80101d05:	c7 40 4c 00 00 00 00 	movl   $0x0,0x4c(%eax)
  }

  ip->size = 0;
80101d0c:	8b 45 08             	mov    0x8(%ebp),%eax
80101d0f:	c7 40 18 00 00 00 00 	movl   $0x0,0x18(%eax)
  iupdate(ip);
80101d16:	8b 45 08             	mov    0x8(%ebp),%eax
80101d19:	89 04 24             	mov    %eax,(%esp)
80101d1c:	e8 7e f9 ff ff       	call   8010169f <iupdate>
}
80101d21:	c9                   	leave  
80101d22:	c3                   	ret    

80101d23 <stati>:

// Copy stat information from inode.
void
stati(struct inode *ip, struct stat *st)
{
80101d23:	55                   	push   %ebp
80101d24:	89 e5                	mov    %esp,%ebp
  st->dev = ip->dev;
80101d26:	8b 45 08             	mov    0x8(%ebp),%eax
80101d29:	8b 00                	mov    (%eax),%eax
80101d2b:	89 c2                	mov    %eax,%edx
80101d2d:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d30:	89 50 04             	mov    %edx,0x4(%eax)
  st->ino = ip->inum;
80101d33:	8b 45 08             	mov    0x8(%ebp),%eax
80101d36:	8b 50 04             	mov    0x4(%eax),%edx
80101d39:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d3c:	89 50 08             	mov    %edx,0x8(%eax)
  st->type = ip->type;
80101d3f:	8b 45 08             	mov    0x8(%ebp),%eax
80101d42:	0f b7 50 10          	movzwl 0x10(%eax),%edx
80101d46:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d49:	66 89 10             	mov    %dx,(%eax)
  st->nlink = ip->nlink;
80101d4c:	8b 45 08             	mov    0x8(%ebp),%eax
80101d4f:	0f b7 50 16          	movzwl 0x16(%eax),%edx
80101d53:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d56:	66 89 50 0c          	mov    %dx,0xc(%eax)
  st->size = ip->size;
80101d5a:	8b 45 08             	mov    0x8(%ebp),%eax
80101d5d:	8b 50 18             	mov    0x18(%eax),%edx
80101d60:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d63:	89 50 10             	mov    %edx,0x10(%eax)
}
80101d66:	5d                   	pop    %ebp
80101d67:	c3                   	ret    

80101d68 <readi>:

//PAGEBREAK!
// Read data from inode.
int
readi(struct inode *ip, char *dst, uint off, uint n)
{
80101d68:	55                   	push   %ebp
80101d69:	89 e5                	mov    %esp,%ebp
80101d6b:	83 ec 28             	sub    $0x28,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101d6e:	8b 45 08             	mov    0x8(%ebp),%eax
80101d71:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101d75:	66 83 f8 03          	cmp    $0x3,%ax
80101d79:	75 60                	jne    80101ddb <readi+0x73>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
80101d7b:	8b 45 08             	mov    0x8(%ebp),%eax
80101d7e:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d82:	66 85 c0             	test   %ax,%ax
80101d85:	78 20                	js     80101da7 <readi+0x3f>
80101d87:	8b 45 08             	mov    0x8(%ebp),%eax
80101d8a:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d8e:	66 83 f8 09          	cmp    $0x9,%ax
80101d92:	7f 13                	jg     80101da7 <readi+0x3f>
80101d94:	8b 45 08             	mov    0x8(%ebp),%eax
80101d97:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d9b:	98                   	cwtl   
80101d9c:	8b 04 c5 00 e8 10 80 	mov    -0x7fef1800(,%eax,8),%eax
80101da3:	85 c0                	test   %eax,%eax
80101da5:	75 0a                	jne    80101db1 <readi+0x49>
      return -1;
80101da7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101dac:	e9 19 01 00 00       	jmp    80101eca <readi+0x162>
    return devsw[ip->major].read(ip, dst, n);
80101db1:	8b 45 08             	mov    0x8(%ebp),%eax
80101db4:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101db8:	98                   	cwtl   
80101db9:	8b 04 c5 00 e8 10 80 	mov    -0x7fef1800(,%eax,8),%eax
80101dc0:	8b 55 14             	mov    0x14(%ebp),%edx
80101dc3:	89 54 24 08          	mov    %edx,0x8(%esp)
80101dc7:	8b 55 0c             	mov    0xc(%ebp),%edx
80101dca:	89 54 24 04          	mov    %edx,0x4(%esp)
80101dce:	8b 55 08             	mov    0x8(%ebp),%edx
80101dd1:	89 14 24             	mov    %edx,(%esp)
80101dd4:	ff d0                	call   *%eax
80101dd6:	e9 ef 00 00 00       	jmp    80101eca <readi+0x162>
  }

  if(off > ip->size || off + n < off)
80101ddb:	8b 45 08             	mov    0x8(%ebp),%eax
80101dde:	8b 40 18             	mov    0x18(%eax),%eax
80101de1:	3b 45 10             	cmp    0x10(%ebp),%eax
80101de4:	72 0d                	jb     80101df3 <readi+0x8b>
80101de6:	8b 45 14             	mov    0x14(%ebp),%eax
80101de9:	8b 55 10             	mov    0x10(%ebp),%edx
80101dec:	01 d0                	add    %edx,%eax
80101dee:	3b 45 10             	cmp    0x10(%ebp),%eax
80101df1:	73 0a                	jae    80101dfd <readi+0x95>
    return -1;
80101df3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101df8:	e9 cd 00 00 00       	jmp    80101eca <readi+0x162>
  if(off + n > ip->size)
80101dfd:	8b 45 14             	mov    0x14(%ebp),%eax
80101e00:	8b 55 10             	mov    0x10(%ebp),%edx
80101e03:	01 c2                	add    %eax,%edx
80101e05:	8b 45 08             	mov    0x8(%ebp),%eax
80101e08:	8b 40 18             	mov    0x18(%eax),%eax
80101e0b:	39 c2                	cmp    %eax,%edx
80101e0d:	76 0c                	jbe    80101e1b <readi+0xb3>
    n = ip->size - off;
80101e0f:	8b 45 08             	mov    0x8(%ebp),%eax
80101e12:	8b 40 18             	mov    0x18(%eax),%eax
80101e15:	2b 45 10             	sub    0x10(%ebp),%eax
80101e18:	89 45 14             	mov    %eax,0x14(%ebp)

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101e1b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101e22:	e9 94 00 00 00       	jmp    80101ebb <readi+0x153>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101e27:	8b 45 10             	mov    0x10(%ebp),%eax
80101e2a:	c1 e8 09             	shr    $0x9,%eax
80101e2d:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e31:	8b 45 08             	mov    0x8(%ebp),%eax
80101e34:	89 04 24             	mov    %eax,(%esp)
80101e37:	e8 c1 fc ff ff       	call   80101afd <bmap>
80101e3c:	8b 55 08             	mov    0x8(%ebp),%edx
80101e3f:	8b 12                	mov    (%edx),%edx
80101e41:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e45:	89 14 24             	mov    %edx,(%esp)
80101e48:	e8 59 e3 ff ff       	call   801001a6 <bread>
80101e4d:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101e50:	8b 45 10             	mov    0x10(%ebp),%eax
80101e53:	25 ff 01 00 00       	and    $0x1ff,%eax
80101e58:	89 c2                	mov    %eax,%edx
80101e5a:	b8 00 02 00 00       	mov    $0x200,%eax
80101e5f:	29 d0                	sub    %edx,%eax
80101e61:	89 c2                	mov    %eax,%edx
80101e63:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101e66:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101e69:	29 c1                	sub    %eax,%ecx
80101e6b:	89 c8                	mov    %ecx,%eax
80101e6d:	39 c2                	cmp    %eax,%edx
80101e6f:	0f 46 c2             	cmovbe %edx,%eax
80101e72:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dst, bp->data + off%BSIZE, m);
80101e75:	8b 45 10             	mov    0x10(%ebp),%eax
80101e78:	25 ff 01 00 00       	and    $0x1ff,%eax
80101e7d:	8d 50 10             	lea    0x10(%eax),%edx
80101e80:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101e83:	01 d0                	add    %edx,%eax
80101e85:	8d 50 08             	lea    0x8(%eax),%edx
80101e88:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101e8b:	89 44 24 08          	mov    %eax,0x8(%esp)
80101e8f:	89 54 24 04          	mov    %edx,0x4(%esp)
80101e93:	8b 45 0c             	mov    0xc(%ebp),%eax
80101e96:	89 04 24             	mov    %eax,(%esp)
80101e99:	e8 1d 2f 00 00       	call   80104dbb <memmove>
    brelse(bp);
80101e9e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ea1:	89 04 24             	mov    %eax,(%esp)
80101ea4:	e8 6e e3 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > ip->size)
    n = ip->size - off;

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101ea9:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eac:	01 45 f4             	add    %eax,-0xc(%ebp)
80101eaf:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eb2:	01 45 10             	add    %eax,0x10(%ebp)
80101eb5:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101eb8:	01 45 0c             	add    %eax,0xc(%ebp)
80101ebb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101ebe:	3b 45 14             	cmp    0x14(%ebp),%eax
80101ec1:	0f 82 60 ff ff ff    	jb     80101e27 <readi+0xbf>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    memmove(dst, bp->data + off%BSIZE, m);
    brelse(bp);
  }
  return n;
80101ec7:	8b 45 14             	mov    0x14(%ebp),%eax
}
80101eca:	c9                   	leave  
80101ecb:	c3                   	ret    

80101ecc <writei>:

// PAGEBREAK!
// Write data to inode.
int
writei(struct inode *ip, char *src, uint off, uint n)
{
80101ecc:	55                   	push   %ebp
80101ecd:	89 e5                	mov    %esp,%ebp
80101ecf:	83 ec 28             	sub    $0x28,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101ed2:	8b 45 08             	mov    0x8(%ebp),%eax
80101ed5:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101ed9:	66 83 f8 03          	cmp    $0x3,%ax
80101edd:	75 60                	jne    80101f3f <writei+0x73>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
80101edf:	8b 45 08             	mov    0x8(%ebp),%eax
80101ee2:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ee6:	66 85 c0             	test   %ax,%ax
80101ee9:	78 20                	js     80101f0b <writei+0x3f>
80101eeb:	8b 45 08             	mov    0x8(%ebp),%eax
80101eee:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ef2:	66 83 f8 09          	cmp    $0x9,%ax
80101ef6:	7f 13                	jg     80101f0b <writei+0x3f>
80101ef8:	8b 45 08             	mov    0x8(%ebp),%eax
80101efb:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101eff:	98                   	cwtl   
80101f00:	8b 04 c5 04 e8 10 80 	mov    -0x7fef17fc(,%eax,8),%eax
80101f07:	85 c0                	test   %eax,%eax
80101f09:	75 0a                	jne    80101f15 <writei+0x49>
      return -1;
80101f0b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f10:	e9 44 01 00 00       	jmp    80102059 <writei+0x18d>
    return devsw[ip->major].write(ip, src, n);
80101f15:	8b 45 08             	mov    0x8(%ebp),%eax
80101f18:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101f1c:	98                   	cwtl   
80101f1d:	8b 04 c5 04 e8 10 80 	mov    -0x7fef17fc(,%eax,8),%eax
80101f24:	8b 55 14             	mov    0x14(%ebp),%edx
80101f27:	89 54 24 08          	mov    %edx,0x8(%esp)
80101f2b:	8b 55 0c             	mov    0xc(%ebp),%edx
80101f2e:	89 54 24 04          	mov    %edx,0x4(%esp)
80101f32:	8b 55 08             	mov    0x8(%ebp),%edx
80101f35:	89 14 24             	mov    %edx,(%esp)
80101f38:	ff d0                	call   *%eax
80101f3a:	e9 1a 01 00 00       	jmp    80102059 <writei+0x18d>
  }

  if(off > ip->size || off + n < off)
80101f3f:	8b 45 08             	mov    0x8(%ebp),%eax
80101f42:	8b 40 18             	mov    0x18(%eax),%eax
80101f45:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f48:	72 0d                	jb     80101f57 <writei+0x8b>
80101f4a:	8b 45 14             	mov    0x14(%ebp),%eax
80101f4d:	8b 55 10             	mov    0x10(%ebp),%edx
80101f50:	01 d0                	add    %edx,%eax
80101f52:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f55:	73 0a                	jae    80101f61 <writei+0x95>
    return -1;
80101f57:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f5c:	e9 f8 00 00 00       	jmp    80102059 <writei+0x18d>
  if(off + n > MAXFILE*BSIZE)
80101f61:	8b 45 14             	mov    0x14(%ebp),%eax
80101f64:	8b 55 10             	mov    0x10(%ebp),%edx
80101f67:	01 d0                	add    %edx,%eax
80101f69:	3d 00 18 01 00       	cmp    $0x11800,%eax
80101f6e:	76 0a                	jbe    80101f7a <writei+0xae>
    return -1;
80101f70:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f75:	e9 df 00 00 00       	jmp    80102059 <writei+0x18d>

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80101f7a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101f81:	e9 9f 00 00 00       	jmp    80102025 <writei+0x159>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101f86:	8b 45 10             	mov    0x10(%ebp),%eax
80101f89:	c1 e8 09             	shr    $0x9,%eax
80101f8c:	89 44 24 04          	mov    %eax,0x4(%esp)
80101f90:	8b 45 08             	mov    0x8(%ebp),%eax
80101f93:	89 04 24             	mov    %eax,(%esp)
80101f96:	e8 62 fb ff ff       	call   80101afd <bmap>
80101f9b:	8b 55 08             	mov    0x8(%ebp),%edx
80101f9e:	8b 12                	mov    (%edx),%edx
80101fa0:	89 44 24 04          	mov    %eax,0x4(%esp)
80101fa4:	89 14 24             	mov    %edx,(%esp)
80101fa7:	e8 fa e1 ff ff       	call   801001a6 <bread>
80101fac:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101faf:	8b 45 10             	mov    0x10(%ebp),%eax
80101fb2:	25 ff 01 00 00       	and    $0x1ff,%eax
80101fb7:	89 c2                	mov    %eax,%edx
80101fb9:	b8 00 02 00 00       	mov    $0x200,%eax
80101fbe:	29 d0                	sub    %edx,%eax
80101fc0:	89 c2                	mov    %eax,%edx
80101fc2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101fc5:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101fc8:	29 c1                	sub    %eax,%ecx
80101fca:	89 c8                	mov    %ecx,%eax
80101fcc:	39 c2                	cmp    %eax,%edx
80101fce:	0f 46 c2             	cmovbe %edx,%eax
80101fd1:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(bp->data + off%BSIZE, src, m);
80101fd4:	8b 45 10             	mov    0x10(%ebp),%eax
80101fd7:	25 ff 01 00 00       	and    $0x1ff,%eax
80101fdc:	8d 50 10             	lea    0x10(%eax),%edx
80101fdf:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101fe2:	01 d0                	add    %edx,%eax
80101fe4:	8d 50 08             	lea    0x8(%eax),%edx
80101fe7:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101fea:	89 44 24 08          	mov    %eax,0x8(%esp)
80101fee:	8b 45 0c             	mov    0xc(%ebp),%eax
80101ff1:	89 44 24 04          	mov    %eax,0x4(%esp)
80101ff5:	89 14 24             	mov    %edx,(%esp)
80101ff8:	e8 be 2d 00 00       	call   80104dbb <memmove>
    log_write(bp);
80101ffd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102000:	89 04 24             	mov    %eax,(%esp)
80102003:	e8 7e 12 00 00       	call   80103286 <log_write>
    brelse(bp);
80102008:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010200b:	89 04 24             	mov    %eax,(%esp)
8010200e:	e8 04 e2 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > MAXFILE*BSIZE)
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80102013:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102016:	01 45 f4             	add    %eax,-0xc(%ebp)
80102019:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010201c:	01 45 10             	add    %eax,0x10(%ebp)
8010201f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102022:	01 45 0c             	add    %eax,0xc(%ebp)
80102025:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102028:	3b 45 14             	cmp    0x14(%ebp),%eax
8010202b:	0f 82 55 ff ff ff    	jb     80101f86 <writei+0xba>
    memmove(bp->data + off%BSIZE, src, m);
    log_write(bp);
    brelse(bp);
  }

  if(n > 0 && off > ip->size){
80102031:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
80102035:	74 1f                	je     80102056 <writei+0x18a>
80102037:	8b 45 08             	mov    0x8(%ebp),%eax
8010203a:	8b 40 18             	mov    0x18(%eax),%eax
8010203d:	3b 45 10             	cmp    0x10(%ebp),%eax
80102040:	73 14                	jae    80102056 <writei+0x18a>
    ip->size = off;
80102042:	8b 45 08             	mov    0x8(%ebp),%eax
80102045:	8b 55 10             	mov    0x10(%ebp),%edx
80102048:	89 50 18             	mov    %edx,0x18(%eax)
    iupdate(ip);
8010204b:	8b 45 08             	mov    0x8(%ebp),%eax
8010204e:	89 04 24             	mov    %eax,(%esp)
80102051:	e8 49 f6 ff ff       	call   8010169f <iupdate>
  }
  return n;
80102056:	8b 45 14             	mov    0x14(%ebp),%eax
}
80102059:	c9                   	leave  
8010205a:	c3                   	ret    

8010205b <namecmp>:
//PAGEBREAK!
// Directories

int
namecmp(const char *s, const char *t)
{
8010205b:	55                   	push   %ebp
8010205c:	89 e5                	mov    %esp,%ebp
8010205e:	83 ec 18             	sub    $0x18,%esp
  return strncmp(s, t, DIRSIZ);
80102061:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
80102068:	00 
80102069:	8b 45 0c             	mov    0xc(%ebp),%eax
8010206c:	89 44 24 04          	mov    %eax,0x4(%esp)
80102070:	8b 45 08             	mov    0x8(%ebp),%eax
80102073:	89 04 24             	mov    %eax,(%esp)
80102076:	e8 e3 2d 00 00       	call   80104e5e <strncmp>
}
8010207b:	c9                   	leave  
8010207c:	c3                   	ret    

8010207d <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
8010207d:	55                   	push   %ebp
8010207e:	89 e5                	mov    %esp,%ebp
80102080:	83 ec 38             	sub    $0x38,%esp
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
80102083:	8b 45 08             	mov    0x8(%ebp),%eax
80102086:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010208a:	66 83 f8 01          	cmp    $0x1,%ax
8010208e:	74 0c                	je     8010209c <dirlookup+0x1f>
    panic("dirlookup not DIR");
80102090:	c7 04 24 81 81 10 80 	movl   $0x80108181,(%esp)
80102097:	e8 9e e4 ff ff       	call   8010053a <panic>

  for(off = 0; off < dp->size; off += sizeof(de)){
8010209c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801020a3:	e9 88 00 00 00       	jmp    80102130 <dirlookup+0xb3>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801020a8:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
801020af:	00 
801020b0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801020b3:	89 44 24 08          	mov    %eax,0x8(%esp)
801020b7:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020ba:	89 44 24 04          	mov    %eax,0x4(%esp)
801020be:	8b 45 08             	mov    0x8(%ebp),%eax
801020c1:	89 04 24             	mov    %eax,(%esp)
801020c4:	e8 9f fc ff ff       	call   80101d68 <readi>
801020c9:	83 f8 10             	cmp    $0x10,%eax
801020cc:	74 0c                	je     801020da <dirlookup+0x5d>
      panic("dirlink read");
801020ce:	c7 04 24 93 81 10 80 	movl   $0x80108193,(%esp)
801020d5:	e8 60 e4 ff ff       	call   8010053a <panic>
    if(de.inum == 0)
801020da:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801020de:	66 85 c0             	test   %ax,%ax
801020e1:	75 02                	jne    801020e5 <dirlookup+0x68>
      continue;
801020e3:	eb 47                	jmp    8010212c <dirlookup+0xaf>
    if(namecmp(name, de.name) == 0){
801020e5:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020e8:	83 c0 02             	add    $0x2,%eax
801020eb:	89 44 24 04          	mov    %eax,0x4(%esp)
801020ef:	8b 45 0c             	mov    0xc(%ebp),%eax
801020f2:	89 04 24             	mov    %eax,(%esp)
801020f5:	e8 61 ff ff ff       	call   8010205b <namecmp>
801020fa:	85 c0                	test   %eax,%eax
801020fc:	75 2e                	jne    8010212c <dirlookup+0xaf>
      // entry matches path element
      if(poff)
801020fe:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80102102:	74 08                	je     8010210c <dirlookup+0x8f>
        *poff = off;
80102104:	8b 45 10             	mov    0x10(%ebp),%eax
80102107:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010210a:	89 10                	mov    %edx,(%eax)
      inum = de.inum;
8010210c:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
80102110:	0f b7 c0             	movzwl %ax,%eax
80102113:	89 45 f0             	mov    %eax,-0x10(%ebp)
      return iget(dp->dev, inum);
80102116:	8b 45 08             	mov    0x8(%ebp),%eax
80102119:	8b 00                	mov    (%eax),%eax
8010211b:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010211e:	89 54 24 04          	mov    %edx,0x4(%esp)
80102122:	89 04 24             	mov    %eax,(%esp)
80102125:	e8 2d f6 ff ff       	call   80101757 <iget>
8010212a:	eb 18                	jmp    80102144 <dirlookup+0xc7>
  struct dirent de;

  if(dp->type != T_DIR)
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
8010212c:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80102130:	8b 45 08             	mov    0x8(%ebp),%eax
80102133:	8b 40 18             	mov    0x18(%eax),%eax
80102136:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80102139:	0f 87 69 ff ff ff    	ja     801020a8 <dirlookup+0x2b>
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
8010213f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102144:	c9                   	leave  
80102145:	c3                   	ret    

80102146 <dirlink>:

// Write a new directory entry (name, inum) into the directory dp.
int
dirlink(struct inode *dp, char *name, uint inum)
{
80102146:	55                   	push   %ebp
80102147:	89 e5                	mov    %esp,%ebp
80102149:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;
  struct inode *ip;

  // Check that name is not present.
  if((ip = dirlookup(dp, name, 0)) != 0){
8010214c:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80102153:	00 
80102154:	8b 45 0c             	mov    0xc(%ebp),%eax
80102157:	89 44 24 04          	mov    %eax,0x4(%esp)
8010215b:	8b 45 08             	mov    0x8(%ebp),%eax
8010215e:	89 04 24             	mov    %eax,(%esp)
80102161:	e8 17 ff ff ff       	call   8010207d <dirlookup>
80102166:	89 45 f0             	mov    %eax,-0x10(%ebp)
80102169:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010216d:	74 15                	je     80102184 <dirlink+0x3e>
    iput(ip);
8010216f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102172:	89 04 24             	mov    %eax,(%esp)
80102175:	e8 94 f8 ff ff       	call   80101a0e <iput>
    return -1;
8010217a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010217f:	e9 b7 00 00 00       	jmp    8010223b <dirlink+0xf5>
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
80102184:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010218b:	eb 46                	jmp    801021d3 <dirlink+0x8d>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
8010218d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102190:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80102197:	00 
80102198:	89 44 24 08          	mov    %eax,0x8(%esp)
8010219c:	8d 45 e0             	lea    -0x20(%ebp),%eax
8010219f:	89 44 24 04          	mov    %eax,0x4(%esp)
801021a3:	8b 45 08             	mov    0x8(%ebp),%eax
801021a6:	89 04 24             	mov    %eax,(%esp)
801021a9:	e8 ba fb ff ff       	call   80101d68 <readi>
801021ae:	83 f8 10             	cmp    $0x10,%eax
801021b1:	74 0c                	je     801021bf <dirlink+0x79>
      panic("dirlink read");
801021b3:	c7 04 24 93 81 10 80 	movl   $0x80108193,(%esp)
801021ba:	e8 7b e3 ff ff       	call   8010053a <panic>
    if(de.inum == 0)
801021bf:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801021c3:	66 85 c0             	test   %ax,%ax
801021c6:	75 02                	jne    801021ca <dirlink+0x84>
      break;
801021c8:	eb 16                	jmp    801021e0 <dirlink+0x9a>
    iput(ip);
    return -1;
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
801021ca:	8b 45 f4             	mov    -0xc(%ebp),%eax
801021cd:	83 c0 10             	add    $0x10,%eax
801021d0:	89 45 f4             	mov    %eax,-0xc(%ebp)
801021d3:	8b 55 f4             	mov    -0xc(%ebp),%edx
801021d6:	8b 45 08             	mov    0x8(%ebp),%eax
801021d9:	8b 40 18             	mov    0x18(%eax),%eax
801021dc:	39 c2                	cmp    %eax,%edx
801021de:	72 ad                	jb     8010218d <dirlink+0x47>
      panic("dirlink read");
    if(de.inum == 0)
      break;
  }

  strncpy(de.name, name, DIRSIZ);
801021e0:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801021e7:	00 
801021e8:	8b 45 0c             	mov    0xc(%ebp),%eax
801021eb:	89 44 24 04          	mov    %eax,0x4(%esp)
801021ef:	8d 45 e0             	lea    -0x20(%ebp),%eax
801021f2:	83 c0 02             	add    $0x2,%eax
801021f5:	89 04 24             	mov    %eax,(%esp)
801021f8:	e8 b7 2c 00 00       	call   80104eb4 <strncpy>
  de.inum = inum;
801021fd:	8b 45 10             	mov    0x10(%ebp),%eax
80102200:	66 89 45 e0          	mov    %ax,-0x20(%ebp)
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80102204:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102207:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
8010220e:	00 
8010220f:	89 44 24 08          	mov    %eax,0x8(%esp)
80102213:	8d 45 e0             	lea    -0x20(%ebp),%eax
80102216:	89 44 24 04          	mov    %eax,0x4(%esp)
8010221a:	8b 45 08             	mov    0x8(%ebp),%eax
8010221d:	89 04 24             	mov    %eax,(%esp)
80102220:	e8 a7 fc ff ff       	call   80101ecc <writei>
80102225:	83 f8 10             	cmp    $0x10,%eax
80102228:	74 0c                	je     80102236 <dirlink+0xf0>
    panic("dirlink");
8010222a:	c7 04 24 a0 81 10 80 	movl   $0x801081a0,(%esp)
80102231:	e8 04 e3 ff ff       	call   8010053a <panic>
  
  return 0;
80102236:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010223b:	c9                   	leave  
8010223c:	c3                   	ret    

8010223d <skipelem>:
//   skipelem("a", name) = "", setting name = "a"
//   skipelem("", name) = skipelem("////", name) = 0
//
static char*
skipelem(char *path, char *name)
{
8010223d:	55                   	push   %ebp
8010223e:	89 e5                	mov    %esp,%ebp
80102240:	83 ec 28             	sub    $0x28,%esp
  char *s;
  int len;

  while(*path == '/')
80102243:	eb 04                	jmp    80102249 <skipelem+0xc>
    path++;
80102245:	83 45 08 01          	addl   $0x1,0x8(%ebp)
skipelem(char *path, char *name)
{
  char *s;
  int len;

  while(*path == '/')
80102249:	8b 45 08             	mov    0x8(%ebp),%eax
8010224c:	0f b6 00             	movzbl (%eax),%eax
8010224f:	3c 2f                	cmp    $0x2f,%al
80102251:	74 f2                	je     80102245 <skipelem+0x8>
    path++;
  if(*path == 0)
80102253:	8b 45 08             	mov    0x8(%ebp),%eax
80102256:	0f b6 00             	movzbl (%eax),%eax
80102259:	84 c0                	test   %al,%al
8010225b:	75 0a                	jne    80102267 <skipelem+0x2a>
    return 0;
8010225d:	b8 00 00 00 00       	mov    $0x0,%eax
80102262:	e9 86 00 00 00       	jmp    801022ed <skipelem+0xb0>
  s = path;
80102267:	8b 45 08             	mov    0x8(%ebp),%eax
8010226a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(*path != '/' && *path != 0)
8010226d:	eb 04                	jmp    80102273 <skipelem+0x36>
    path++;
8010226f:	83 45 08 01          	addl   $0x1,0x8(%ebp)
  while(*path == '/')
    path++;
  if(*path == 0)
    return 0;
  s = path;
  while(*path != '/' && *path != 0)
80102273:	8b 45 08             	mov    0x8(%ebp),%eax
80102276:	0f b6 00             	movzbl (%eax),%eax
80102279:	3c 2f                	cmp    $0x2f,%al
8010227b:	74 0a                	je     80102287 <skipelem+0x4a>
8010227d:	8b 45 08             	mov    0x8(%ebp),%eax
80102280:	0f b6 00             	movzbl (%eax),%eax
80102283:	84 c0                	test   %al,%al
80102285:	75 e8                	jne    8010226f <skipelem+0x32>
    path++;
  len = path - s;
80102287:	8b 55 08             	mov    0x8(%ebp),%edx
8010228a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010228d:	29 c2                	sub    %eax,%edx
8010228f:	89 d0                	mov    %edx,%eax
80102291:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(len >= DIRSIZ)
80102294:	83 7d f0 0d          	cmpl   $0xd,-0x10(%ebp)
80102298:	7e 1c                	jle    801022b6 <skipelem+0x79>
    memmove(name, s, DIRSIZ);
8010229a:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801022a1:	00 
801022a2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022a5:	89 44 24 04          	mov    %eax,0x4(%esp)
801022a9:	8b 45 0c             	mov    0xc(%ebp),%eax
801022ac:	89 04 24             	mov    %eax,(%esp)
801022af:	e8 07 2b 00 00       	call   80104dbb <memmove>
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022b4:	eb 2a                	jmp    801022e0 <skipelem+0xa3>
    path++;
  len = path - s;
  if(len >= DIRSIZ)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
801022b6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801022b9:	89 44 24 08          	mov    %eax,0x8(%esp)
801022bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022c0:	89 44 24 04          	mov    %eax,0x4(%esp)
801022c4:	8b 45 0c             	mov    0xc(%ebp),%eax
801022c7:	89 04 24             	mov    %eax,(%esp)
801022ca:	e8 ec 2a 00 00       	call   80104dbb <memmove>
    name[len] = 0;
801022cf:	8b 55 f0             	mov    -0x10(%ebp),%edx
801022d2:	8b 45 0c             	mov    0xc(%ebp),%eax
801022d5:	01 d0                	add    %edx,%eax
801022d7:	c6 00 00             	movb   $0x0,(%eax)
  }
  while(*path == '/')
801022da:	eb 04                	jmp    801022e0 <skipelem+0xa3>
    path++;
801022dc:	83 45 08 01          	addl   $0x1,0x8(%ebp)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022e0:	8b 45 08             	mov    0x8(%ebp),%eax
801022e3:	0f b6 00             	movzbl (%eax),%eax
801022e6:	3c 2f                	cmp    $0x2f,%al
801022e8:	74 f2                	je     801022dc <skipelem+0x9f>
    path++;
  return path;
801022ea:	8b 45 08             	mov    0x8(%ebp),%eax
}
801022ed:	c9                   	leave  
801022ee:	c3                   	ret    

801022ef <namex>:
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
// Must be called inside a transaction since it calls iput().
static struct inode*
namex(char *path, int nameiparent, char *name)
{
801022ef:	55                   	push   %ebp
801022f0:	89 e5                	mov    %esp,%ebp
801022f2:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *next;

  if(*path == '/')
801022f5:	8b 45 08             	mov    0x8(%ebp),%eax
801022f8:	0f b6 00             	movzbl (%eax),%eax
801022fb:	3c 2f                	cmp    $0x2f,%al
801022fd:	75 1c                	jne    8010231b <namex+0x2c>
    ip = iget(ROOTDEV, ROOTINO);
801022ff:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102306:	00 
80102307:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010230e:	e8 44 f4 ff ff       	call   80101757 <iget>
80102313:	89 45 f4             	mov    %eax,-0xc(%ebp)
  else
    ip = idup(proc->cwd);

  while((path = skipelem(path, name)) != 0){
80102316:	e9 af 00 00 00       	jmp    801023ca <namex+0xdb>
  struct inode *ip, *next;

  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);
8010231b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80102321:	8b 40 68             	mov    0x68(%eax),%eax
80102324:	89 04 24             	mov    %eax,(%esp)
80102327:	e8 fd f4 ff ff       	call   80101829 <idup>
8010232c:	89 45 f4             	mov    %eax,-0xc(%ebp)

  while((path = skipelem(path, name)) != 0){
8010232f:	e9 96 00 00 00       	jmp    801023ca <namex+0xdb>
    ilock(ip);
80102334:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102337:	89 04 24             	mov    %eax,(%esp)
8010233a:	e8 1c f5 ff ff       	call   8010185b <ilock>
    if(ip->type != T_DIR){
8010233f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102342:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80102346:	66 83 f8 01          	cmp    $0x1,%ax
8010234a:	74 15                	je     80102361 <namex+0x72>
      iunlockput(ip);
8010234c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010234f:	89 04 24             	mov    %eax,(%esp)
80102352:	e8 88 f7 ff ff       	call   80101adf <iunlockput>
      return 0;
80102357:	b8 00 00 00 00       	mov    $0x0,%eax
8010235c:	e9 a3 00 00 00       	jmp    80102404 <namex+0x115>
    }
    if(nameiparent && *path == '\0'){
80102361:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80102365:	74 1d                	je     80102384 <namex+0x95>
80102367:	8b 45 08             	mov    0x8(%ebp),%eax
8010236a:	0f b6 00             	movzbl (%eax),%eax
8010236d:	84 c0                	test   %al,%al
8010236f:	75 13                	jne    80102384 <namex+0x95>
      // Stop one level early.
      iunlock(ip);
80102371:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102374:	89 04 24             	mov    %eax,(%esp)
80102377:	e8 2d f6 ff ff       	call   801019a9 <iunlock>
      return ip;
8010237c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010237f:	e9 80 00 00 00       	jmp    80102404 <namex+0x115>
    }
    if((next = dirlookup(ip, name, 0)) == 0){
80102384:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010238b:	00 
8010238c:	8b 45 10             	mov    0x10(%ebp),%eax
8010238f:	89 44 24 04          	mov    %eax,0x4(%esp)
80102393:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102396:	89 04 24             	mov    %eax,(%esp)
80102399:	e8 df fc ff ff       	call   8010207d <dirlookup>
8010239e:	89 45 f0             	mov    %eax,-0x10(%ebp)
801023a1:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801023a5:	75 12                	jne    801023b9 <namex+0xca>
      iunlockput(ip);
801023a7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023aa:	89 04 24             	mov    %eax,(%esp)
801023ad:	e8 2d f7 ff ff       	call   80101adf <iunlockput>
      return 0;
801023b2:	b8 00 00 00 00       	mov    $0x0,%eax
801023b7:	eb 4b                	jmp    80102404 <namex+0x115>
    }
    iunlockput(ip);
801023b9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023bc:	89 04 24             	mov    %eax,(%esp)
801023bf:	e8 1b f7 ff ff       	call   80101adf <iunlockput>
    ip = next;
801023c4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801023c7:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);

  while((path = skipelem(path, name)) != 0){
801023ca:	8b 45 10             	mov    0x10(%ebp),%eax
801023cd:	89 44 24 04          	mov    %eax,0x4(%esp)
801023d1:	8b 45 08             	mov    0x8(%ebp),%eax
801023d4:	89 04 24             	mov    %eax,(%esp)
801023d7:	e8 61 fe ff ff       	call   8010223d <skipelem>
801023dc:	89 45 08             	mov    %eax,0x8(%ebp)
801023df:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801023e3:	0f 85 4b ff ff ff    	jne    80102334 <namex+0x45>
      return 0;
    }
    iunlockput(ip);
    ip = next;
  }
  if(nameiparent){
801023e9:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
801023ed:	74 12                	je     80102401 <namex+0x112>
    iput(ip);
801023ef:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023f2:	89 04 24             	mov    %eax,(%esp)
801023f5:	e8 14 f6 ff ff       	call   80101a0e <iput>
    return 0;
801023fa:	b8 00 00 00 00       	mov    $0x0,%eax
801023ff:	eb 03                	jmp    80102404 <namex+0x115>
  }
  return ip;
80102401:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102404:	c9                   	leave  
80102405:	c3                   	ret    

80102406 <namei>:

struct inode*
namei(char *path)
{
80102406:	55                   	push   %ebp
80102407:	89 e5                	mov    %esp,%ebp
80102409:	83 ec 28             	sub    $0x28,%esp
  char name[DIRSIZ];
  return namex(path, 0, name);
8010240c:	8d 45 ea             	lea    -0x16(%ebp),%eax
8010240f:	89 44 24 08          	mov    %eax,0x8(%esp)
80102413:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010241a:	00 
8010241b:	8b 45 08             	mov    0x8(%ebp),%eax
8010241e:	89 04 24             	mov    %eax,(%esp)
80102421:	e8 c9 fe ff ff       	call   801022ef <namex>
}
80102426:	c9                   	leave  
80102427:	c3                   	ret    

80102428 <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
80102428:	55                   	push   %ebp
80102429:	89 e5                	mov    %esp,%ebp
8010242b:	83 ec 18             	sub    $0x18,%esp
  return namex(path, 1, name);
8010242e:	8b 45 0c             	mov    0xc(%ebp),%eax
80102431:	89 44 24 08          	mov    %eax,0x8(%esp)
80102435:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
8010243c:	00 
8010243d:	8b 45 08             	mov    0x8(%ebp),%eax
80102440:	89 04 24             	mov    %eax,(%esp)
80102443:	e8 a7 fe ff ff       	call   801022ef <namex>
}
80102448:	c9                   	leave  
80102449:	c3                   	ret    

8010244a <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
8010244a:	55                   	push   %ebp
8010244b:	89 e5                	mov    %esp,%ebp
8010244d:	83 ec 14             	sub    $0x14,%esp
80102450:	8b 45 08             	mov    0x8(%ebp),%eax
80102453:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102457:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
8010245b:	89 c2                	mov    %eax,%edx
8010245d:	ec                   	in     (%dx),%al
8010245e:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
80102461:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
80102465:	c9                   	leave  
80102466:	c3                   	ret    

80102467 <insl>:

static inline void
insl(int port, void *addr, int cnt)
{
80102467:	55                   	push   %ebp
80102468:	89 e5                	mov    %esp,%ebp
8010246a:	57                   	push   %edi
8010246b:	53                   	push   %ebx
  asm volatile("cld; rep insl" :
8010246c:	8b 55 08             	mov    0x8(%ebp),%edx
8010246f:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80102472:	8b 45 10             	mov    0x10(%ebp),%eax
80102475:	89 cb                	mov    %ecx,%ebx
80102477:	89 df                	mov    %ebx,%edi
80102479:	89 c1                	mov    %eax,%ecx
8010247b:	fc                   	cld    
8010247c:	f3 6d                	rep insl (%dx),%es:(%edi)
8010247e:	89 c8                	mov    %ecx,%eax
80102480:	89 fb                	mov    %edi,%ebx
80102482:	89 5d 0c             	mov    %ebx,0xc(%ebp)
80102485:	89 45 10             	mov    %eax,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "memory", "cc");
}
80102488:	5b                   	pop    %ebx
80102489:	5f                   	pop    %edi
8010248a:	5d                   	pop    %ebp
8010248b:	c3                   	ret    

8010248c <outb>:

static inline void
outb(ushort port, uchar data)
{
8010248c:	55                   	push   %ebp
8010248d:	89 e5                	mov    %esp,%ebp
8010248f:	83 ec 08             	sub    $0x8,%esp
80102492:	8b 55 08             	mov    0x8(%ebp),%edx
80102495:	8b 45 0c             	mov    0xc(%ebp),%eax
80102498:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
8010249c:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
8010249f:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801024a3:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801024a7:	ee                   	out    %al,(%dx)
}
801024a8:	c9                   	leave  
801024a9:	c3                   	ret    

801024aa <outsl>:
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
}

static inline void
outsl(int port, const void *addr, int cnt)
{
801024aa:	55                   	push   %ebp
801024ab:	89 e5                	mov    %esp,%ebp
801024ad:	56                   	push   %esi
801024ae:	53                   	push   %ebx
  asm volatile("cld; rep outsl" :
801024af:	8b 55 08             	mov    0x8(%ebp),%edx
801024b2:	8b 4d 0c             	mov    0xc(%ebp),%ecx
801024b5:	8b 45 10             	mov    0x10(%ebp),%eax
801024b8:	89 cb                	mov    %ecx,%ebx
801024ba:	89 de                	mov    %ebx,%esi
801024bc:	89 c1                	mov    %eax,%ecx
801024be:	fc                   	cld    
801024bf:	f3 6f                	rep outsl %ds:(%esi),(%dx)
801024c1:	89 c8                	mov    %ecx,%eax
801024c3:	89 f3                	mov    %esi,%ebx
801024c5:	89 5d 0c             	mov    %ebx,0xc(%ebp)
801024c8:	89 45 10             	mov    %eax,0x10(%ebp)
               "=S" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "cc");
}
801024cb:	5b                   	pop    %ebx
801024cc:	5e                   	pop    %esi
801024cd:	5d                   	pop    %ebp
801024ce:	c3                   	ret    

801024cf <idewait>:
static void idestart(struct buf*);

// Wait for IDE disk to become ready.
static int
idewait(int checkerr)
{
801024cf:	55                   	push   %ebp
801024d0:	89 e5                	mov    %esp,%ebp
801024d2:	83 ec 14             	sub    $0x14,%esp
  int r;

  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY) 
801024d5:	90                   	nop
801024d6:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801024dd:	e8 68 ff ff ff       	call   8010244a <inb>
801024e2:	0f b6 c0             	movzbl %al,%eax
801024e5:	89 45 fc             	mov    %eax,-0x4(%ebp)
801024e8:	8b 45 fc             	mov    -0x4(%ebp),%eax
801024eb:	25 c0 00 00 00       	and    $0xc0,%eax
801024f0:	83 f8 40             	cmp    $0x40,%eax
801024f3:	75 e1                	jne    801024d6 <idewait+0x7>
    ;
  if(checkerr && (r & (IDE_DF|IDE_ERR)) != 0)
801024f5:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801024f9:	74 11                	je     8010250c <idewait+0x3d>
801024fb:	8b 45 fc             	mov    -0x4(%ebp),%eax
801024fe:	83 e0 21             	and    $0x21,%eax
80102501:	85 c0                	test   %eax,%eax
80102503:	74 07                	je     8010250c <idewait+0x3d>
    return -1;
80102505:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010250a:	eb 05                	jmp    80102511 <idewait+0x42>
  return 0;
8010250c:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102511:	c9                   	leave  
80102512:	c3                   	ret    

80102513 <ideinit>:

void
ideinit(void)
{
80102513:	55                   	push   %ebp
80102514:	89 e5                	mov    %esp,%ebp
80102516:	83 ec 28             	sub    $0x28,%esp
  int i;

  initlock(&idelock, "ide");
80102519:	c7 44 24 04 a8 81 10 	movl   $0x801081a8,0x4(%esp)
80102520:	80 
80102521:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
80102528:	e8 4a 25 00 00       	call   80104a77 <initlock>
  picenable(IRQ_IDE);
8010252d:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
80102534:	e8 29 15 00 00       	call   80103a62 <picenable>
  ioapicenable(IRQ_IDE, ncpu - 1);
80102539:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
8010253e:	83 e8 01             	sub    $0x1,%eax
80102541:	89 44 24 04          	mov    %eax,0x4(%esp)
80102545:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
8010254c:	e8 0c 04 00 00       	call   8010295d <ioapicenable>
  idewait(0);
80102551:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80102558:	e8 72 ff ff ff       	call   801024cf <idewait>
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
8010255d:	c7 44 24 04 f0 00 00 	movl   $0xf0,0x4(%esp)
80102564:	00 
80102565:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
8010256c:	e8 1b ff ff ff       	call   8010248c <outb>
  for(i=0; i<1000; i++){
80102571:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102578:	eb 20                	jmp    8010259a <ideinit+0x87>
    if(inb(0x1f7) != 0){
8010257a:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
80102581:	e8 c4 fe ff ff       	call   8010244a <inb>
80102586:	84 c0                	test   %al,%al
80102588:	74 0c                	je     80102596 <ideinit+0x83>
      havedisk1 = 1;
8010258a:	c7 05 38 b6 10 80 01 	movl   $0x1,0x8010b638
80102591:	00 00 00 
      break;
80102594:	eb 0d                	jmp    801025a3 <ideinit+0x90>
  ioapicenable(IRQ_IDE, ncpu - 1);
  idewait(0);
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
  for(i=0; i<1000; i++){
80102596:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
8010259a:	81 7d f4 e7 03 00 00 	cmpl   $0x3e7,-0xc(%ebp)
801025a1:	7e d7                	jle    8010257a <ideinit+0x67>
      break;
    }
  }
  
  // Switch back to disk 0.
  outb(0x1f6, 0xe0 | (0<<4));
801025a3:	c7 44 24 04 e0 00 00 	movl   $0xe0,0x4(%esp)
801025aa:	00 
801025ab:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
801025b2:	e8 d5 fe ff ff       	call   8010248c <outb>
}
801025b7:	c9                   	leave  
801025b8:	c3                   	ret    

801025b9 <idestart>:

// Start the request for b.  Caller must hold idelock.
static void
idestart(struct buf *b)
{
801025b9:	55                   	push   %ebp
801025ba:	89 e5                	mov    %esp,%ebp
801025bc:	83 ec 18             	sub    $0x18,%esp
  if(b == 0)
801025bf:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801025c3:	75 0c                	jne    801025d1 <idestart+0x18>
    panic("idestart");
801025c5:	c7 04 24 ac 81 10 80 	movl   $0x801081ac,(%esp)
801025cc:	e8 69 df ff ff       	call   8010053a <panic>

  idewait(0);
801025d1:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801025d8:	e8 f2 fe ff ff       	call   801024cf <idewait>
  outb(0x3f6, 0);  // generate interrupt
801025dd:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801025e4:	00 
801025e5:	c7 04 24 f6 03 00 00 	movl   $0x3f6,(%esp)
801025ec:	e8 9b fe ff ff       	call   8010248c <outb>
  outb(0x1f2, 1);  // number of sectors
801025f1:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
801025f8:	00 
801025f9:	c7 04 24 f2 01 00 00 	movl   $0x1f2,(%esp)
80102600:	e8 87 fe ff ff       	call   8010248c <outb>
  outb(0x1f3, b->sector & 0xff);
80102605:	8b 45 08             	mov    0x8(%ebp),%eax
80102608:	8b 40 08             	mov    0x8(%eax),%eax
8010260b:	0f b6 c0             	movzbl %al,%eax
8010260e:	89 44 24 04          	mov    %eax,0x4(%esp)
80102612:	c7 04 24 f3 01 00 00 	movl   $0x1f3,(%esp)
80102619:	e8 6e fe ff ff       	call   8010248c <outb>
  outb(0x1f4, (b->sector >> 8) & 0xff);
8010261e:	8b 45 08             	mov    0x8(%ebp),%eax
80102621:	8b 40 08             	mov    0x8(%eax),%eax
80102624:	c1 e8 08             	shr    $0x8,%eax
80102627:	0f b6 c0             	movzbl %al,%eax
8010262a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010262e:	c7 04 24 f4 01 00 00 	movl   $0x1f4,(%esp)
80102635:	e8 52 fe ff ff       	call   8010248c <outb>
  outb(0x1f5, (b->sector >> 16) & 0xff);
8010263a:	8b 45 08             	mov    0x8(%ebp),%eax
8010263d:	8b 40 08             	mov    0x8(%eax),%eax
80102640:	c1 e8 10             	shr    $0x10,%eax
80102643:	0f b6 c0             	movzbl %al,%eax
80102646:	89 44 24 04          	mov    %eax,0x4(%esp)
8010264a:	c7 04 24 f5 01 00 00 	movl   $0x1f5,(%esp)
80102651:	e8 36 fe ff ff       	call   8010248c <outb>
  outb(0x1f6, 0xe0 | ((b->dev&1)<<4) | ((b->sector>>24)&0x0f));
80102656:	8b 45 08             	mov    0x8(%ebp),%eax
80102659:	8b 40 04             	mov    0x4(%eax),%eax
8010265c:	83 e0 01             	and    $0x1,%eax
8010265f:	c1 e0 04             	shl    $0x4,%eax
80102662:	89 c2                	mov    %eax,%edx
80102664:	8b 45 08             	mov    0x8(%ebp),%eax
80102667:	8b 40 08             	mov    0x8(%eax),%eax
8010266a:	c1 e8 18             	shr    $0x18,%eax
8010266d:	83 e0 0f             	and    $0xf,%eax
80102670:	09 d0                	or     %edx,%eax
80102672:	83 c8 e0             	or     $0xffffffe0,%eax
80102675:	0f b6 c0             	movzbl %al,%eax
80102678:	89 44 24 04          	mov    %eax,0x4(%esp)
8010267c:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
80102683:	e8 04 fe ff ff       	call   8010248c <outb>
  if(b->flags & B_DIRTY){
80102688:	8b 45 08             	mov    0x8(%ebp),%eax
8010268b:	8b 00                	mov    (%eax),%eax
8010268d:	83 e0 04             	and    $0x4,%eax
80102690:	85 c0                	test   %eax,%eax
80102692:	74 34                	je     801026c8 <idestart+0x10f>
    outb(0x1f7, IDE_CMD_WRITE);
80102694:	c7 44 24 04 30 00 00 	movl   $0x30,0x4(%esp)
8010269b:	00 
8010269c:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026a3:	e8 e4 fd ff ff       	call   8010248c <outb>
    outsl(0x1f0, b->data, 512/4);
801026a8:	8b 45 08             	mov    0x8(%ebp),%eax
801026ab:	83 c0 18             	add    $0x18,%eax
801026ae:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
801026b5:	00 
801026b6:	89 44 24 04          	mov    %eax,0x4(%esp)
801026ba:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
801026c1:	e8 e4 fd ff ff       	call   801024aa <outsl>
801026c6:	eb 14                	jmp    801026dc <idestart+0x123>
  } else {
    outb(0x1f7, IDE_CMD_READ);
801026c8:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
801026cf:	00 
801026d0:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026d7:	e8 b0 fd ff ff       	call   8010248c <outb>
  }
}
801026dc:	c9                   	leave  
801026dd:	c3                   	ret    

801026de <ideintr>:

// Interrupt handler.
void
ideintr(void)
{
801026de:	55                   	push   %ebp
801026df:	89 e5                	mov    %esp,%ebp
801026e1:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  // First queued buffer is the active request.
  acquire(&idelock);
801026e4:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
801026eb:	e8 a8 23 00 00       	call   80104a98 <acquire>
  if((b = idequeue) == 0){
801026f0:	a1 34 b6 10 80       	mov    0x8010b634,%eax
801026f5:	89 45 f4             	mov    %eax,-0xc(%ebp)
801026f8:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801026fc:	75 11                	jne    8010270f <ideintr+0x31>
    release(&idelock);
801026fe:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
80102705:	e8 f0 23 00 00       	call   80104afa <release>
    // cprintf("spurious IDE interrupt\n");
    return;
8010270a:	e9 90 00 00 00       	jmp    8010279f <ideintr+0xc1>
  }
  idequeue = b->qnext;
8010270f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102712:	8b 40 14             	mov    0x14(%eax),%eax
80102715:	a3 34 b6 10 80       	mov    %eax,0x8010b634

  // Read data if needed.
  if(!(b->flags & B_DIRTY) && idewait(1) >= 0)
8010271a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010271d:	8b 00                	mov    (%eax),%eax
8010271f:	83 e0 04             	and    $0x4,%eax
80102722:	85 c0                	test   %eax,%eax
80102724:	75 2e                	jne    80102754 <ideintr+0x76>
80102726:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010272d:	e8 9d fd ff ff       	call   801024cf <idewait>
80102732:	85 c0                	test   %eax,%eax
80102734:	78 1e                	js     80102754 <ideintr+0x76>
    insl(0x1f0, b->data, 512/4);
80102736:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102739:	83 c0 18             	add    $0x18,%eax
8010273c:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
80102743:	00 
80102744:	89 44 24 04          	mov    %eax,0x4(%esp)
80102748:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
8010274f:	e8 13 fd ff ff       	call   80102467 <insl>
  
  // Wake process waiting for this buf.
  b->flags |= B_VALID;
80102754:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102757:	8b 00                	mov    (%eax),%eax
80102759:	83 c8 02             	or     $0x2,%eax
8010275c:	89 c2                	mov    %eax,%edx
8010275e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102761:	89 10                	mov    %edx,(%eax)
  b->flags &= ~B_DIRTY;
80102763:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102766:	8b 00                	mov    (%eax),%eax
80102768:	83 e0 fb             	and    $0xfffffffb,%eax
8010276b:	89 c2                	mov    %eax,%edx
8010276d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102770:	89 10                	mov    %edx,(%eax)
  wakeup(b);
80102772:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102775:	89 04 24             	mov    %eax,(%esp)
80102778:	e8 2a 21 00 00       	call   801048a7 <wakeup>
  
  // Start disk on next buf in queue.
  if(idequeue != 0)
8010277d:	a1 34 b6 10 80       	mov    0x8010b634,%eax
80102782:	85 c0                	test   %eax,%eax
80102784:	74 0d                	je     80102793 <ideintr+0xb5>
    idestart(idequeue);
80102786:	a1 34 b6 10 80       	mov    0x8010b634,%eax
8010278b:	89 04 24             	mov    %eax,(%esp)
8010278e:	e8 26 fe ff ff       	call   801025b9 <idestart>

  release(&idelock);
80102793:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
8010279a:	e8 5b 23 00 00       	call   80104afa <release>
}
8010279f:	c9                   	leave  
801027a0:	c3                   	ret    

801027a1 <iderw>:
// Sync buf with disk. 
// If B_DIRTY is set, write buf to disk, clear B_DIRTY, set B_VALID.
// Else if B_VALID is not set, read buf from disk, set B_VALID.
void
iderw(struct buf *b)
{
801027a1:	55                   	push   %ebp
801027a2:	89 e5                	mov    %esp,%ebp
801027a4:	83 ec 28             	sub    $0x28,%esp
  struct buf **pp;

  if(!(b->flags & B_BUSY))
801027a7:	8b 45 08             	mov    0x8(%ebp),%eax
801027aa:	8b 00                	mov    (%eax),%eax
801027ac:	83 e0 01             	and    $0x1,%eax
801027af:	85 c0                	test   %eax,%eax
801027b1:	75 0c                	jne    801027bf <iderw+0x1e>
    panic("iderw: buf not busy");
801027b3:	c7 04 24 b5 81 10 80 	movl   $0x801081b5,(%esp)
801027ba:	e8 7b dd ff ff       	call   8010053a <panic>
  if((b->flags & (B_VALID|B_DIRTY)) == B_VALID)
801027bf:	8b 45 08             	mov    0x8(%ebp),%eax
801027c2:	8b 00                	mov    (%eax),%eax
801027c4:	83 e0 06             	and    $0x6,%eax
801027c7:	83 f8 02             	cmp    $0x2,%eax
801027ca:	75 0c                	jne    801027d8 <iderw+0x37>
    panic("iderw: nothing to do");
801027cc:	c7 04 24 c9 81 10 80 	movl   $0x801081c9,(%esp)
801027d3:	e8 62 dd ff ff       	call   8010053a <panic>
  if(b->dev != 0 && !havedisk1)
801027d8:	8b 45 08             	mov    0x8(%ebp),%eax
801027db:	8b 40 04             	mov    0x4(%eax),%eax
801027de:	85 c0                	test   %eax,%eax
801027e0:	74 15                	je     801027f7 <iderw+0x56>
801027e2:	a1 38 b6 10 80       	mov    0x8010b638,%eax
801027e7:	85 c0                	test   %eax,%eax
801027e9:	75 0c                	jne    801027f7 <iderw+0x56>
    panic("iderw: ide disk 1 not present");
801027eb:	c7 04 24 de 81 10 80 	movl   $0x801081de,(%esp)
801027f2:	e8 43 dd ff ff       	call   8010053a <panic>

  acquire(&idelock);  //DOC:acquire-lock
801027f7:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
801027fe:	e8 95 22 00 00       	call   80104a98 <acquire>

  // Append b to idequeue.
  b->qnext = 0;
80102803:	8b 45 08             	mov    0x8(%ebp),%eax
80102806:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
8010280d:	c7 45 f4 34 b6 10 80 	movl   $0x8010b634,-0xc(%ebp)
80102814:	eb 0b                	jmp    80102821 <iderw+0x80>
80102816:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102819:	8b 00                	mov    (%eax),%eax
8010281b:	83 c0 14             	add    $0x14,%eax
8010281e:	89 45 f4             	mov    %eax,-0xc(%ebp)
80102821:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102824:	8b 00                	mov    (%eax),%eax
80102826:	85 c0                	test   %eax,%eax
80102828:	75 ec                	jne    80102816 <iderw+0x75>
    ;
  *pp = b;
8010282a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010282d:	8b 55 08             	mov    0x8(%ebp),%edx
80102830:	89 10                	mov    %edx,(%eax)
  
  // Start disk if necessary.
  if(idequeue == b)
80102832:	a1 34 b6 10 80       	mov    0x8010b634,%eax
80102837:	3b 45 08             	cmp    0x8(%ebp),%eax
8010283a:	75 0d                	jne    80102849 <iderw+0xa8>
    idestart(b);
8010283c:	8b 45 08             	mov    0x8(%ebp),%eax
8010283f:	89 04 24             	mov    %eax,(%esp)
80102842:	e8 72 fd ff ff       	call   801025b9 <idestart>
  
  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
80102847:	eb 15                	jmp    8010285e <iderw+0xbd>
80102849:	eb 13                	jmp    8010285e <iderw+0xbd>
    sleep(b, &idelock);
8010284b:	c7 44 24 04 00 b6 10 	movl   $0x8010b600,0x4(%esp)
80102852:	80 
80102853:	8b 45 08             	mov    0x8(%ebp),%eax
80102856:	89 04 24             	mov    %eax,(%esp)
80102859:	e8 70 1f 00 00       	call   801047ce <sleep>
  // Start disk if necessary.
  if(idequeue == b)
    idestart(b);
  
  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
8010285e:	8b 45 08             	mov    0x8(%ebp),%eax
80102861:	8b 00                	mov    (%eax),%eax
80102863:	83 e0 06             	and    $0x6,%eax
80102866:	83 f8 02             	cmp    $0x2,%eax
80102869:	75 e0                	jne    8010284b <iderw+0xaa>
    sleep(b, &idelock);
  }

  release(&idelock);
8010286b:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
80102872:	e8 83 22 00 00       	call   80104afa <release>
}
80102877:	c9                   	leave  
80102878:	c3                   	ret    

80102879 <ioapicread>:
  uint data;
};

static uint
ioapicread(int reg)
{
80102879:	55                   	push   %ebp
8010287a:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
8010287c:	a1 34 f8 10 80       	mov    0x8010f834,%eax
80102881:	8b 55 08             	mov    0x8(%ebp),%edx
80102884:	89 10                	mov    %edx,(%eax)
  return ioapic->data;
80102886:	a1 34 f8 10 80       	mov    0x8010f834,%eax
8010288b:	8b 40 10             	mov    0x10(%eax),%eax
}
8010288e:	5d                   	pop    %ebp
8010288f:	c3                   	ret    

80102890 <ioapicwrite>:

static void
ioapicwrite(int reg, uint data)
{
80102890:	55                   	push   %ebp
80102891:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
80102893:	a1 34 f8 10 80       	mov    0x8010f834,%eax
80102898:	8b 55 08             	mov    0x8(%ebp),%edx
8010289b:	89 10                	mov    %edx,(%eax)
  ioapic->data = data;
8010289d:	a1 34 f8 10 80       	mov    0x8010f834,%eax
801028a2:	8b 55 0c             	mov    0xc(%ebp),%edx
801028a5:	89 50 10             	mov    %edx,0x10(%eax)
}
801028a8:	5d                   	pop    %ebp
801028a9:	c3                   	ret    

801028aa <ioapicinit>:

void
ioapicinit(void)
{
801028aa:	55                   	push   %ebp
801028ab:	89 e5                	mov    %esp,%ebp
801028ad:	83 ec 28             	sub    $0x28,%esp
  int i, id, maxintr;

  if(!ismp)
801028b0:	a1 04 f9 10 80       	mov    0x8010f904,%eax
801028b5:	85 c0                	test   %eax,%eax
801028b7:	75 05                	jne    801028be <ioapicinit+0x14>
    return;
801028b9:	e9 9d 00 00 00       	jmp    8010295b <ioapicinit+0xb1>

  ioapic = (volatile struct ioapic*)IOAPIC;
801028be:	c7 05 34 f8 10 80 00 	movl   $0xfec00000,0x8010f834
801028c5:	00 c0 fe 
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
801028c8:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801028cf:	e8 a5 ff ff ff       	call   80102879 <ioapicread>
801028d4:	c1 e8 10             	shr    $0x10,%eax
801028d7:	25 ff 00 00 00       	and    $0xff,%eax
801028dc:	89 45 f0             	mov    %eax,-0x10(%ebp)
  id = ioapicread(REG_ID) >> 24;
801028df:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801028e6:	e8 8e ff ff ff       	call   80102879 <ioapicread>
801028eb:	c1 e8 18             	shr    $0x18,%eax
801028ee:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if(id != ioapicid)
801028f1:	0f b6 05 00 f9 10 80 	movzbl 0x8010f900,%eax
801028f8:	0f b6 c0             	movzbl %al,%eax
801028fb:	3b 45 ec             	cmp    -0x14(%ebp),%eax
801028fe:	74 0c                	je     8010290c <ioapicinit+0x62>
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");
80102900:	c7 04 24 fc 81 10 80 	movl   $0x801081fc,(%esp)
80102907:	e8 94 da ff ff       	call   801003a0 <cprintf>

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
8010290c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102913:	eb 3e                	jmp    80102953 <ioapicinit+0xa9>
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
80102915:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102918:	83 c0 20             	add    $0x20,%eax
8010291b:	0d 00 00 01 00       	or     $0x10000,%eax
80102920:	8b 55 f4             	mov    -0xc(%ebp),%edx
80102923:	83 c2 08             	add    $0x8,%edx
80102926:	01 d2                	add    %edx,%edx
80102928:	89 44 24 04          	mov    %eax,0x4(%esp)
8010292c:	89 14 24             	mov    %edx,(%esp)
8010292f:	e8 5c ff ff ff       	call   80102890 <ioapicwrite>
    ioapicwrite(REG_TABLE+2*i+1, 0);
80102934:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102937:	83 c0 08             	add    $0x8,%eax
8010293a:	01 c0                	add    %eax,%eax
8010293c:	83 c0 01             	add    $0x1,%eax
8010293f:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102946:	00 
80102947:	89 04 24             	mov    %eax,(%esp)
8010294a:	e8 41 ff ff ff       	call   80102890 <ioapicwrite>
  if(id != ioapicid)
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
8010294f:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80102953:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102956:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80102959:	7e ba                	jle    80102915 <ioapicinit+0x6b>
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
    ioapicwrite(REG_TABLE+2*i+1, 0);
  }
}
8010295b:	c9                   	leave  
8010295c:	c3                   	ret    

8010295d <ioapicenable>:

void
ioapicenable(int irq, int cpunum)
{
8010295d:	55                   	push   %ebp
8010295e:	89 e5                	mov    %esp,%ebp
80102960:	83 ec 08             	sub    $0x8,%esp
  if(!ismp)
80102963:	a1 04 f9 10 80       	mov    0x8010f904,%eax
80102968:	85 c0                	test   %eax,%eax
8010296a:	75 02                	jne    8010296e <ioapicenable+0x11>
    return;
8010296c:	eb 37                	jmp    801029a5 <ioapicenable+0x48>

  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
8010296e:	8b 45 08             	mov    0x8(%ebp),%eax
80102971:	83 c0 20             	add    $0x20,%eax
80102974:	8b 55 08             	mov    0x8(%ebp),%edx
80102977:	83 c2 08             	add    $0x8,%edx
8010297a:	01 d2                	add    %edx,%edx
8010297c:	89 44 24 04          	mov    %eax,0x4(%esp)
80102980:	89 14 24             	mov    %edx,(%esp)
80102983:	e8 08 ff ff ff       	call   80102890 <ioapicwrite>
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
80102988:	8b 45 0c             	mov    0xc(%ebp),%eax
8010298b:	c1 e0 18             	shl    $0x18,%eax
8010298e:	8b 55 08             	mov    0x8(%ebp),%edx
80102991:	83 c2 08             	add    $0x8,%edx
80102994:	01 d2                	add    %edx,%edx
80102996:	83 c2 01             	add    $0x1,%edx
80102999:	89 44 24 04          	mov    %eax,0x4(%esp)
8010299d:	89 14 24             	mov    %edx,(%esp)
801029a0:	e8 eb fe ff ff       	call   80102890 <ioapicwrite>
}
801029a5:	c9                   	leave  
801029a6:	c3                   	ret    

801029a7 <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
801029a7:	55                   	push   %ebp
801029a8:	89 e5                	mov    %esp,%ebp
801029aa:	8b 45 08             	mov    0x8(%ebp),%eax
801029ad:	05 00 00 00 80       	add    $0x80000000,%eax
801029b2:	5d                   	pop    %ebp
801029b3:	c3                   	ret    

801029b4 <kinit1>:
// the pages mapped by entrypgdir on free list.
// 2. main() calls kinit2() with the rest of the physical pages
// after installing a full page table that maps them on all cores.
void
kinit1(void *vstart, void *vend)
{
801029b4:	55                   	push   %ebp
801029b5:	89 e5                	mov    %esp,%ebp
801029b7:	83 ec 18             	sub    $0x18,%esp
  initlock(&kmem.lock, "kmem");
801029ba:	c7 44 24 04 2e 82 10 	movl   $0x8010822e,0x4(%esp)
801029c1:	80 
801029c2:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
801029c9:	e8 a9 20 00 00       	call   80104a77 <initlock>
  kmem.use_lock = 0;
801029ce:	c7 05 74 f8 10 80 00 	movl   $0x0,0x8010f874
801029d5:	00 00 00 
  freerange(vstart, vend);
801029d8:	8b 45 0c             	mov    0xc(%ebp),%eax
801029db:	89 44 24 04          	mov    %eax,0x4(%esp)
801029df:	8b 45 08             	mov    0x8(%ebp),%eax
801029e2:	89 04 24             	mov    %eax,(%esp)
801029e5:	e8 26 00 00 00       	call   80102a10 <freerange>
}
801029ea:	c9                   	leave  
801029eb:	c3                   	ret    

801029ec <kinit2>:

void
kinit2(void *vstart, void *vend)
{
801029ec:	55                   	push   %ebp
801029ed:	89 e5                	mov    %esp,%ebp
801029ef:	83 ec 18             	sub    $0x18,%esp
  freerange(vstart, vend);
801029f2:	8b 45 0c             	mov    0xc(%ebp),%eax
801029f5:	89 44 24 04          	mov    %eax,0x4(%esp)
801029f9:	8b 45 08             	mov    0x8(%ebp),%eax
801029fc:	89 04 24             	mov    %eax,(%esp)
801029ff:	e8 0c 00 00 00       	call   80102a10 <freerange>
  kmem.use_lock = 1;
80102a04:	c7 05 74 f8 10 80 01 	movl   $0x1,0x8010f874
80102a0b:	00 00 00 
}
80102a0e:	c9                   	leave  
80102a0f:	c3                   	ret    

80102a10 <freerange>:

void
freerange(void *vstart, void *vend)
{
80102a10:	55                   	push   %ebp
80102a11:	89 e5                	mov    %esp,%ebp
80102a13:	83 ec 28             	sub    $0x28,%esp
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
80102a16:	8b 45 08             	mov    0x8(%ebp),%eax
80102a19:	05 ff 0f 00 00       	add    $0xfff,%eax
80102a1e:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80102a23:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a26:	eb 12                	jmp    80102a3a <freerange+0x2a>
    kfree(p);
80102a28:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a2b:	89 04 24             	mov    %eax,(%esp)
80102a2e:	e8 16 00 00 00       	call   80102a49 <kfree>
void
freerange(void *vstart, void *vend)
{
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a33:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80102a3a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a3d:	05 00 10 00 00       	add    $0x1000,%eax
80102a42:	3b 45 0c             	cmp    0xc(%ebp),%eax
80102a45:	76 e1                	jbe    80102a28 <freerange+0x18>
    kfree(p);
}
80102a47:	c9                   	leave  
80102a48:	c3                   	ret    

80102a49 <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(char *v)
{
80102a49:	55                   	push   %ebp
80102a4a:	89 e5                	mov    %esp,%ebp
80102a4c:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if((uint)v % PGSIZE || v < end || v2p(v) >= PHYSTOP)
80102a4f:	8b 45 08             	mov    0x8(%ebp),%eax
80102a52:	25 ff 0f 00 00       	and    $0xfff,%eax
80102a57:	85 c0                	test   %eax,%eax
80102a59:	75 1b                	jne    80102a76 <kfree+0x2d>
80102a5b:	81 7d 08 fc 26 11 80 	cmpl   $0x801126fc,0x8(%ebp)
80102a62:	72 12                	jb     80102a76 <kfree+0x2d>
80102a64:	8b 45 08             	mov    0x8(%ebp),%eax
80102a67:	89 04 24             	mov    %eax,(%esp)
80102a6a:	e8 38 ff ff ff       	call   801029a7 <v2p>
80102a6f:	3d ff ff ff 0d       	cmp    $0xdffffff,%eax
80102a74:	76 0c                	jbe    80102a82 <kfree+0x39>
    panic("kfree");
80102a76:	c7 04 24 33 82 10 80 	movl   $0x80108233,(%esp)
80102a7d:	e8 b8 da ff ff       	call   8010053a <panic>

  // Fill with junk to catch dangling refs.
  memset(v, 1, PGSIZE);
80102a82:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80102a89:	00 
80102a8a:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102a91:	00 
80102a92:	8b 45 08             	mov    0x8(%ebp),%eax
80102a95:	89 04 24             	mov    %eax,(%esp)
80102a98:	e8 4f 22 00 00       	call   80104cec <memset>

  if(kmem.use_lock)
80102a9d:	a1 74 f8 10 80       	mov    0x8010f874,%eax
80102aa2:	85 c0                	test   %eax,%eax
80102aa4:	74 0c                	je     80102ab2 <kfree+0x69>
    acquire(&kmem.lock);
80102aa6:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
80102aad:	e8 e6 1f 00 00       	call   80104a98 <acquire>
  r = (struct run*)v;
80102ab2:	8b 45 08             	mov    0x8(%ebp),%eax
80102ab5:	89 45 f4             	mov    %eax,-0xc(%ebp)
  r->next = kmem.freelist;
80102ab8:	8b 15 78 f8 10 80    	mov    0x8010f878,%edx
80102abe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ac1:	89 10                	mov    %edx,(%eax)
  kmem.freelist = r;
80102ac3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ac6:	a3 78 f8 10 80       	mov    %eax,0x8010f878
  if(kmem.use_lock)
80102acb:	a1 74 f8 10 80       	mov    0x8010f874,%eax
80102ad0:	85 c0                	test   %eax,%eax
80102ad2:	74 0c                	je     80102ae0 <kfree+0x97>
    release(&kmem.lock);
80102ad4:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
80102adb:	e8 1a 20 00 00       	call   80104afa <release>
}
80102ae0:	c9                   	leave  
80102ae1:	c3                   	ret    

80102ae2 <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
char*
kalloc(void)
{
80102ae2:	55                   	push   %ebp
80102ae3:	89 e5                	mov    %esp,%ebp
80102ae5:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if(kmem.use_lock)
80102ae8:	a1 74 f8 10 80       	mov    0x8010f874,%eax
80102aed:	85 c0                	test   %eax,%eax
80102aef:	74 0c                	je     80102afd <kalloc+0x1b>
    acquire(&kmem.lock);
80102af1:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
80102af8:	e8 9b 1f 00 00       	call   80104a98 <acquire>
  r = kmem.freelist;
80102afd:	a1 78 f8 10 80       	mov    0x8010f878,%eax
80102b02:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(r)
80102b05:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80102b09:	74 0a                	je     80102b15 <kalloc+0x33>
    kmem.freelist = r->next;
80102b0b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102b0e:	8b 00                	mov    (%eax),%eax
80102b10:	a3 78 f8 10 80       	mov    %eax,0x8010f878
  if(kmem.use_lock)
80102b15:	a1 74 f8 10 80       	mov    0x8010f874,%eax
80102b1a:	85 c0                	test   %eax,%eax
80102b1c:	74 0c                	je     80102b2a <kalloc+0x48>
    release(&kmem.lock);
80102b1e:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
80102b25:	e8 d0 1f 00 00       	call   80104afa <release>
  return (char*)r;
80102b2a:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102b2d:	c9                   	leave  
80102b2e:	c3                   	ret    

80102b2f <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80102b2f:	55                   	push   %ebp
80102b30:	89 e5                	mov    %esp,%ebp
80102b32:	83 ec 14             	sub    $0x14,%esp
80102b35:	8b 45 08             	mov    0x8(%ebp),%eax
80102b38:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b3c:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
80102b40:	89 c2                	mov    %eax,%edx
80102b42:	ec                   	in     (%dx),%al
80102b43:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
80102b46:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
80102b4a:	c9                   	leave  
80102b4b:	c3                   	ret    

80102b4c <kbdgetc>:
#include "defs.h"
#include "kbd.h"

int
kbdgetc(void)
{
80102b4c:	55                   	push   %ebp
80102b4d:	89 e5                	mov    %esp,%ebp
80102b4f:	83 ec 14             	sub    $0x14,%esp
  static uchar *charcode[4] = {
    normalmap, shiftmap, ctlmap, ctlmap
  };
  uint st, data, c;

  st = inb(KBSTATP);
80102b52:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
80102b59:	e8 d1 ff ff ff       	call   80102b2f <inb>
80102b5e:	0f b6 c0             	movzbl %al,%eax
80102b61:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((st & KBS_DIB) == 0)
80102b64:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102b67:	83 e0 01             	and    $0x1,%eax
80102b6a:	85 c0                	test   %eax,%eax
80102b6c:	75 0a                	jne    80102b78 <kbdgetc+0x2c>
    return -1;
80102b6e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102b73:	e9 25 01 00 00       	jmp    80102c9d <kbdgetc+0x151>
  data = inb(KBDATAP);
80102b78:	c7 04 24 60 00 00 00 	movl   $0x60,(%esp)
80102b7f:	e8 ab ff ff ff       	call   80102b2f <inb>
80102b84:	0f b6 c0             	movzbl %al,%eax
80102b87:	89 45 fc             	mov    %eax,-0x4(%ebp)

  if(data == 0xE0){
80102b8a:	81 7d fc e0 00 00 00 	cmpl   $0xe0,-0x4(%ebp)
80102b91:	75 17                	jne    80102baa <kbdgetc+0x5e>
    shift |= E0ESC;
80102b93:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102b98:	83 c8 40             	or     $0x40,%eax
80102b9b:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
    return 0;
80102ba0:	b8 00 00 00 00       	mov    $0x0,%eax
80102ba5:	e9 f3 00 00 00       	jmp    80102c9d <kbdgetc+0x151>
  } else if(data & 0x80){
80102baa:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bad:	25 80 00 00 00       	and    $0x80,%eax
80102bb2:	85 c0                	test   %eax,%eax
80102bb4:	74 45                	je     80102bfb <kbdgetc+0xaf>
    // Key released
    data = (shift & E0ESC ? data : data & 0x7F);
80102bb6:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102bbb:	83 e0 40             	and    $0x40,%eax
80102bbe:	85 c0                	test   %eax,%eax
80102bc0:	75 08                	jne    80102bca <kbdgetc+0x7e>
80102bc2:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bc5:	83 e0 7f             	and    $0x7f,%eax
80102bc8:	eb 03                	jmp    80102bcd <kbdgetc+0x81>
80102bca:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bcd:	89 45 fc             	mov    %eax,-0x4(%ebp)
    shift &= ~(shiftcode[data] | E0ESC);
80102bd0:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bd3:	05 20 90 10 80       	add    $0x80109020,%eax
80102bd8:	0f b6 00             	movzbl (%eax),%eax
80102bdb:	83 c8 40             	or     $0x40,%eax
80102bde:	0f b6 c0             	movzbl %al,%eax
80102be1:	f7 d0                	not    %eax
80102be3:	89 c2                	mov    %eax,%edx
80102be5:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102bea:	21 d0                	and    %edx,%eax
80102bec:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
    return 0;
80102bf1:	b8 00 00 00 00       	mov    $0x0,%eax
80102bf6:	e9 a2 00 00 00       	jmp    80102c9d <kbdgetc+0x151>
  } else if(shift & E0ESC){
80102bfb:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c00:	83 e0 40             	and    $0x40,%eax
80102c03:	85 c0                	test   %eax,%eax
80102c05:	74 14                	je     80102c1b <kbdgetc+0xcf>
    // Last character was an E0 escape; or with 0x80
    data |= 0x80;
80102c07:	81 4d fc 80 00 00 00 	orl    $0x80,-0x4(%ebp)
    shift &= ~E0ESC;
80102c0e:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c13:	83 e0 bf             	and    $0xffffffbf,%eax
80102c16:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
  }

  shift |= shiftcode[data];
80102c1b:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c1e:	05 20 90 10 80       	add    $0x80109020,%eax
80102c23:	0f b6 00             	movzbl (%eax),%eax
80102c26:	0f b6 d0             	movzbl %al,%edx
80102c29:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c2e:	09 d0                	or     %edx,%eax
80102c30:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
  shift ^= togglecode[data];
80102c35:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c38:	05 20 91 10 80       	add    $0x80109120,%eax
80102c3d:	0f b6 00             	movzbl (%eax),%eax
80102c40:	0f b6 d0             	movzbl %al,%edx
80102c43:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c48:	31 d0                	xor    %edx,%eax
80102c4a:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
  c = charcode[shift & (CTL | SHIFT)][data];
80102c4f:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c54:	83 e0 03             	and    $0x3,%eax
80102c57:	8b 14 85 20 95 10 80 	mov    -0x7fef6ae0(,%eax,4),%edx
80102c5e:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c61:	01 d0                	add    %edx,%eax
80102c63:	0f b6 00             	movzbl (%eax),%eax
80102c66:	0f b6 c0             	movzbl %al,%eax
80102c69:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(shift & CAPSLOCK){
80102c6c:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c71:	83 e0 08             	and    $0x8,%eax
80102c74:	85 c0                	test   %eax,%eax
80102c76:	74 22                	je     80102c9a <kbdgetc+0x14e>
    if('a' <= c && c <= 'z')
80102c78:	83 7d f8 60          	cmpl   $0x60,-0x8(%ebp)
80102c7c:	76 0c                	jbe    80102c8a <kbdgetc+0x13e>
80102c7e:	83 7d f8 7a          	cmpl   $0x7a,-0x8(%ebp)
80102c82:	77 06                	ja     80102c8a <kbdgetc+0x13e>
      c += 'A' - 'a';
80102c84:	83 6d f8 20          	subl   $0x20,-0x8(%ebp)
80102c88:	eb 10                	jmp    80102c9a <kbdgetc+0x14e>
    else if('A' <= c && c <= 'Z')
80102c8a:	83 7d f8 40          	cmpl   $0x40,-0x8(%ebp)
80102c8e:	76 0a                	jbe    80102c9a <kbdgetc+0x14e>
80102c90:	83 7d f8 5a          	cmpl   $0x5a,-0x8(%ebp)
80102c94:	77 04                	ja     80102c9a <kbdgetc+0x14e>
      c += 'a' - 'A';
80102c96:	83 45 f8 20          	addl   $0x20,-0x8(%ebp)
  }
  return c;
80102c9a:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80102c9d:	c9                   	leave  
80102c9e:	c3                   	ret    

80102c9f <kbdintr>:

void
kbdintr(void)
{
80102c9f:	55                   	push   %ebp
80102ca0:	89 e5                	mov    %esp,%ebp
80102ca2:	83 ec 18             	sub    $0x18,%esp
  consoleintr(kbdgetc);
80102ca5:	c7 04 24 4c 2b 10 80 	movl   $0x80102b4c,(%esp)
80102cac:	e8 fc da ff ff       	call   801007ad <consoleintr>
}
80102cb1:	c9                   	leave  
80102cb2:	c3                   	ret    

80102cb3 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80102cb3:	55                   	push   %ebp
80102cb4:	89 e5                	mov    %esp,%ebp
80102cb6:	83 ec 08             	sub    $0x8,%esp
80102cb9:	8b 55 08             	mov    0x8(%ebp),%edx
80102cbc:	8b 45 0c             	mov    0xc(%ebp),%eax
80102cbf:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80102cc3:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102cc6:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80102cca:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80102cce:	ee                   	out    %al,(%dx)
}
80102ccf:	c9                   	leave  
80102cd0:	c3                   	ret    

80102cd1 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80102cd1:	55                   	push   %ebp
80102cd2:	89 e5                	mov    %esp,%ebp
80102cd4:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80102cd7:	9c                   	pushf  
80102cd8:	58                   	pop    %eax
80102cd9:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return eflags;
80102cdc:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80102cdf:	c9                   	leave  
80102ce0:	c3                   	ret    

80102ce1 <lapicw>:

volatile uint *lapic;  // Initialized in mp.c

static void
lapicw(int index, int value)
{
80102ce1:	55                   	push   %ebp
80102ce2:	89 e5                	mov    %esp,%ebp
  lapic[index] = value;
80102ce4:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102ce9:	8b 55 08             	mov    0x8(%ebp),%edx
80102cec:	c1 e2 02             	shl    $0x2,%edx
80102cef:	01 c2                	add    %eax,%edx
80102cf1:	8b 45 0c             	mov    0xc(%ebp),%eax
80102cf4:	89 02                	mov    %eax,(%edx)
  lapic[ID];  // wait for write to finish, by reading
80102cf6:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102cfb:	83 c0 20             	add    $0x20,%eax
80102cfe:	8b 00                	mov    (%eax),%eax
}
80102d00:	5d                   	pop    %ebp
80102d01:	c3                   	ret    

80102d02 <lapicinit>:
//PAGEBREAK!

void
lapicinit(void)
{
80102d02:	55                   	push   %ebp
80102d03:	89 e5                	mov    %esp,%ebp
80102d05:	83 ec 08             	sub    $0x8,%esp
  if(!lapic) 
80102d08:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102d0d:	85 c0                	test   %eax,%eax
80102d0f:	75 05                	jne    80102d16 <lapicinit+0x14>
    return;
80102d11:	e9 43 01 00 00       	jmp    80102e59 <lapicinit+0x157>

  // Enable local APIC; set spurious interrupt vector.
  lapicw(SVR, ENABLE | (T_IRQ0 + IRQ_SPURIOUS));
80102d16:	c7 44 24 04 3f 01 00 	movl   $0x13f,0x4(%esp)
80102d1d:	00 
80102d1e:	c7 04 24 3c 00 00 00 	movl   $0x3c,(%esp)
80102d25:	e8 b7 ff ff ff       	call   80102ce1 <lapicw>

  // The timer repeatedly counts down at bus frequency
  // from lapic[TICR] and then issues an interrupt.  
  // If xv6 cared more about precise timekeeping,
  // TICR would be calibrated using an external time source.
  lapicw(TDCR, X1);
80102d2a:	c7 44 24 04 0b 00 00 	movl   $0xb,0x4(%esp)
80102d31:	00 
80102d32:	c7 04 24 f8 00 00 00 	movl   $0xf8,(%esp)
80102d39:	e8 a3 ff ff ff       	call   80102ce1 <lapicw>
  lapicw(TIMER, PERIODIC | (T_IRQ0 + IRQ_TIMER));
80102d3e:	c7 44 24 04 20 00 02 	movl   $0x20020,0x4(%esp)
80102d45:	00 
80102d46:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102d4d:	e8 8f ff ff ff       	call   80102ce1 <lapicw>
  lapicw(TICR, 10000000); 
80102d52:	c7 44 24 04 80 96 98 	movl   $0x989680,0x4(%esp)
80102d59:	00 
80102d5a:	c7 04 24 e0 00 00 00 	movl   $0xe0,(%esp)
80102d61:	e8 7b ff ff ff       	call   80102ce1 <lapicw>

  // Disable logical interrupt lines.
  lapicw(LINT0, MASKED);
80102d66:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102d6d:	00 
80102d6e:	c7 04 24 d4 00 00 00 	movl   $0xd4,(%esp)
80102d75:	e8 67 ff ff ff       	call   80102ce1 <lapicw>
  lapicw(LINT1, MASKED);
80102d7a:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102d81:	00 
80102d82:	c7 04 24 d8 00 00 00 	movl   $0xd8,(%esp)
80102d89:	e8 53 ff ff ff       	call   80102ce1 <lapicw>

  // Disable performance counter overflow interrupts
  // on machines that provide that interrupt entry.
  if(((lapic[VER]>>16) & 0xFF) >= 4)
80102d8e:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102d93:	83 c0 30             	add    $0x30,%eax
80102d96:	8b 00                	mov    (%eax),%eax
80102d98:	c1 e8 10             	shr    $0x10,%eax
80102d9b:	0f b6 c0             	movzbl %al,%eax
80102d9e:	83 f8 03             	cmp    $0x3,%eax
80102da1:	76 14                	jbe    80102db7 <lapicinit+0xb5>
    lapicw(PCINT, MASKED);
80102da3:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102daa:	00 
80102dab:	c7 04 24 d0 00 00 00 	movl   $0xd0,(%esp)
80102db2:	e8 2a ff ff ff       	call   80102ce1 <lapicw>

  // Map error interrupt to IRQ_ERROR.
  lapicw(ERROR, T_IRQ0 + IRQ_ERROR);
80102db7:	c7 44 24 04 33 00 00 	movl   $0x33,0x4(%esp)
80102dbe:	00 
80102dbf:	c7 04 24 dc 00 00 00 	movl   $0xdc,(%esp)
80102dc6:	e8 16 ff ff ff       	call   80102ce1 <lapicw>

  // Clear error status register (requires back-to-back writes).
  lapicw(ESR, 0);
80102dcb:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102dd2:	00 
80102dd3:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102dda:	e8 02 ff ff ff       	call   80102ce1 <lapicw>
  lapicw(ESR, 0);
80102ddf:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102de6:	00 
80102de7:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102dee:	e8 ee fe ff ff       	call   80102ce1 <lapicw>

  // Ack any outstanding interrupts.
  lapicw(EOI, 0);
80102df3:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102dfa:	00 
80102dfb:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80102e02:	e8 da fe ff ff       	call   80102ce1 <lapicw>

  // Send an Init Level De-Assert to synchronise arbitration ID's.
  lapicw(ICRHI, 0);
80102e07:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e0e:	00 
80102e0f:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102e16:	e8 c6 fe ff ff       	call   80102ce1 <lapicw>
  lapicw(ICRLO, BCAST | INIT | LEVEL);
80102e1b:	c7 44 24 04 00 85 08 	movl   $0x88500,0x4(%esp)
80102e22:	00 
80102e23:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102e2a:	e8 b2 fe ff ff       	call   80102ce1 <lapicw>
  while(lapic[ICRLO] & DELIVS)
80102e2f:	90                   	nop
80102e30:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102e35:	05 00 03 00 00       	add    $0x300,%eax
80102e3a:	8b 00                	mov    (%eax),%eax
80102e3c:	25 00 10 00 00       	and    $0x1000,%eax
80102e41:	85 c0                	test   %eax,%eax
80102e43:	75 eb                	jne    80102e30 <lapicinit+0x12e>
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
80102e45:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e4c:	00 
80102e4d:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80102e54:	e8 88 fe ff ff       	call   80102ce1 <lapicw>
}
80102e59:	c9                   	leave  
80102e5a:	c3                   	ret    

80102e5b <cpunum>:

int
cpunum(void)
{
80102e5b:	55                   	push   %ebp
80102e5c:	89 e5                	mov    %esp,%ebp
80102e5e:	83 ec 18             	sub    $0x18,%esp
  // Cannot call cpu when interrupts are enabled:
  // result not guaranteed to last long enough to be used!
  // Would prefer to panic but even printing is chancy here:
  // almost everything, including cprintf and panic, calls cpu,
  // often indirectly through acquire and release.
  if(readeflags()&FL_IF){
80102e61:	e8 6b fe ff ff       	call   80102cd1 <readeflags>
80102e66:	25 00 02 00 00       	and    $0x200,%eax
80102e6b:	85 c0                	test   %eax,%eax
80102e6d:	74 25                	je     80102e94 <cpunum+0x39>
    static int n;
    if(n++ == 0)
80102e6f:	a1 40 b6 10 80       	mov    0x8010b640,%eax
80102e74:	8d 50 01             	lea    0x1(%eax),%edx
80102e77:	89 15 40 b6 10 80    	mov    %edx,0x8010b640
80102e7d:	85 c0                	test   %eax,%eax
80102e7f:	75 13                	jne    80102e94 <cpunum+0x39>
      cprintf("cpu called from %x with interrupts enabled\n",
80102e81:	8b 45 04             	mov    0x4(%ebp),%eax
80102e84:	89 44 24 04          	mov    %eax,0x4(%esp)
80102e88:	c7 04 24 3c 82 10 80 	movl   $0x8010823c,(%esp)
80102e8f:	e8 0c d5 ff ff       	call   801003a0 <cprintf>
        __builtin_return_address(0));
  }

  if(lapic)
80102e94:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102e99:	85 c0                	test   %eax,%eax
80102e9b:	74 0f                	je     80102eac <cpunum+0x51>
    return lapic[ID]>>24;
80102e9d:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102ea2:	83 c0 20             	add    $0x20,%eax
80102ea5:	8b 00                	mov    (%eax),%eax
80102ea7:	c1 e8 18             	shr    $0x18,%eax
80102eaa:	eb 05                	jmp    80102eb1 <cpunum+0x56>
  return 0;
80102eac:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102eb1:	c9                   	leave  
80102eb2:	c3                   	ret    

80102eb3 <lapiceoi>:

// Acknowledge interrupt.
void
lapiceoi(void)
{
80102eb3:	55                   	push   %ebp
80102eb4:	89 e5                	mov    %esp,%ebp
80102eb6:	83 ec 08             	sub    $0x8,%esp
  if(lapic)
80102eb9:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102ebe:	85 c0                	test   %eax,%eax
80102ec0:	74 14                	je     80102ed6 <lapiceoi+0x23>
    lapicw(EOI, 0);
80102ec2:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102ec9:	00 
80102eca:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80102ed1:	e8 0b fe ff ff       	call   80102ce1 <lapicw>
}
80102ed6:	c9                   	leave  
80102ed7:	c3                   	ret    

80102ed8 <microdelay>:

// Spin for a given number of microseconds.
// On real hardware would want to tune this dynamically.
void
microdelay(int us)
{
80102ed8:	55                   	push   %ebp
80102ed9:	89 e5                	mov    %esp,%ebp
}
80102edb:	5d                   	pop    %ebp
80102edc:	c3                   	ret    

80102edd <lapicstartap>:

// Start additional processor running entry code at addr.
// See Appendix B of MultiProcessor Specification.
void
lapicstartap(uchar apicid, uint addr)
{
80102edd:	55                   	push   %ebp
80102ede:	89 e5                	mov    %esp,%ebp
80102ee0:	83 ec 1c             	sub    $0x1c,%esp
80102ee3:	8b 45 08             	mov    0x8(%ebp),%eax
80102ee6:	88 45 ec             	mov    %al,-0x14(%ebp)
  ushort *wrv;
  
  // "The BSP must initialize CMOS shutdown code to 0AH
  // and the warm reset vector (DWORD based at 40:67) to point at
  // the AP startup code prior to the [universal startup algorithm]."
  outb(IO_RTC, 0xF);  // offset 0xF is shutdown code
80102ee9:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80102ef0:	00 
80102ef1:	c7 04 24 70 00 00 00 	movl   $0x70,(%esp)
80102ef8:	e8 b6 fd ff ff       	call   80102cb3 <outb>
  outb(IO_RTC+1, 0x0A);
80102efd:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80102f04:	00 
80102f05:	c7 04 24 71 00 00 00 	movl   $0x71,(%esp)
80102f0c:	e8 a2 fd ff ff       	call   80102cb3 <outb>
  wrv = (ushort*)P2V((0x40<<4 | 0x67));  // Warm reset vector
80102f11:	c7 45 f8 67 04 00 80 	movl   $0x80000467,-0x8(%ebp)
  wrv[0] = 0;
80102f18:	8b 45 f8             	mov    -0x8(%ebp),%eax
80102f1b:	66 c7 00 00 00       	movw   $0x0,(%eax)
  wrv[1] = addr >> 4;
80102f20:	8b 45 f8             	mov    -0x8(%ebp),%eax
80102f23:	8d 50 02             	lea    0x2(%eax),%edx
80102f26:	8b 45 0c             	mov    0xc(%ebp),%eax
80102f29:	c1 e8 04             	shr    $0x4,%eax
80102f2c:	66 89 02             	mov    %ax,(%edx)

  // "Universal startup algorithm."
  // Send INIT (level-triggered) interrupt to reset other CPU.
  lapicw(ICRHI, apicid<<24);
80102f2f:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80102f33:	c1 e0 18             	shl    $0x18,%eax
80102f36:	89 44 24 04          	mov    %eax,0x4(%esp)
80102f3a:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102f41:	e8 9b fd ff ff       	call   80102ce1 <lapicw>
  lapicw(ICRLO, INIT | LEVEL | ASSERT);
80102f46:	c7 44 24 04 00 c5 00 	movl   $0xc500,0x4(%esp)
80102f4d:	00 
80102f4e:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102f55:	e8 87 fd ff ff       	call   80102ce1 <lapicw>
  microdelay(200);
80102f5a:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102f61:	e8 72 ff ff ff       	call   80102ed8 <microdelay>
  lapicw(ICRLO, INIT | LEVEL);
80102f66:	c7 44 24 04 00 85 00 	movl   $0x8500,0x4(%esp)
80102f6d:	00 
80102f6e:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102f75:	e8 67 fd ff ff       	call   80102ce1 <lapicw>
  microdelay(100);    // should be 10ms, but too slow in Bochs!
80102f7a:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
80102f81:	e8 52 ff ff ff       	call   80102ed8 <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
80102f86:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80102f8d:	eb 40                	jmp    80102fcf <lapicstartap+0xf2>
    lapicw(ICRHI, apicid<<24);
80102f8f:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80102f93:	c1 e0 18             	shl    $0x18,%eax
80102f96:	89 44 24 04          	mov    %eax,0x4(%esp)
80102f9a:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102fa1:	e8 3b fd ff ff       	call   80102ce1 <lapicw>
    lapicw(ICRLO, STARTUP | (addr>>12));
80102fa6:	8b 45 0c             	mov    0xc(%ebp),%eax
80102fa9:	c1 e8 0c             	shr    $0xc,%eax
80102fac:	80 cc 06             	or     $0x6,%ah
80102faf:	89 44 24 04          	mov    %eax,0x4(%esp)
80102fb3:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102fba:	e8 22 fd ff ff       	call   80102ce1 <lapicw>
    microdelay(200);
80102fbf:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102fc6:	e8 0d ff ff ff       	call   80102ed8 <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
80102fcb:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80102fcf:	83 7d fc 01          	cmpl   $0x1,-0x4(%ebp)
80102fd3:	7e ba                	jle    80102f8f <lapicstartap+0xb2>
    lapicw(ICRHI, apicid<<24);
    lapicw(ICRLO, STARTUP | (addr>>12));
    microdelay(200);
  }
}
80102fd5:	c9                   	leave  
80102fd6:	c3                   	ret    

80102fd7 <initlog>:

static void recover_from_log(void);

void
initlog(void)
{
80102fd7:	55                   	push   %ebp
80102fd8:	89 e5                	mov    %esp,%ebp
80102fda:	83 ec 28             	sub    $0x28,%esp
  if (sizeof(struct logheader) >= BSIZE)
    panic("initlog: too big logheader");

  struct superblock sb;
  initlock(&log.lock, "log");
80102fdd:	c7 44 24 04 68 82 10 	movl   $0x80108268,0x4(%esp)
80102fe4:	80 
80102fe5:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
80102fec:	e8 86 1a 00 00       	call   80104a77 <initlock>
  readsb(ROOTDEV, &sb);
80102ff1:	8d 45 e8             	lea    -0x18(%ebp),%eax
80102ff4:	89 44 24 04          	mov    %eax,0x4(%esp)
80102ff8:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80102fff:	e8 ed e2 ff ff       	call   801012f1 <readsb>
  log.start = sb.size - sb.nlog;
80103004:	8b 55 e8             	mov    -0x18(%ebp),%edx
80103007:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010300a:	29 c2                	sub    %eax,%edx
8010300c:	89 d0                	mov    %edx,%eax
8010300e:	a3 b4 f8 10 80       	mov    %eax,0x8010f8b4
  log.size = sb.nlog;
80103013:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103016:	a3 b8 f8 10 80       	mov    %eax,0x8010f8b8
  log.dev = ROOTDEV;
8010301b:	c7 05 c0 f8 10 80 01 	movl   $0x1,0x8010f8c0
80103022:	00 00 00 
  recover_from_log();
80103025:	e8 9a 01 00 00       	call   801031c4 <recover_from_log>
}
8010302a:	c9                   	leave  
8010302b:	c3                   	ret    

8010302c <install_trans>:

// Copy committed blocks from log to their home location
static void 
install_trans(void)
{
8010302c:	55                   	push   %ebp
8010302d:	89 e5                	mov    %esp,%ebp
8010302f:	83 ec 28             	sub    $0x28,%esp
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
80103032:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103039:	e9 8c 00 00 00       	jmp    801030ca <install_trans+0x9e>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
8010303e:	8b 15 b4 f8 10 80    	mov    0x8010f8b4,%edx
80103044:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103047:	01 d0                	add    %edx,%eax
80103049:	83 c0 01             	add    $0x1,%eax
8010304c:	89 c2                	mov    %eax,%edx
8010304e:	a1 c0 f8 10 80       	mov    0x8010f8c0,%eax
80103053:	89 54 24 04          	mov    %edx,0x4(%esp)
80103057:	89 04 24             	mov    %eax,(%esp)
8010305a:	e8 47 d1 ff ff       	call   801001a6 <bread>
8010305f:	89 45 f0             	mov    %eax,-0x10(%ebp)
    struct buf *dbuf = bread(log.dev, log.lh.sector[tail]); // read dst
80103062:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103065:	83 c0 10             	add    $0x10,%eax
80103068:	8b 04 85 88 f8 10 80 	mov    -0x7fef0778(,%eax,4),%eax
8010306f:	89 c2                	mov    %eax,%edx
80103071:	a1 c0 f8 10 80       	mov    0x8010f8c0,%eax
80103076:	89 54 24 04          	mov    %edx,0x4(%esp)
8010307a:	89 04 24             	mov    %eax,(%esp)
8010307d:	e8 24 d1 ff ff       	call   801001a6 <bread>
80103082:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
80103085:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103088:	8d 50 18             	lea    0x18(%eax),%edx
8010308b:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010308e:	83 c0 18             	add    $0x18,%eax
80103091:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
80103098:	00 
80103099:	89 54 24 04          	mov    %edx,0x4(%esp)
8010309d:	89 04 24             	mov    %eax,(%esp)
801030a0:	e8 16 1d 00 00       	call   80104dbb <memmove>
    bwrite(dbuf);  // write dst to disk
801030a5:	8b 45 ec             	mov    -0x14(%ebp),%eax
801030a8:	89 04 24             	mov    %eax,(%esp)
801030ab:	e8 2d d1 ff ff       	call   801001dd <bwrite>
    brelse(lbuf); 
801030b0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801030b3:	89 04 24             	mov    %eax,(%esp)
801030b6:	e8 5c d1 ff ff       	call   80100217 <brelse>
    brelse(dbuf);
801030bb:	8b 45 ec             	mov    -0x14(%ebp),%eax
801030be:	89 04 24             	mov    %eax,(%esp)
801030c1:	e8 51 d1 ff ff       	call   80100217 <brelse>
static void 
install_trans(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
801030c6:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801030ca:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
801030cf:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801030d2:	0f 8f 66 ff ff ff    	jg     8010303e <install_trans+0x12>
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    bwrite(dbuf);  // write dst to disk
    brelse(lbuf); 
    brelse(dbuf);
  }
}
801030d8:	c9                   	leave  
801030d9:	c3                   	ret    

801030da <read_head>:

// Read the log header from disk into the in-memory log header
static void
read_head(void)
{
801030da:	55                   	push   %ebp
801030db:	89 e5                	mov    %esp,%ebp
801030dd:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
801030e0:	a1 b4 f8 10 80       	mov    0x8010f8b4,%eax
801030e5:	89 c2                	mov    %eax,%edx
801030e7:	a1 c0 f8 10 80       	mov    0x8010f8c0,%eax
801030ec:	89 54 24 04          	mov    %edx,0x4(%esp)
801030f0:	89 04 24             	mov    %eax,(%esp)
801030f3:	e8 ae d0 ff ff       	call   801001a6 <bread>
801030f8:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *lh = (struct logheader *) (buf->data);
801030fb:	8b 45 f0             	mov    -0x10(%ebp),%eax
801030fe:	83 c0 18             	add    $0x18,%eax
80103101:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  log.lh.n = lh->n;
80103104:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103107:	8b 00                	mov    (%eax),%eax
80103109:	a3 c4 f8 10 80       	mov    %eax,0x8010f8c4
  for (i = 0; i < log.lh.n; i++) {
8010310e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103115:	eb 1b                	jmp    80103132 <read_head+0x58>
    log.lh.sector[i] = lh->sector[i];
80103117:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010311a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010311d:	8b 44 90 04          	mov    0x4(%eax,%edx,4),%eax
80103121:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103124:	83 c2 10             	add    $0x10,%edx
80103127:	89 04 95 88 f8 10 80 	mov    %eax,-0x7fef0778(,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *lh = (struct logheader *) (buf->data);
  int i;
  log.lh.n = lh->n;
  for (i = 0; i < log.lh.n; i++) {
8010312e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103132:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
80103137:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010313a:	7f db                	jg     80103117 <read_head+0x3d>
    log.lh.sector[i] = lh->sector[i];
  }
  brelse(buf);
8010313c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010313f:	89 04 24             	mov    %eax,(%esp)
80103142:	e8 d0 d0 ff ff       	call   80100217 <brelse>
}
80103147:	c9                   	leave  
80103148:	c3                   	ret    

80103149 <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
80103149:	55                   	push   %ebp
8010314a:	89 e5                	mov    %esp,%ebp
8010314c:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
8010314f:	a1 b4 f8 10 80       	mov    0x8010f8b4,%eax
80103154:	89 c2                	mov    %eax,%edx
80103156:	a1 c0 f8 10 80       	mov    0x8010f8c0,%eax
8010315b:	89 54 24 04          	mov    %edx,0x4(%esp)
8010315f:	89 04 24             	mov    %eax,(%esp)
80103162:	e8 3f d0 ff ff       	call   801001a6 <bread>
80103167:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *hb = (struct logheader *) (buf->data);
8010316a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010316d:	83 c0 18             	add    $0x18,%eax
80103170:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  hb->n = log.lh.n;
80103173:	8b 15 c4 f8 10 80    	mov    0x8010f8c4,%edx
80103179:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010317c:	89 10                	mov    %edx,(%eax)
  for (i = 0; i < log.lh.n; i++) {
8010317e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103185:	eb 1b                	jmp    801031a2 <write_head+0x59>
    hb->sector[i] = log.lh.sector[i];
80103187:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010318a:	83 c0 10             	add    $0x10,%eax
8010318d:	8b 0c 85 88 f8 10 80 	mov    -0x7fef0778(,%eax,4),%ecx
80103194:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103197:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010319a:	89 4c 90 04          	mov    %ecx,0x4(%eax,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
  for (i = 0; i < log.lh.n; i++) {
8010319e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801031a2:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
801031a7:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801031aa:	7f db                	jg     80103187 <write_head+0x3e>
    hb->sector[i] = log.lh.sector[i];
  }
  bwrite(buf);
801031ac:	8b 45 f0             	mov    -0x10(%ebp),%eax
801031af:	89 04 24             	mov    %eax,(%esp)
801031b2:	e8 26 d0 ff ff       	call   801001dd <bwrite>
  brelse(buf);
801031b7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801031ba:	89 04 24             	mov    %eax,(%esp)
801031bd:	e8 55 d0 ff ff       	call   80100217 <brelse>
}
801031c2:	c9                   	leave  
801031c3:	c3                   	ret    

801031c4 <recover_from_log>:

static void
recover_from_log(void)
{
801031c4:	55                   	push   %ebp
801031c5:	89 e5                	mov    %esp,%ebp
801031c7:	83 ec 08             	sub    $0x8,%esp
  read_head();      
801031ca:	e8 0b ff ff ff       	call   801030da <read_head>
  install_trans(); // if committed, copy from log to disk
801031cf:	e8 58 fe ff ff       	call   8010302c <install_trans>
  log.lh.n = 0;
801031d4:	c7 05 c4 f8 10 80 00 	movl   $0x0,0x8010f8c4
801031db:	00 00 00 
  write_head(); // clear the log
801031de:	e8 66 ff ff ff       	call   80103149 <write_head>
}
801031e3:	c9                   	leave  
801031e4:	c3                   	ret    

801031e5 <begin_trans>:

void
begin_trans(void)
{
801031e5:	55                   	push   %ebp
801031e6:	89 e5                	mov    %esp,%ebp
801031e8:	83 ec 18             	sub    $0x18,%esp
  acquire(&log.lock);
801031eb:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
801031f2:	e8 a1 18 00 00       	call   80104a98 <acquire>
  while (log.busy) {
801031f7:	eb 14                	jmp    8010320d <begin_trans+0x28>
    sleep(&log, &log.lock);
801031f9:	c7 44 24 04 80 f8 10 	movl   $0x8010f880,0x4(%esp)
80103200:	80 
80103201:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
80103208:	e8 c1 15 00 00       	call   801047ce <sleep>

void
begin_trans(void)
{
  acquire(&log.lock);
  while (log.busy) {
8010320d:	a1 bc f8 10 80       	mov    0x8010f8bc,%eax
80103212:	85 c0                	test   %eax,%eax
80103214:	75 e3                	jne    801031f9 <begin_trans+0x14>
    sleep(&log, &log.lock);
  }
  log.busy = 1;
80103216:	c7 05 bc f8 10 80 01 	movl   $0x1,0x8010f8bc
8010321d:	00 00 00 
  release(&log.lock);
80103220:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
80103227:	e8 ce 18 00 00       	call   80104afa <release>
}
8010322c:	c9                   	leave  
8010322d:	c3                   	ret    

8010322e <commit_trans>:

void
commit_trans(void)
{
8010322e:	55                   	push   %ebp
8010322f:	89 e5                	mov    %esp,%ebp
80103231:	83 ec 18             	sub    $0x18,%esp
  if (log.lh.n > 0) {
80103234:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
80103239:	85 c0                	test   %eax,%eax
8010323b:	7e 19                	jle    80103256 <commit_trans+0x28>
    write_head();    // Write header to disk -- the real commit
8010323d:	e8 07 ff ff ff       	call   80103149 <write_head>
    install_trans(); // Now install writes to home locations
80103242:	e8 e5 fd ff ff       	call   8010302c <install_trans>
    log.lh.n = 0; 
80103247:	c7 05 c4 f8 10 80 00 	movl   $0x0,0x8010f8c4
8010324e:	00 00 00 
    write_head();    // Erase the transaction from the log
80103251:	e8 f3 fe ff ff       	call   80103149 <write_head>
  }
  
  acquire(&log.lock);
80103256:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
8010325d:	e8 36 18 00 00       	call   80104a98 <acquire>
  log.busy = 0;
80103262:	c7 05 bc f8 10 80 00 	movl   $0x0,0x8010f8bc
80103269:	00 00 00 
  wakeup(&log);
8010326c:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
80103273:	e8 2f 16 00 00       	call   801048a7 <wakeup>
  release(&log.lock);
80103278:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
8010327f:	e8 76 18 00 00       	call   80104afa <release>
}
80103284:	c9                   	leave  
80103285:	c3                   	ret    

80103286 <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
80103286:	55                   	push   %ebp
80103287:	89 e5                	mov    %esp,%ebp
80103289:	83 ec 28             	sub    $0x28,%esp
  int i;

  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
8010328c:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
80103291:	83 f8 09             	cmp    $0x9,%eax
80103294:	7f 12                	jg     801032a8 <log_write+0x22>
80103296:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
8010329b:	8b 15 b8 f8 10 80    	mov    0x8010f8b8,%edx
801032a1:	83 ea 01             	sub    $0x1,%edx
801032a4:	39 d0                	cmp    %edx,%eax
801032a6:	7c 0c                	jl     801032b4 <log_write+0x2e>
    panic("too big a transaction");
801032a8:	c7 04 24 6c 82 10 80 	movl   $0x8010826c,(%esp)
801032af:	e8 86 d2 ff ff       	call   8010053a <panic>
  if (!log.busy)
801032b4:	a1 bc f8 10 80       	mov    0x8010f8bc,%eax
801032b9:	85 c0                	test   %eax,%eax
801032bb:	75 0c                	jne    801032c9 <log_write+0x43>
    panic("write outside of trans");
801032bd:	c7 04 24 82 82 10 80 	movl   $0x80108282,(%esp)
801032c4:	e8 71 d2 ff ff       	call   8010053a <panic>

  for (i = 0; i < log.lh.n; i++) {
801032c9:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801032d0:	eb 1f                	jmp    801032f1 <log_write+0x6b>
    if (log.lh.sector[i] == b->sector)   // log absorbtion?
801032d2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801032d5:	83 c0 10             	add    $0x10,%eax
801032d8:	8b 04 85 88 f8 10 80 	mov    -0x7fef0778(,%eax,4),%eax
801032df:	89 c2                	mov    %eax,%edx
801032e1:	8b 45 08             	mov    0x8(%ebp),%eax
801032e4:	8b 40 08             	mov    0x8(%eax),%eax
801032e7:	39 c2                	cmp    %eax,%edx
801032e9:	75 02                	jne    801032ed <log_write+0x67>
      break;
801032eb:	eb 0e                	jmp    801032fb <log_write+0x75>
  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
    panic("too big a transaction");
  if (!log.busy)
    panic("write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
801032ed:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801032f1:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
801032f6:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801032f9:	7f d7                	jg     801032d2 <log_write+0x4c>
    if (log.lh.sector[i] == b->sector)   // log absorbtion?
      break;
  }
  log.lh.sector[i] = b->sector;
801032fb:	8b 45 08             	mov    0x8(%ebp),%eax
801032fe:	8b 40 08             	mov    0x8(%eax),%eax
80103301:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103304:	83 c2 10             	add    $0x10,%edx
80103307:	89 04 95 88 f8 10 80 	mov    %eax,-0x7fef0778(,%edx,4)
  struct buf *lbuf = bread(b->dev, log.start+i+1);
8010330e:	8b 15 b4 f8 10 80    	mov    0x8010f8b4,%edx
80103314:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103317:	01 d0                	add    %edx,%eax
80103319:	83 c0 01             	add    $0x1,%eax
8010331c:	89 c2                	mov    %eax,%edx
8010331e:	8b 45 08             	mov    0x8(%ebp),%eax
80103321:	8b 40 04             	mov    0x4(%eax),%eax
80103324:	89 54 24 04          	mov    %edx,0x4(%esp)
80103328:	89 04 24             	mov    %eax,(%esp)
8010332b:	e8 76 ce ff ff       	call   801001a6 <bread>
80103330:	89 45 f0             	mov    %eax,-0x10(%ebp)
  memmove(lbuf->data, b->data, BSIZE);
80103333:	8b 45 08             	mov    0x8(%ebp),%eax
80103336:	8d 50 18             	lea    0x18(%eax),%edx
80103339:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010333c:	83 c0 18             	add    $0x18,%eax
8010333f:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
80103346:	00 
80103347:	89 54 24 04          	mov    %edx,0x4(%esp)
8010334b:	89 04 24             	mov    %eax,(%esp)
8010334e:	e8 68 1a 00 00       	call   80104dbb <memmove>
  bwrite(lbuf);
80103353:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103356:	89 04 24             	mov    %eax,(%esp)
80103359:	e8 7f ce ff ff       	call   801001dd <bwrite>
  brelse(lbuf);
8010335e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103361:	89 04 24             	mov    %eax,(%esp)
80103364:	e8 ae ce ff ff       	call   80100217 <brelse>
  if (i == log.lh.n)
80103369:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
8010336e:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103371:	75 0d                	jne    80103380 <log_write+0xfa>
    log.lh.n++;
80103373:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
80103378:	83 c0 01             	add    $0x1,%eax
8010337b:	a3 c4 f8 10 80       	mov    %eax,0x8010f8c4
  b->flags |= B_DIRTY; // XXX prevent eviction
80103380:	8b 45 08             	mov    0x8(%ebp),%eax
80103383:	8b 00                	mov    (%eax),%eax
80103385:	83 c8 04             	or     $0x4,%eax
80103388:	89 c2                	mov    %eax,%edx
8010338a:	8b 45 08             	mov    0x8(%ebp),%eax
8010338d:	89 10                	mov    %edx,(%eax)
}
8010338f:	c9                   	leave  
80103390:	c3                   	ret    

80103391 <v2p>:
80103391:	55                   	push   %ebp
80103392:	89 e5                	mov    %esp,%ebp
80103394:	8b 45 08             	mov    0x8(%ebp),%eax
80103397:	05 00 00 00 80       	add    $0x80000000,%eax
8010339c:	5d                   	pop    %ebp
8010339d:	c3                   	ret    

8010339e <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
8010339e:	55                   	push   %ebp
8010339f:	89 e5                	mov    %esp,%ebp
801033a1:	8b 45 08             	mov    0x8(%ebp),%eax
801033a4:	05 00 00 00 80       	add    $0x80000000,%eax
801033a9:	5d                   	pop    %ebp
801033aa:	c3                   	ret    

801033ab <xchg>:
  asm volatile("sti");
}

static inline uint
xchg(volatile uint *addr, uint newval)
{
801033ab:	55                   	push   %ebp
801033ac:	89 e5                	mov    %esp,%ebp
801033ae:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
801033b1:	8b 55 08             	mov    0x8(%ebp),%edx
801033b4:	8b 45 0c             	mov    0xc(%ebp),%eax
801033b7:	8b 4d 08             	mov    0x8(%ebp),%ecx
801033ba:	f0 87 02             	lock xchg %eax,(%edx)
801033bd:	89 45 fc             	mov    %eax,-0x4(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
801033c0:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
801033c3:	c9                   	leave  
801033c4:	c3                   	ret    

801033c5 <main>:
// Bootstrap processor starts running C code here.
// Allocate a real stack and switch to it, first
// doing some setup required for memory allocator to work.
int
main(void)
{
801033c5:	55                   	push   %ebp
801033c6:	89 e5                	mov    %esp,%ebp
801033c8:	83 e4 f0             	and    $0xfffffff0,%esp
801033cb:	83 ec 10             	sub    $0x10,%esp
  kinit1(end, P2V(4*1024*1024)); // phys page allocator
801033ce:	c7 44 24 04 00 00 40 	movl   $0x80400000,0x4(%esp)
801033d5:	80 
801033d6:	c7 04 24 fc 26 11 80 	movl   $0x801126fc,(%esp)
801033dd:	e8 d2 f5 ff ff       	call   801029b4 <kinit1>
  kvmalloc();      // kernel page table
801033e2:	e8 ca 44 00 00       	call   801078b1 <kvmalloc>
  mpinit();        // collect info about this machine
801033e7:	e8 46 04 00 00       	call   80103832 <mpinit>
  lapicinit();
801033ec:	e8 11 f9 ff ff       	call   80102d02 <lapicinit>
  seginit();       // set up segments
801033f1:	e8 4e 3e 00 00       	call   80107244 <seginit>
  cprintf("\ncpu%d: starting xv6\n\n", cpu->id);
801033f6:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801033fc:	0f b6 00             	movzbl (%eax),%eax
801033ff:	0f b6 c0             	movzbl %al,%eax
80103402:	89 44 24 04          	mov    %eax,0x4(%esp)
80103406:	c7 04 24 99 82 10 80 	movl   $0x80108299,(%esp)
8010340d:	e8 8e cf ff ff       	call   801003a0 <cprintf>
  picinit();       // interrupt controller
80103412:	e8 79 06 00 00       	call   80103a90 <picinit>
  ioapicinit();    // another interrupt controller
80103417:	e8 8e f4 ff ff       	call   801028aa <ioapicinit>
  consoleinit();   // I/O devices & their interrupts
8010341c:	e8 60 d6 ff ff       	call   80100a81 <consoleinit>
  uartinit();      // serial port
80103421:	e8 6d 31 00 00       	call   80106593 <uartinit>
  pinit();         // process table
80103426:	e8 6f 0b 00 00       	call   80103f9a <pinit>
  tvinit();        // trap vectors
8010342b:	e8 15 2d 00 00       	call   80106145 <tvinit>
  binit();         // buffer cache
80103430:	e8 ff cb ff ff       	call   80100034 <binit>
  fileinit();      // file table
80103435:	e8 d0 da ff ff       	call   80100f0a <fileinit>
  iinit();         // inode cache
8010343a:	e8 65 e1 ff ff       	call   801015a4 <iinit>
  ideinit();       // disk
8010343f:	e8 cf f0 ff ff       	call   80102513 <ideinit>
  if(!ismp)
80103444:	a1 04 f9 10 80       	mov    0x8010f904,%eax
80103449:	85 c0                	test   %eax,%eax
8010344b:	75 05                	jne    80103452 <main+0x8d>
    timerinit();   // uniprocessor timer
8010344d:	e8 3e 2c 00 00       	call   80106090 <timerinit>
  startothers();   // start other processors
80103452:	e8 7f 00 00 00       	call   801034d6 <startothers>
  kinit2(P2V(4*1024*1024), P2V(PHYSTOP)); // must come after startothers()
80103457:	c7 44 24 04 00 00 00 	movl   $0x8e000000,0x4(%esp)
8010345e:	8e 
8010345f:	c7 04 24 00 00 40 80 	movl   $0x80400000,(%esp)
80103466:	e8 81 f5 ff ff       	call   801029ec <kinit2>
  userinit();      // first user process
8010346b:	e8 45 0c 00 00       	call   801040b5 <userinit>
  // Finish setting up this processor in mpmain.
  mpmain();
80103470:	e8 1a 00 00 00       	call   8010348f <mpmain>

80103475 <mpenter>:
}

// Other CPUs jump here from entryother.S.
static void
mpenter(void)
{
80103475:	55                   	push   %ebp
80103476:	89 e5                	mov    %esp,%ebp
80103478:	83 ec 08             	sub    $0x8,%esp
  switchkvm(); 
8010347b:	e8 48 44 00 00       	call   801078c8 <switchkvm>
  seginit();
80103480:	e8 bf 3d 00 00       	call   80107244 <seginit>
  lapicinit();
80103485:	e8 78 f8 ff ff       	call   80102d02 <lapicinit>
  mpmain();
8010348a:	e8 00 00 00 00       	call   8010348f <mpmain>

8010348f <mpmain>:
}

// Common CPU setup code.
static void
mpmain(void)
{
8010348f:	55                   	push   %ebp
80103490:	89 e5                	mov    %esp,%ebp
80103492:	83 ec 18             	sub    $0x18,%esp
  cprintf("cpu%d: starting\n", cpu->id);
80103495:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010349b:	0f b6 00             	movzbl (%eax),%eax
8010349e:	0f b6 c0             	movzbl %al,%eax
801034a1:	89 44 24 04          	mov    %eax,0x4(%esp)
801034a5:	c7 04 24 b0 82 10 80 	movl   $0x801082b0,(%esp)
801034ac:	e8 ef ce ff ff       	call   801003a0 <cprintf>
  idtinit();       // load idt register
801034b1:	e8 03 2e 00 00       	call   801062b9 <idtinit>
  xchg(&cpu->started, 1); // tell startothers() we're up
801034b6:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801034bc:	05 a8 00 00 00       	add    $0xa8,%eax
801034c1:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
801034c8:	00 
801034c9:	89 04 24             	mov    %eax,(%esp)
801034cc:	e8 da fe ff ff       	call   801033ab <xchg>
  scheduler();     // start running processes
801034d1:	e8 50 11 00 00       	call   80104626 <scheduler>

801034d6 <startothers>:
pde_t entrypgdir[];  // For entry.S

// Start the non-boot (AP) processors.
static void
startothers(void)
{
801034d6:	55                   	push   %ebp
801034d7:	89 e5                	mov    %esp,%ebp
801034d9:	53                   	push   %ebx
801034da:	83 ec 24             	sub    $0x24,%esp
  char *stack;

  // Write entry code to unused memory at 0x7000.
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
801034dd:	c7 04 24 00 70 00 00 	movl   $0x7000,(%esp)
801034e4:	e8 b5 fe ff ff       	call   8010339e <p2v>
801034e9:	89 45 f0             	mov    %eax,-0x10(%ebp)
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);
801034ec:	b8 8a 00 00 00       	mov    $0x8a,%eax
801034f1:	89 44 24 08          	mov    %eax,0x8(%esp)
801034f5:	c7 44 24 04 0c b5 10 	movl   $0x8010b50c,0x4(%esp)
801034fc:	80 
801034fd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103500:	89 04 24             	mov    %eax,(%esp)
80103503:	e8 b3 18 00 00       	call   80104dbb <memmove>

  for(c = cpus; c < cpus+ncpu; c++){
80103508:	c7 45 f4 20 f9 10 80 	movl   $0x8010f920,-0xc(%ebp)
8010350f:	e9 85 00 00 00       	jmp    80103599 <startothers+0xc3>
    if(c == cpus+cpunum())  // We've started already.
80103514:	e8 42 f9 ff ff       	call   80102e5b <cpunum>
80103519:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
8010351f:	05 20 f9 10 80       	add    $0x8010f920,%eax
80103524:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103527:	75 02                	jne    8010352b <startothers+0x55>
      continue;
80103529:	eb 67                	jmp    80103592 <startothers+0xbc>

    // Tell entryother.S what stack to use, where to enter, and what 
    // pgdir to use. We cannot use kpgdir yet, because the AP processor
    // is running in low  memory, so we use entrypgdir for the APs too.
    stack = kalloc();
8010352b:	e8 b2 f5 ff ff       	call   80102ae2 <kalloc>
80103530:	89 45 ec             	mov    %eax,-0x14(%ebp)
    *(void**)(code-4) = stack + KSTACKSIZE;
80103533:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103536:	83 e8 04             	sub    $0x4,%eax
80103539:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010353c:	81 c2 00 10 00 00    	add    $0x1000,%edx
80103542:	89 10                	mov    %edx,(%eax)
    *(void**)(code-8) = mpenter;
80103544:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103547:	83 e8 08             	sub    $0x8,%eax
8010354a:	c7 00 75 34 10 80    	movl   $0x80103475,(%eax)
    *(int**)(code-12) = (void *) v2p(entrypgdir);
80103550:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103553:	8d 58 f4             	lea    -0xc(%eax),%ebx
80103556:	c7 04 24 00 a0 10 80 	movl   $0x8010a000,(%esp)
8010355d:	e8 2f fe ff ff       	call   80103391 <v2p>
80103562:	89 03                	mov    %eax,(%ebx)

    lapicstartap(c->id, v2p(code));
80103564:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103567:	89 04 24             	mov    %eax,(%esp)
8010356a:	e8 22 fe ff ff       	call   80103391 <v2p>
8010356f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103572:	0f b6 12             	movzbl (%edx),%edx
80103575:	0f b6 d2             	movzbl %dl,%edx
80103578:	89 44 24 04          	mov    %eax,0x4(%esp)
8010357c:	89 14 24             	mov    %edx,(%esp)
8010357f:	e8 59 f9 ff ff       	call   80102edd <lapicstartap>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
80103584:	90                   	nop
80103585:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103588:	8b 80 a8 00 00 00    	mov    0xa8(%eax),%eax
8010358e:	85 c0                	test   %eax,%eax
80103590:	74 f3                	je     80103585 <startothers+0xaf>
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);

  for(c = cpus; c < cpus+ncpu; c++){
80103592:	81 45 f4 bc 00 00 00 	addl   $0xbc,-0xc(%ebp)
80103599:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
8010359e:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
801035a4:	05 20 f9 10 80       	add    $0x8010f920,%eax
801035a9:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801035ac:	0f 87 62 ff ff ff    	ja     80103514 <startothers+0x3e>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
      ;
  }
}
801035b2:	83 c4 24             	add    $0x24,%esp
801035b5:	5b                   	pop    %ebx
801035b6:	5d                   	pop    %ebp
801035b7:	c3                   	ret    

801035b8 <p2v>:
801035b8:	55                   	push   %ebp
801035b9:	89 e5                	mov    %esp,%ebp
801035bb:	8b 45 08             	mov    0x8(%ebp),%eax
801035be:	05 00 00 00 80       	add    $0x80000000,%eax
801035c3:	5d                   	pop    %ebp
801035c4:	c3                   	ret    

801035c5 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
801035c5:	55                   	push   %ebp
801035c6:	89 e5                	mov    %esp,%ebp
801035c8:	83 ec 14             	sub    $0x14,%esp
801035cb:	8b 45 08             	mov    0x8(%ebp),%eax
801035ce:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801035d2:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
801035d6:	89 c2                	mov    %eax,%edx
801035d8:	ec                   	in     (%dx),%al
801035d9:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
801035dc:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
801035e0:	c9                   	leave  
801035e1:	c3                   	ret    

801035e2 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801035e2:	55                   	push   %ebp
801035e3:	89 e5                	mov    %esp,%ebp
801035e5:	83 ec 08             	sub    $0x8,%esp
801035e8:	8b 55 08             	mov    0x8(%ebp),%edx
801035eb:	8b 45 0c             	mov    0xc(%ebp),%eax
801035ee:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801035f2:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801035f5:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801035f9:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801035fd:	ee                   	out    %al,(%dx)
}
801035fe:	c9                   	leave  
801035ff:	c3                   	ret    

80103600 <mpbcpu>:
int ncpu;
uchar ioapicid;

int
mpbcpu(void)
{
80103600:	55                   	push   %ebp
80103601:	89 e5                	mov    %esp,%ebp
  return bcpu-cpus;
80103603:	a1 44 b6 10 80       	mov    0x8010b644,%eax
80103608:	89 c2                	mov    %eax,%edx
8010360a:	b8 20 f9 10 80       	mov    $0x8010f920,%eax
8010360f:	29 c2                	sub    %eax,%edx
80103611:	89 d0                	mov    %edx,%eax
80103613:	c1 f8 02             	sar    $0x2,%eax
80103616:	69 c0 cf 46 7d 67    	imul   $0x677d46cf,%eax,%eax
}
8010361c:	5d                   	pop    %ebp
8010361d:	c3                   	ret    

8010361e <sum>:

static uchar
sum(uchar *addr, int len)
{
8010361e:	55                   	push   %ebp
8010361f:	89 e5                	mov    %esp,%ebp
80103621:	83 ec 10             	sub    $0x10,%esp
  int i, sum;
  
  sum = 0;
80103624:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
  for(i=0; i<len; i++)
8010362b:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80103632:	eb 15                	jmp    80103649 <sum+0x2b>
    sum += addr[i];
80103634:	8b 55 fc             	mov    -0x4(%ebp),%edx
80103637:	8b 45 08             	mov    0x8(%ebp),%eax
8010363a:	01 d0                	add    %edx,%eax
8010363c:	0f b6 00             	movzbl (%eax),%eax
8010363f:	0f b6 c0             	movzbl %al,%eax
80103642:	01 45 f8             	add    %eax,-0x8(%ebp)
sum(uchar *addr, int len)
{
  int i, sum;
  
  sum = 0;
  for(i=0; i<len; i++)
80103645:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80103649:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010364c:	3b 45 0c             	cmp    0xc(%ebp),%eax
8010364f:	7c e3                	jl     80103634 <sum+0x16>
    sum += addr[i];
  return sum;
80103651:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80103654:	c9                   	leave  
80103655:	c3                   	ret    

80103656 <mpsearch1>:

// Look for an MP structure in the len bytes at addr.
static struct mp*
mpsearch1(uint a, int len)
{
80103656:	55                   	push   %ebp
80103657:	89 e5                	mov    %esp,%ebp
80103659:	83 ec 28             	sub    $0x28,%esp
  uchar *e, *p, *addr;

  addr = p2v(a);
8010365c:	8b 45 08             	mov    0x8(%ebp),%eax
8010365f:	89 04 24             	mov    %eax,(%esp)
80103662:	e8 51 ff ff ff       	call   801035b8 <p2v>
80103667:	89 45 f0             	mov    %eax,-0x10(%ebp)
  e = addr+len;
8010366a:	8b 55 0c             	mov    0xc(%ebp),%edx
8010366d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103670:	01 d0                	add    %edx,%eax
80103672:	89 45 ec             	mov    %eax,-0x14(%ebp)
  for(p = addr; p < e; p += sizeof(struct mp))
80103675:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103678:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010367b:	eb 3f                	jmp    801036bc <mpsearch1+0x66>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
8010367d:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
80103684:	00 
80103685:	c7 44 24 04 c4 82 10 	movl   $0x801082c4,0x4(%esp)
8010368c:	80 
8010368d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103690:	89 04 24             	mov    %eax,(%esp)
80103693:	e8 cb 16 00 00       	call   80104d63 <memcmp>
80103698:	85 c0                	test   %eax,%eax
8010369a:	75 1c                	jne    801036b8 <mpsearch1+0x62>
8010369c:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
801036a3:	00 
801036a4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801036a7:	89 04 24             	mov    %eax,(%esp)
801036aa:	e8 6f ff ff ff       	call   8010361e <sum>
801036af:	84 c0                	test   %al,%al
801036b1:	75 05                	jne    801036b8 <mpsearch1+0x62>
      return (struct mp*)p;
801036b3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801036b6:	eb 11                	jmp    801036c9 <mpsearch1+0x73>
{
  uchar *e, *p, *addr;

  addr = p2v(a);
  e = addr+len;
  for(p = addr; p < e; p += sizeof(struct mp))
801036b8:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
801036bc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801036bf:	3b 45 ec             	cmp    -0x14(%ebp),%eax
801036c2:	72 b9                	jb     8010367d <mpsearch1+0x27>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
      return (struct mp*)p;
  return 0;
801036c4:	b8 00 00 00 00       	mov    $0x0,%eax
}
801036c9:	c9                   	leave  
801036ca:	c3                   	ret    

801036cb <mpsearch>:
// 1) in the first KB of the EBDA;
// 2) in the last KB of system base memory;
// 3) in the BIOS ROM between 0xE0000 and 0xFFFFF.
static struct mp*
mpsearch(void)
{
801036cb:	55                   	push   %ebp
801036cc:	89 e5                	mov    %esp,%ebp
801036ce:	83 ec 28             	sub    $0x28,%esp
  uchar *bda;
  uint p;
  struct mp *mp;

  bda = (uchar *) P2V(0x400);
801036d1:	c7 45 f4 00 04 00 80 	movl   $0x80000400,-0xc(%ebp)
  if((p = ((bda[0x0F]<<8)| bda[0x0E]) << 4)){
801036d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801036db:	83 c0 0f             	add    $0xf,%eax
801036de:	0f b6 00             	movzbl (%eax),%eax
801036e1:	0f b6 c0             	movzbl %al,%eax
801036e4:	c1 e0 08             	shl    $0x8,%eax
801036e7:	89 c2                	mov    %eax,%edx
801036e9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801036ec:	83 c0 0e             	add    $0xe,%eax
801036ef:	0f b6 00             	movzbl (%eax),%eax
801036f2:	0f b6 c0             	movzbl %al,%eax
801036f5:	09 d0                	or     %edx,%eax
801036f7:	c1 e0 04             	shl    $0x4,%eax
801036fa:	89 45 f0             	mov    %eax,-0x10(%ebp)
801036fd:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80103701:	74 21                	je     80103724 <mpsearch+0x59>
    if((mp = mpsearch1(p, 1024)))
80103703:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
8010370a:	00 
8010370b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010370e:	89 04 24             	mov    %eax,(%esp)
80103711:	e8 40 ff ff ff       	call   80103656 <mpsearch1>
80103716:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103719:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
8010371d:	74 50                	je     8010376f <mpsearch+0xa4>
      return mp;
8010371f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103722:	eb 5f                	jmp    80103783 <mpsearch+0xb8>
  } else {
    p = ((bda[0x14]<<8)|bda[0x13])*1024;
80103724:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103727:	83 c0 14             	add    $0x14,%eax
8010372a:	0f b6 00             	movzbl (%eax),%eax
8010372d:	0f b6 c0             	movzbl %al,%eax
80103730:	c1 e0 08             	shl    $0x8,%eax
80103733:	89 c2                	mov    %eax,%edx
80103735:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103738:	83 c0 13             	add    $0x13,%eax
8010373b:	0f b6 00             	movzbl (%eax),%eax
8010373e:	0f b6 c0             	movzbl %al,%eax
80103741:	09 d0                	or     %edx,%eax
80103743:	c1 e0 0a             	shl    $0xa,%eax
80103746:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if((mp = mpsearch1(p-1024, 1024)))
80103749:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010374c:	2d 00 04 00 00       	sub    $0x400,%eax
80103751:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
80103758:	00 
80103759:	89 04 24             	mov    %eax,(%esp)
8010375c:	e8 f5 fe ff ff       	call   80103656 <mpsearch1>
80103761:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103764:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80103768:	74 05                	je     8010376f <mpsearch+0xa4>
      return mp;
8010376a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010376d:	eb 14                	jmp    80103783 <mpsearch+0xb8>
  }
  return mpsearch1(0xF0000, 0x10000);
8010376f:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80103776:	00 
80103777:	c7 04 24 00 00 0f 00 	movl   $0xf0000,(%esp)
8010377e:	e8 d3 fe ff ff       	call   80103656 <mpsearch1>
}
80103783:	c9                   	leave  
80103784:	c3                   	ret    

80103785 <mpconfig>:
// Check for correct signature, calculate the checksum and,
// if correct, check the version.
// To do: check extended table checksum.
static struct mpconf*
mpconfig(struct mp **pmp)
{
80103785:	55                   	push   %ebp
80103786:	89 e5                	mov    %esp,%ebp
80103788:	83 ec 28             	sub    $0x28,%esp
  struct mpconf *conf;
  struct mp *mp;

  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
8010378b:	e8 3b ff ff ff       	call   801036cb <mpsearch>
80103790:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103793:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103797:	74 0a                	je     801037a3 <mpconfig+0x1e>
80103799:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010379c:	8b 40 04             	mov    0x4(%eax),%eax
8010379f:	85 c0                	test   %eax,%eax
801037a1:	75 0a                	jne    801037ad <mpconfig+0x28>
    return 0;
801037a3:	b8 00 00 00 00       	mov    $0x0,%eax
801037a8:	e9 83 00 00 00       	jmp    80103830 <mpconfig+0xab>
  conf = (struct mpconf*) p2v((uint) mp->physaddr);
801037ad:	8b 45 f4             	mov    -0xc(%ebp),%eax
801037b0:	8b 40 04             	mov    0x4(%eax),%eax
801037b3:	89 04 24             	mov    %eax,(%esp)
801037b6:	e8 fd fd ff ff       	call   801035b8 <p2v>
801037bb:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(memcmp(conf, "PCMP", 4) != 0)
801037be:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
801037c5:	00 
801037c6:	c7 44 24 04 c9 82 10 	movl   $0x801082c9,0x4(%esp)
801037cd:	80 
801037ce:	8b 45 f0             	mov    -0x10(%ebp),%eax
801037d1:	89 04 24             	mov    %eax,(%esp)
801037d4:	e8 8a 15 00 00       	call   80104d63 <memcmp>
801037d9:	85 c0                	test   %eax,%eax
801037db:	74 07                	je     801037e4 <mpconfig+0x5f>
    return 0;
801037dd:	b8 00 00 00 00       	mov    $0x0,%eax
801037e2:	eb 4c                	jmp    80103830 <mpconfig+0xab>
  if(conf->version != 1 && conf->version != 4)
801037e4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801037e7:	0f b6 40 06          	movzbl 0x6(%eax),%eax
801037eb:	3c 01                	cmp    $0x1,%al
801037ed:	74 12                	je     80103801 <mpconfig+0x7c>
801037ef:	8b 45 f0             	mov    -0x10(%ebp),%eax
801037f2:	0f b6 40 06          	movzbl 0x6(%eax),%eax
801037f6:	3c 04                	cmp    $0x4,%al
801037f8:	74 07                	je     80103801 <mpconfig+0x7c>
    return 0;
801037fa:	b8 00 00 00 00       	mov    $0x0,%eax
801037ff:	eb 2f                	jmp    80103830 <mpconfig+0xab>
  if(sum((uchar*)conf, conf->length) != 0)
80103801:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103804:	0f b7 40 04          	movzwl 0x4(%eax),%eax
80103808:	0f b7 c0             	movzwl %ax,%eax
8010380b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010380f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103812:	89 04 24             	mov    %eax,(%esp)
80103815:	e8 04 fe ff ff       	call   8010361e <sum>
8010381a:	84 c0                	test   %al,%al
8010381c:	74 07                	je     80103825 <mpconfig+0xa0>
    return 0;
8010381e:	b8 00 00 00 00       	mov    $0x0,%eax
80103823:	eb 0b                	jmp    80103830 <mpconfig+0xab>
  *pmp = mp;
80103825:	8b 45 08             	mov    0x8(%ebp),%eax
80103828:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010382b:	89 10                	mov    %edx,(%eax)
  return conf;
8010382d:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80103830:	c9                   	leave  
80103831:	c3                   	ret    

80103832 <mpinit>:

void
mpinit(void)
{
80103832:	55                   	push   %ebp
80103833:	89 e5                	mov    %esp,%ebp
80103835:	83 ec 38             	sub    $0x38,%esp
  struct mp *mp;
  struct mpconf *conf;
  struct mpproc *proc;
  struct mpioapic *ioapic;

  bcpu = &cpus[0];
80103838:	c7 05 44 b6 10 80 20 	movl   $0x8010f920,0x8010b644
8010383f:	f9 10 80 
  if((conf = mpconfig(&mp)) == 0)
80103842:	8d 45 e0             	lea    -0x20(%ebp),%eax
80103845:	89 04 24             	mov    %eax,(%esp)
80103848:	e8 38 ff ff ff       	call   80103785 <mpconfig>
8010384d:	89 45 f0             	mov    %eax,-0x10(%ebp)
80103850:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80103854:	75 05                	jne    8010385b <mpinit+0x29>
    return;
80103856:	e9 9c 01 00 00       	jmp    801039f7 <mpinit+0x1c5>
  ismp = 1;
8010385b:	c7 05 04 f9 10 80 01 	movl   $0x1,0x8010f904
80103862:	00 00 00 
  lapic = (uint*)conf->lapicaddr;
80103865:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103868:	8b 40 24             	mov    0x24(%eax),%eax
8010386b:	a3 7c f8 10 80       	mov    %eax,0x8010f87c
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
80103870:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103873:	83 c0 2c             	add    $0x2c,%eax
80103876:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103879:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010387c:	0f b7 40 04          	movzwl 0x4(%eax),%eax
80103880:	0f b7 d0             	movzwl %ax,%edx
80103883:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103886:	01 d0                	add    %edx,%eax
80103888:	89 45 ec             	mov    %eax,-0x14(%ebp)
8010388b:	e9 f4 00 00 00       	jmp    80103984 <mpinit+0x152>
    switch(*p){
80103890:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103893:	0f b6 00             	movzbl (%eax),%eax
80103896:	0f b6 c0             	movzbl %al,%eax
80103899:	83 f8 04             	cmp    $0x4,%eax
8010389c:	0f 87 bf 00 00 00    	ja     80103961 <mpinit+0x12f>
801038a2:	8b 04 85 0c 83 10 80 	mov    -0x7fef7cf4(,%eax,4),%eax
801038a9:	ff e0                	jmp    *%eax
    case MPPROC:
      proc = (struct mpproc*)p;
801038ab:	8b 45 f4             	mov    -0xc(%ebp),%eax
801038ae:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if(ncpu != proc->apicid){
801038b1:	8b 45 e8             	mov    -0x18(%ebp),%eax
801038b4:	0f b6 40 01          	movzbl 0x1(%eax),%eax
801038b8:	0f b6 d0             	movzbl %al,%edx
801038bb:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
801038c0:	39 c2                	cmp    %eax,%edx
801038c2:	74 2d                	je     801038f1 <mpinit+0xbf>
        cprintf("mpinit: ncpu=%d apicid=%d\n", ncpu, proc->apicid);
801038c4:	8b 45 e8             	mov    -0x18(%ebp),%eax
801038c7:	0f b6 40 01          	movzbl 0x1(%eax),%eax
801038cb:	0f b6 d0             	movzbl %al,%edx
801038ce:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
801038d3:	89 54 24 08          	mov    %edx,0x8(%esp)
801038d7:	89 44 24 04          	mov    %eax,0x4(%esp)
801038db:	c7 04 24 ce 82 10 80 	movl   $0x801082ce,(%esp)
801038e2:	e8 b9 ca ff ff       	call   801003a0 <cprintf>
        ismp = 0;
801038e7:	c7 05 04 f9 10 80 00 	movl   $0x0,0x8010f904
801038ee:	00 00 00 
      }
      if(proc->flags & MPBOOT)
801038f1:	8b 45 e8             	mov    -0x18(%ebp),%eax
801038f4:	0f b6 40 03          	movzbl 0x3(%eax),%eax
801038f8:	0f b6 c0             	movzbl %al,%eax
801038fb:	83 e0 02             	and    $0x2,%eax
801038fe:	85 c0                	test   %eax,%eax
80103900:	74 15                	je     80103917 <mpinit+0xe5>
        bcpu = &cpus[ncpu];
80103902:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
80103907:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
8010390d:	05 20 f9 10 80       	add    $0x8010f920,%eax
80103912:	a3 44 b6 10 80       	mov    %eax,0x8010b644
      cpus[ncpu].id = ncpu;
80103917:	8b 15 00 ff 10 80    	mov    0x8010ff00,%edx
8010391d:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
80103922:	69 d2 bc 00 00 00    	imul   $0xbc,%edx,%edx
80103928:	81 c2 20 f9 10 80    	add    $0x8010f920,%edx
8010392e:	88 02                	mov    %al,(%edx)
      ncpu++;
80103930:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
80103935:	83 c0 01             	add    $0x1,%eax
80103938:	a3 00 ff 10 80       	mov    %eax,0x8010ff00
      p += sizeof(struct mpproc);
8010393d:	83 45 f4 14          	addl   $0x14,-0xc(%ebp)
      continue;
80103941:	eb 41                	jmp    80103984 <mpinit+0x152>
    case MPIOAPIC:
      ioapic = (struct mpioapic*)p;
80103943:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103946:	89 45 e4             	mov    %eax,-0x1c(%ebp)
      ioapicid = ioapic->apicno;
80103949:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010394c:	0f b6 40 01          	movzbl 0x1(%eax),%eax
80103950:	a2 00 f9 10 80       	mov    %al,0x8010f900
      p += sizeof(struct mpioapic);
80103955:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
80103959:	eb 29                	jmp    80103984 <mpinit+0x152>
    case MPBUS:
    case MPIOINTR:
    case MPLINTR:
      p += 8;
8010395b:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
8010395f:	eb 23                	jmp    80103984 <mpinit+0x152>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
80103961:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103964:	0f b6 00             	movzbl (%eax),%eax
80103967:	0f b6 c0             	movzbl %al,%eax
8010396a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010396e:	c7 04 24 ec 82 10 80 	movl   $0x801082ec,(%esp)
80103975:	e8 26 ca ff ff       	call   801003a0 <cprintf>
      ismp = 0;
8010397a:	c7 05 04 f9 10 80 00 	movl   $0x0,0x8010f904
80103981:	00 00 00 
  bcpu = &cpus[0];
  if((conf = mpconfig(&mp)) == 0)
    return;
  ismp = 1;
  lapic = (uint*)conf->lapicaddr;
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
80103984:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103987:	3b 45 ec             	cmp    -0x14(%ebp),%eax
8010398a:	0f 82 00 ff ff ff    	jb     80103890 <mpinit+0x5e>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
      ismp = 0;
    }
  }
  if(!ismp){
80103990:	a1 04 f9 10 80       	mov    0x8010f904,%eax
80103995:	85 c0                	test   %eax,%eax
80103997:	75 1d                	jne    801039b6 <mpinit+0x184>
    // Didn't like what we found; fall back to no MP.
    ncpu = 1;
80103999:	c7 05 00 ff 10 80 01 	movl   $0x1,0x8010ff00
801039a0:	00 00 00 
    lapic = 0;
801039a3:	c7 05 7c f8 10 80 00 	movl   $0x0,0x8010f87c
801039aa:	00 00 00 
    ioapicid = 0;
801039ad:	c6 05 00 f9 10 80 00 	movb   $0x0,0x8010f900
    return;
801039b4:	eb 41                	jmp    801039f7 <mpinit+0x1c5>
  }

  if(mp->imcrp){
801039b6:	8b 45 e0             	mov    -0x20(%ebp),%eax
801039b9:	0f b6 40 0c          	movzbl 0xc(%eax),%eax
801039bd:	84 c0                	test   %al,%al
801039bf:	74 36                	je     801039f7 <mpinit+0x1c5>
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
801039c1:	c7 44 24 04 70 00 00 	movl   $0x70,0x4(%esp)
801039c8:	00 
801039c9:	c7 04 24 22 00 00 00 	movl   $0x22,(%esp)
801039d0:	e8 0d fc ff ff       	call   801035e2 <outb>
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
801039d5:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
801039dc:	e8 e4 fb ff ff       	call   801035c5 <inb>
801039e1:	83 c8 01             	or     $0x1,%eax
801039e4:	0f b6 c0             	movzbl %al,%eax
801039e7:	89 44 24 04          	mov    %eax,0x4(%esp)
801039eb:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
801039f2:	e8 eb fb ff ff       	call   801035e2 <outb>
  }
}
801039f7:	c9                   	leave  
801039f8:	c3                   	ret    

801039f9 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801039f9:	55                   	push   %ebp
801039fa:	89 e5                	mov    %esp,%ebp
801039fc:	83 ec 08             	sub    $0x8,%esp
801039ff:	8b 55 08             	mov    0x8(%ebp),%edx
80103a02:	8b 45 0c             	mov    0xc(%ebp),%eax
80103a05:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80103a09:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80103a0c:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80103a10:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80103a14:	ee                   	out    %al,(%dx)
}
80103a15:	c9                   	leave  
80103a16:	c3                   	ret    

80103a17 <picsetmask>:
// Initial IRQ mask has interrupt 2 enabled (for slave 8259A).
static ushort irqmask = 0xFFFF & ~(1<<IRQ_SLAVE);

static void
picsetmask(ushort mask)
{
80103a17:	55                   	push   %ebp
80103a18:	89 e5                	mov    %esp,%ebp
80103a1a:	83 ec 0c             	sub    $0xc,%esp
80103a1d:	8b 45 08             	mov    0x8(%ebp),%eax
80103a20:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  irqmask = mask;
80103a24:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103a28:	66 a3 00 b0 10 80    	mov    %ax,0x8010b000
  outb(IO_PIC1+1, mask);
80103a2e:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103a32:	0f b6 c0             	movzbl %al,%eax
80103a35:	89 44 24 04          	mov    %eax,0x4(%esp)
80103a39:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103a40:	e8 b4 ff ff ff       	call   801039f9 <outb>
  outb(IO_PIC2+1, mask >> 8);
80103a45:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103a49:	66 c1 e8 08          	shr    $0x8,%ax
80103a4d:	0f b6 c0             	movzbl %al,%eax
80103a50:	89 44 24 04          	mov    %eax,0x4(%esp)
80103a54:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103a5b:	e8 99 ff ff ff       	call   801039f9 <outb>
}
80103a60:	c9                   	leave  
80103a61:	c3                   	ret    

80103a62 <picenable>:

void
picenable(int irq)
{
80103a62:	55                   	push   %ebp
80103a63:	89 e5                	mov    %esp,%ebp
80103a65:	83 ec 04             	sub    $0x4,%esp
  picsetmask(irqmask & ~(1<<irq));
80103a68:	8b 45 08             	mov    0x8(%ebp),%eax
80103a6b:	ba 01 00 00 00       	mov    $0x1,%edx
80103a70:	89 c1                	mov    %eax,%ecx
80103a72:	d3 e2                	shl    %cl,%edx
80103a74:	89 d0                	mov    %edx,%eax
80103a76:	f7 d0                	not    %eax
80103a78:	89 c2                	mov    %eax,%edx
80103a7a:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103a81:	21 d0                	and    %edx,%eax
80103a83:	0f b7 c0             	movzwl %ax,%eax
80103a86:	89 04 24             	mov    %eax,(%esp)
80103a89:	e8 89 ff ff ff       	call   80103a17 <picsetmask>
}
80103a8e:	c9                   	leave  
80103a8f:	c3                   	ret    

80103a90 <picinit>:

// Initialize the 8259A interrupt controllers.
void
picinit(void)
{
80103a90:	55                   	push   %ebp
80103a91:	89 e5                	mov    %esp,%ebp
80103a93:	83 ec 08             	sub    $0x8,%esp
  // mask all interrupts
  outb(IO_PIC1+1, 0xFF);
80103a96:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103a9d:	00 
80103a9e:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103aa5:	e8 4f ff ff ff       	call   801039f9 <outb>
  outb(IO_PIC2+1, 0xFF);
80103aaa:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103ab1:	00 
80103ab2:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103ab9:	e8 3b ff ff ff       	call   801039f9 <outb>

  // ICW1:  0001g0hi
  //    g:  0 = edge triggering, 1 = level triggering
  //    h:  0 = cascaded PICs, 1 = master only
  //    i:  0 = no ICW4, 1 = ICW4 required
  outb(IO_PIC1, 0x11);
80103abe:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80103ac5:	00 
80103ac6:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103acd:	e8 27 ff ff ff       	call   801039f9 <outb>

  // ICW2:  Vector offset
  outb(IO_PIC1+1, T_IRQ0);
80103ad2:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
80103ad9:	00 
80103ada:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103ae1:	e8 13 ff ff ff       	call   801039f9 <outb>

  // ICW3:  (master PIC) bit mask of IR lines connected to slaves
  //        (slave PIC) 3-bit # of slave's connection to master
  outb(IO_PIC1+1, 1<<IRQ_SLAVE);
80103ae6:	c7 44 24 04 04 00 00 	movl   $0x4,0x4(%esp)
80103aed:	00 
80103aee:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103af5:	e8 ff fe ff ff       	call   801039f9 <outb>
  //    m:  0 = slave PIC, 1 = master PIC
  //      (ignored when b is 0, as the master/slave role
  //      can be hardwired).
  //    a:  1 = Automatic EOI mode
  //    p:  0 = MCS-80/85 mode, 1 = intel x86 mode
  outb(IO_PIC1+1, 0x3);
80103afa:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80103b01:	00 
80103b02:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103b09:	e8 eb fe ff ff       	call   801039f9 <outb>

  // Set up slave (8259A-2)
  outb(IO_PIC2, 0x11);                  // ICW1
80103b0e:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80103b15:	00 
80103b16:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103b1d:	e8 d7 fe ff ff       	call   801039f9 <outb>
  outb(IO_PIC2+1, T_IRQ0 + 8);      // ICW2
80103b22:	c7 44 24 04 28 00 00 	movl   $0x28,0x4(%esp)
80103b29:	00 
80103b2a:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103b31:	e8 c3 fe ff ff       	call   801039f9 <outb>
  outb(IO_PIC2+1, IRQ_SLAVE);           // ICW3
80103b36:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80103b3d:	00 
80103b3e:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103b45:	e8 af fe ff ff       	call   801039f9 <outb>
  // NB Automatic EOI mode doesn't tend to work on the slave.
  // Linux source code says it's "to be investigated".
  outb(IO_PIC2+1, 0x3);                 // ICW4
80103b4a:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80103b51:	00 
80103b52:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103b59:	e8 9b fe ff ff       	call   801039f9 <outb>

  // OCW3:  0ef01prs
  //   ef:  0x = NOP, 10 = clear specific mask, 11 = set specific mask
  //    p:  0 = no polling, 1 = polling mode
  //   rs:  0x = NOP, 10 = read IRR, 11 = read ISR
  outb(IO_PIC1, 0x68);             // clear specific mask
80103b5e:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
80103b65:	00 
80103b66:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103b6d:	e8 87 fe ff ff       	call   801039f9 <outb>
  outb(IO_PIC1, 0x0a);             // read IRR by default
80103b72:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103b79:	00 
80103b7a:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103b81:	e8 73 fe ff ff       	call   801039f9 <outb>

  outb(IO_PIC2, 0x68);             // OCW3
80103b86:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
80103b8d:	00 
80103b8e:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103b95:	e8 5f fe ff ff       	call   801039f9 <outb>
  outb(IO_PIC2, 0x0a);             // OCW3
80103b9a:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103ba1:	00 
80103ba2:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103ba9:	e8 4b fe ff ff       	call   801039f9 <outb>

  if(irqmask != 0xFFFF)
80103bae:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103bb5:	66 83 f8 ff          	cmp    $0xffff,%ax
80103bb9:	74 12                	je     80103bcd <picinit+0x13d>
    picsetmask(irqmask);
80103bbb:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103bc2:	0f b7 c0             	movzwl %ax,%eax
80103bc5:	89 04 24             	mov    %eax,(%esp)
80103bc8:	e8 4a fe ff ff       	call   80103a17 <picsetmask>
}
80103bcd:	c9                   	leave  
80103bce:	c3                   	ret    

80103bcf <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
80103bcf:	55                   	push   %ebp
80103bd0:	89 e5                	mov    %esp,%ebp
80103bd2:	83 ec 28             	sub    $0x28,%esp
  struct pipe *p;

  p = 0;
80103bd5:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  *f0 = *f1 = 0;
80103bdc:	8b 45 0c             	mov    0xc(%ebp),%eax
80103bdf:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
80103be5:	8b 45 0c             	mov    0xc(%ebp),%eax
80103be8:	8b 10                	mov    (%eax),%edx
80103bea:	8b 45 08             	mov    0x8(%ebp),%eax
80103bed:	89 10                	mov    %edx,(%eax)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
80103bef:	e8 32 d3 ff ff       	call   80100f26 <filealloc>
80103bf4:	8b 55 08             	mov    0x8(%ebp),%edx
80103bf7:	89 02                	mov    %eax,(%edx)
80103bf9:	8b 45 08             	mov    0x8(%ebp),%eax
80103bfc:	8b 00                	mov    (%eax),%eax
80103bfe:	85 c0                	test   %eax,%eax
80103c00:	0f 84 c8 00 00 00    	je     80103cce <pipealloc+0xff>
80103c06:	e8 1b d3 ff ff       	call   80100f26 <filealloc>
80103c0b:	8b 55 0c             	mov    0xc(%ebp),%edx
80103c0e:	89 02                	mov    %eax,(%edx)
80103c10:	8b 45 0c             	mov    0xc(%ebp),%eax
80103c13:	8b 00                	mov    (%eax),%eax
80103c15:	85 c0                	test   %eax,%eax
80103c17:	0f 84 b1 00 00 00    	je     80103cce <pipealloc+0xff>
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
80103c1d:	e8 c0 ee ff ff       	call   80102ae2 <kalloc>
80103c22:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103c25:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103c29:	75 05                	jne    80103c30 <pipealloc+0x61>
    goto bad;
80103c2b:	e9 9e 00 00 00       	jmp    80103cce <pipealloc+0xff>
  p->readopen = 1;
80103c30:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c33:	c7 80 3c 02 00 00 01 	movl   $0x1,0x23c(%eax)
80103c3a:	00 00 00 
  p->writeopen = 1;
80103c3d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c40:	c7 80 40 02 00 00 01 	movl   $0x1,0x240(%eax)
80103c47:	00 00 00 
  p->nwrite = 0;
80103c4a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c4d:	c7 80 38 02 00 00 00 	movl   $0x0,0x238(%eax)
80103c54:	00 00 00 
  p->nread = 0;
80103c57:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c5a:	c7 80 34 02 00 00 00 	movl   $0x0,0x234(%eax)
80103c61:	00 00 00 
  initlock(&p->lock, "pipe");
80103c64:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c67:	c7 44 24 04 20 83 10 	movl   $0x80108320,0x4(%esp)
80103c6e:	80 
80103c6f:	89 04 24             	mov    %eax,(%esp)
80103c72:	e8 00 0e 00 00       	call   80104a77 <initlock>
  (*f0)->type = FD_PIPE;
80103c77:	8b 45 08             	mov    0x8(%ebp),%eax
80103c7a:	8b 00                	mov    (%eax),%eax
80103c7c:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f0)->readable = 1;
80103c82:	8b 45 08             	mov    0x8(%ebp),%eax
80103c85:	8b 00                	mov    (%eax),%eax
80103c87:	c6 40 08 01          	movb   $0x1,0x8(%eax)
  (*f0)->writable = 0;
80103c8b:	8b 45 08             	mov    0x8(%ebp),%eax
80103c8e:	8b 00                	mov    (%eax),%eax
80103c90:	c6 40 09 00          	movb   $0x0,0x9(%eax)
  (*f0)->pipe = p;
80103c94:	8b 45 08             	mov    0x8(%ebp),%eax
80103c97:	8b 00                	mov    (%eax),%eax
80103c99:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103c9c:	89 50 0c             	mov    %edx,0xc(%eax)
  (*f1)->type = FD_PIPE;
80103c9f:	8b 45 0c             	mov    0xc(%ebp),%eax
80103ca2:	8b 00                	mov    (%eax),%eax
80103ca4:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f1)->readable = 0;
80103caa:	8b 45 0c             	mov    0xc(%ebp),%eax
80103cad:	8b 00                	mov    (%eax),%eax
80103caf:	c6 40 08 00          	movb   $0x0,0x8(%eax)
  (*f1)->writable = 1;
80103cb3:	8b 45 0c             	mov    0xc(%ebp),%eax
80103cb6:	8b 00                	mov    (%eax),%eax
80103cb8:	c6 40 09 01          	movb   $0x1,0x9(%eax)
  (*f1)->pipe = p;
80103cbc:	8b 45 0c             	mov    0xc(%ebp),%eax
80103cbf:	8b 00                	mov    (%eax),%eax
80103cc1:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103cc4:	89 50 0c             	mov    %edx,0xc(%eax)
  return 0;
80103cc7:	b8 00 00 00 00       	mov    $0x0,%eax
80103ccc:	eb 42                	jmp    80103d10 <pipealloc+0x141>

//PAGEBREAK: 20
 bad:
  if(p)
80103cce:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103cd2:	74 0b                	je     80103cdf <pipealloc+0x110>
    kfree((char*)p);
80103cd4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103cd7:	89 04 24             	mov    %eax,(%esp)
80103cda:	e8 6a ed ff ff       	call   80102a49 <kfree>
  if(*f0)
80103cdf:	8b 45 08             	mov    0x8(%ebp),%eax
80103ce2:	8b 00                	mov    (%eax),%eax
80103ce4:	85 c0                	test   %eax,%eax
80103ce6:	74 0d                	je     80103cf5 <pipealloc+0x126>
    fileclose(*f0);
80103ce8:	8b 45 08             	mov    0x8(%ebp),%eax
80103ceb:	8b 00                	mov    (%eax),%eax
80103ced:	89 04 24             	mov    %eax,(%esp)
80103cf0:	e8 d9 d2 ff ff       	call   80100fce <fileclose>
  if(*f1)
80103cf5:	8b 45 0c             	mov    0xc(%ebp),%eax
80103cf8:	8b 00                	mov    (%eax),%eax
80103cfa:	85 c0                	test   %eax,%eax
80103cfc:	74 0d                	je     80103d0b <pipealloc+0x13c>
    fileclose(*f1);
80103cfe:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d01:	8b 00                	mov    (%eax),%eax
80103d03:	89 04 24             	mov    %eax,(%esp)
80103d06:	e8 c3 d2 ff ff       	call   80100fce <fileclose>
  return -1;
80103d0b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80103d10:	c9                   	leave  
80103d11:	c3                   	ret    

80103d12 <pipeclose>:

void
pipeclose(struct pipe *p, int writable)
{
80103d12:	55                   	push   %ebp
80103d13:	89 e5                	mov    %esp,%ebp
80103d15:	83 ec 18             	sub    $0x18,%esp
  acquire(&p->lock);
80103d18:	8b 45 08             	mov    0x8(%ebp),%eax
80103d1b:	89 04 24             	mov    %eax,(%esp)
80103d1e:	e8 75 0d 00 00       	call   80104a98 <acquire>
  if(writable){
80103d23:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80103d27:	74 1f                	je     80103d48 <pipeclose+0x36>
    p->writeopen = 0;
80103d29:	8b 45 08             	mov    0x8(%ebp),%eax
80103d2c:	c7 80 40 02 00 00 00 	movl   $0x0,0x240(%eax)
80103d33:	00 00 00 
    wakeup(&p->nread);
80103d36:	8b 45 08             	mov    0x8(%ebp),%eax
80103d39:	05 34 02 00 00       	add    $0x234,%eax
80103d3e:	89 04 24             	mov    %eax,(%esp)
80103d41:	e8 61 0b 00 00       	call   801048a7 <wakeup>
80103d46:	eb 1d                	jmp    80103d65 <pipeclose+0x53>
  } else {
    p->readopen = 0;
80103d48:	8b 45 08             	mov    0x8(%ebp),%eax
80103d4b:	c7 80 3c 02 00 00 00 	movl   $0x0,0x23c(%eax)
80103d52:	00 00 00 
    wakeup(&p->nwrite);
80103d55:	8b 45 08             	mov    0x8(%ebp),%eax
80103d58:	05 38 02 00 00       	add    $0x238,%eax
80103d5d:	89 04 24             	mov    %eax,(%esp)
80103d60:	e8 42 0b 00 00       	call   801048a7 <wakeup>
  }
  if(p->readopen == 0 && p->writeopen == 0){
80103d65:	8b 45 08             	mov    0x8(%ebp),%eax
80103d68:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80103d6e:	85 c0                	test   %eax,%eax
80103d70:	75 25                	jne    80103d97 <pipeclose+0x85>
80103d72:	8b 45 08             	mov    0x8(%ebp),%eax
80103d75:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
80103d7b:	85 c0                	test   %eax,%eax
80103d7d:	75 18                	jne    80103d97 <pipeclose+0x85>
    release(&p->lock);
80103d7f:	8b 45 08             	mov    0x8(%ebp),%eax
80103d82:	89 04 24             	mov    %eax,(%esp)
80103d85:	e8 70 0d 00 00       	call   80104afa <release>
    kfree((char*)p);
80103d8a:	8b 45 08             	mov    0x8(%ebp),%eax
80103d8d:	89 04 24             	mov    %eax,(%esp)
80103d90:	e8 b4 ec ff ff       	call   80102a49 <kfree>
80103d95:	eb 0b                	jmp    80103da2 <pipeclose+0x90>
  } else
    release(&p->lock);
80103d97:	8b 45 08             	mov    0x8(%ebp),%eax
80103d9a:	89 04 24             	mov    %eax,(%esp)
80103d9d:	e8 58 0d 00 00       	call   80104afa <release>
}
80103da2:	c9                   	leave  
80103da3:	c3                   	ret    

80103da4 <pipewrite>:

//PAGEBREAK: 40
int
pipewrite(struct pipe *p, char *addr, int n)
{
80103da4:	55                   	push   %ebp
80103da5:	89 e5                	mov    %esp,%ebp
80103da7:	83 ec 28             	sub    $0x28,%esp
  int i;

  acquire(&p->lock);
80103daa:	8b 45 08             	mov    0x8(%ebp),%eax
80103dad:	89 04 24             	mov    %eax,(%esp)
80103db0:	e8 e3 0c 00 00       	call   80104a98 <acquire>
  for(i = 0; i < n; i++){
80103db5:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103dbc:	e9 a6 00 00 00       	jmp    80103e67 <pipewrite+0xc3>
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103dc1:	eb 57                	jmp    80103e1a <pipewrite+0x76>
      if(p->readopen == 0 || proc->killed){
80103dc3:	8b 45 08             	mov    0x8(%ebp),%eax
80103dc6:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80103dcc:	85 c0                	test   %eax,%eax
80103dce:	74 0d                	je     80103ddd <pipewrite+0x39>
80103dd0:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80103dd6:	8b 40 24             	mov    0x24(%eax),%eax
80103dd9:	85 c0                	test   %eax,%eax
80103ddb:	74 15                	je     80103df2 <pipewrite+0x4e>
        release(&p->lock);
80103ddd:	8b 45 08             	mov    0x8(%ebp),%eax
80103de0:	89 04 24             	mov    %eax,(%esp)
80103de3:	e8 12 0d 00 00       	call   80104afa <release>
        return -1;
80103de8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103ded:	e9 9f 00 00 00       	jmp    80103e91 <pipewrite+0xed>
      }
      wakeup(&p->nread);
80103df2:	8b 45 08             	mov    0x8(%ebp),%eax
80103df5:	05 34 02 00 00       	add    $0x234,%eax
80103dfa:	89 04 24             	mov    %eax,(%esp)
80103dfd:	e8 a5 0a 00 00       	call   801048a7 <wakeup>
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
80103e02:	8b 45 08             	mov    0x8(%ebp),%eax
80103e05:	8b 55 08             	mov    0x8(%ebp),%edx
80103e08:	81 c2 38 02 00 00    	add    $0x238,%edx
80103e0e:	89 44 24 04          	mov    %eax,0x4(%esp)
80103e12:	89 14 24             	mov    %edx,(%esp)
80103e15:	e8 b4 09 00 00       	call   801047ce <sleep>
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103e1a:	8b 45 08             	mov    0x8(%ebp),%eax
80103e1d:	8b 90 38 02 00 00    	mov    0x238(%eax),%edx
80103e23:	8b 45 08             	mov    0x8(%ebp),%eax
80103e26:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80103e2c:	05 00 02 00 00       	add    $0x200,%eax
80103e31:	39 c2                	cmp    %eax,%edx
80103e33:	74 8e                	je     80103dc3 <pipewrite+0x1f>
        return -1;
      }
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
80103e35:	8b 45 08             	mov    0x8(%ebp),%eax
80103e38:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103e3e:	8d 48 01             	lea    0x1(%eax),%ecx
80103e41:	8b 55 08             	mov    0x8(%ebp),%edx
80103e44:	89 8a 38 02 00 00    	mov    %ecx,0x238(%edx)
80103e4a:	25 ff 01 00 00       	and    $0x1ff,%eax
80103e4f:	89 c1                	mov    %eax,%ecx
80103e51:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103e54:	8b 45 0c             	mov    0xc(%ebp),%eax
80103e57:	01 d0                	add    %edx,%eax
80103e59:	0f b6 10             	movzbl (%eax),%edx
80103e5c:	8b 45 08             	mov    0x8(%ebp),%eax
80103e5f:	88 54 08 34          	mov    %dl,0x34(%eax,%ecx,1)
pipewrite(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
80103e63:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103e67:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103e6a:	3b 45 10             	cmp    0x10(%ebp),%eax
80103e6d:	0f 8c 4e ff ff ff    	jl     80103dc1 <pipewrite+0x1d>
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
  }
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
80103e73:	8b 45 08             	mov    0x8(%ebp),%eax
80103e76:	05 34 02 00 00       	add    $0x234,%eax
80103e7b:	89 04 24             	mov    %eax,(%esp)
80103e7e:	e8 24 0a 00 00       	call   801048a7 <wakeup>
  release(&p->lock);
80103e83:	8b 45 08             	mov    0x8(%ebp),%eax
80103e86:	89 04 24             	mov    %eax,(%esp)
80103e89:	e8 6c 0c 00 00       	call   80104afa <release>
  return n;
80103e8e:	8b 45 10             	mov    0x10(%ebp),%eax
}
80103e91:	c9                   	leave  
80103e92:	c3                   	ret    

80103e93 <piperead>:

int
piperead(struct pipe *p, char *addr, int n)
{
80103e93:	55                   	push   %ebp
80103e94:	89 e5                	mov    %esp,%ebp
80103e96:	53                   	push   %ebx
80103e97:	83 ec 24             	sub    $0x24,%esp
  int i;

  acquire(&p->lock);
80103e9a:	8b 45 08             	mov    0x8(%ebp),%eax
80103e9d:	89 04 24             	mov    %eax,(%esp)
80103ea0:	e8 f3 0b 00 00       	call   80104a98 <acquire>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80103ea5:	eb 3a                	jmp    80103ee1 <piperead+0x4e>
    if(proc->killed){
80103ea7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80103ead:	8b 40 24             	mov    0x24(%eax),%eax
80103eb0:	85 c0                	test   %eax,%eax
80103eb2:	74 15                	je     80103ec9 <piperead+0x36>
      release(&p->lock);
80103eb4:	8b 45 08             	mov    0x8(%ebp),%eax
80103eb7:	89 04 24             	mov    %eax,(%esp)
80103eba:	e8 3b 0c 00 00       	call   80104afa <release>
      return -1;
80103ebf:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103ec4:	e9 b5 00 00 00       	jmp    80103f7e <piperead+0xeb>
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
80103ec9:	8b 45 08             	mov    0x8(%ebp),%eax
80103ecc:	8b 55 08             	mov    0x8(%ebp),%edx
80103ecf:	81 c2 34 02 00 00    	add    $0x234,%edx
80103ed5:	89 44 24 04          	mov    %eax,0x4(%esp)
80103ed9:	89 14 24             	mov    %edx,(%esp)
80103edc:	e8 ed 08 00 00       	call   801047ce <sleep>
piperead(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80103ee1:	8b 45 08             	mov    0x8(%ebp),%eax
80103ee4:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
80103eea:	8b 45 08             	mov    0x8(%ebp),%eax
80103eed:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103ef3:	39 c2                	cmp    %eax,%edx
80103ef5:	75 0d                	jne    80103f04 <piperead+0x71>
80103ef7:	8b 45 08             	mov    0x8(%ebp),%eax
80103efa:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
80103f00:	85 c0                	test   %eax,%eax
80103f02:	75 a3                	jne    80103ea7 <piperead+0x14>
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80103f04:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103f0b:	eb 4b                	jmp    80103f58 <piperead+0xc5>
    if(p->nread == p->nwrite)
80103f0d:	8b 45 08             	mov    0x8(%ebp),%eax
80103f10:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
80103f16:	8b 45 08             	mov    0x8(%ebp),%eax
80103f19:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103f1f:	39 c2                	cmp    %eax,%edx
80103f21:	75 02                	jne    80103f25 <piperead+0x92>
      break;
80103f23:	eb 3b                	jmp    80103f60 <piperead+0xcd>
    addr[i] = p->data[p->nread++ % PIPESIZE];
80103f25:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103f28:	8b 45 0c             	mov    0xc(%ebp),%eax
80103f2b:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
80103f2e:	8b 45 08             	mov    0x8(%ebp),%eax
80103f31:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80103f37:	8d 48 01             	lea    0x1(%eax),%ecx
80103f3a:	8b 55 08             	mov    0x8(%ebp),%edx
80103f3d:	89 8a 34 02 00 00    	mov    %ecx,0x234(%edx)
80103f43:	25 ff 01 00 00       	and    $0x1ff,%eax
80103f48:	89 c2                	mov    %eax,%edx
80103f4a:	8b 45 08             	mov    0x8(%ebp),%eax
80103f4d:	0f b6 44 10 34       	movzbl 0x34(%eax,%edx,1),%eax
80103f52:	88 03                	mov    %al,(%ebx)
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80103f54:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103f58:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f5b:	3b 45 10             	cmp    0x10(%ebp),%eax
80103f5e:	7c ad                	jl     80103f0d <piperead+0x7a>
    if(p->nread == p->nwrite)
      break;
    addr[i] = p->data[p->nread++ % PIPESIZE];
  }
  wakeup(&p->nwrite);  //DOC: piperead-wakeup
80103f60:	8b 45 08             	mov    0x8(%ebp),%eax
80103f63:	05 38 02 00 00       	add    $0x238,%eax
80103f68:	89 04 24             	mov    %eax,(%esp)
80103f6b:	e8 37 09 00 00       	call   801048a7 <wakeup>
  release(&p->lock);
80103f70:	8b 45 08             	mov    0x8(%ebp),%eax
80103f73:	89 04 24             	mov    %eax,(%esp)
80103f76:	e8 7f 0b 00 00       	call   80104afa <release>
  return i;
80103f7b:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80103f7e:	83 c4 24             	add    $0x24,%esp
80103f81:	5b                   	pop    %ebx
80103f82:	5d                   	pop    %ebp
80103f83:	c3                   	ret    

80103f84 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80103f84:	55                   	push   %ebp
80103f85:	89 e5                	mov    %esp,%ebp
80103f87:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80103f8a:	9c                   	pushf  
80103f8b:	58                   	pop    %eax
80103f8c:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return eflags;
80103f8f:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80103f92:	c9                   	leave  
80103f93:	c3                   	ret    

80103f94 <sti>:
  asm volatile("cli");
}

static inline void
sti(void)
{
80103f94:	55                   	push   %ebp
80103f95:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
80103f97:	fb                   	sti    
}
80103f98:	5d                   	pop    %ebp
80103f99:	c3                   	ret    

80103f9a <pinit>:

static void wakeup1(void *chan);

void
pinit(void)
{
80103f9a:	55                   	push   %ebp
80103f9b:	89 e5                	mov    %esp,%ebp
80103f9d:	83 ec 18             	sub    $0x18,%esp
  initlock(&ptable.lock, "ptable");
80103fa0:	c7 44 24 04 25 83 10 	movl   $0x80108325,0x4(%esp)
80103fa7:	80 
80103fa8:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80103faf:	e8 c3 0a 00 00       	call   80104a77 <initlock>
}
80103fb4:	c9                   	leave  
80103fb5:	c3                   	ret    

80103fb6 <allocproc>:
// If found, change state to EMBRYO and initialize
// state required to run in the kernel.
// Otherwise return 0.
static struct proc*
allocproc(void)
{
80103fb6:	55                   	push   %ebp
80103fb7:	89 e5                	mov    %esp,%ebp
80103fb9:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);
80103fbc:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80103fc3:	e8 d0 0a 00 00       	call   80104a98 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80103fc8:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
80103fcf:	eb 50                	jmp    80104021 <allocproc+0x6b>
    if(p->state == UNUSED)
80103fd1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103fd4:	8b 40 0c             	mov    0xc(%eax),%eax
80103fd7:	85 c0                	test   %eax,%eax
80103fd9:	75 42                	jne    8010401d <allocproc+0x67>
      goto found;
80103fdb:	90                   	nop
  release(&ptable.lock);
  return 0;

found:
  p->state = EMBRYO;
80103fdc:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103fdf:	c7 40 0c 01 00 00 00 	movl   $0x1,0xc(%eax)
  p->pid = nextpid++;
80103fe6:	a1 04 b0 10 80       	mov    0x8010b004,%eax
80103feb:	8d 50 01             	lea    0x1(%eax),%edx
80103fee:	89 15 04 b0 10 80    	mov    %edx,0x8010b004
80103ff4:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103ff7:	89 42 10             	mov    %eax,0x10(%edx)
  release(&ptable.lock);
80103ffa:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104001:	e8 f4 0a 00 00       	call   80104afa <release>

  // Allocate kernel stack.
  if((p->kstack = kalloc()) == 0){
80104006:	e8 d7 ea ff ff       	call   80102ae2 <kalloc>
8010400b:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010400e:	89 42 08             	mov    %eax,0x8(%edx)
80104011:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104014:	8b 40 08             	mov    0x8(%eax),%eax
80104017:	85 c0                	test   %eax,%eax
80104019:	75 33                	jne    8010404e <allocproc+0x98>
8010401b:	eb 20                	jmp    8010403d <allocproc+0x87>
{
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
8010401d:	83 45 f4 7c          	addl   $0x7c,-0xc(%ebp)
80104021:	81 7d f4 54 1e 11 80 	cmpl   $0x80111e54,-0xc(%ebp)
80104028:	72 a7                	jb     80103fd1 <allocproc+0x1b>
    if(p->state == UNUSED)
      goto found;
  release(&ptable.lock);
8010402a:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104031:	e8 c4 0a 00 00       	call   80104afa <release>
  return 0;
80104036:	b8 00 00 00 00       	mov    $0x0,%eax
8010403b:	eb 76                	jmp    801040b3 <allocproc+0xfd>
  p->pid = nextpid++;
  release(&ptable.lock);

  // Allocate kernel stack.
  if((p->kstack = kalloc()) == 0){
    p->state = UNUSED;
8010403d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104040:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    return 0;
80104047:	b8 00 00 00 00       	mov    $0x0,%eax
8010404c:	eb 65                	jmp    801040b3 <allocproc+0xfd>
  }
  sp = p->kstack + KSTACKSIZE;
8010404e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104051:	8b 40 08             	mov    0x8(%eax),%eax
80104054:	05 00 10 00 00       	add    $0x1000,%eax
80104059:	89 45 f0             	mov    %eax,-0x10(%ebp)
  
  // Leave room for trap frame.
  sp -= sizeof *p->tf;
8010405c:	83 6d f0 4c          	subl   $0x4c,-0x10(%ebp)
  p->tf = (struct trapframe*)sp;
80104060:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104063:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104066:	89 50 18             	mov    %edx,0x18(%eax)
  
  // Set up new context to start executing at forkret,
  // which returns to trapret.
  sp -= 4;
80104069:	83 6d f0 04          	subl   $0x4,-0x10(%ebp)
  *(uint*)sp = (uint)trapret;
8010406d:	ba 00 61 10 80       	mov    $0x80106100,%edx
80104072:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104075:	89 10                	mov    %edx,(%eax)

  sp -= sizeof *p->context;
80104077:	83 6d f0 14          	subl   $0x14,-0x10(%ebp)
  p->context = (struct context*)sp;
8010407b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010407e:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104081:	89 50 1c             	mov    %edx,0x1c(%eax)
  memset(p->context, 0, sizeof *p->context);
80104084:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104087:	8b 40 1c             	mov    0x1c(%eax),%eax
8010408a:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
80104091:	00 
80104092:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104099:	00 
8010409a:	89 04 24             	mov    %eax,(%esp)
8010409d:	e8 4a 0c 00 00       	call   80104cec <memset>
  p->context->eip = (uint)forkret;
801040a2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040a5:	8b 40 1c             	mov    0x1c(%eax),%eax
801040a8:	ba a2 47 10 80       	mov    $0x801047a2,%edx
801040ad:	89 50 10             	mov    %edx,0x10(%eax)

  return p;
801040b0:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801040b3:	c9                   	leave  
801040b4:	c3                   	ret    

801040b5 <userinit>:

//PAGEBREAK: 32
// Set up first user process.
void
userinit(void)
{
801040b5:	55                   	push   %ebp
801040b6:	89 e5                	mov    %esp,%ebp
801040b8:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  extern char _binary_initcode_start[], _binary_initcode_size[];
  
  p = allocproc();
801040bb:	e8 f6 fe ff ff       	call   80103fb6 <allocproc>
801040c0:	89 45 f4             	mov    %eax,-0xc(%ebp)
  initproc = p;
801040c3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040c6:	a3 48 b6 10 80       	mov    %eax,0x8010b648
  if((p->pgdir = setupkvm()) == 0)
801040cb:	e8 24 37 00 00       	call   801077f4 <setupkvm>
801040d0:	8b 55 f4             	mov    -0xc(%ebp),%edx
801040d3:	89 42 04             	mov    %eax,0x4(%edx)
801040d6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040d9:	8b 40 04             	mov    0x4(%eax),%eax
801040dc:	85 c0                	test   %eax,%eax
801040de:	75 0c                	jne    801040ec <userinit+0x37>
    panic("userinit: out of memory?");
801040e0:	c7 04 24 2c 83 10 80 	movl   $0x8010832c,(%esp)
801040e7:	e8 4e c4 ff ff       	call   8010053a <panic>
  inituvm(p->pgdir, _binary_initcode_start, (int)_binary_initcode_size);
801040ec:	ba 2c 00 00 00       	mov    $0x2c,%edx
801040f1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040f4:	8b 40 04             	mov    0x4(%eax),%eax
801040f7:	89 54 24 08          	mov    %edx,0x8(%esp)
801040fb:	c7 44 24 04 e0 b4 10 	movl   $0x8010b4e0,0x4(%esp)
80104102:	80 
80104103:	89 04 24             	mov    %eax,(%esp)
80104106:	e8 41 39 00 00       	call   80107a4c <inituvm>
  p->sz = PGSIZE;
8010410b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010410e:	c7 00 00 10 00 00    	movl   $0x1000,(%eax)
  memset(p->tf, 0, sizeof(*p->tf));
80104114:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104117:	8b 40 18             	mov    0x18(%eax),%eax
8010411a:	c7 44 24 08 4c 00 00 	movl   $0x4c,0x8(%esp)
80104121:	00 
80104122:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104129:	00 
8010412a:	89 04 24             	mov    %eax,(%esp)
8010412d:	e8 ba 0b 00 00       	call   80104cec <memset>
  p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
80104132:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104135:	8b 40 18             	mov    0x18(%eax),%eax
80104138:	66 c7 40 3c 23 00    	movw   $0x23,0x3c(%eax)
  p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
8010413e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104141:	8b 40 18             	mov    0x18(%eax),%eax
80104144:	66 c7 40 2c 2b 00    	movw   $0x2b,0x2c(%eax)
  p->tf->es = p->tf->ds;
8010414a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010414d:	8b 40 18             	mov    0x18(%eax),%eax
80104150:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104153:	8b 52 18             	mov    0x18(%edx),%edx
80104156:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
8010415a:	66 89 50 28          	mov    %dx,0x28(%eax)
  p->tf->ss = p->tf->ds;
8010415e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104161:	8b 40 18             	mov    0x18(%eax),%eax
80104164:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104167:	8b 52 18             	mov    0x18(%edx),%edx
8010416a:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
8010416e:	66 89 50 48          	mov    %dx,0x48(%eax)
  p->tf->eflags = FL_IF;
80104172:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104175:	8b 40 18             	mov    0x18(%eax),%eax
80104178:	c7 40 40 00 02 00 00 	movl   $0x200,0x40(%eax)
  p->tf->esp = PGSIZE;
8010417f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104182:	8b 40 18             	mov    0x18(%eax),%eax
80104185:	c7 40 44 00 10 00 00 	movl   $0x1000,0x44(%eax)
  p->tf->eip = 0;  // beginning of initcode.S
8010418c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010418f:	8b 40 18             	mov    0x18(%eax),%eax
80104192:	c7 40 38 00 00 00 00 	movl   $0x0,0x38(%eax)

  safestrcpy(p->name, "initcode", sizeof(p->name));
80104199:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010419c:	83 c0 6c             	add    $0x6c,%eax
8010419f:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
801041a6:	00 
801041a7:	c7 44 24 04 45 83 10 	movl   $0x80108345,0x4(%esp)
801041ae:	80 
801041af:	89 04 24             	mov    %eax,(%esp)
801041b2:	e8 55 0d 00 00       	call   80104f0c <safestrcpy>
  p->cwd = namei("/");
801041b7:	c7 04 24 4e 83 10 80 	movl   $0x8010834e,(%esp)
801041be:	e8 43 e2 ff ff       	call   80102406 <namei>
801041c3:	8b 55 f4             	mov    -0xc(%ebp),%edx
801041c6:	89 42 68             	mov    %eax,0x68(%edx)

  p->state = RUNNABLE;
801041c9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041cc:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
}
801041d3:	c9                   	leave  
801041d4:	c3                   	ret    

801041d5 <growproc>:

// Grow current process's memory by n bytes.
// Return 0 on success, -1 on failure.
int
growproc(int n)
{
801041d5:	55                   	push   %ebp
801041d6:	89 e5                	mov    %esp,%ebp
801041d8:	83 ec 28             	sub    $0x28,%esp
  uint sz;
  
  sz = proc->sz;
801041db:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801041e1:	8b 00                	mov    (%eax),%eax
801041e3:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(n > 0){
801041e6:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801041ea:	7e 34                	jle    80104220 <growproc+0x4b>
    if((sz = allocuvm(proc->pgdir, sz, sz + n)) == 0)
801041ec:	8b 55 08             	mov    0x8(%ebp),%edx
801041ef:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041f2:	01 c2                	add    %eax,%edx
801041f4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801041fa:	8b 40 04             	mov    0x4(%eax),%eax
801041fd:	89 54 24 08          	mov    %edx,0x8(%esp)
80104201:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104204:	89 54 24 04          	mov    %edx,0x4(%esp)
80104208:	89 04 24             	mov    %eax,(%esp)
8010420b:	e8 b2 39 00 00       	call   80107bc2 <allocuvm>
80104210:	89 45 f4             	mov    %eax,-0xc(%ebp)
80104213:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80104217:	75 41                	jne    8010425a <growproc+0x85>
      return -1;
80104219:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010421e:	eb 58                	jmp    80104278 <growproc+0xa3>
  } else if(n < 0){
80104220:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80104224:	79 34                	jns    8010425a <growproc+0x85>
    if((sz = deallocuvm(proc->pgdir, sz, sz + n)) == 0)
80104226:	8b 55 08             	mov    0x8(%ebp),%edx
80104229:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010422c:	01 c2                	add    %eax,%edx
8010422e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104234:	8b 40 04             	mov    0x4(%eax),%eax
80104237:	89 54 24 08          	mov    %edx,0x8(%esp)
8010423b:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010423e:	89 54 24 04          	mov    %edx,0x4(%esp)
80104242:	89 04 24             	mov    %eax,(%esp)
80104245:	e8 52 3a 00 00       	call   80107c9c <deallocuvm>
8010424a:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010424d:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80104251:	75 07                	jne    8010425a <growproc+0x85>
      return -1;
80104253:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104258:	eb 1e                	jmp    80104278 <growproc+0xa3>
  }
  proc->sz = sz;
8010425a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104260:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104263:	89 10                	mov    %edx,(%eax)
  switchuvm(proc);
80104265:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010426b:	89 04 24             	mov    %eax,(%esp)
8010426e:	e8 72 36 00 00       	call   801078e5 <switchuvm>
  return 0;
80104273:	b8 00 00 00 00       	mov    $0x0,%eax
}
80104278:	c9                   	leave  
80104279:	c3                   	ret    

8010427a <fork>:
// Create a new process copying p as the parent.
// Sets up stack to return as if from system call.
// Caller must set state of returned proc to RUNNABLE.
int
fork(void)
{
8010427a:	55                   	push   %ebp
8010427b:	89 e5                	mov    %esp,%ebp
8010427d:	57                   	push   %edi
8010427e:	56                   	push   %esi
8010427f:	53                   	push   %ebx
80104280:	83 ec 2c             	sub    $0x2c,%esp
  int i, pid;
  struct proc *np;

  // Allocate process.
  if((np = allocproc()) == 0)
80104283:	e8 2e fd ff ff       	call   80103fb6 <allocproc>
80104288:	89 45 e0             	mov    %eax,-0x20(%ebp)
8010428b:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
8010428f:	75 0a                	jne    8010429b <fork+0x21>
    return -1;
80104291:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104296:	e9 52 01 00 00       	jmp    801043ed <fork+0x173>

  // Copy process state from p.
  if((np->pgdir = copyuvm(proc->pgdir, proc->sz)) == 0){
8010429b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801042a1:	8b 10                	mov    (%eax),%edx
801042a3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801042a9:	8b 40 04             	mov    0x4(%eax),%eax
801042ac:	89 54 24 04          	mov    %edx,0x4(%esp)
801042b0:	89 04 24             	mov    %eax,(%esp)
801042b3:	e8 80 3b 00 00       	call   80107e38 <copyuvm>
801042b8:	8b 55 e0             	mov    -0x20(%ebp),%edx
801042bb:	89 42 04             	mov    %eax,0x4(%edx)
801042be:	8b 45 e0             	mov    -0x20(%ebp),%eax
801042c1:	8b 40 04             	mov    0x4(%eax),%eax
801042c4:	85 c0                	test   %eax,%eax
801042c6:	75 2c                	jne    801042f4 <fork+0x7a>
    kfree(np->kstack);
801042c8:	8b 45 e0             	mov    -0x20(%ebp),%eax
801042cb:	8b 40 08             	mov    0x8(%eax),%eax
801042ce:	89 04 24             	mov    %eax,(%esp)
801042d1:	e8 73 e7 ff ff       	call   80102a49 <kfree>
    np->kstack = 0;
801042d6:	8b 45 e0             	mov    -0x20(%ebp),%eax
801042d9:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
    np->state = UNUSED;
801042e0:	8b 45 e0             	mov    -0x20(%ebp),%eax
801042e3:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    return -1;
801042ea:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801042ef:	e9 f9 00 00 00       	jmp    801043ed <fork+0x173>
  }
  np->sz = proc->sz;
801042f4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801042fa:	8b 10                	mov    (%eax),%edx
801042fc:	8b 45 e0             	mov    -0x20(%ebp),%eax
801042ff:	89 10                	mov    %edx,(%eax)
  np->parent = proc;
80104301:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104308:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010430b:	89 50 14             	mov    %edx,0x14(%eax)
  *np->tf = *proc->tf;
8010430e:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104311:	8b 50 18             	mov    0x18(%eax),%edx
80104314:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010431a:	8b 40 18             	mov    0x18(%eax),%eax
8010431d:	89 c3                	mov    %eax,%ebx
8010431f:	b8 13 00 00 00       	mov    $0x13,%eax
80104324:	89 d7                	mov    %edx,%edi
80104326:	89 de                	mov    %ebx,%esi
80104328:	89 c1                	mov    %eax,%ecx
8010432a:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;
8010432c:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010432f:	8b 40 18             	mov    0x18(%eax),%eax
80104332:	c7 40 1c 00 00 00 00 	movl   $0x0,0x1c(%eax)

  for(i = 0; i < NOFILE; i++)
80104339:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
80104340:	eb 3d                	jmp    8010437f <fork+0x105>
    if(proc->ofile[i])
80104342:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104348:	8b 55 e4             	mov    -0x1c(%ebp),%edx
8010434b:	83 c2 08             	add    $0x8,%edx
8010434e:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80104352:	85 c0                	test   %eax,%eax
80104354:	74 25                	je     8010437b <fork+0x101>
      np->ofile[i] = filedup(proc->ofile[i]);
80104356:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010435c:	8b 55 e4             	mov    -0x1c(%ebp),%edx
8010435f:	83 c2 08             	add    $0x8,%edx
80104362:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80104366:	89 04 24             	mov    %eax,(%esp)
80104369:	e8 18 cc ff ff       	call   80100f86 <filedup>
8010436e:	8b 55 e0             	mov    -0x20(%ebp),%edx
80104371:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80104374:	83 c1 08             	add    $0x8,%ecx
80104377:	89 44 8a 08          	mov    %eax,0x8(%edx,%ecx,4)
  *np->tf = *proc->tf;

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;

  for(i = 0; i < NOFILE; i++)
8010437b:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
8010437f:	83 7d e4 0f          	cmpl   $0xf,-0x1c(%ebp)
80104383:	7e bd                	jle    80104342 <fork+0xc8>
    if(proc->ofile[i])
      np->ofile[i] = filedup(proc->ofile[i]);
  np->cwd = idup(proc->cwd);
80104385:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010438b:	8b 40 68             	mov    0x68(%eax),%eax
8010438e:	89 04 24             	mov    %eax,(%esp)
80104391:	e8 93 d4 ff ff       	call   80101829 <idup>
80104396:	8b 55 e0             	mov    -0x20(%ebp),%edx
80104399:	89 42 68             	mov    %eax,0x68(%edx)

  safestrcpy(np->name, proc->name, sizeof(proc->name));
8010439c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801043a2:	8d 50 6c             	lea    0x6c(%eax),%edx
801043a5:	8b 45 e0             	mov    -0x20(%ebp),%eax
801043a8:	83 c0 6c             	add    $0x6c,%eax
801043ab:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
801043b2:	00 
801043b3:	89 54 24 04          	mov    %edx,0x4(%esp)
801043b7:	89 04 24             	mov    %eax,(%esp)
801043ba:	e8 4d 0b 00 00       	call   80104f0c <safestrcpy>
 
  pid = np->pid;
801043bf:	8b 45 e0             	mov    -0x20(%ebp),%eax
801043c2:	8b 40 10             	mov    0x10(%eax),%eax
801043c5:	89 45 dc             	mov    %eax,-0x24(%ebp)

  // lock to force the compiler to emit the np->state write last.
  acquire(&ptable.lock);
801043c8:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801043cf:	e8 c4 06 00 00       	call   80104a98 <acquire>
  np->state = RUNNABLE;
801043d4:	8b 45 e0             	mov    -0x20(%ebp),%eax
801043d7:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  release(&ptable.lock);
801043de:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801043e5:	e8 10 07 00 00       	call   80104afa <release>
  
  return pid;
801043ea:	8b 45 dc             	mov    -0x24(%ebp),%eax
}
801043ed:	83 c4 2c             	add    $0x2c,%esp
801043f0:	5b                   	pop    %ebx
801043f1:	5e                   	pop    %esi
801043f2:	5f                   	pop    %edi
801043f3:	5d                   	pop    %ebp
801043f4:	c3                   	ret    

801043f5 <exit>:
// Exit the current process.  Does not return.
// An exited process remains in the zombie state
// until its parent calls wait() to find out it exited.
void
exit(void)
{
801043f5:	55                   	push   %ebp
801043f6:	89 e5                	mov    %esp,%ebp
801043f8:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int fd;

  if(proc == initproc)
801043fb:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104402:	a1 48 b6 10 80       	mov    0x8010b648,%eax
80104407:	39 c2                	cmp    %eax,%edx
80104409:	75 0c                	jne    80104417 <exit+0x22>
    panic("init exiting");
8010440b:	c7 04 24 50 83 10 80 	movl   $0x80108350,(%esp)
80104412:	e8 23 c1 ff ff       	call   8010053a <panic>

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
80104417:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
8010441e:	eb 44                	jmp    80104464 <exit+0x6f>
    if(proc->ofile[fd]){
80104420:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104426:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104429:	83 c2 08             	add    $0x8,%edx
8010442c:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80104430:	85 c0                	test   %eax,%eax
80104432:	74 2c                	je     80104460 <exit+0x6b>
      fileclose(proc->ofile[fd]);
80104434:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010443a:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010443d:	83 c2 08             	add    $0x8,%edx
80104440:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80104444:	89 04 24             	mov    %eax,(%esp)
80104447:	e8 82 cb ff ff       	call   80100fce <fileclose>
      proc->ofile[fd] = 0;
8010444c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104452:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104455:	83 c2 08             	add    $0x8,%edx
80104458:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
8010445f:	00 

  if(proc == initproc)
    panic("init exiting");

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
80104460:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80104464:	83 7d f0 0f          	cmpl   $0xf,-0x10(%ebp)
80104468:	7e b6                	jle    80104420 <exit+0x2b>
      fileclose(proc->ofile[fd]);
      proc->ofile[fd] = 0;
    }
  }

  begin_trans();
8010446a:	e8 76 ed ff ff       	call   801031e5 <begin_trans>
  iput(proc->cwd);
8010446f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104475:	8b 40 68             	mov    0x68(%eax),%eax
80104478:	89 04 24             	mov    %eax,(%esp)
8010447b:	e8 8e d5 ff ff       	call   80101a0e <iput>
  commit_trans();
80104480:	e8 a9 ed ff ff       	call   8010322e <commit_trans>
  proc->cwd = 0;
80104485:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010448b:	c7 40 68 00 00 00 00 	movl   $0x0,0x68(%eax)

  acquire(&ptable.lock);
80104492:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104499:	e8 fa 05 00 00       	call   80104a98 <acquire>

  // Parent might be sleeping in wait().
  wakeup1(proc->parent);
8010449e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044a4:	8b 40 14             	mov    0x14(%eax),%eax
801044a7:	89 04 24             	mov    %eax,(%esp)
801044aa:	e8 ba 03 00 00       	call   80104869 <wakeup1>

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801044af:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
801044b6:	eb 38                	jmp    801044f0 <exit+0xfb>
    if(p->parent == proc){
801044b8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044bb:	8b 50 14             	mov    0x14(%eax),%edx
801044be:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044c4:	39 c2                	cmp    %eax,%edx
801044c6:	75 24                	jne    801044ec <exit+0xf7>
      p->parent = initproc;
801044c8:	8b 15 48 b6 10 80    	mov    0x8010b648,%edx
801044ce:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044d1:	89 50 14             	mov    %edx,0x14(%eax)
      if(p->state == ZOMBIE)
801044d4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044d7:	8b 40 0c             	mov    0xc(%eax),%eax
801044da:	83 f8 05             	cmp    $0x5,%eax
801044dd:	75 0d                	jne    801044ec <exit+0xf7>
        wakeup1(initproc);
801044df:	a1 48 b6 10 80       	mov    0x8010b648,%eax
801044e4:	89 04 24             	mov    %eax,(%esp)
801044e7:	e8 7d 03 00 00       	call   80104869 <wakeup1>

  // Parent might be sleeping in wait().
  wakeup1(proc->parent);

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801044ec:	83 45 f4 7c          	addl   $0x7c,-0xc(%ebp)
801044f0:	81 7d f4 54 1e 11 80 	cmpl   $0x80111e54,-0xc(%ebp)
801044f7:	72 bf                	jb     801044b8 <exit+0xc3>
        wakeup1(initproc);
    }
  }

  // Jump into the scheduler, never to return.
  proc->state = ZOMBIE;
801044f9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044ff:	c7 40 0c 05 00 00 00 	movl   $0x5,0xc(%eax)
  sched();
80104506:	e8 b3 01 00 00       	call   801046be <sched>
  panic("zombie exit");
8010450b:	c7 04 24 5d 83 10 80 	movl   $0x8010835d,(%esp)
80104512:	e8 23 c0 ff ff       	call   8010053a <panic>

80104517 <wait>:

// Wait for a child process to exit and return its pid.
// Return -1 if this process has no children.
int
wait(void)
{
80104517:	55                   	push   %ebp
80104518:	89 e5                	mov    %esp,%ebp
8010451a:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int havekids, pid;

  acquire(&ptable.lock);
8010451d:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104524:	e8 6f 05 00 00       	call   80104a98 <acquire>
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
80104529:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104530:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
80104537:	e9 9a 00 00 00       	jmp    801045d6 <wait+0xbf>
      if(p->parent != proc)
8010453c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010453f:	8b 50 14             	mov    0x14(%eax),%edx
80104542:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104548:	39 c2                	cmp    %eax,%edx
8010454a:	74 05                	je     80104551 <wait+0x3a>
        continue;
8010454c:	e9 81 00 00 00       	jmp    801045d2 <wait+0xbb>
      havekids = 1;
80104551:	c7 45 f0 01 00 00 00 	movl   $0x1,-0x10(%ebp)
      if(p->state == ZOMBIE){
80104558:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010455b:	8b 40 0c             	mov    0xc(%eax),%eax
8010455e:	83 f8 05             	cmp    $0x5,%eax
80104561:	75 6f                	jne    801045d2 <wait+0xbb>
        // Found one.
        pid = p->pid;
80104563:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104566:	8b 40 10             	mov    0x10(%eax),%eax
80104569:	89 45 ec             	mov    %eax,-0x14(%ebp)
        kfree(p->kstack);
8010456c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010456f:	8b 40 08             	mov    0x8(%eax),%eax
80104572:	89 04 24             	mov    %eax,(%esp)
80104575:	e8 cf e4 ff ff       	call   80102a49 <kfree>
        p->kstack = 0;
8010457a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010457d:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
        freevm(p->pgdir);
80104584:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104587:	8b 40 04             	mov    0x4(%eax),%eax
8010458a:	89 04 24             	mov    %eax,(%esp)
8010458d:	e8 c6 37 00 00       	call   80107d58 <freevm>
        p->state = UNUSED;
80104592:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104595:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
        p->pid = 0;
8010459c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010459f:	c7 40 10 00 00 00 00 	movl   $0x0,0x10(%eax)
        p->parent = 0;
801045a6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801045a9:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
        p->name[0] = 0;
801045b0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801045b3:	c6 40 6c 00          	movb   $0x0,0x6c(%eax)
        p->killed = 0;
801045b7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801045ba:	c7 40 24 00 00 00 00 	movl   $0x0,0x24(%eax)
        release(&ptable.lock);
801045c1:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801045c8:	e8 2d 05 00 00       	call   80104afa <release>
        return pid;
801045cd:	8b 45 ec             	mov    -0x14(%ebp),%eax
801045d0:	eb 52                	jmp    80104624 <wait+0x10d>

  acquire(&ptable.lock);
  for(;;){
    // Scan through table looking for zombie children.
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801045d2:	83 45 f4 7c          	addl   $0x7c,-0xc(%ebp)
801045d6:	81 7d f4 54 1e 11 80 	cmpl   $0x80111e54,-0xc(%ebp)
801045dd:	0f 82 59 ff ff ff    	jb     8010453c <wait+0x25>
        return pid;
      }
    }

    // No point waiting if we don't have any children.
    if(!havekids || proc->killed){
801045e3:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801045e7:	74 0d                	je     801045f6 <wait+0xdf>
801045e9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801045ef:	8b 40 24             	mov    0x24(%eax),%eax
801045f2:	85 c0                	test   %eax,%eax
801045f4:	74 13                	je     80104609 <wait+0xf2>
      release(&ptable.lock);
801045f6:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801045fd:	e8 f8 04 00 00       	call   80104afa <release>
      return -1;
80104602:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104607:	eb 1b                	jmp    80104624 <wait+0x10d>
    }

    // Wait for children to exit.  (See wakeup1 call in proc_exit.)
    sleep(proc, &ptable.lock);  //DOC: wait-sleep
80104609:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010460f:	c7 44 24 04 20 ff 10 	movl   $0x8010ff20,0x4(%esp)
80104616:	80 
80104617:	89 04 24             	mov    %eax,(%esp)
8010461a:	e8 af 01 00 00       	call   801047ce <sleep>
  }
8010461f:	e9 05 ff ff ff       	jmp    80104529 <wait+0x12>
}
80104624:	c9                   	leave  
80104625:	c3                   	ret    

80104626 <scheduler>:
//  - swtch to start running that process
//  - eventually that process transfers control
//      via swtch back to the scheduler.
void
scheduler(void)
{
80104626:	55                   	push   %ebp
80104627:	89 e5                	mov    %esp,%ebp
80104629:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;

  for(;;){
    // Enable interrupts on this processor.
    sti();
8010462c:	e8 63 f9 ff ff       	call   80103f94 <sti>

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
80104631:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104638:	e8 5b 04 00 00       	call   80104a98 <acquire>
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
8010463d:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
80104644:	eb 5e                	jmp    801046a4 <scheduler+0x7e>
      if(p->state != RUNNABLE)
80104646:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104649:	8b 40 0c             	mov    0xc(%eax),%eax
8010464c:	83 f8 03             	cmp    $0x3,%eax
8010464f:	74 02                	je     80104653 <scheduler+0x2d>
        continue;
80104651:	eb 4d                	jmp    801046a0 <scheduler+0x7a>

      // Switch to chosen process.  It is the process's job
      // to release ptable.lock and then reacquire it
      // before jumping back to us.
      proc = p;
80104653:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104656:	65 a3 04 00 00 00    	mov    %eax,%gs:0x4
      switchuvm(p);
8010465c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010465f:	89 04 24             	mov    %eax,(%esp)
80104662:	e8 7e 32 00 00       	call   801078e5 <switchuvm>
      p->state = RUNNING;
80104667:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010466a:	c7 40 0c 04 00 00 00 	movl   $0x4,0xc(%eax)
      swtch(&cpu->scheduler, proc->context);
80104671:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104677:	8b 40 1c             	mov    0x1c(%eax),%eax
8010467a:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80104681:	83 c2 04             	add    $0x4,%edx
80104684:	89 44 24 04          	mov    %eax,0x4(%esp)
80104688:	89 14 24             	mov    %edx,(%esp)
8010468b:	e8 ed 08 00 00       	call   80104f7d <swtch>
      switchkvm();
80104690:	e8 33 32 00 00       	call   801078c8 <switchkvm>

      // Process is done running for now.
      // It should have changed its p->state before coming back.
      proc = 0;
80104695:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
8010469c:	00 00 00 00 
    // Enable interrupts on this processor.
    sti();

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801046a0:	83 45 f4 7c          	addl   $0x7c,-0xc(%ebp)
801046a4:	81 7d f4 54 1e 11 80 	cmpl   $0x80111e54,-0xc(%ebp)
801046ab:	72 99                	jb     80104646 <scheduler+0x20>

      // Process is done running for now.
      // It should have changed its p->state before coming back.
      proc = 0;
    }
    release(&ptable.lock);
801046ad:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801046b4:	e8 41 04 00 00       	call   80104afa <release>

  }
801046b9:	e9 6e ff ff ff       	jmp    8010462c <scheduler+0x6>

801046be <sched>:

// Enter scheduler.  Must hold only ptable.lock
// and have changed proc->state.
void
sched(void)
{
801046be:	55                   	push   %ebp
801046bf:	89 e5                	mov    %esp,%ebp
801046c1:	83 ec 28             	sub    $0x28,%esp
  int intena;

  if(!holding(&ptable.lock))
801046c4:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801046cb:	e8 f2 04 00 00       	call   80104bc2 <holding>
801046d0:	85 c0                	test   %eax,%eax
801046d2:	75 0c                	jne    801046e0 <sched+0x22>
    panic("sched ptable.lock");
801046d4:	c7 04 24 69 83 10 80 	movl   $0x80108369,(%esp)
801046db:	e8 5a be ff ff       	call   8010053a <panic>
  if(cpu->ncli != 1)
801046e0:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801046e6:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
801046ec:	83 f8 01             	cmp    $0x1,%eax
801046ef:	74 0c                	je     801046fd <sched+0x3f>
    panic("sched locks");
801046f1:	c7 04 24 7b 83 10 80 	movl   $0x8010837b,(%esp)
801046f8:	e8 3d be ff ff       	call   8010053a <panic>
  if(proc->state == RUNNING)
801046fd:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104703:	8b 40 0c             	mov    0xc(%eax),%eax
80104706:	83 f8 04             	cmp    $0x4,%eax
80104709:	75 0c                	jne    80104717 <sched+0x59>
    panic("sched running");
8010470b:	c7 04 24 87 83 10 80 	movl   $0x80108387,(%esp)
80104712:	e8 23 be ff ff       	call   8010053a <panic>
  if(readeflags()&FL_IF)
80104717:	e8 68 f8 ff ff       	call   80103f84 <readeflags>
8010471c:	25 00 02 00 00       	and    $0x200,%eax
80104721:	85 c0                	test   %eax,%eax
80104723:	74 0c                	je     80104731 <sched+0x73>
    panic("sched interruptible");
80104725:	c7 04 24 95 83 10 80 	movl   $0x80108395,(%esp)
8010472c:	e8 09 be ff ff       	call   8010053a <panic>
  intena = cpu->intena;
80104731:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104737:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
8010473d:	89 45 f4             	mov    %eax,-0xc(%ebp)
  swtch(&proc->context, cpu->scheduler);
80104740:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104746:	8b 40 04             	mov    0x4(%eax),%eax
80104749:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104750:	83 c2 1c             	add    $0x1c,%edx
80104753:	89 44 24 04          	mov    %eax,0x4(%esp)
80104757:	89 14 24             	mov    %edx,(%esp)
8010475a:	e8 1e 08 00 00       	call   80104f7d <swtch>
  cpu->intena = intena;
8010475f:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104765:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104768:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
8010476e:	c9                   	leave  
8010476f:	c3                   	ret    

80104770 <yield>:

// Give up the CPU for one scheduling round.
void
yield(void)
{
80104770:	55                   	push   %ebp
80104771:	89 e5                	mov    %esp,%ebp
80104773:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);  //DOC: yieldlock
80104776:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010477d:	e8 16 03 00 00       	call   80104a98 <acquire>
  proc->state = RUNNABLE;
80104782:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104788:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  sched();
8010478f:	e8 2a ff ff ff       	call   801046be <sched>
  release(&ptable.lock);
80104794:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010479b:	e8 5a 03 00 00       	call   80104afa <release>
}
801047a0:	c9                   	leave  
801047a1:	c3                   	ret    

801047a2 <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch here.  "Return" to user space.
void
forkret(void)
{
801047a2:	55                   	push   %ebp
801047a3:	89 e5                	mov    %esp,%ebp
801047a5:	83 ec 18             	sub    $0x18,%esp
  static int first = 1;
  // Still holding ptable.lock from scheduler.
  release(&ptable.lock);
801047a8:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801047af:	e8 46 03 00 00       	call   80104afa <release>

  if (first) {
801047b4:	a1 08 b0 10 80       	mov    0x8010b008,%eax
801047b9:	85 c0                	test   %eax,%eax
801047bb:	74 0f                	je     801047cc <forkret+0x2a>
    // Some initialization functions must be run in the context
    // of a regular process (e.g., they call sleep), and thus cannot 
    // be run from main().
    first = 0;
801047bd:	c7 05 08 b0 10 80 00 	movl   $0x0,0x8010b008
801047c4:	00 00 00 
    initlog();
801047c7:	e8 0b e8 ff ff       	call   80102fd7 <initlog>
  }
  
  // Return to "caller", actually trapret (see allocproc).
}
801047cc:	c9                   	leave  
801047cd:	c3                   	ret    

801047ce <sleep>:

// Atomically release lock and sleep on chan.
// Reacquires lock when awakened.
void
sleep(void *chan, struct spinlock *lk)
{
801047ce:	55                   	push   %ebp
801047cf:	89 e5                	mov    %esp,%ebp
801047d1:	83 ec 18             	sub    $0x18,%esp
  if(proc == 0)
801047d4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801047da:	85 c0                	test   %eax,%eax
801047dc:	75 0c                	jne    801047ea <sleep+0x1c>
    panic("sleep");
801047de:	c7 04 24 a9 83 10 80 	movl   $0x801083a9,(%esp)
801047e5:	e8 50 bd ff ff       	call   8010053a <panic>

  if(lk == 0)
801047ea:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
801047ee:	75 0c                	jne    801047fc <sleep+0x2e>
    panic("sleep without lk");
801047f0:	c7 04 24 af 83 10 80 	movl   $0x801083af,(%esp)
801047f7:	e8 3e bd ff ff       	call   8010053a <panic>
  // change p->state and then call sched.
  // Once we hold ptable.lock, we can be
  // guaranteed that we won't miss any wakeup
  // (wakeup runs with ptable.lock locked),
  // so it's okay to release lk.
  if(lk != &ptable.lock){  //DOC: sleeplock0
801047fc:	81 7d 0c 20 ff 10 80 	cmpl   $0x8010ff20,0xc(%ebp)
80104803:	74 17                	je     8010481c <sleep+0x4e>
    acquire(&ptable.lock);  //DOC: sleeplock1
80104805:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010480c:	e8 87 02 00 00       	call   80104a98 <acquire>
    release(lk);
80104811:	8b 45 0c             	mov    0xc(%ebp),%eax
80104814:	89 04 24             	mov    %eax,(%esp)
80104817:	e8 de 02 00 00       	call   80104afa <release>
  }

  // Go to sleep.
  proc->chan = chan;
8010481c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104822:	8b 55 08             	mov    0x8(%ebp),%edx
80104825:	89 50 20             	mov    %edx,0x20(%eax)
  proc->state = SLEEPING;
80104828:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010482e:	c7 40 0c 02 00 00 00 	movl   $0x2,0xc(%eax)
  sched();
80104835:	e8 84 fe ff ff       	call   801046be <sched>

  // Tidy up.
  proc->chan = 0;
8010483a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104840:	c7 40 20 00 00 00 00 	movl   $0x0,0x20(%eax)

  // Reacquire original lock.
  if(lk != &ptable.lock){  //DOC: sleeplock2
80104847:	81 7d 0c 20 ff 10 80 	cmpl   $0x8010ff20,0xc(%ebp)
8010484e:	74 17                	je     80104867 <sleep+0x99>
    release(&ptable.lock);
80104850:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104857:	e8 9e 02 00 00       	call   80104afa <release>
    acquire(lk);
8010485c:	8b 45 0c             	mov    0xc(%ebp),%eax
8010485f:	89 04 24             	mov    %eax,(%esp)
80104862:	e8 31 02 00 00       	call   80104a98 <acquire>
  }
}
80104867:	c9                   	leave  
80104868:	c3                   	ret    

80104869 <wakeup1>:
//PAGEBREAK!
// Wake up all processes sleeping on chan.
// The ptable lock must be held.
static void
wakeup1(void *chan)
{
80104869:	55                   	push   %ebp
8010486a:	89 e5                	mov    %esp,%ebp
8010486c:	83 ec 10             	sub    $0x10,%esp
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
8010486f:	c7 45 fc 54 ff 10 80 	movl   $0x8010ff54,-0x4(%ebp)
80104876:	eb 24                	jmp    8010489c <wakeup1+0x33>
    if(p->state == SLEEPING && p->chan == chan)
80104878:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010487b:	8b 40 0c             	mov    0xc(%eax),%eax
8010487e:	83 f8 02             	cmp    $0x2,%eax
80104881:	75 15                	jne    80104898 <wakeup1+0x2f>
80104883:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104886:	8b 40 20             	mov    0x20(%eax),%eax
80104889:	3b 45 08             	cmp    0x8(%ebp),%eax
8010488c:	75 0a                	jne    80104898 <wakeup1+0x2f>
      p->state = RUNNABLE;
8010488e:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104891:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
static void
wakeup1(void *chan)
{
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104898:	83 45 fc 7c          	addl   $0x7c,-0x4(%ebp)
8010489c:	81 7d fc 54 1e 11 80 	cmpl   $0x80111e54,-0x4(%ebp)
801048a3:	72 d3                	jb     80104878 <wakeup1+0xf>
    if(p->state == SLEEPING && p->chan == chan)
      p->state = RUNNABLE;
}
801048a5:	c9                   	leave  
801048a6:	c3                   	ret    

801048a7 <wakeup>:

// Wake up all processes sleeping on chan.
void
wakeup(void *chan)
{
801048a7:	55                   	push   %ebp
801048a8:	89 e5                	mov    %esp,%ebp
801048aa:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);
801048ad:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801048b4:	e8 df 01 00 00       	call   80104a98 <acquire>
  wakeup1(chan);
801048b9:	8b 45 08             	mov    0x8(%ebp),%eax
801048bc:	89 04 24             	mov    %eax,(%esp)
801048bf:	e8 a5 ff ff ff       	call   80104869 <wakeup1>
  release(&ptable.lock);
801048c4:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801048cb:	e8 2a 02 00 00       	call   80104afa <release>
}
801048d0:	c9                   	leave  
801048d1:	c3                   	ret    

801048d2 <kill>:
// Kill the process with the given pid.
// Process won't exit until it returns
// to user space (see trap in trap.c).
int
kill(int pid)
{
801048d2:	55                   	push   %ebp
801048d3:	89 e5                	mov    %esp,%ebp
801048d5:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;

  acquire(&ptable.lock);
801048d8:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801048df:	e8 b4 01 00 00       	call   80104a98 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801048e4:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
801048eb:	eb 41                	jmp    8010492e <kill+0x5c>
    if(p->pid == pid){
801048ed:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048f0:	8b 40 10             	mov    0x10(%eax),%eax
801048f3:	3b 45 08             	cmp    0x8(%ebp),%eax
801048f6:	75 32                	jne    8010492a <kill+0x58>
      p->killed = 1;
801048f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048fb:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
      // Wake process from sleep if necessary.
      if(p->state == SLEEPING)
80104902:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104905:	8b 40 0c             	mov    0xc(%eax),%eax
80104908:	83 f8 02             	cmp    $0x2,%eax
8010490b:	75 0a                	jne    80104917 <kill+0x45>
        p->state = RUNNABLE;
8010490d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104910:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
      release(&ptable.lock);
80104917:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010491e:	e8 d7 01 00 00       	call   80104afa <release>
      return 0;
80104923:	b8 00 00 00 00       	mov    $0x0,%eax
80104928:	eb 1e                	jmp    80104948 <kill+0x76>
kill(int pid)
{
  struct proc *p;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
8010492a:	83 45 f4 7c          	addl   $0x7c,-0xc(%ebp)
8010492e:	81 7d f4 54 1e 11 80 	cmpl   $0x80111e54,-0xc(%ebp)
80104935:	72 b6                	jb     801048ed <kill+0x1b>
        p->state = RUNNABLE;
      release(&ptable.lock);
      return 0;
    }
  }
  release(&ptable.lock);
80104937:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010493e:	e8 b7 01 00 00       	call   80104afa <release>
  return -1;
80104943:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104948:	c9                   	leave  
80104949:	c3                   	ret    

8010494a <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
8010494a:	55                   	push   %ebp
8010494b:	89 e5                	mov    %esp,%ebp
8010494d:	83 ec 58             	sub    $0x58,%esp
  int i;
  struct proc *p;
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104950:	c7 45 f0 54 ff 10 80 	movl   $0x8010ff54,-0x10(%ebp)
80104957:	e9 d6 00 00 00       	jmp    80104a32 <procdump+0xe8>
    if(p->state == UNUSED)
8010495c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010495f:	8b 40 0c             	mov    0xc(%eax),%eax
80104962:	85 c0                	test   %eax,%eax
80104964:	75 05                	jne    8010496b <procdump+0x21>
      continue;
80104966:	e9 c3 00 00 00       	jmp    80104a2e <procdump+0xe4>
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
8010496b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010496e:	8b 40 0c             	mov    0xc(%eax),%eax
80104971:	83 f8 05             	cmp    $0x5,%eax
80104974:	77 23                	ja     80104999 <procdump+0x4f>
80104976:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104979:	8b 40 0c             	mov    0xc(%eax),%eax
8010497c:	8b 04 85 0c b0 10 80 	mov    -0x7fef4ff4(,%eax,4),%eax
80104983:	85 c0                	test   %eax,%eax
80104985:	74 12                	je     80104999 <procdump+0x4f>
      state = states[p->state];
80104987:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010498a:	8b 40 0c             	mov    0xc(%eax),%eax
8010498d:	8b 04 85 0c b0 10 80 	mov    -0x7fef4ff4(,%eax,4),%eax
80104994:	89 45 ec             	mov    %eax,-0x14(%ebp)
80104997:	eb 07                	jmp    801049a0 <procdump+0x56>
    else
      state = "???";
80104999:	c7 45 ec c0 83 10 80 	movl   $0x801083c0,-0x14(%ebp)
    cprintf("%d %s %s", p->pid, state, p->name);
801049a0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801049a3:	8d 50 6c             	lea    0x6c(%eax),%edx
801049a6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801049a9:	8b 40 10             	mov    0x10(%eax),%eax
801049ac:	89 54 24 0c          	mov    %edx,0xc(%esp)
801049b0:	8b 55 ec             	mov    -0x14(%ebp),%edx
801049b3:	89 54 24 08          	mov    %edx,0x8(%esp)
801049b7:	89 44 24 04          	mov    %eax,0x4(%esp)
801049bb:	c7 04 24 c4 83 10 80 	movl   $0x801083c4,(%esp)
801049c2:	e8 d9 b9 ff ff       	call   801003a0 <cprintf>
    if(p->state == SLEEPING){
801049c7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801049ca:	8b 40 0c             	mov    0xc(%eax),%eax
801049cd:	83 f8 02             	cmp    $0x2,%eax
801049d0:	75 50                	jne    80104a22 <procdump+0xd8>
      getcallerpcs((uint*)p->context->ebp+2, pc);
801049d2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801049d5:	8b 40 1c             	mov    0x1c(%eax),%eax
801049d8:	8b 40 0c             	mov    0xc(%eax),%eax
801049db:	83 c0 08             	add    $0x8,%eax
801049de:	8d 55 c4             	lea    -0x3c(%ebp),%edx
801049e1:	89 54 24 04          	mov    %edx,0x4(%esp)
801049e5:	89 04 24             	mov    %eax,(%esp)
801049e8:	e8 5c 01 00 00       	call   80104b49 <getcallerpcs>
      for(i=0; i<10 && pc[i] != 0; i++)
801049ed:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801049f4:	eb 1b                	jmp    80104a11 <procdump+0xc7>
        cprintf(" %p", pc[i]);
801049f6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801049f9:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
801049fd:	89 44 24 04          	mov    %eax,0x4(%esp)
80104a01:	c7 04 24 cd 83 10 80 	movl   $0x801083cd,(%esp)
80104a08:	e8 93 b9 ff ff       	call   801003a0 <cprintf>
    else
      state = "???";
    cprintf("%d %s %s", p->pid, state, p->name);
    if(p->state == SLEEPING){
      getcallerpcs((uint*)p->context->ebp+2, pc);
      for(i=0; i<10 && pc[i] != 0; i++)
80104a0d:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80104a11:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
80104a15:	7f 0b                	jg     80104a22 <procdump+0xd8>
80104a17:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a1a:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
80104a1e:	85 c0                	test   %eax,%eax
80104a20:	75 d4                	jne    801049f6 <procdump+0xac>
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
80104a22:	c7 04 24 d1 83 10 80 	movl   $0x801083d1,(%esp)
80104a29:	e8 72 b9 ff ff       	call   801003a0 <cprintf>
  int i;
  struct proc *p;
  char *state;
  uint pc[10];
  
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104a2e:	83 45 f0 7c          	addl   $0x7c,-0x10(%ebp)
80104a32:	81 7d f0 54 1e 11 80 	cmpl   $0x80111e54,-0x10(%ebp)
80104a39:	0f 82 1d ff ff ff    	jb     8010495c <procdump+0x12>
      for(i=0; i<10 && pc[i] != 0; i++)
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
  }
}
80104a3f:	c9                   	leave  
80104a40:	c3                   	ret    

80104a41 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80104a41:	55                   	push   %ebp
80104a42:	89 e5                	mov    %esp,%ebp
80104a44:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80104a47:	9c                   	pushf  
80104a48:	58                   	pop    %eax
80104a49:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return eflags;
80104a4c:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80104a4f:	c9                   	leave  
80104a50:	c3                   	ret    

80104a51 <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
80104a51:	55                   	push   %ebp
80104a52:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
80104a54:	fa                   	cli    
}
80104a55:	5d                   	pop    %ebp
80104a56:	c3                   	ret    

80104a57 <sti>:

static inline void
sti(void)
{
80104a57:	55                   	push   %ebp
80104a58:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
80104a5a:	fb                   	sti    
}
80104a5b:	5d                   	pop    %ebp
80104a5c:	c3                   	ret    

80104a5d <xchg>:

static inline uint
xchg(volatile uint *addr, uint newval)
{
80104a5d:	55                   	push   %ebp
80104a5e:	89 e5                	mov    %esp,%ebp
80104a60:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
80104a63:	8b 55 08             	mov    0x8(%ebp),%edx
80104a66:	8b 45 0c             	mov    0xc(%ebp),%eax
80104a69:	8b 4d 08             	mov    0x8(%ebp),%ecx
80104a6c:	f0 87 02             	lock xchg %eax,(%edx)
80104a6f:	89 45 fc             	mov    %eax,-0x4(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
80104a72:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80104a75:	c9                   	leave  
80104a76:	c3                   	ret    

80104a77 <initlock>:
#include "proc.h"
#include "spinlock.h"

void
initlock(struct spinlock *lk, char *name)
{
80104a77:	55                   	push   %ebp
80104a78:	89 e5                	mov    %esp,%ebp
  lk->name = name;
80104a7a:	8b 45 08             	mov    0x8(%ebp),%eax
80104a7d:	8b 55 0c             	mov    0xc(%ebp),%edx
80104a80:	89 50 04             	mov    %edx,0x4(%eax)
  lk->locked = 0;
80104a83:	8b 45 08             	mov    0x8(%ebp),%eax
80104a86:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lk->cpu = 0;
80104a8c:	8b 45 08             	mov    0x8(%ebp),%eax
80104a8f:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
}
80104a96:	5d                   	pop    %ebp
80104a97:	c3                   	ret    

80104a98 <acquire>:
// Loops (spins) until the lock is acquired.
// Holding a lock for a long time may cause
// other CPUs to waste time spinning to acquire it.
void
acquire(struct spinlock *lk)
{
80104a98:	55                   	push   %ebp
80104a99:	89 e5                	mov    %esp,%ebp
80104a9b:	83 ec 18             	sub    $0x18,%esp
  pushcli(); // disable interrupts to avoid deadlock.
80104a9e:	e8 49 01 00 00       	call   80104bec <pushcli>
  if(holding(lk))
80104aa3:	8b 45 08             	mov    0x8(%ebp),%eax
80104aa6:	89 04 24             	mov    %eax,(%esp)
80104aa9:	e8 14 01 00 00       	call   80104bc2 <holding>
80104aae:	85 c0                	test   %eax,%eax
80104ab0:	74 0c                	je     80104abe <acquire+0x26>
    panic("acquire");
80104ab2:	c7 04 24 fd 83 10 80 	movl   $0x801083fd,(%esp)
80104ab9:	e8 7c ba ff ff       	call   8010053a <panic>

  // The xchg is atomic.
  // It also serializes, so that reads after acquire are not
  // reordered before it. 
  while(xchg(&lk->locked, 1) != 0)
80104abe:	90                   	nop
80104abf:	8b 45 08             	mov    0x8(%ebp),%eax
80104ac2:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80104ac9:	00 
80104aca:	89 04 24             	mov    %eax,(%esp)
80104acd:	e8 8b ff ff ff       	call   80104a5d <xchg>
80104ad2:	85 c0                	test   %eax,%eax
80104ad4:	75 e9                	jne    80104abf <acquire+0x27>
    ;

  // Record info about lock acquisition for debugging.
  lk->cpu = cpu;
80104ad6:	8b 45 08             	mov    0x8(%ebp),%eax
80104ad9:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80104ae0:	89 50 08             	mov    %edx,0x8(%eax)
  getcallerpcs(&lk, lk->pcs);
80104ae3:	8b 45 08             	mov    0x8(%ebp),%eax
80104ae6:	83 c0 0c             	add    $0xc,%eax
80104ae9:	89 44 24 04          	mov    %eax,0x4(%esp)
80104aed:	8d 45 08             	lea    0x8(%ebp),%eax
80104af0:	89 04 24             	mov    %eax,(%esp)
80104af3:	e8 51 00 00 00       	call   80104b49 <getcallerpcs>
}
80104af8:	c9                   	leave  
80104af9:	c3                   	ret    

80104afa <release>:

// Release the lock.
void
release(struct spinlock *lk)
{
80104afa:	55                   	push   %ebp
80104afb:	89 e5                	mov    %esp,%ebp
80104afd:	83 ec 18             	sub    $0x18,%esp
  if(!holding(lk))
80104b00:	8b 45 08             	mov    0x8(%ebp),%eax
80104b03:	89 04 24             	mov    %eax,(%esp)
80104b06:	e8 b7 00 00 00       	call   80104bc2 <holding>
80104b0b:	85 c0                	test   %eax,%eax
80104b0d:	75 0c                	jne    80104b1b <release+0x21>
    panic("release");
80104b0f:	c7 04 24 05 84 10 80 	movl   $0x80108405,(%esp)
80104b16:	e8 1f ba ff ff       	call   8010053a <panic>

  lk->pcs[0] = 0;
80104b1b:	8b 45 08             	mov    0x8(%ebp),%eax
80104b1e:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  lk->cpu = 0;
80104b25:	8b 45 08             	mov    0x8(%ebp),%eax
80104b28:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
  // But the 2007 Intel 64 Architecture Memory Ordering White
  // Paper says that Intel 64 and IA-32 will not move a load
  // after a store. So lock->locked = 0 would work here.
  // The xchg being asm volatile ensures gcc emits it after
  // the above assignments (and after the critical section).
  xchg(&lk->locked, 0);
80104b2f:	8b 45 08             	mov    0x8(%ebp),%eax
80104b32:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104b39:	00 
80104b3a:	89 04 24             	mov    %eax,(%esp)
80104b3d:	e8 1b ff ff ff       	call   80104a5d <xchg>

  popcli();
80104b42:	e8 e9 00 00 00       	call   80104c30 <popcli>
}
80104b47:	c9                   	leave  
80104b48:	c3                   	ret    

80104b49 <getcallerpcs>:

// Record the current call stack in pcs[] by following the %ebp chain.
void
getcallerpcs(void *v, uint pcs[])
{
80104b49:	55                   	push   %ebp
80104b4a:	89 e5                	mov    %esp,%ebp
80104b4c:	83 ec 10             	sub    $0x10,%esp
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
80104b4f:	8b 45 08             	mov    0x8(%ebp),%eax
80104b52:	83 e8 08             	sub    $0x8,%eax
80104b55:	89 45 fc             	mov    %eax,-0x4(%ebp)
  for(i = 0; i < 10; i++){
80104b58:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
80104b5f:	eb 38                	jmp    80104b99 <getcallerpcs+0x50>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
80104b61:	83 7d fc 00          	cmpl   $0x0,-0x4(%ebp)
80104b65:	74 38                	je     80104b9f <getcallerpcs+0x56>
80104b67:	81 7d fc ff ff ff 7f 	cmpl   $0x7fffffff,-0x4(%ebp)
80104b6e:	76 2f                	jbe    80104b9f <getcallerpcs+0x56>
80104b70:	83 7d fc ff          	cmpl   $0xffffffff,-0x4(%ebp)
80104b74:	74 29                	je     80104b9f <getcallerpcs+0x56>
      break;
    pcs[i] = ebp[1];     // saved %eip
80104b76:	8b 45 f8             	mov    -0x8(%ebp),%eax
80104b79:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80104b80:	8b 45 0c             	mov    0xc(%ebp),%eax
80104b83:	01 c2                	add    %eax,%edx
80104b85:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104b88:	8b 40 04             	mov    0x4(%eax),%eax
80104b8b:	89 02                	mov    %eax,(%edx)
    ebp = (uint*)ebp[0]; // saved %ebp
80104b8d:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104b90:	8b 00                	mov    (%eax),%eax
80104b92:	89 45 fc             	mov    %eax,-0x4(%ebp)
{
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
  for(i = 0; i < 10; i++){
80104b95:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80104b99:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
80104b9d:	7e c2                	jle    80104b61 <getcallerpcs+0x18>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
80104b9f:	eb 19                	jmp    80104bba <getcallerpcs+0x71>
    pcs[i] = 0;
80104ba1:	8b 45 f8             	mov    -0x8(%ebp),%eax
80104ba4:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80104bab:	8b 45 0c             	mov    0xc(%ebp),%eax
80104bae:	01 d0                	add    %edx,%eax
80104bb0:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
80104bb6:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80104bba:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
80104bbe:	7e e1                	jle    80104ba1 <getcallerpcs+0x58>
    pcs[i] = 0;
}
80104bc0:	c9                   	leave  
80104bc1:	c3                   	ret    

80104bc2 <holding>:

// Check whether this cpu is holding the lock.
int
holding(struct spinlock *lock)
{
80104bc2:	55                   	push   %ebp
80104bc3:	89 e5                	mov    %esp,%ebp
  return lock->locked && lock->cpu == cpu;
80104bc5:	8b 45 08             	mov    0x8(%ebp),%eax
80104bc8:	8b 00                	mov    (%eax),%eax
80104bca:	85 c0                	test   %eax,%eax
80104bcc:	74 17                	je     80104be5 <holding+0x23>
80104bce:	8b 45 08             	mov    0x8(%ebp),%eax
80104bd1:	8b 50 08             	mov    0x8(%eax),%edx
80104bd4:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104bda:	39 c2                	cmp    %eax,%edx
80104bdc:	75 07                	jne    80104be5 <holding+0x23>
80104bde:	b8 01 00 00 00       	mov    $0x1,%eax
80104be3:	eb 05                	jmp    80104bea <holding+0x28>
80104be5:	b8 00 00 00 00       	mov    $0x0,%eax
}
80104bea:	5d                   	pop    %ebp
80104beb:	c3                   	ret    

80104bec <pushcli>:
// it takes two popcli to undo two pushcli.  Also, if interrupts
// are off, then pushcli, popcli leaves them off.

void
pushcli(void)
{
80104bec:	55                   	push   %ebp
80104bed:	89 e5                	mov    %esp,%ebp
80104bef:	83 ec 10             	sub    $0x10,%esp
  int eflags;
  
  eflags = readeflags();
80104bf2:	e8 4a fe ff ff       	call   80104a41 <readeflags>
80104bf7:	89 45 fc             	mov    %eax,-0x4(%ebp)
  cli();
80104bfa:	e8 52 fe ff ff       	call   80104a51 <cli>
  if(cpu->ncli++ == 0)
80104bff:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80104c06:	8b 82 ac 00 00 00    	mov    0xac(%edx),%eax
80104c0c:	8d 48 01             	lea    0x1(%eax),%ecx
80104c0f:	89 8a ac 00 00 00    	mov    %ecx,0xac(%edx)
80104c15:	85 c0                	test   %eax,%eax
80104c17:	75 15                	jne    80104c2e <pushcli+0x42>
    cpu->intena = eflags & FL_IF;
80104c19:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104c1f:	8b 55 fc             	mov    -0x4(%ebp),%edx
80104c22:	81 e2 00 02 00 00    	and    $0x200,%edx
80104c28:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
80104c2e:	c9                   	leave  
80104c2f:	c3                   	ret    

80104c30 <popcli>:

void
popcli(void)
{
80104c30:	55                   	push   %ebp
80104c31:	89 e5                	mov    %esp,%ebp
80104c33:	83 ec 18             	sub    $0x18,%esp
  if(readeflags()&FL_IF)
80104c36:	e8 06 fe ff ff       	call   80104a41 <readeflags>
80104c3b:	25 00 02 00 00       	and    $0x200,%eax
80104c40:	85 c0                	test   %eax,%eax
80104c42:	74 0c                	je     80104c50 <popcli+0x20>
    panic("popcli - interruptible");
80104c44:	c7 04 24 0d 84 10 80 	movl   $0x8010840d,(%esp)
80104c4b:	e8 ea b8 ff ff       	call   8010053a <panic>
  if(--cpu->ncli < 0)
80104c50:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104c56:	8b 90 ac 00 00 00    	mov    0xac(%eax),%edx
80104c5c:	83 ea 01             	sub    $0x1,%edx
80104c5f:	89 90 ac 00 00 00    	mov    %edx,0xac(%eax)
80104c65:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80104c6b:	85 c0                	test   %eax,%eax
80104c6d:	79 0c                	jns    80104c7b <popcli+0x4b>
    panic("popcli");
80104c6f:	c7 04 24 24 84 10 80 	movl   $0x80108424,(%esp)
80104c76:	e8 bf b8 ff ff       	call   8010053a <panic>
  if(cpu->ncli == 0 && cpu->intena)
80104c7b:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104c81:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
80104c87:	85 c0                	test   %eax,%eax
80104c89:	75 15                	jne    80104ca0 <popcli+0x70>
80104c8b:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104c91:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
80104c97:	85 c0                	test   %eax,%eax
80104c99:	74 05                	je     80104ca0 <popcli+0x70>
    sti();
80104c9b:	e8 b7 fd ff ff       	call   80104a57 <sti>
}
80104ca0:	c9                   	leave  
80104ca1:	c3                   	ret    

80104ca2 <stosb>:
               "cc");
}

static inline void
stosb(void *addr, int data, int cnt)
{
80104ca2:	55                   	push   %ebp
80104ca3:	89 e5                	mov    %esp,%ebp
80104ca5:	57                   	push   %edi
80104ca6:	53                   	push   %ebx
  asm volatile("cld; rep stosb" :
80104ca7:	8b 4d 08             	mov    0x8(%ebp),%ecx
80104caa:	8b 55 10             	mov    0x10(%ebp),%edx
80104cad:	8b 45 0c             	mov    0xc(%ebp),%eax
80104cb0:	89 cb                	mov    %ecx,%ebx
80104cb2:	89 df                	mov    %ebx,%edi
80104cb4:	89 d1                	mov    %edx,%ecx
80104cb6:	fc                   	cld    
80104cb7:	f3 aa                	rep stos %al,%es:(%edi)
80104cb9:	89 ca                	mov    %ecx,%edx
80104cbb:	89 fb                	mov    %edi,%ebx
80104cbd:	89 5d 08             	mov    %ebx,0x8(%ebp)
80104cc0:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
80104cc3:	5b                   	pop    %ebx
80104cc4:	5f                   	pop    %edi
80104cc5:	5d                   	pop    %ebp
80104cc6:	c3                   	ret    

80104cc7 <stosl>:

static inline void
stosl(void *addr, int data, int cnt)
{
80104cc7:	55                   	push   %ebp
80104cc8:	89 e5                	mov    %esp,%ebp
80104cca:	57                   	push   %edi
80104ccb:	53                   	push   %ebx
  asm volatile("cld; rep stosl" :
80104ccc:	8b 4d 08             	mov    0x8(%ebp),%ecx
80104ccf:	8b 55 10             	mov    0x10(%ebp),%edx
80104cd2:	8b 45 0c             	mov    0xc(%ebp),%eax
80104cd5:	89 cb                	mov    %ecx,%ebx
80104cd7:	89 df                	mov    %ebx,%edi
80104cd9:	89 d1                	mov    %edx,%ecx
80104cdb:	fc                   	cld    
80104cdc:	f3 ab                	rep stos %eax,%es:(%edi)
80104cde:	89 ca                	mov    %ecx,%edx
80104ce0:	89 fb                	mov    %edi,%ebx
80104ce2:	89 5d 08             	mov    %ebx,0x8(%ebp)
80104ce5:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
80104ce8:	5b                   	pop    %ebx
80104ce9:	5f                   	pop    %edi
80104cea:	5d                   	pop    %ebp
80104ceb:	c3                   	ret    

80104cec <memset>:
#include "types.h"
#include "x86.h"

void*
memset(void *dst, int c, uint n)
{
80104cec:	55                   	push   %ebp
80104ced:	89 e5                	mov    %esp,%ebp
80104cef:	83 ec 0c             	sub    $0xc,%esp
  if ((int)dst%4 == 0 && n%4 == 0){
80104cf2:	8b 45 08             	mov    0x8(%ebp),%eax
80104cf5:	83 e0 03             	and    $0x3,%eax
80104cf8:	85 c0                	test   %eax,%eax
80104cfa:	75 49                	jne    80104d45 <memset+0x59>
80104cfc:	8b 45 10             	mov    0x10(%ebp),%eax
80104cff:	83 e0 03             	and    $0x3,%eax
80104d02:	85 c0                	test   %eax,%eax
80104d04:	75 3f                	jne    80104d45 <memset+0x59>
    c &= 0xFF;
80104d06:	81 65 0c ff 00 00 00 	andl   $0xff,0xc(%ebp)
    stosl(dst, (c<<24)|(c<<16)|(c<<8)|c, n/4);
80104d0d:	8b 45 10             	mov    0x10(%ebp),%eax
80104d10:	c1 e8 02             	shr    $0x2,%eax
80104d13:	89 c2                	mov    %eax,%edx
80104d15:	8b 45 0c             	mov    0xc(%ebp),%eax
80104d18:	c1 e0 18             	shl    $0x18,%eax
80104d1b:	89 c1                	mov    %eax,%ecx
80104d1d:	8b 45 0c             	mov    0xc(%ebp),%eax
80104d20:	c1 e0 10             	shl    $0x10,%eax
80104d23:	09 c1                	or     %eax,%ecx
80104d25:	8b 45 0c             	mov    0xc(%ebp),%eax
80104d28:	c1 e0 08             	shl    $0x8,%eax
80104d2b:	09 c8                	or     %ecx,%eax
80104d2d:	0b 45 0c             	or     0xc(%ebp),%eax
80104d30:	89 54 24 08          	mov    %edx,0x8(%esp)
80104d34:	89 44 24 04          	mov    %eax,0x4(%esp)
80104d38:	8b 45 08             	mov    0x8(%ebp),%eax
80104d3b:	89 04 24             	mov    %eax,(%esp)
80104d3e:	e8 84 ff ff ff       	call   80104cc7 <stosl>
80104d43:	eb 19                	jmp    80104d5e <memset+0x72>
  } else
    stosb(dst, c, n);
80104d45:	8b 45 10             	mov    0x10(%ebp),%eax
80104d48:	89 44 24 08          	mov    %eax,0x8(%esp)
80104d4c:	8b 45 0c             	mov    0xc(%ebp),%eax
80104d4f:	89 44 24 04          	mov    %eax,0x4(%esp)
80104d53:	8b 45 08             	mov    0x8(%ebp),%eax
80104d56:	89 04 24             	mov    %eax,(%esp)
80104d59:	e8 44 ff ff ff       	call   80104ca2 <stosb>
  return dst;
80104d5e:	8b 45 08             	mov    0x8(%ebp),%eax
}
80104d61:	c9                   	leave  
80104d62:	c3                   	ret    

80104d63 <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
80104d63:	55                   	push   %ebp
80104d64:	89 e5                	mov    %esp,%ebp
80104d66:	83 ec 10             	sub    $0x10,%esp
  const uchar *s1, *s2;
  
  s1 = v1;
80104d69:	8b 45 08             	mov    0x8(%ebp),%eax
80104d6c:	89 45 fc             	mov    %eax,-0x4(%ebp)
  s2 = v2;
80104d6f:	8b 45 0c             	mov    0xc(%ebp),%eax
80104d72:	89 45 f8             	mov    %eax,-0x8(%ebp)
  while(n-- > 0){
80104d75:	eb 30                	jmp    80104da7 <memcmp+0x44>
    if(*s1 != *s2)
80104d77:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104d7a:	0f b6 10             	movzbl (%eax),%edx
80104d7d:	8b 45 f8             	mov    -0x8(%ebp),%eax
80104d80:	0f b6 00             	movzbl (%eax),%eax
80104d83:	38 c2                	cmp    %al,%dl
80104d85:	74 18                	je     80104d9f <memcmp+0x3c>
      return *s1 - *s2;
80104d87:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104d8a:	0f b6 00             	movzbl (%eax),%eax
80104d8d:	0f b6 d0             	movzbl %al,%edx
80104d90:	8b 45 f8             	mov    -0x8(%ebp),%eax
80104d93:	0f b6 00             	movzbl (%eax),%eax
80104d96:	0f b6 c0             	movzbl %al,%eax
80104d99:	29 c2                	sub    %eax,%edx
80104d9b:	89 d0                	mov    %edx,%eax
80104d9d:	eb 1a                	jmp    80104db9 <memcmp+0x56>
    s1++, s2++;
80104d9f:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80104da3:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
{
  const uchar *s1, *s2;
  
  s1 = v1;
  s2 = v2;
  while(n-- > 0){
80104da7:	8b 45 10             	mov    0x10(%ebp),%eax
80104daa:	8d 50 ff             	lea    -0x1(%eax),%edx
80104dad:	89 55 10             	mov    %edx,0x10(%ebp)
80104db0:	85 c0                	test   %eax,%eax
80104db2:	75 c3                	jne    80104d77 <memcmp+0x14>
    if(*s1 != *s2)
      return *s1 - *s2;
    s1++, s2++;
  }

  return 0;
80104db4:	b8 00 00 00 00       	mov    $0x0,%eax
}
80104db9:	c9                   	leave  
80104dba:	c3                   	ret    

80104dbb <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
80104dbb:	55                   	push   %ebp
80104dbc:	89 e5                	mov    %esp,%ebp
80104dbe:	83 ec 10             	sub    $0x10,%esp
  const char *s;
  char *d;

  s = src;
80104dc1:	8b 45 0c             	mov    0xc(%ebp),%eax
80104dc4:	89 45 fc             	mov    %eax,-0x4(%ebp)
  d = dst;
80104dc7:	8b 45 08             	mov    0x8(%ebp),%eax
80104dca:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(s < d && s + n > d){
80104dcd:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104dd0:	3b 45 f8             	cmp    -0x8(%ebp),%eax
80104dd3:	73 3d                	jae    80104e12 <memmove+0x57>
80104dd5:	8b 45 10             	mov    0x10(%ebp),%eax
80104dd8:	8b 55 fc             	mov    -0x4(%ebp),%edx
80104ddb:	01 d0                	add    %edx,%eax
80104ddd:	3b 45 f8             	cmp    -0x8(%ebp),%eax
80104de0:	76 30                	jbe    80104e12 <memmove+0x57>
    s += n;
80104de2:	8b 45 10             	mov    0x10(%ebp),%eax
80104de5:	01 45 fc             	add    %eax,-0x4(%ebp)
    d += n;
80104de8:	8b 45 10             	mov    0x10(%ebp),%eax
80104deb:	01 45 f8             	add    %eax,-0x8(%ebp)
    while(n-- > 0)
80104dee:	eb 13                	jmp    80104e03 <memmove+0x48>
      *--d = *--s;
80104df0:	83 6d f8 01          	subl   $0x1,-0x8(%ebp)
80104df4:	83 6d fc 01          	subl   $0x1,-0x4(%ebp)
80104df8:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104dfb:	0f b6 10             	movzbl (%eax),%edx
80104dfe:	8b 45 f8             	mov    -0x8(%ebp),%eax
80104e01:	88 10                	mov    %dl,(%eax)
  s = src;
  d = dst;
  if(s < d && s + n > d){
    s += n;
    d += n;
    while(n-- > 0)
80104e03:	8b 45 10             	mov    0x10(%ebp),%eax
80104e06:	8d 50 ff             	lea    -0x1(%eax),%edx
80104e09:	89 55 10             	mov    %edx,0x10(%ebp)
80104e0c:	85 c0                	test   %eax,%eax
80104e0e:	75 e0                	jne    80104df0 <memmove+0x35>
  const char *s;
  char *d;

  s = src;
  d = dst;
  if(s < d && s + n > d){
80104e10:	eb 26                	jmp    80104e38 <memmove+0x7d>
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
80104e12:	eb 17                	jmp    80104e2b <memmove+0x70>
      *d++ = *s++;
80104e14:	8b 45 f8             	mov    -0x8(%ebp),%eax
80104e17:	8d 50 01             	lea    0x1(%eax),%edx
80104e1a:	89 55 f8             	mov    %edx,-0x8(%ebp)
80104e1d:	8b 55 fc             	mov    -0x4(%ebp),%edx
80104e20:	8d 4a 01             	lea    0x1(%edx),%ecx
80104e23:	89 4d fc             	mov    %ecx,-0x4(%ebp)
80104e26:	0f b6 12             	movzbl (%edx),%edx
80104e29:	88 10                	mov    %dl,(%eax)
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
80104e2b:	8b 45 10             	mov    0x10(%ebp),%eax
80104e2e:	8d 50 ff             	lea    -0x1(%eax),%edx
80104e31:	89 55 10             	mov    %edx,0x10(%ebp)
80104e34:	85 c0                	test   %eax,%eax
80104e36:	75 dc                	jne    80104e14 <memmove+0x59>
      *d++ = *s++;

  return dst;
80104e38:	8b 45 08             	mov    0x8(%ebp),%eax
}
80104e3b:	c9                   	leave  
80104e3c:	c3                   	ret    

80104e3d <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
80104e3d:	55                   	push   %ebp
80104e3e:	89 e5                	mov    %esp,%ebp
80104e40:	83 ec 0c             	sub    $0xc,%esp
  return memmove(dst, src, n);
80104e43:	8b 45 10             	mov    0x10(%ebp),%eax
80104e46:	89 44 24 08          	mov    %eax,0x8(%esp)
80104e4a:	8b 45 0c             	mov    0xc(%ebp),%eax
80104e4d:	89 44 24 04          	mov    %eax,0x4(%esp)
80104e51:	8b 45 08             	mov    0x8(%ebp),%eax
80104e54:	89 04 24             	mov    %eax,(%esp)
80104e57:	e8 5f ff ff ff       	call   80104dbb <memmove>
}
80104e5c:	c9                   	leave  
80104e5d:	c3                   	ret    

80104e5e <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
80104e5e:	55                   	push   %ebp
80104e5f:	89 e5                	mov    %esp,%ebp
  while(n > 0 && *p && *p == *q)
80104e61:	eb 0c                	jmp    80104e6f <strncmp+0x11>
    n--, p++, q++;
80104e63:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80104e67:	83 45 08 01          	addl   $0x1,0x8(%ebp)
80104e6b:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
}

int
strncmp(const char *p, const char *q, uint n)
{
  while(n > 0 && *p && *p == *q)
80104e6f:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80104e73:	74 1a                	je     80104e8f <strncmp+0x31>
80104e75:	8b 45 08             	mov    0x8(%ebp),%eax
80104e78:	0f b6 00             	movzbl (%eax),%eax
80104e7b:	84 c0                	test   %al,%al
80104e7d:	74 10                	je     80104e8f <strncmp+0x31>
80104e7f:	8b 45 08             	mov    0x8(%ebp),%eax
80104e82:	0f b6 10             	movzbl (%eax),%edx
80104e85:	8b 45 0c             	mov    0xc(%ebp),%eax
80104e88:	0f b6 00             	movzbl (%eax),%eax
80104e8b:	38 c2                	cmp    %al,%dl
80104e8d:	74 d4                	je     80104e63 <strncmp+0x5>
    n--, p++, q++;
  if(n == 0)
80104e8f:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80104e93:	75 07                	jne    80104e9c <strncmp+0x3e>
    return 0;
80104e95:	b8 00 00 00 00       	mov    $0x0,%eax
80104e9a:	eb 16                	jmp    80104eb2 <strncmp+0x54>
  return (uchar)*p - (uchar)*q;
80104e9c:	8b 45 08             	mov    0x8(%ebp),%eax
80104e9f:	0f b6 00             	movzbl (%eax),%eax
80104ea2:	0f b6 d0             	movzbl %al,%edx
80104ea5:	8b 45 0c             	mov    0xc(%ebp),%eax
80104ea8:	0f b6 00             	movzbl (%eax),%eax
80104eab:	0f b6 c0             	movzbl %al,%eax
80104eae:	29 c2                	sub    %eax,%edx
80104eb0:	89 d0                	mov    %edx,%eax
}
80104eb2:	5d                   	pop    %ebp
80104eb3:	c3                   	ret    

80104eb4 <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
80104eb4:	55                   	push   %ebp
80104eb5:	89 e5                	mov    %esp,%ebp
80104eb7:	83 ec 10             	sub    $0x10,%esp
  char *os;
  
  os = s;
80104eba:	8b 45 08             	mov    0x8(%ebp),%eax
80104ebd:	89 45 fc             	mov    %eax,-0x4(%ebp)
  while(n-- > 0 && (*s++ = *t++) != 0)
80104ec0:	90                   	nop
80104ec1:	8b 45 10             	mov    0x10(%ebp),%eax
80104ec4:	8d 50 ff             	lea    -0x1(%eax),%edx
80104ec7:	89 55 10             	mov    %edx,0x10(%ebp)
80104eca:	85 c0                	test   %eax,%eax
80104ecc:	7e 1e                	jle    80104eec <strncpy+0x38>
80104ece:	8b 45 08             	mov    0x8(%ebp),%eax
80104ed1:	8d 50 01             	lea    0x1(%eax),%edx
80104ed4:	89 55 08             	mov    %edx,0x8(%ebp)
80104ed7:	8b 55 0c             	mov    0xc(%ebp),%edx
80104eda:	8d 4a 01             	lea    0x1(%edx),%ecx
80104edd:	89 4d 0c             	mov    %ecx,0xc(%ebp)
80104ee0:	0f b6 12             	movzbl (%edx),%edx
80104ee3:	88 10                	mov    %dl,(%eax)
80104ee5:	0f b6 00             	movzbl (%eax),%eax
80104ee8:	84 c0                	test   %al,%al
80104eea:	75 d5                	jne    80104ec1 <strncpy+0xd>
    ;
  while(n-- > 0)
80104eec:	eb 0c                	jmp    80104efa <strncpy+0x46>
    *s++ = 0;
80104eee:	8b 45 08             	mov    0x8(%ebp),%eax
80104ef1:	8d 50 01             	lea    0x1(%eax),%edx
80104ef4:	89 55 08             	mov    %edx,0x8(%ebp)
80104ef7:	c6 00 00             	movb   $0x0,(%eax)
  char *os;
  
  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
    ;
  while(n-- > 0)
80104efa:	8b 45 10             	mov    0x10(%ebp),%eax
80104efd:	8d 50 ff             	lea    -0x1(%eax),%edx
80104f00:	89 55 10             	mov    %edx,0x10(%ebp)
80104f03:	85 c0                	test   %eax,%eax
80104f05:	7f e7                	jg     80104eee <strncpy+0x3a>
    *s++ = 0;
  return os;
80104f07:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80104f0a:	c9                   	leave  
80104f0b:	c3                   	ret    

80104f0c <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
80104f0c:	55                   	push   %ebp
80104f0d:	89 e5                	mov    %esp,%ebp
80104f0f:	83 ec 10             	sub    $0x10,%esp
  char *os;
  
  os = s;
80104f12:	8b 45 08             	mov    0x8(%ebp),%eax
80104f15:	89 45 fc             	mov    %eax,-0x4(%ebp)
  if(n <= 0)
80104f18:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80104f1c:	7f 05                	jg     80104f23 <safestrcpy+0x17>
    return os;
80104f1e:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104f21:	eb 31                	jmp    80104f54 <safestrcpy+0x48>
  while(--n > 0 && (*s++ = *t++) != 0)
80104f23:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80104f27:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80104f2b:	7e 1e                	jle    80104f4b <safestrcpy+0x3f>
80104f2d:	8b 45 08             	mov    0x8(%ebp),%eax
80104f30:	8d 50 01             	lea    0x1(%eax),%edx
80104f33:	89 55 08             	mov    %edx,0x8(%ebp)
80104f36:	8b 55 0c             	mov    0xc(%ebp),%edx
80104f39:	8d 4a 01             	lea    0x1(%edx),%ecx
80104f3c:	89 4d 0c             	mov    %ecx,0xc(%ebp)
80104f3f:	0f b6 12             	movzbl (%edx),%edx
80104f42:	88 10                	mov    %dl,(%eax)
80104f44:	0f b6 00             	movzbl (%eax),%eax
80104f47:	84 c0                	test   %al,%al
80104f49:	75 d8                	jne    80104f23 <safestrcpy+0x17>
    ;
  *s = 0;
80104f4b:	8b 45 08             	mov    0x8(%ebp),%eax
80104f4e:	c6 00 00             	movb   $0x0,(%eax)
  return os;
80104f51:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80104f54:	c9                   	leave  
80104f55:	c3                   	ret    

80104f56 <strlen>:

int
strlen(const char *s)
{
80104f56:	55                   	push   %ebp
80104f57:	89 e5                	mov    %esp,%ebp
80104f59:	83 ec 10             	sub    $0x10,%esp
  int n;

  for(n = 0; s[n]; n++)
80104f5c:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80104f63:	eb 04                	jmp    80104f69 <strlen+0x13>
80104f65:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80104f69:	8b 55 fc             	mov    -0x4(%ebp),%edx
80104f6c:	8b 45 08             	mov    0x8(%ebp),%eax
80104f6f:	01 d0                	add    %edx,%eax
80104f71:	0f b6 00             	movzbl (%eax),%eax
80104f74:	84 c0                	test   %al,%al
80104f76:	75 ed                	jne    80104f65 <strlen+0xf>
    ;
  return n;
80104f78:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80104f7b:	c9                   	leave  
80104f7c:	c3                   	ret    

80104f7d <swtch>:
# Save current register context in old
# and then load register context from new.

.globl swtch
swtch:
  movl 4(%esp), %eax
80104f7d:	8b 44 24 04          	mov    0x4(%esp),%eax
  movl 8(%esp), %edx
80104f81:	8b 54 24 08          	mov    0x8(%esp),%edx

  # Save old callee-save registers
  pushl %ebp
80104f85:	55                   	push   %ebp
  pushl %ebx
80104f86:	53                   	push   %ebx
  pushl %esi
80104f87:	56                   	push   %esi
  pushl %edi
80104f88:	57                   	push   %edi

  # Switch stacks
  movl %esp, (%eax)
80104f89:	89 20                	mov    %esp,(%eax)
  movl %edx, %esp
80104f8b:	89 d4                	mov    %edx,%esp

  # Load new callee-save registers
  popl %edi
80104f8d:	5f                   	pop    %edi
  popl %esi
80104f8e:	5e                   	pop    %esi
  popl %ebx
80104f8f:	5b                   	pop    %ebx
  popl %ebp
80104f90:	5d                   	pop    %ebp
  ret
80104f91:	c3                   	ret    

80104f92 <fetchint>:
// to a saved program counter, and then the first argument.

// Fetch the int at addr from the current process.
int
fetchint(uint addr, int *ip)
{
80104f92:	55                   	push   %ebp
80104f93:	89 e5                	mov    %esp,%ebp
  if(addr >= proc->sz || addr+4 > proc->sz)
80104f95:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104f9b:	8b 00                	mov    (%eax),%eax
80104f9d:	3b 45 08             	cmp    0x8(%ebp),%eax
80104fa0:	76 12                	jbe    80104fb4 <fetchint+0x22>
80104fa2:	8b 45 08             	mov    0x8(%ebp),%eax
80104fa5:	8d 50 04             	lea    0x4(%eax),%edx
80104fa8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104fae:	8b 00                	mov    (%eax),%eax
80104fb0:	39 c2                	cmp    %eax,%edx
80104fb2:	76 07                	jbe    80104fbb <fetchint+0x29>
    return -1;
80104fb4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104fb9:	eb 0f                	jmp    80104fca <fetchint+0x38>
  *ip = *(int*)(addr);
80104fbb:	8b 45 08             	mov    0x8(%ebp),%eax
80104fbe:	8b 10                	mov    (%eax),%edx
80104fc0:	8b 45 0c             	mov    0xc(%ebp),%eax
80104fc3:	89 10                	mov    %edx,(%eax)
  return 0;
80104fc5:	b8 00 00 00 00       	mov    $0x0,%eax
}
80104fca:	5d                   	pop    %ebp
80104fcb:	c3                   	ret    

80104fcc <fetchstr>:
// Fetch the nul-terminated string at addr from the current process.
// Doesn't actually copy the string - just sets *pp to point at it.
// Returns length of string, not including nul.
int
fetchstr(uint addr, char **pp)
{
80104fcc:	55                   	push   %ebp
80104fcd:	89 e5                	mov    %esp,%ebp
80104fcf:	83 ec 10             	sub    $0x10,%esp
  char *s, *ep;

  if(addr >= proc->sz)
80104fd2:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104fd8:	8b 00                	mov    (%eax),%eax
80104fda:	3b 45 08             	cmp    0x8(%ebp),%eax
80104fdd:	77 07                	ja     80104fe6 <fetchstr+0x1a>
    return -1;
80104fdf:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104fe4:	eb 46                	jmp    8010502c <fetchstr+0x60>
  *pp = (char*)addr;
80104fe6:	8b 55 08             	mov    0x8(%ebp),%edx
80104fe9:	8b 45 0c             	mov    0xc(%ebp),%eax
80104fec:	89 10                	mov    %edx,(%eax)
  ep = (char*)proc->sz;
80104fee:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104ff4:	8b 00                	mov    (%eax),%eax
80104ff6:	89 45 f8             	mov    %eax,-0x8(%ebp)
  for(s = *pp; s < ep; s++)
80104ff9:	8b 45 0c             	mov    0xc(%ebp),%eax
80104ffc:	8b 00                	mov    (%eax),%eax
80104ffe:	89 45 fc             	mov    %eax,-0x4(%ebp)
80105001:	eb 1c                	jmp    8010501f <fetchstr+0x53>
    if(*s == 0)
80105003:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105006:	0f b6 00             	movzbl (%eax),%eax
80105009:	84 c0                	test   %al,%al
8010500b:	75 0e                	jne    8010501b <fetchstr+0x4f>
      return s - *pp;
8010500d:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105010:	8b 45 0c             	mov    0xc(%ebp),%eax
80105013:	8b 00                	mov    (%eax),%eax
80105015:	29 c2                	sub    %eax,%edx
80105017:	89 d0                	mov    %edx,%eax
80105019:	eb 11                	jmp    8010502c <fetchstr+0x60>

  if(addr >= proc->sz)
    return -1;
  *pp = (char*)addr;
  ep = (char*)proc->sz;
  for(s = *pp; s < ep; s++)
8010501b:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
8010501f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105022:	3b 45 f8             	cmp    -0x8(%ebp),%eax
80105025:	72 dc                	jb     80105003 <fetchstr+0x37>
    if(*s == 0)
      return s - *pp;
  return -1;
80105027:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010502c:	c9                   	leave  
8010502d:	c3                   	ret    

8010502e <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
8010502e:	55                   	push   %ebp
8010502f:	89 e5                	mov    %esp,%ebp
80105031:	83 ec 08             	sub    $0x8,%esp
  return fetchint(proc->tf->esp + 4 + 4*n, ip);
80105034:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010503a:	8b 40 18             	mov    0x18(%eax),%eax
8010503d:	8b 50 44             	mov    0x44(%eax),%edx
80105040:	8b 45 08             	mov    0x8(%ebp),%eax
80105043:	c1 e0 02             	shl    $0x2,%eax
80105046:	01 d0                	add    %edx,%eax
80105048:	8d 50 04             	lea    0x4(%eax),%edx
8010504b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010504e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105052:	89 14 24             	mov    %edx,(%esp)
80105055:	e8 38 ff ff ff       	call   80104f92 <fetchint>
}
8010505a:	c9                   	leave  
8010505b:	c3                   	ret    

8010505c <argptr>:
// Fetch the nth word-sized system call argument as a pointer
// to a block of memory of size n bytes.  Check that the pointer
// lies within the process address space.
int
argptr(int n, char **pp, int size)
{
8010505c:	55                   	push   %ebp
8010505d:	89 e5                	mov    %esp,%ebp
8010505f:	83 ec 18             	sub    $0x18,%esp
  int i;
  
  if(argint(n, &i) < 0)
80105062:	8d 45 fc             	lea    -0x4(%ebp),%eax
80105065:	89 44 24 04          	mov    %eax,0x4(%esp)
80105069:	8b 45 08             	mov    0x8(%ebp),%eax
8010506c:	89 04 24             	mov    %eax,(%esp)
8010506f:	e8 ba ff ff ff       	call   8010502e <argint>
80105074:	85 c0                	test   %eax,%eax
80105076:	79 07                	jns    8010507f <argptr+0x23>
    return -1;
80105078:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010507d:	eb 3d                	jmp    801050bc <argptr+0x60>
  if((uint)i >= proc->sz || (uint)i+size > proc->sz)
8010507f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105082:	89 c2                	mov    %eax,%edx
80105084:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010508a:	8b 00                	mov    (%eax),%eax
8010508c:	39 c2                	cmp    %eax,%edx
8010508e:	73 16                	jae    801050a6 <argptr+0x4a>
80105090:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105093:	89 c2                	mov    %eax,%edx
80105095:	8b 45 10             	mov    0x10(%ebp),%eax
80105098:	01 c2                	add    %eax,%edx
8010509a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801050a0:	8b 00                	mov    (%eax),%eax
801050a2:	39 c2                	cmp    %eax,%edx
801050a4:	76 07                	jbe    801050ad <argptr+0x51>
    return -1;
801050a6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801050ab:	eb 0f                	jmp    801050bc <argptr+0x60>
  *pp = (char*)i;
801050ad:	8b 45 fc             	mov    -0x4(%ebp),%eax
801050b0:	89 c2                	mov    %eax,%edx
801050b2:	8b 45 0c             	mov    0xc(%ebp),%eax
801050b5:	89 10                	mov    %edx,(%eax)
  return 0;
801050b7:	b8 00 00 00 00       	mov    $0x0,%eax
}
801050bc:	c9                   	leave  
801050bd:	c3                   	ret    

801050be <argstr>:
// Check that the pointer is valid and the string is nul-terminated.
// (There is no shared writable memory, so the string can't change
// between this check and being used by the kernel.)
int
argstr(int n, char **pp)
{
801050be:	55                   	push   %ebp
801050bf:	89 e5                	mov    %esp,%ebp
801050c1:	83 ec 18             	sub    $0x18,%esp
  int addr;
  if(argint(n, &addr) < 0)
801050c4:	8d 45 fc             	lea    -0x4(%ebp),%eax
801050c7:	89 44 24 04          	mov    %eax,0x4(%esp)
801050cb:	8b 45 08             	mov    0x8(%ebp),%eax
801050ce:	89 04 24             	mov    %eax,(%esp)
801050d1:	e8 58 ff ff ff       	call   8010502e <argint>
801050d6:	85 c0                	test   %eax,%eax
801050d8:	79 07                	jns    801050e1 <argstr+0x23>
    return -1;
801050da:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801050df:	eb 12                	jmp    801050f3 <argstr+0x35>
  return fetchstr(addr, pp);
801050e1:	8b 45 fc             	mov    -0x4(%ebp),%eax
801050e4:	8b 55 0c             	mov    0xc(%ebp),%edx
801050e7:	89 54 24 04          	mov    %edx,0x4(%esp)
801050eb:	89 04 24             	mov    %eax,(%esp)
801050ee:	e8 d9 fe ff ff       	call   80104fcc <fetchstr>
}
801050f3:	c9                   	leave  
801050f4:	c3                   	ret    

801050f5 <syscall>:
[SYS_close]   sys_close,
};

void
syscall(void)
{
801050f5:	55                   	push   %ebp
801050f6:	89 e5                	mov    %esp,%ebp
801050f8:	53                   	push   %ebx
801050f9:	83 ec 24             	sub    $0x24,%esp
  int num;

  num = proc->tf->eax;
801050fc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105102:	8b 40 18             	mov    0x18(%eax),%eax
80105105:	8b 40 1c             	mov    0x1c(%eax),%eax
80105108:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
8010510b:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010510f:	7e 30                	jle    80105141 <syscall+0x4c>
80105111:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105114:	83 f8 15             	cmp    $0x15,%eax
80105117:	77 28                	ja     80105141 <syscall+0x4c>
80105119:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010511c:	8b 04 85 40 b0 10 80 	mov    -0x7fef4fc0(,%eax,4),%eax
80105123:	85 c0                	test   %eax,%eax
80105125:	74 1a                	je     80105141 <syscall+0x4c>
    proc->tf->eax = syscalls[num]();
80105127:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010512d:	8b 58 18             	mov    0x18(%eax),%ebx
80105130:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105133:	8b 04 85 40 b0 10 80 	mov    -0x7fef4fc0(,%eax,4),%eax
8010513a:	ff d0                	call   *%eax
8010513c:	89 43 1c             	mov    %eax,0x1c(%ebx)
8010513f:	eb 3d                	jmp    8010517e <syscall+0x89>
  } else {
    cprintf("%d %s: unknown sys call %d\n",
            proc->pid, proc->name, num);
80105141:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105147:	8d 48 6c             	lea    0x6c(%eax),%ecx
8010514a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax

  num = proc->tf->eax;
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    proc->tf->eax = syscalls[num]();
  } else {
    cprintf("%d %s: unknown sys call %d\n",
80105150:	8b 40 10             	mov    0x10(%eax),%eax
80105153:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105156:	89 54 24 0c          	mov    %edx,0xc(%esp)
8010515a:	89 4c 24 08          	mov    %ecx,0x8(%esp)
8010515e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105162:	c7 04 24 2b 84 10 80 	movl   $0x8010842b,(%esp)
80105169:	e8 32 b2 ff ff       	call   801003a0 <cprintf>
            proc->pid, proc->name, num);
    proc->tf->eax = -1;
8010516e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105174:	8b 40 18             	mov    0x18(%eax),%eax
80105177:	c7 40 1c ff ff ff ff 	movl   $0xffffffff,0x1c(%eax)
  }
}
8010517e:	83 c4 24             	add    $0x24,%esp
80105181:	5b                   	pop    %ebx
80105182:	5d                   	pop    %ebp
80105183:	c3                   	ret    

80105184 <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
80105184:	55                   	push   %ebp
80105185:	89 e5                	mov    %esp,%ebp
80105187:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;

  if(argint(n, &fd) < 0)
8010518a:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010518d:	89 44 24 04          	mov    %eax,0x4(%esp)
80105191:	8b 45 08             	mov    0x8(%ebp),%eax
80105194:	89 04 24             	mov    %eax,(%esp)
80105197:	e8 92 fe ff ff       	call   8010502e <argint>
8010519c:	85 c0                	test   %eax,%eax
8010519e:	79 07                	jns    801051a7 <argfd+0x23>
    return -1;
801051a0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801051a5:	eb 50                	jmp    801051f7 <argfd+0x73>
  if(fd < 0 || fd >= NOFILE || (f=proc->ofile[fd]) == 0)
801051a7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801051aa:	85 c0                	test   %eax,%eax
801051ac:	78 21                	js     801051cf <argfd+0x4b>
801051ae:	8b 45 f0             	mov    -0x10(%ebp),%eax
801051b1:	83 f8 0f             	cmp    $0xf,%eax
801051b4:	7f 19                	jg     801051cf <argfd+0x4b>
801051b6:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801051bc:	8b 55 f0             	mov    -0x10(%ebp),%edx
801051bf:	83 c2 08             	add    $0x8,%edx
801051c2:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801051c6:	89 45 f4             	mov    %eax,-0xc(%ebp)
801051c9:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801051cd:	75 07                	jne    801051d6 <argfd+0x52>
    return -1;
801051cf:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801051d4:	eb 21                	jmp    801051f7 <argfd+0x73>
  if(pfd)
801051d6:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
801051da:	74 08                	je     801051e4 <argfd+0x60>
    *pfd = fd;
801051dc:	8b 55 f0             	mov    -0x10(%ebp),%edx
801051df:	8b 45 0c             	mov    0xc(%ebp),%eax
801051e2:	89 10                	mov    %edx,(%eax)
  if(pf)
801051e4:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801051e8:	74 08                	je     801051f2 <argfd+0x6e>
    *pf = f;
801051ea:	8b 45 10             	mov    0x10(%ebp),%eax
801051ed:	8b 55 f4             	mov    -0xc(%ebp),%edx
801051f0:	89 10                	mov    %edx,(%eax)
  return 0;
801051f2:	b8 00 00 00 00       	mov    $0x0,%eax
}
801051f7:	c9                   	leave  
801051f8:	c3                   	ret    

801051f9 <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
801051f9:	55                   	push   %ebp
801051fa:	89 e5                	mov    %esp,%ebp
801051fc:	83 ec 10             	sub    $0x10,%esp
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
801051ff:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80105206:	eb 30                	jmp    80105238 <fdalloc+0x3f>
    if(proc->ofile[fd] == 0){
80105208:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010520e:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105211:	83 c2 08             	add    $0x8,%edx
80105214:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80105218:	85 c0                	test   %eax,%eax
8010521a:	75 18                	jne    80105234 <fdalloc+0x3b>
      proc->ofile[fd] = f;
8010521c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105222:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105225:	8d 4a 08             	lea    0x8(%edx),%ecx
80105228:	8b 55 08             	mov    0x8(%ebp),%edx
8010522b:	89 54 88 08          	mov    %edx,0x8(%eax,%ecx,4)
      return fd;
8010522f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105232:	eb 0f                	jmp    80105243 <fdalloc+0x4a>
static int
fdalloc(struct file *f)
{
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
80105234:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105238:	83 7d fc 0f          	cmpl   $0xf,-0x4(%ebp)
8010523c:	7e ca                	jle    80105208 <fdalloc+0xf>
    if(proc->ofile[fd] == 0){
      proc->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
8010523e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105243:	c9                   	leave  
80105244:	c3                   	ret    

80105245 <sys_dup>:

int
sys_dup(void)
{
80105245:	55                   	push   %ebp
80105246:	89 e5                	mov    %esp,%ebp
80105248:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int fd;
  
  if(argfd(0, 0, &f) < 0)
8010524b:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010524e:	89 44 24 08          	mov    %eax,0x8(%esp)
80105252:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105259:	00 
8010525a:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105261:	e8 1e ff ff ff       	call   80105184 <argfd>
80105266:	85 c0                	test   %eax,%eax
80105268:	79 07                	jns    80105271 <sys_dup+0x2c>
    return -1;
8010526a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010526f:	eb 29                	jmp    8010529a <sys_dup+0x55>
  if((fd=fdalloc(f)) < 0)
80105271:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105274:	89 04 24             	mov    %eax,(%esp)
80105277:	e8 7d ff ff ff       	call   801051f9 <fdalloc>
8010527c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010527f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105283:	79 07                	jns    8010528c <sys_dup+0x47>
    return -1;
80105285:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010528a:	eb 0e                	jmp    8010529a <sys_dup+0x55>
  filedup(f);
8010528c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010528f:	89 04 24             	mov    %eax,(%esp)
80105292:	e8 ef bc ff ff       	call   80100f86 <filedup>
  return fd;
80105297:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
8010529a:	c9                   	leave  
8010529b:	c3                   	ret    

8010529c <sys_read>:

int
sys_read(void)
{
8010529c:	55                   	push   %ebp
8010529d:	89 e5                	mov    %esp,%ebp
8010529f:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
801052a2:	8d 45 f4             	lea    -0xc(%ebp),%eax
801052a5:	89 44 24 08          	mov    %eax,0x8(%esp)
801052a9:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801052b0:	00 
801052b1:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801052b8:	e8 c7 fe ff ff       	call   80105184 <argfd>
801052bd:	85 c0                	test   %eax,%eax
801052bf:	78 35                	js     801052f6 <sys_read+0x5a>
801052c1:	8d 45 f0             	lea    -0x10(%ebp),%eax
801052c4:	89 44 24 04          	mov    %eax,0x4(%esp)
801052c8:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
801052cf:	e8 5a fd ff ff       	call   8010502e <argint>
801052d4:	85 c0                	test   %eax,%eax
801052d6:	78 1e                	js     801052f6 <sys_read+0x5a>
801052d8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801052db:	89 44 24 08          	mov    %eax,0x8(%esp)
801052df:	8d 45 ec             	lea    -0x14(%ebp),%eax
801052e2:	89 44 24 04          	mov    %eax,0x4(%esp)
801052e6:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801052ed:	e8 6a fd ff ff       	call   8010505c <argptr>
801052f2:	85 c0                	test   %eax,%eax
801052f4:	79 07                	jns    801052fd <sys_read+0x61>
    return -1;
801052f6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801052fb:	eb 19                	jmp    80105316 <sys_read+0x7a>
  return fileread(f, p, n);
801052fd:	8b 4d f0             	mov    -0x10(%ebp),%ecx
80105300:	8b 55 ec             	mov    -0x14(%ebp),%edx
80105303:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105306:	89 4c 24 08          	mov    %ecx,0x8(%esp)
8010530a:	89 54 24 04          	mov    %edx,0x4(%esp)
8010530e:	89 04 24             	mov    %eax,(%esp)
80105311:	e8 dd bd ff ff       	call   801010f3 <fileread>
}
80105316:	c9                   	leave  
80105317:	c3                   	ret    

80105318 <sys_write>:

int
sys_write(void)
{
80105318:	55                   	push   %ebp
80105319:	89 e5                	mov    %esp,%ebp
8010531b:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
8010531e:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105321:	89 44 24 08          	mov    %eax,0x8(%esp)
80105325:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010532c:	00 
8010532d:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105334:	e8 4b fe ff ff       	call   80105184 <argfd>
80105339:	85 c0                	test   %eax,%eax
8010533b:	78 35                	js     80105372 <sys_write+0x5a>
8010533d:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105340:	89 44 24 04          	mov    %eax,0x4(%esp)
80105344:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
8010534b:	e8 de fc ff ff       	call   8010502e <argint>
80105350:	85 c0                	test   %eax,%eax
80105352:	78 1e                	js     80105372 <sys_write+0x5a>
80105354:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105357:	89 44 24 08          	mov    %eax,0x8(%esp)
8010535b:	8d 45 ec             	lea    -0x14(%ebp),%eax
8010535e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105362:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105369:	e8 ee fc ff ff       	call   8010505c <argptr>
8010536e:	85 c0                	test   %eax,%eax
80105370:	79 07                	jns    80105379 <sys_write+0x61>
    return -1;
80105372:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105377:	eb 19                	jmp    80105392 <sys_write+0x7a>
  return filewrite(f, p, n);
80105379:	8b 4d f0             	mov    -0x10(%ebp),%ecx
8010537c:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010537f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105382:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80105386:	89 54 24 04          	mov    %edx,0x4(%esp)
8010538a:	89 04 24             	mov    %eax,(%esp)
8010538d:	e8 1d be ff ff       	call   801011af <filewrite>
}
80105392:	c9                   	leave  
80105393:	c3                   	ret    

80105394 <sys_close>:

int
sys_close(void)
{
80105394:	55                   	push   %ebp
80105395:	89 e5                	mov    %esp,%ebp
80105397:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;
  
  if(argfd(0, &fd, &f) < 0)
8010539a:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010539d:	89 44 24 08          	mov    %eax,0x8(%esp)
801053a1:	8d 45 f4             	lea    -0xc(%ebp),%eax
801053a4:	89 44 24 04          	mov    %eax,0x4(%esp)
801053a8:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801053af:	e8 d0 fd ff ff       	call   80105184 <argfd>
801053b4:	85 c0                	test   %eax,%eax
801053b6:	79 07                	jns    801053bf <sys_close+0x2b>
    return -1;
801053b8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801053bd:	eb 24                	jmp    801053e3 <sys_close+0x4f>
  proc->ofile[fd] = 0;
801053bf:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801053c5:	8b 55 f4             	mov    -0xc(%ebp),%edx
801053c8:	83 c2 08             	add    $0x8,%edx
801053cb:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
801053d2:	00 
  fileclose(f);
801053d3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801053d6:	89 04 24             	mov    %eax,(%esp)
801053d9:	e8 f0 bb ff ff       	call   80100fce <fileclose>
  return 0;
801053de:	b8 00 00 00 00       	mov    $0x0,%eax
}
801053e3:	c9                   	leave  
801053e4:	c3                   	ret    

801053e5 <sys_fstat>:

int
sys_fstat(void)
{
801053e5:	55                   	push   %ebp
801053e6:	89 e5                	mov    %esp,%ebp
801053e8:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  struct stat *st;
  
  if(argfd(0, 0, &f) < 0 || argptr(1, (void*)&st, sizeof(*st)) < 0)
801053eb:	8d 45 f4             	lea    -0xc(%ebp),%eax
801053ee:	89 44 24 08          	mov    %eax,0x8(%esp)
801053f2:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801053f9:	00 
801053fa:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105401:	e8 7e fd ff ff       	call   80105184 <argfd>
80105406:	85 c0                	test   %eax,%eax
80105408:	78 1f                	js     80105429 <sys_fstat+0x44>
8010540a:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
80105411:	00 
80105412:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105415:	89 44 24 04          	mov    %eax,0x4(%esp)
80105419:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105420:	e8 37 fc ff ff       	call   8010505c <argptr>
80105425:	85 c0                	test   %eax,%eax
80105427:	79 07                	jns    80105430 <sys_fstat+0x4b>
    return -1;
80105429:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010542e:	eb 12                	jmp    80105442 <sys_fstat+0x5d>
  return filestat(f, st);
80105430:	8b 55 f0             	mov    -0x10(%ebp),%edx
80105433:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105436:	89 54 24 04          	mov    %edx,0x4(%esp)
8010543a:	89 04 24             	mov    %eax,(%esp)
8010543d:	e8 62 bc ff ff       	call   801010a4 <filestat>
}
80105442:	c9                   	leave  
80105443:	c3                   	ret    

80105444 <sys_link>:

// Create the path new as a link to the same inode as old.
int
sys_link(void)
{
80105444:	55                   	push   %ebp
80105445:	89 e5                	mov    %esp,%ebp
80105447:	83 ec 38             	sub    $0x38,%esp
  char name[DIRSIZ], *new, *old;
  struct inode *dp, *ip;

  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
8010544a:	8d 45 d8             	lea    -0x28(%ebp),%eax
8010544d:	89 44 24 04          	mov    %eax,0x4(%esp)
80105451:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105458:	e8 61 fc ff ff       	call   801050be <argstr>
8010545d:	85 c0                	test   %eax,%eax
8010545f:	78 17                	js     80105478 <sys_link+0x34>
80105461:	8d 45 dc             	lea    -0x24(%ebp),%eax
80105464:	89 44 24 04          	mov    %eax,0x4(%esp)
80105468:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010546f:	e8 4a fc ff ff       	call   801050be <argstr>
80105474:	85 c0                	test   %eax,%eax
80105476:	79 0a                	jns    80105482 <sys_link+0x3e>
    return -1;
80105478:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010547d:	e9 42 01 00 00       	jmp    801055c4 <sys_link+0x180>

  begin_trans();
80105482:	e8 5e dd ff ff       	call   801031e5 <begin_trans>
  if((ip = namei(old)) == 0){
80105487:	8b 45 d8             	mov    -0x28(%ebp),%eax
8010548a:	89 04 24             	mov    %eax,(%esp)
8010548d:	e8 74 cf ff ff       	call   80102406 <namei>
80105492:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105495:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105499:	75 0f                	jne    801054aa <sys_link+0x66>
    commit_trans();
8010549b:	e8 8e dd ff ff       	call   8010322e <commit_trans>
    return -1;
801054a0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801054a5:	e9 1a 01 00 00       	jmp    801055c4 <sys_link+0x180>
  }

  ilock(ip);
801054aa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801054ad:	89 04 24             	mov    %eax,(%esp)
801054b0:	e8 a6 c3 ff ff       	call   8010185b <ilock>
  if(ip->type == T_DIR){
801054b5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801054b8:	0f b7 40 10          	movzwl 0x10(%eax),%eax
801054bc:	66 83 f8 01          	cmp    $0x1,%ax
801054c0:	75 1a                	jne    801054dc <sys_link+0x98>
    iunlockput(ip);
801054c2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801054c5:	89 04 24             	mov    %eax,(%esp)
801054c8:	e8 12 c6 ff ff       	call   80101adf <iunlockput>
    commit_trans();
801054cd:	e8 5c dd ff ff       	call   8010322e <commit_trans>
    return -1;
801054d2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801054d7:	e9 e8 00 00 00       	jmp    801055c4 <sys_link+0x180>
  }

  ip->nlink++;
801054dc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801054df:	0f b7 40 16          	movzwl 0x16(%eax),%eax
801054e3:	8d 50 01             	lea    0x1(%eax),%edx
801054e6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801054e9:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
801054ed:	8b 45 f4             	mov    -0xc(%ebp),%eax
801054f0:	89 04 24             	mov    %eax,(%esp)
801054f3:	e8 a7 c1 ff ff       	call   8010169f <iupdate>
  iunlock(ip);
801054f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801054fb:	89 04 24             	mov    %eax,(%esp)
801054fe:	e8 a6 c4 ff ff       	call   801019a9 <iunlock>

  if((dp = nameiparent(new, name)) == 0)
80105503:	8b 45 dc             	mov    -0x24(%ebp),%eax
80105506:	8d 55 e2             	lea    -0x1e(%ebp),%edx
80105509:	89 54 24 04          	mov    %edx,0x4(%esp)
8010550d:	89 04 24             	mov    %eax,(%esp)
80105510:	e8 13 cf ff ff       	call   80102428 <nameiparent>
80105515:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105518:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010551c:	75 02                	jne    80105520 <sys_link+0xdc>
    goto bad;
8010551e:	eb 68                	jmp    80105588 <sys_link+0x144>
  ilock(dp);
80105520:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105523:	89 04 24             	mov    %eax,(%esp)
80105526:	e8 30 c3 ff ff       	call   8010185b <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
8010552b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010552e:	8b 10                	mov    (%eax),%edx
80105530:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105533:	8b 00                	mov    (%eax),%eax
80105535:	39 c2                	cmp    %eax,%edx
80105537:	75 20                	jne    80105559 <sys_link+0x115>
80105539:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010553c:	8b 40 04             	mov    0x4(%eax),%eax
8010553f:	89 44 24 08          	mov    %eax,0x8(%esp)
80105543:	8d 45 e2             	lea    -0x1e(%ebp),%eax
80105546:	89 44 24 04          	mov    %eax,0x4(%esp)
8010554a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010554d:	89 04 24             	mov    %eax,(%esp)
80105550:	e8 f1 cb ff ff       	call   80102146 <dirlink>
80105555:	85 c0                	test   %eax,%eax
80105557:	79 0d                	jns    80105566 <sys_link+0x122>
    iunlockput(dp);
80105559:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010555c:	89 04 24             	mov    %eax,(%esp)
8010555f:	e8 7b c5 ff ff       	call   80101adf <iunlockput>
    goto bad;
80105564:	eb 22                	jmp    80105588 <sys_link+0x144>
  }
  iunlockput(dp);
80105566:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105569:	89 04 24             	mov    %eax,(%esp)
8010556c:	e8 6e c5 ff ff       	call   80101adf <iunlockput>
  iput(ip);
80105571:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105574:	89 04 24             	mov    %eax,(%esp)
80105577:	e8 92 c4 ff ff       	call   80101a0e <iput>

  commit_trans();
8010557c:	e8 ad dc ff ff       	call   8010322e <commit_trans>

  return 0;
80105581:	b8 00 00 00 00       	mov    $0x0,%eax
80105586:	eb 3c                	jmp    801055c4 <sys_link+0x180>

bad:
  ilock(ip);
80105588:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010558b:	89 04 24             	mov    %eax,(%esp)
8010558e:	e8 c8 c2 ff ff       	call   8010185b <ilock>
  ip->nlink--;
80105593:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105596:	0f b7 40 16          	movzwl 0x16(%eax),%eax
8010559a:	8d 50 ff             	lea    -0x1(%eax),%edx
8010559d:	8b 45 f4             	mov    -0xc(%ebp),%eax
801055a0:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
801055a4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801055a7:	89 04 24             	mov    %eax,(%esp)
801055aa:	e8 f0 c0 ff ff       	call   8010169f <iupdate>
  iunlockput(ip);
801055af:	8b 45 f4             	mov    -0xc(%ebp),%eax
801055b2:	89 04 24             	mov    %eax,(%esp)
801055b5:	e8 25 c5 ff ff       	call   80101adf <iunlockput>
  commit_trans();
801055ba:	e8 6f dc ff ff       	call   8010322e <commit_trans>
  return -1;
801055bf:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801055c4:	c9                   	leave  
801055c5:	c3                   	ret    

801055c6 <isdirempty>:

// Is the directory dp empty except for "." and ".." ?
static int
isdirempty(struct inode *dp)
{
801055c6:	55                   	push   %ebp
801055c7:	89 e5                	mov    %esp,%ebp
801055c9:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
801055cc:	c7 45 f4 20 00 00 00 	movl   $0x20,-0xc(%ebp)
801055d3:	eb 4b                	jmp    80105620 <isdirempty+0x5a>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801055d5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801055d8:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
801055df:	00 
801055e0:	89 44 24 08          	mov    %eax,0x8(%esp)
801055e4:	8d 45 e4             	lea    -0x1c(%ebp),%eax
801055e7:	89 44 24 04          	mov    %eax,0x4(%esp)
801055eb:	8b 45 08             	mov    0x8(%ebp),%eax
801055ee:	89 04 24             	mov    %eax,(%esp)
801055f1:	e8 72 c7 ff ff       	call   80101d68 <readi>
801055f6:	83 f8 10             	cmp    $0x10,%eax
801055f9:	74 0c                	je     80105607 <isdirempty+0x41>
      panic("isdirempty: readi");
801055fb:	c7 04 24 47 84 10 80 	movl   $0x80108447,(%esp)
80105602:	e8 33 af ff ff       	call   8010053a <panic>
    if(de.inum != 0)
80105607:	0f b7 45 e4          	movzwl -0x1c(%ebp),%eax
8010560b:	66 85 c0             	test   %ax,%ax
8010560e:	74 07                	je     80105617 <isdirempty+0x51>
      return 0;
80105610:	b8 00 00 00 00       	mov    $0x0,%eax
80105615:	eb 1b                	jmp    80105632 <isdirempty+0x6c>
isdirempty(struct inode *dp)
{
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
80105617:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010561a:	83 c0 10             	add    $0x10,%eax
8010561d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105620:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105623:	8b 45 08             	mov    0x8(%ebp),%eax
80105626:	8b 40 18             	mov    0x18(%eax),%eax
80105629:	39 c2                	cmp    %eax,%edx
8010562b:	72 a8                	jb     801055d5 <isdirempty+0xf>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("isdirempty: readi");
    if(de.inum != 0)
      return 0;
  }
  return 1;
8010562d:	b8 01 00 00 00       	mov    $0x1,%eax
}
80105632:	c9                   	leave  
80105633:	c3                   	ret    

80105634 <sys_unlink>:

//PAGEBREAK!
int
sys_unlink(void)
{
80105634:	55                   	push   %ebp
80105635:	89 e5                	mov    %esp,%ebp
80105637:	83 ec 48             	sub    $0x48,%esp
  struct inode *ip, *dp;
  struct dirent de;
  char name[DIRSIZ], *path;
  uint off;

  if(argstr(0, &path) < 0)
8010563a:	8d 45 cc             	lea    -0x34(%ebp),%eax
8010563d:	89 44 24 04          	mov    %eax,0x4(%esp)
80105641:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105648:	e8 71 fa ff ff       	call   801050be <argstr>
8010564d:	85 c0                	test   %eax,%eax
8010564f:	79 0a                	jns    8010565b <sys_unlink+0x27>
    return -1;
80105651:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105656:	e9 af 01 00 00       	jmp    8010580a <sys_unlink+0x1d6>

  begin_trans();
8010565b:	e8 85 db ff ff       	call   801031e5 <begin_trans>
  if((dp = nameiparent(path, name)) == 0){
80105660:	8b 45 cc             	mov    -0x34(%ebp),%eax
80105663:	8d 55 d2             	lea    -0x2e(%ebp),%edx
80105666:	89 54 24 04          	mov    %edx,0x4(%esp)
8010566a:	89 04 24             	mov    %eax,(%esp)
8010566d:	e8 b6 cd ff ff       	call   80102428 <nameiparent>
80105672:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105675:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105679:	75 0f                	jne    8010568a <sys_unlink+0x56>
    commit_trans();
8010567b:	e8 ae db ff ff       	call   8010322e <commit_trans>
    return -1;
80105680:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105685:	e9 80 01 00 00       	jmp    8010580a <sys_unlink+0x1d6>
  }

  ilock(dp);
8010568a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010568d:	89 04 24             	mov    %eax,(%esp)
80105690:	e8 c6 c1 ff ff       	call   8010185b <ilock>

  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
80105695:	c7 44 24 04 59 84 10 	movl   $0x80108459,0x4(%esp)
8010569c:	80 
8010569d:	8d 45 d2             	lea    -0x2e(%ebp),%eax
801056a0:	89 04 24             	mov    %eax,(%esp)
801056a3:	e8 b3 c9 ff ff       	call   8010205b <namecmp>
801056a8:	85 c0                	test   %eax,%eax
801056aa:	0f 84 45 01 00 00    	je     801057f5 <sys_unlink+0x1c1>
801056b0:	c7 44 24 04 5b 84 10 	movl   $0x8010845b,0x4(%esp)
801056b7:	80 
801056b8:	8d 45 d2             	lea    -0x2e(%ebp),%eax
801056bb:	89 04 24             	mov    %eax,(%esp)
801056be:	e8 98 c9 ff ff       	call   8010205b <namecmp>
801056c3:	85 c0                	test   %eax,%eax
801056c5:	0f 84 2a 01 00 00    	je     801057f5 <sys_unlink+0x1c1>
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
801056cb:	8d 45 c8             	lea    -0x38(%ebp),%eax
801056ce:	89 44 24 08          	mov    %eax,0x8(%esp)
801056d2:	8d 45 d2             	lea    -0x2e(%ebp),%eax
801056d5:	89 44 24 04          	mov    %eax,0x4(%esp)
801056d9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801056dc:	89 04 24             	mov    %eax,(%esp)
801056df:	e8 99 c9 ff ff       	call   8010207d <dirlookup>
801056e4:	89 45 f0             	mov    %eax,-0x10(%ebp)
801056e7:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801056eb:	75 05                	jne    801056f2 <sys_unlink+0xbe>
    goto bad;
801056ed:	e9 03 01 00 00       	jmp    801057f5 <sys_unlink+0x1c1>
  ilock(ip);
801056f2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801056f5:	89 04 24             	mov    %eax,(%esp)
801056f8:	e8 5e c1 ff ff       	call   8010185b <ilock>

  if(ip->nlink < 1)
801056fd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105700:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105704:	66 85 c0             	test   %ax,%ax
80105707:	7f 0c                	jg     80105715 <sys_unlink+0xe1>
    panic("unlink: nlink < 1");
80105709:	c7 04 24 5e 84 10 80 	movl   $0x8010845e,(%esp)
80105710:	e8 25 ae ff ff       	call   8010053a <panic>
  if(ip->type == T_DIR && !isdirempty(ip)){
80105715:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105718:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010571c:	66 83 f8 01          	cmp    $0x1,%ax
80105720:	75 1f                	jne    80105741 <sys_unlink+0x10d>
80105722:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105725:	89 04 24             	mov    %eax,(%esp)
80105728:	e8 99 fe ff ff       	call   801055c6 <isdirempty>
8010572d:	85 c0                	test   %eax,%eax
8010572f:	75 10                	jne    80105741 <sys_unlink+0x10d>
    iunlockput(ip);
80105731:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105734:	89 04 24             	mov    %eax,(%esp)
80105737:	e8 a3 c3 ff ff       	call   80101adf <iunlockput>
    goto bad;
8010573c:	e9 b4 00 00 00       	jmp    801057f5 <sys_unlink+0x1c1>
  }

  memset(&de, 0, sizeof(de));
80105741:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80105748:	00 
80105749:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105750:	00 
80105751:	8d 45 e0             	lea    -0x20(%ebp),%eax
80105754:	89 04 24             	mov    %eax,(%esp)
80105757:	e8 90 f5 ff ff       	call   80104cec <memset>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
8010575c:	8b 45 c8             	mov    -0x38(%ebp),%eax
8010575f:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80105766:	00 
80105767:	89 44 24 08          	mov    %eax,0x8(%esp)
8010576b:	8d 45 e0             	lea    -0x20(%ebp),%eax
8010576e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105772:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105775:	89 04 24             	mov    %eax,(%esp)
80105778:	e8 4f c7 ff ff       	call   80101ecc <writei>
8010577d:	83 f8 10             	cmp    $0x10,%eax
80105780:	74 0c                	je     8010578e <sys_unlink+0x15a>
    panic("unlink: writei");
80105782:	c7 04 24 70 84 10 80 	movl   $0x80108470,(%esp)
80105789:	e8 ac ad ff ff       	call   8010053a <panic>
  if(ip->type == T_DIR){
8010578e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105791:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105795:	66 83 f8 01          	cmp    $0x1,%ax
80105799:	75 1c                	jne    801057b7 <sys_unlink+0x183>
    dp->nlink--;
8010579b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010579e:	0f b7 40 16          	movzwl 0x16(%eax),%eax
801057a2:	8d 50 ff             	lea    -0x1(%eax),%edx
801057a5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801057a8:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
801057ac:	8b 45 f4             	mov    -0xc(%ebp),%eax
801057af:	89 04 24             	mov    %eax,(%esp)
801057b2:	e8 e8 be ff ff       	call   8010169f <iupdate>
  }
  iunlockput(dp);
801057b7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801057ba:	89 04 24             	mov    %eax,(%esp)
801057bd:	e8 1d c3 ff ff       	call   80101adf <iunlockput>

  ip->nlink--;
801057c2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801057c5:	0f b7 40 16          	movzwl 0x16(%eax),%eax
801057c9:	8d 50 ff             	lea    -0x1(%eax),%edx
801057cc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801057cf:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
801057d3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801057d6:	89 04 24             	mov    %eax,(%esp)
801057d9:	e8 c1 be ff ff       	call   8010169f <iupdate>
  iunlockput(ip);
801057de:	8b 45 f0             	mov    -0x10(%ebp),%eax
801057e1:	89 04 24             	mov    %eax,(%esp)
801057e4:	e8 f6 c2 ff ff       	call   80101adf <iunlockput>

  commit_trans();
801057e9:	e8 40 da ff ff       	call   8010322e <commit_trans>

  return 0;
801057ee:	b8 00 00 00 00       	mov    $0x0,%eax
801057f3:	eb 15                	jmp    8010580a <sys_unlink+0x1d6>

bad:
  iunlockput(dp);
801057f5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801057f8:	89 04 24             	mov    %eax,(%esp)
801057fb:	e8 df c2 ff ff       	call   80101adf <iunlockput>
  commit_trans();
80105800:	e8 29 da ff ff       	call   8010322e <commit_trans>
  return -1;
80105805:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010580a:	c9                   	leave  
8010580b:	c3                   	ret    

8010580c <create>:

static struct inode*
create(char *path, short type, short major, short minor)
{
8010580c:	55                   	push   %ebp
8010580d:	89 e5                	mov    %esp,%ebp
8010580f:	83 ec 48             	sub    $0x48,%esp
80105812:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80105815:	8b 55 10             	mov    0x10(%ebp),%edx
80105818:	8b 45 14             	mov    0x14(%ebp),%eax
8010581b:	66 89 4d d4          	mov    %cx,-0x2c(%ebp)
8010581f:	66 89 55 d0          	mov    %dx,-0x30(%ebp)
80105823:	66 89 45 cc          	mov    %ax,-0x34(%ebp)
  uint off;
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
80105827:	8d 45 de             	lea    -0x22(%ebp),%eax
8010582a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010582e:	8b 45 08             	mov    0x8(%ebp),%eax
80105831:	89 04 24             	mov    %eax,(%esp)
80105834:	e8 ef cb ff ff       	call   80102428 <nameiparent>
80105839:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010583c:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105840:	75 0a                	jne    8010584c <create+0x40>
    return 0;
80105842:	b8 00 00 00 00       	mov    $0x0,%eax
80105847:	e9 7e 01 00 00       	jmp    801059ca <create+0x1be>
  ilock(dp);
8010584c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010584f:	89 04 24             	mov    %eax,(%esp)
80105852:	e8 04 c0 ff ff       	call   8010185b <ilock>

  if((ip = dirlookup(dp, name, &off)) != 0){
80105857:	8d 45 ec             	lea    -0x14(%ebp),%eax
8010585a:	89 44 24 08          	mov    %eax,0x8(%esp)
8010585e:	8d 45 de             	lea    -0x22(%ebp),%eax
80105861:	89 44 24 04          	mov    %eax,0x4(%esp)
80105865:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105868:	89 04 24             	mov    %eax,(%esp)
8010586b:	e8 0d c8 ff ff       	call   8010207d <dirlookup>
80105870:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105873:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105877:	74 47                	je     801058c0 <create+0xb4>
    iunlockput(dp);
80105879:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010587c:	89 04 24             	mov    %eax,(%esp)
8010587f:	e8 5b c2 ff ff       	call   80101adf <iunlockput>
    ilock(ip);
80105884:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105887:	89 04 24             	mov    %eax,(%esp)
8010588a:	e8 cc bf ff ff       	call   8010185b <ilock>
    if(type == T_FILE && ip->type == T_FILE)
8010588f:	66 83 7d d4 02       	cmpw   $0x2,-0x2c(%ebp)
80105894:	75 15                	jne    801058ab <create+0x9f>
80105896:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105899:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010589d:	66 83 f8 02          	cmp    $0x2,%ax
801058a1:	75 08                	jne    801058ab <create+0x9f>
      return ip;
801058a3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801058a6:	e9 1f 01 00 00       	jmp    801059ca <create+0x1be>
    iunlockput(ip);
801058ab:	8b 45 f0             	mov    -0x10(%ebp),%eax
801058ae:	89 04 24             	mov    %eax,(%esp)
801058b1:	e8 29 c2 ff ff       	call   80101adf <iunlockput>
    return 0;
801058b6:	b8 00 00 00 00       	mov    $0x0,%eax
801058bb:	e9 0a 01 00 00       	jmp    801059ca <create+0x1be>
  }

  if((ip = ialloc(dp->dev, type)) == 0)
801058c0:	0f bf 55 d4          	movswl -0x2c(%ebp),%edx
801058c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058c7:	8b 00                	mov    (%eax),%eax
801058c9:	89 54 24 04          	mov    %edx,0x4(%esp)
801058cd:	89 04 24             	mov    %eax,(%esp)
801058d0:	e8 eb bc ff ff       	call   801015c0 <ialloc>
801058d5:	89 45 f0             	mov    %eax,-0x10(%ebp)
801058d8:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801058dc:	75 0c                	jne    801058ea <create+0xde>
    panic("create: ialloc");
801058de:	c7 04 24 7f 84 10 80 	movl   $0x8010847f,(%esp)
801058e5:	e8 50 ac ff ff       	call   8010053a <panic>

  ilock(ip);
801058ea:	8b 45 f0             	mov    -0x10(%ebp),%eax
801058ed:	89 04 24             	mov    %eax,(%esp)
801058f0:	e8 66 bf ff ff       	call   8010185b <ilock>
  ip->major = major;
801058f5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801058f8:	0f b7 55 d0          	movzwl -0x30(%ebp),%edx
801058fc:	66 89 50 12          	mov    %dx,0x12(%eax)
  ip->minor = minor;
80105900:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105903:	0f b7 55 cc          	movzwl -0x34(%ebp),%edx
80105907:	66 89 50 14          	mov    %dx,0x14(%eax)
  ip->nlink = 1;
8010590b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010590e:	66 c7 40 16 01 00    	movw   $0x1,0x16(%eax)
  iupdate(ip);
80105914:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105917:	89 04 24             	mov    %eax,(%esp)
8010591a:	e8 80 bd ff ff       	call   8010169f <iupdate>

  if(type == T_DIR){  // Create . and .. entries.
8010591f:	66 83 7d d4 01       	cmpw   $0x1,-0x2c(%ebp)
80105924:	75 6a                	jne    80105990 <create+0x184>
    dp->nlink++;  // for ".."
80105926:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105929:	0f b7 40 16          	movzwl 0x16(%eax),%eax
8010592d:	8d 50 01             	lea    0x1(%eax),%edx
80105930:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105933:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
80105937:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010593a:	89 04 24             	mov    %eax,(%esp)
8010593d:	e8 5d bd ff ff       	call   8010169f <iupdate>
    // No ip->nlink++ for ".": avoid cyclic ref count.
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
80105942:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105945:	8b 40 04             	mov    0x4(%eax),%eax
80105948:	89 44 24 08          	mov    %eax,0x8(%esp)
8010594c:	c7 44 24 04 59 84 10 	movl   $0x80108459,0x4(%esp)
80105953:	80 
80105954:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105957:	89 04 24             	mov    %eax,(%esp)
8010595a:	e8 e7 c7 ff ff       	call   80102146 <dirlink>
8010595f:	85 c0                	test   %eax,%eax
80105961:	78 21                	js     80105984 <create+0x178>
80105963:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105966:	8b 40 04             	mov    0x4(%eax),%eax
80105969:	89 44 24 08          	mov    %eax,0x8(%esp)
8010596d:	c7 44 24 04 5b 84 10 	movl   $0x8010845b,0x4(%esp)
80105974:	80 
80105975:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105978:	89 04 24             	mov    %eax,(%esp)
8010597b:	e8 c6 c7 ff ff       	call   80102146 <dirlink>
80105980:	85 c0                	test   %eax,%eax
80105982:	79 0c                	jns    80105990 <create+0x184>
      panic("create dots");
80105984:	c7 04 24 8e 84 10 80 	movl   $0x8010848e,(%esp)
8010598b:	e8 aa ab ff ff       	call   8010053a <panic>
  }

  if(dirlink(dp, name, ip->inum) < 0)
80105990:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105993:	8b 40 04             	mov    0x4(%eax),%eax
80105996:	89 44 24 08          	mov    %eax,0x8(%esp)
8010599a:	8d 45 de             	lea    -0x22(%ebp),%eax
8010599d:	89 44 24 04          	mov    %eax,0x4(%esp)
801059a1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801059a4:	89 04 24             	mov    %eax,(%esp)
801059a7:	e8 9a c7 ff ff       	call   80102146 <dirlink>
801059ac:	85 c0                	test   %eax,%eax
801059ae:	79 0c                	jns    801059bc <create+0x1b0>
    panic("create: dirlink");
801059b0:	c7 04 24 9a 84 10 80 	movl   $0x8010849a,(%esp)
801059b7:	e8 7e ab ff ff       	call   8010053a <panic>

  iunlockput(dp);
801059bc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801059bf:	89 04 24             	mov    %eax,(%esp)
801059c2:	e8 18 c1 ff ff       	call   80101adf <iunlockput>

  return ip;
801059c7:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
801059ca:	c9                   	leave  
801059cb:	c3                   	ret    

801059cc <sys_open>:

int
sys_open(void)
{
801059cc:	55                   	push   %ebp
801059cd:	89 e5                	mov    %esp,%ebp
801059cf:	83 ec 38             	sub    $0x38,%esp
  char *path;
  int fd, omode;
  struct file *f;
  struct inode *ip;

  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
801059d2:	8d 45 e8             	lea    -0x18(%ebp),%eax
801059d5:	89 44 24 04          	mov    %eax,0x4(%esp)
801059d9:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801059e0:	e8 d9 f6 ff ff       	call   801050be <argstr>
801059e5:	85 c0                	test   %eax,%eax
801059e7:	78 17                	js     80105a00 <sys_open+0x34>
801059e9:	8d 45 e4             	lea    -0x1c(%ebp),%eax
801059ec:	89 44 24 04          	mov    %eax,0x4(%esp)
801059f0:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801059f7:	e8 32 f6 ff ff       	call   8010502e <argint>
801059fc:	85 c0                	test   %eax,%eax
801059fe:	79 0a                	jns    80105a0a <sys_open+0x3e>
    return -1;
80105a00:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105a05:	e9 5c 01 00 00       	jmp    80105b66 <sys_open+0x19a>

  begin_trans();
80105a0a:	e8 d6 d7 ff ff       	call   801031e5 <begin_trans>

  if(omode & O_CREATE){
80105a0f:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105a12:	25 00 02 00 00       	and    $0x200,%eax
80105a17:	85 c0                	test   %eax,%eax
80105a19:	74 3b                	je     80105a56 <sys_open+0x8a>
    ip = create(path, T_FILE, 0, 0);
80105a1b:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105a1e:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80105a25:	00 
80105a26:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80105a2d:	00 
80105a2e:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80105a35:	00 
80105a36:	89 04 24             	mov    %eax,(%esp)
80105a39:	e8 ce fd ff ff       	call   8010580c <create>
80105a3e:	89 45 f4             	mov    %eax,-0xc(%ebp)
    if(ip == 0){
80105a41:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105a45:	75 6b                	jne    80105ab2 <sys_open+0xe6>
      commit_trans();
80105a47:	e8 e2 d7 ff ff       	call   8010322e <commit_trans>
      return -1;
80105a4c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105a51:	e9 10 01 00 00       	jmp    80105b66 <sys_open+0x19a>
    }
  } else {
    if((ip = namei(path)) == 0){
80105a56:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105a59:	89 04 24             	mov    %eax,(%esp)
80105a5c:	e8 a5 c9 ff ff       	call   80102406 <namei>
80105a61:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105a64:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105a68:	75 0f                	jne    80105a79 <sys_open+0xad>
      commit_trans();
80105a6a:	e8 bf d7 ff ff       	call   8010322e <commit_trans>
      return -1;
80105a6f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105a74:	e9 ed 00 00 00       	jmp    80105b66 <sys_open+0x19a>
    }
    ilock(ip);
80105a79:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a7c:	89 04 24             	mov    %eax,(%esp)
80105a7f:	e8 d7 bd ff ff       	call   8010185b <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
80105a84:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a87:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105a8b:	66 83 f8 01          	cmp    $0x1,%ax
80105a8f:	75 21                	jne    80105ab2 <sys_open+0xe6>
80105a91:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105a94:	85 c0                	test   %eax,%eax
80105a96:	74 1a                	je     80105ab2 <sys_open+0xe6>
      iunlockput(ip);
80105a98:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a9b:	89 04 24             	mov    %eax,(%esp)
80105a9e:	e8 3c c0 ff ff       	call   80101adf <iunlockput>
      commit_trans();
80105aa3:	e8 86 d7 ff ff       	call   8010322e <commit_trans>
      return -1;
80105aa8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105aad:	e9 b4 00 00 00       	jmp    80105b66 <sys_open+0x19a>
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
80105ab2:	e8 6f b4 ff ff       	call   80100f26 <filealloc>
80105ab7:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105aba:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105abe:	74 14                	je     80105ad4 <sys_open+0x108>
80105ac0:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ac3:	89 04 24             	mov    %eax,(%esp)
80105ac6:	e8 2e f7 ff ff       	call   801051f9 <fdalloc>
80105acb:	89 45 ec             	mov    %eax,-0x14(%ebp)
80105ace:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80105ad2:	79 28                	jns    80105afc <sys_open+0x130>
    if(f)
80105ad4:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105ad8:	74 0b                	je     80105ae5 <sys_open+0x119>
      fileclose(f);
80105ada:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105add:	89 04 24             	mov    %eax,(%esp)
80105ae0:	e8 e9 b4 ff ff       	call   80100fce <fileclose>
    iunlockput(ip);
80105ae5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ae8:	89 04 24             	mov    %eax,(%esp)
80105aeb:	e8 ef bf ff ff       	call   80101adf <iunlockput>
    commit_trans();
80105af0:	e8 39 d7 ff ff       	call   8010322e <commit_trans>
    return -1;
80105af5:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105afa:	eb 6a                	jmp    80105b66 <sys_open+0x19a>
  }
  iunlock(ip);
80105afc:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105aff:	89 04 24             	mov    %eax,(%esp)
80105b02:	e8 a2 be ff ff       	call   801019a9 <iunlock>
  commit_trans();
80105b07:	e8 22 d7 ff ff       	call   8010322e <commit_trans>

  f->type = FD_INODE;
80105b0c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b0f:	c7 00 02 00 00 00    	movl   $0x2,(%eax)
  f->ip = ip;
80105b15:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b18:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105b1b:	89 50 10             	mov    %edx,0x10(%eax)
  f->off = 0;
80105b1e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b21:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  f->readable = !(omode & O_WRONLY);
80105b28:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105b2b:	83 e0 01             	and    $0x1,%eax
80105b2e:	85 c0                	test   %eax,%eax
80105b30:	0f 94 c0             	sete   %al
80105b33:	89 c2                	mov    %eax,%edx
80105b35:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b38:	88 50 08             	mov    %dl,0x8(%eax)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80105b3b:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105b3e:	83 e0 01             	and    $0x1,%eax
80105b41:	85 c0                	test   %eax,%eax
80105b43:	75 0a                	jne    80105b4f <sys_open+0x183>
80105b45:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105b48:	83 e0 02             	and    $0x2,%eax
80105b4b:	85 c0                	test   %eax,%eax
80105b4d:	74 07                	je     80105b56 <sys_open+0x18a>
80105b4f:	b8 01 00 00 00       	mov    $0x1,%eax
80105b54:	eb 05                	jmp    80105b5b <sys_open+0x18f>
80105b56:	b8 00 00 00 00       	mov    $0x0,%eax
80105b5b:	89 c2                	mov    %eax,%edx
80105b5d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b60:	88 50 09             	mov    %dl,0x9(%eax)
  return fd;
80105b63:	8b 45 ec             	mov    -0x14(%ebp),%eax
}
80105b66:	c9                   	leave  
80105b67:	c3                   	ret    

80105b68 <sys_mkdir>:

int
sys_mkdir(void)
{
80105b68:	55                   	push   %ebp
80105b69:	89 e5                	mov    %esp,%ebp
80105b6b:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  begin_trans();
80105b6e:	e8 72 d6 ff ff       	call   801031e5 <begin_trans>
  if(argstr(0, &path) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
80105b73:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105b76:	89 44 24 04          	mov    %eax,0x4(%esp)
80105b7a:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105b81:	e8 38 f5 ff ff       	call   801050be <argstr>
80105b86:	85 c0                	test   %eax,%eax
80105b88:	78 2c                	js     80105bb6 <sys_mkdir+0x4e>
80105b8a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b8d:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80105b94:	00 
80105b95:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80105b9c:	00 
80105b9d:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80105ba4:	00 
80105ba5:	89 04 24             	mov    %eax,(%esp)
80105ba8:	e8 5f fc ff ff       	call   8010580c <create>
80105bad:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105bb0:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105bb4:	75 0c                	jne    80105bc2 <sys_mkdir+0x5a>
    commit_trans();
80105bb6:	e8 73 d6 ff ff       	call   8010322e <commit_trans>
    return -1;
80105bbb:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105bc0:	eb 15                	jmp    80105bd7 <sys_mkdir+0x6f>
  }
  iunlockput(ip);
80105bc2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105bc5:	89 04 24             	mov    %eax,(%esp)
80105bc8:	e8 12 bf ff ff       	call   80101adf <iunlockput>
  commit_trans();
80105bcd:	e8 5c d6 ff ff       	call   8010322e <commit_trans>
  return 0;
80105bd2:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105bd7:	c9                   	leave  
80105bd8:	c3                   	ret    

80105bd9 <sys_mknod>:

int
sys_mknod(void)
{
80105bd9:	55                   	push   %ebp
80105bda:	89 e5                	mov    %esp,%ebp
80105bdc:	83 ec 38             	sub    $0x38,%esp
  struct inode *ip;
  char *path;
  int len;
  int major, minor;
  
  begin_trans();
80105bdf:	e8 01 d6 ff ff       	call   801031e5 <begin_trans>
  if((len=argstr(0, &path)) < 0 ||
80105be4:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105be7:	89 44 24 04          	mov    %eax,0x4(%esp)
80105beb:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105bf2:	e8 c7 f4 ff ff       	call   801050be <argstr>
80105bf7:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105bfa:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105bfe:	78 5e                	js     80105c5e <sys_mknod+0x85>
     argint(1, &major) < 0 ||
80105c00:	8d 45 e8             	lea    -0x18(%ebp),%eax
80105c03:	89 44 24 04          	mov    %eax,0x4(%esp)
80105c07:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105c0e:	e8 1b f4 ff ff       	call   8010502e <argint>
  char *path;
  int len;
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
80105c13:	85 c0                	test   %eax,%eax
80105c15:	78 47                	js     80105c5e <sys_mknod+0x85>
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
80105c17:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105c1a:	89 44 24 04          	mov    %eax,0x4(%esp)
80105c1e:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
80105c25:	e8 04 f4 ff ff       	call   8010502e <argint>
  int len;
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
80105c2a:	85 c0                	test   %eax,%eax
80105c2c:	78 30                	js     80105c5e <sys_mknod+0x85>
     argint(2, &minor) < 0 ||
     (ip = create(path, T_DEV, major, minor)) == 0){
80105c2e:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105c31:	0f bf c8             	movswl %ax,%ecx
80105c34:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105c37:	0f bf d0             	movswl %ax,%edx
80105c3a:	8b 45 ec             	mov    -0x14(%ebp),%eax
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
80105c3d:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80105c41:	89 54 24 08          	mov    %edx,0x8(%esp)
80105c45:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80105c4c:	00 
80105c4d:	89 04 24             	mov    %eax,(%esp)
80105c50:	e8 b7 fb ff ff       	call   8010580c <create>
80105c55:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105c58:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105c5c:	75 0c                	jne    80105c6a <sys_mknod+0x91>
     (ip = create(path, T_DEV, major, minor)) == 0){
    commit_trans();
80105c5e:	e8 cb d5 ff ff       	call   8010322e <commit_trans>
    return -1;
80105c63:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105c68:	eb 15                	jmp    80105c7f <sys_mknod+0xa6>
  }
  iunlockput(ip);
80105c6a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c6d:	89 04 24             	mov    %eax,(%esp)
80105c70:	e8 6a be ff ff       	call   80101adf <iunlockput>
  commit_trans();
80105c75:	e8 b4 d5 ff ff       	call   8010322e <commit_trans>
  return 0;
80105c7a:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105c7f:	c9                   	leave  
80105c80:	c3                   	ret    

80105c81 <sys_chdir>:

int
sys_chdir(void)
{
80105c81:	55                   	push   %ebp
80105c82:	89 e5                	mov    %esp,%ebp
80105c84:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  begin_trans();
80105c87:	e8 59 d5 ff ff       	call   801031e5 <begin_trans>
  if(argstr(0, &path) < 0 || (ip = namei(path)) == 0){
80105c8c:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105c8f:	89 44 24 04          	mov    %eax,0x4(%esp)
80105c93:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105c9a:	e8 1f f4 ff ff       	call   801050be <argstr>
80105c9f:	85 c0                	test   %eax,%eax
80105ca1:	78 14                	js     80105cb7 <sys_chdir+0x36>
80105ca3:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ca6:	89 04 24             	mov    %eax,(%esp)
80105ca9:	e8 58 c7 ff ff       	call   80102406 <namei>
80105cae:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105cb1:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105cb5:	75 0c                	jne    80105cc3 <sys_chdir+0x42>
    commit_trans();
80105cb7:	e8 72 d5 ff ff       	call   8010322e <commit_trans>
    return -1;
80105cbc:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105cc1:	eb 61                	jmp    80105d24 <sys_chdir+0xa3>
  }
  ilock(ip);
80105cc3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105cc6:	89 04 24             	mov    %eax,(%esp)
80105cc9:	e8 8d bb ff ff       	call   8010185b <ilock>
  if(ip->type != T_DIR){
80105cce:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105cd1:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105cd5:	66 83 f8 01          	cmp    $0x1,%ax
80105cd9:	74 17                	je     80105cf2 <sys_chdir+0x71>
    iunlockput(ip);
80105cdb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105cde:	89 04 24             	mov    %eax,(%esp)
80105ce1:	e8 f9 bd ff ff       	call   80101adf <iunlockput>
    commit_trans();
80105ce6:	e8 43 d5 ff ff       	call   8010322e <commit_trans>
    return -1;
80105ceb:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105cf0:	eb 32                	jmp    80105d24 <sys_chdir+0xa3>
  }
  iunlock(ip);
80105cf2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105cf5:	89 04 24             	mov    %eax,(%esp)
80105cf8:	e8 ac bc ff ff       	call   801019a9 <iunlock>
  iput(proc->cwd);
80105cfd:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105d03:	8b 40 68             	mov    0x68(%eax),%eax
80105d06:	89 04 24             	mov    %eax,(%esp)
80105d09:	e8 00 bd ff ff       	call   80101a0e <iput>
  commit_trans();
80105d0e:	e8 1b d5 ff ff       	call   8010322e <commit_trans>
  proc->cwd = ip;
80105d13:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105d19:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105d1c:	89 50 68             	mov    %edx,0x68(%eax)
  return 0;
80105d1f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105d24:	c9                   	leave  
80105d25:	c3                   	ret    

80105d26 <sys_exec>:

int
sys_exec(void)
{
80105d26:	55                   	push   %ebp
80105d27:	89 e5                	mov    %esp,%ebp
80105d29:	81 ec a8 00 00 00    	sub    $0xa8,%esp
  char *path, *argv[MAXARG];
  int i;
  uint uargv, uarg;

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
80105d2f:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105d32:	89 44 24 04          	mov    %eax,0x4(%esp)
80105d36:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105d3d:	e8 7c f3 ff ff       	call   801050be <argstr>
80105d42:	85 c0                	test   %eax,%eax
80105d44:	78 1a                	js     80105d60 <sys_exec+0x3a>
80105d46:	8d 85 6c ff ff ff    	lea    -0x94(%ebp),%eax
80105d4c:	89 44 24 04          	mov    %eax,0x4(%esp)
80105d50:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105d57:	e8 d2 f2 ff ff       	call   8010502e <argint>
80105d5c:	85 c0                	test   %eax,%eax
80105d5e:	79 0a                	jns    80105d6a <sys_exec+0x44>
    return -1;
80105d60:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105d65:	e9 c8 00 00 00       	jmp    80105e32 <sys_exec+0x10c>
  }
  memset(argv, 0, sizeof(argv));
80105d6a:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
80105d71:	00 
80105d72:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105d79:	00 
80105d7a:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
80105d80:	89 04 24             	mov    %eax,(%esp)
80105d83:	e8 64 ef ff ff       	call   80104cec <memset>
  for(i=0;; i++){
80105d88:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    if(i >= NELEM(argv))
80105d8f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d92:	83 f8 1f             	cmp    $0x1f,%eax
80105d95:	76 0a                	jbe    80105da1 <sys_exec+0x7b>
      return -1;
80105d97:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105d9c:	e9 91 00 00 00       	jmp    80105e32 <sys_exec+0x10c>
    if(fetchint(uargv+4*i, (int*)&uarg) < 0)
80105da1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105da4:	c1 e0 02             	shl    $0x2,%eax
80105da7:	89 c2                	mov    %eax,%edx
80105da9:	8b 85 6c ff ff ff    	mov    -0x94(%ebp),%eax
80105daf:	01 c2                	add    %eax,%edx
80105db1:	8d 85 68 ff ff ff    	lea    -0x98(%ebp),%eax
80105db7:	89 44 24 04          	mov    %eax,0x4(%esp)
80105dbb:	89 14 24             	mov    %edx,(%esp)
80105dbe:	e8 cf f1 ff ff       	call   80104f92 <fetchint>
80105dc3:	85 c0                	test   %eax,%eax
80105dc5:	79 07                	jns    80105dce <sys_exec+0xa8>
      return -1;
80105dc7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105dcc:	eb 64                	jmp    80105e32 <sys_exec+0x10c>
    if(uarg == 0){
80105dce:	8b 85 68 ff ff ff    	mov    -0x98(%ebp),%eax
80105dd4:	85 c0                	test   %eax,%eax
80105dd6:	75 26                	jne    80105dfe <sys_exec+0xd8>
      argv[i] = 0;
80105dd8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ddb:	c7 84 85 70 ff ff ff 	movl   $0x0,-0x90(%ebp,%eax,4)
80105de2:	00 00 00 00 
      break;
80105de6:	90                   	nop
    }
    if(fetchstr(uarg, &argv[i]) < 0)
      return -1;
  }
  return exec(path, argv);
80105de7:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105dea:	8d 95 70 ff ff ff    	lea    -0x90(%ebp),%edx
80105df0:	89 54 24 04          	mov    %edx,0x4(%esp)
80105df4:	89 04 24             	mov    %eax,(%esp)
80105df7:	e8 f3 ac ff ff       	call   80100aef <exec>
80105dfc:	eb 34                	jmp    80105e32 <sys_exec+0x10c>
      return -1;
    if(uarg == 0){
      argv[i] = 0;
      break;
    }
    if(fetchstr(uarg, &argv[i]) < 0)
80105dfe:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
80105e04:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105e07:	c1 e2 02             	shl    $0x2,%edx
80105e0a:	01 c2                	add    %eax,%edx
80105e0c:	8b 85 68 ff ff ff    	mov    -0x98(%ebp),%eax
80105e12:	89 54 24 04          	mov    %edx,0x4(%esp)
80105e16:	89 04 24             	mov    %eax,(%esp)
80105e19:	e8 ae f1 ff ff       	call   80104fcc <fetchstr>
80105e1e:	85 c0                	test   %eax,%eax
80105e20:	79 07                	jns    80105e29 <sys_exec+0x103>
      return -1;
80105e22:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105e27:	eb 09                	jmp    80105e32 <sys_exec+0x10c>

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
    return -1;
  }
  memset(argv, 0, sizeof(argv));
  for(i=0;; i++){
80105e29:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
      argv[i] = 0;
      break;
    }
    if(fetchstr(uarg, &argv[i]) < 0)
      return -1;
  }
80105e2d:	e9 5d ff ff ff       	jmp    80105d8f <sys_exec+0x69>
  return exec(path, argv);
}
80105e32:	c9                   	leave  
80105e33:	c3                   	ret    

80105e34 <sys_pipe>:

int
sys_pipe(void)
{
80105e34:	55                   	push   %ebp
80105e35:	89 e5                	mov    %esp,%ebp
80105e37:	83 ec 38             	sub    $0x38,%esp
  int *fd;
  struct file *rf, *wf;
  int fd0, fd1;

  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
80105e3a:	c7 44 24 08 08 00 00 	movl   $0x8,0x8(%esp)
80105e41:	00 
80105e42:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105e45:	89 44 24 04          	mov    %eax,0x4(%esp)
80105e49:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105e50:	e8 07 f2 ff ff       	call   8010505c <argptr>
80105e55:	85 c0                	test   %eax,%eax
80105e57:	79 0a                	jns    80105e63 <sys_pipe+0x2f>
    return -1;
80105e59:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105e5e:	e9 9b 00 00 00       	jmp    80105efe <sys_pipe+0xca>
  if(pipealloc(&rf, &wf) < 0)
80105e63:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105e66:	89 44 24 04          	mov    %eax,0x4(%esp)
80105e6a:	8d 45 e8             	lea    -0x18(%ebp),%eax
80105e6d:	89 04 24             	mov    %eax,(%esp)
80105e70:	e8 5a dd ff ff       	call   80103bcf <pipealloc>
80105e75:	85 c0                	test   %eax,%eax
80105e77:	79 07                	jns    80105e80 <sys_pipe+0x4c>
    return -1;
80105e79:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105e7e:	eb 7e                	jmp    80105efe <sys_pipe+0xca>
  fd0 = -1;
80105e80:	c7 45 f4 ff ff ff ff 	movl   $0xffffffff,-0xc(%ebp)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
80105e87:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105e8a:	89 04 24             	mov    %eax,(%esp)
80105e8d:	e8 67 f3 ff ff       	call   801051f9 <fdalloc>
80105e92:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105e95:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105e99:	78 14                	js     80105eaf <sys_pipe+0x7b>
80105e9b:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105e9e:	89 04 24             	mov    %eax,(%esp)
80105ea1:	e8 53 f3 ff ff       	call   801051f9 <fdalloc>
80105ea6:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105ea9:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105ead:	79 37                	jns    80105ee6 <sys_pipe+0xb2>
    if(fd0 >= 0)
80105eaf:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105eb3:	78 14                	js     80105ec9 <sys_pipe+0x95>
      proc->ofile[fd0] = 0;
80105eb5:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105ebb:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105ebe:	83 c2 08             	add    $0x8,%edx
80105ec1:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
80105ec8:	00 
    fileclose(rf);
80105ec9:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105ecc:	89 04 24             	mov    %eax,(%esp)
80105ecf:	e8 fa b0 ff ff       	call   80100fce <fileclose>
    fileclose(wf);
80105ed4:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105ed7:	89 04 24             	mov    %eax,(%esp)
80105eda:	e8 ef b0 ff ff       	call   80100fce <fileclose>
    return -1;
80105edf:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105ee4:	eb 18                	jmp    80105efe <sys_pipe+0xca>
  }
  fd[0] = fd0;
80105ee6:	8b 45 ec             	mov    -0x14(%ebp),%eax
80105ee9:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105eec:	89 10                	mov    %edx,(%eax)
  fd[1] = fd1;
80105eee:	8b 45 ec             	mov    -0x14(%ebp),%eax
80105ef1:	8d 50 04             	lea    0x4(%eax),%edx
80105ef4:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ef7:	89 02                	mov    %eax,(%edx)
  return 0;
80105ef9:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105efe:	c9                   	leave  
80105eff:	c3                   	ret    

80105f00 <sys_fork>:
#include "mmu.h"
#include "proc.h"

int
sys_fork(void)
{
80105f00:	55                   	push   %ebp
80105f01:	89 e5                	mov    %esp,%ebp
80105f03:	83 ec 08             	sub    $0x8,%esp
  return fork();
80105f06:	e8 6f e3 ff ff       	call   8010427a <fork>
}
80105f0b:	c9                   	leave  
80105f0c:	c3                   	ret    

80105f0d <sys_exit>:

int
sys_exit(void)
{
80105f0d:	55                   	push   %ebp
80105f0e:	89 e5                	mov    %esp,%ebp
80105f10:	83 ec 08             	sub    $0x8,%esp
  exit();
80105f13:	e8 dd e4 ff ff       	call   801043f5 <exit>
  return 0;  // not reached
80105f18:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105f1d:	c9                   	leave  
80105f1e:	c3                   	ret    

80105f1f <sys_wait>:

int
sys_wait(void)
{
80105f1f:	55                   	push   %ebp
80105f20:	89 e5                	mov    %esp,%ebp
80105f22:	83 ec 08             	sub    $0x8,%esp
  return wait();
80105f25:	e8 ed e5 ff ff       	call   80104517 <wait>
}
80105f2a:	c9                   	leave  
80105f2b:	c3                   	ret    

80105f2c <sys_kill>:

int
sys_kill(void)
{
80105f2c:	55                   	push   %ebp
80105f2d:	89 e5                	mov    %esp,%ebp
80105f2f:	83 ec 28             	sub    $0x28,%esp
  int pid;

  if(argint(0, &pid) < 0)
80105f32:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105f35:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f39:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105f40:	e8 e9 f0 ff ff       	call   8010502e <argint>
80105f45:	85 c0                	test   %eax,%eax
80105f47:	79 07                	jns    80105f50 <sys_kill+0x24>
    return -1;
80105f49:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105f4e:	eb 0b                	jmp    80105f5b <sys_kill+0x2f>
  return kill(pid);
80105f50:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105f53:	89 04 24             	mov    %eax,(%esp)
80105f56:	e8 77 e9 ff ff       	call   801048d2 <kill>
}
80105f5b:	c9                   	leave  
80105f5c:	c3                   	ret    

80105f5d <sys_getpid>:

int
sys_getpid(void)
{
80105f5d:	55                   	push   %ebp
80105f5e:	89 e5                	mov    %esp,%ebp
  return proc->pid;
80105f60:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105f66:	8b 40 10             	mov    0x10(%eax),%eax
}
80105f69:	5d                   	pop    %ebp
80105f6a:	c3                   	ret    

80105f6b <sys_sbrk>:

int
sys_sbrk(void)
{
80105f6b:	55                   	push   %ebp
80105f6c:	89 e5                	mov    %esp,%ebp
80105f6e:	83 ec 28             	sub    $0x28,%esp
  int addr;
  int n;

  if(argint(0, &n) < 0)
80105f71:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105f74:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f78:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105f7f:	e8 aa f0 ff ff       	call   8010502e <argint>
80105f84:	85 c0                	test   %eax,%eax
80105f86:	79 07                	jns    80105f8f <sys_sbrk+0x24>
    return -1;
80105f88:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105f8d:	eb 24                	jmp    80105fb3 <sys_sbrk+0x48>
  addr = proc->sz;
80105f8f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105f95:	8b 00                	mov    (%eax),%eax
80105f97:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(growproc(n) < 0)
80105f9a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f9d:	89 04 24             	mov    %eax,(%esp)
80105fa0:	e8 30 e2 ff ff       	call   801041d5 <growproc>
80105fa5:	85 c0                	test   %eax,%eax
80105fa7:	79 07                	jns    80105fb0 <sys_sbrk+0x45>
    return -1;
80105fa9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105fae:	eb 03                	jmp    80105fb3 <sys_sbrk+0x48>
  return addr;
80105fb0:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80105fb3:	c9                   	leave  
80105fb4:	c3                   	ret    

80105fb5 <sys_sleep>:

int
sys_sleep(void)
{
80105fb5:	55                   	push   %ebp
80105fb6:	89 e5                	mov    %esp,%ebp
80105fb8:	83 ec 28             	sub    $0x28,%esp
  int n;
  uint ticks0;
  
  if(argint(0, &n) < 0)
80105fbb:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105fbe:	89 44 24 04          	mov    %eax,0x4(%esp)
80105fc2:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105fc9:	e8 60 f0 ff ff       	call   8010502e <argint>
80105fce:	85 c0                	test   %eax,%eax
80105fd0:	79 07                	jns    80105fd9 <sys_sleep+0x24>
    return -1;
80105fd2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105fd7:	eb 6c                	jmp    80106045 <sys_sleep+0x90>
  acquire(&tickslock);
80105fd9:	c7 04 24 60 1e 11 80 	movl   $0x80111e60,(%esp)
80105fe0:	e8 b3 ea ff ff       	call   80104a98 <acquire>
  ticks0 = ticks;
80105fe5:	a1 a0 26 11 80       	mov    0x801126a0,%eax
80105fea:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(ticks - ticks0 < n){
80105fed:	eb 34                	jmp    80106023 <sys_sleep+0x6e>
    if(proc->killed){
80105fef:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105ff5:	8b 40 24             	mov    0x24(%eax),%eax
80105ff8:	85 c0                	test   %eax,%eax
80105ffa:	74 13                	je     8010600f <sys_sleep+0x5a>
      release(&tickslock);
80105ffc:	c7 04 24 60 1e 11 80 	movl   $0x80111e60,(%esp)
80106003:	e8 f2 ea ff ff       	call   80104afa <release>
      return -1;
80106008:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010600d:	eb 36                	jmp    80106045 <sys_sleep+0x90>
    }
    sleep(&ticks, &tickslock);
8010600f:	c7 44 24 04 60 1e 11 	movl   $0x80111e60,0x4(%esp)
80106016:	80 
80106017:	c7 04 24 a0 26 11 80 	movl   $0x801126a0,(%esp)
8010601e:	e8 ab e7 ff ff       	call   801047ce <sleep>
  
  if(argint(0, &n) < 0)
    return -1;
  acquire(&tickslock);
  ticks0 = ticks;
  while(ticks - ticks0 < n){
80106023:	a1 a0 26 11 80       	mov    0x801126a0,%eax
80106028:	2b 45 f4             	sub    -0xc(%ebp),%eax
8010602b:	89 c2                	mov    %eax,%edx
8010602d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106030:	39 c2                	cmp    %eax,%edx
80106032:	72 bb                	jb     80105fef <sys_sleep+0x3a>
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
  }
  release(&tickslock);
80106034:	c7 04 24 60 1e 11 80 	movl   $0x80111e60,(%esp)
8010603b:	e8 ba ea ff ff       	call   80104afa <release>
  return 0;
80106040:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106045:	c9                   	leave  
80106046:	c3                   	ret    

80106047 <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
int
sys_uptime(void)
{
80106047:	55                   	push   %ebp
80106048:	89 e5                	mov    %esp,%ebp
8010604a:	83 ec 28             	sub    $0x28,%esp
  uint xticks;
  
  acquire(&tickslock);
8010604d:	c7 04 24 60 1e 11 80 	movl   $0x80111e60,(%esp)
80106054:	e8 3f ea ff ff       	call   80104a98 <acquire>
  xticks = ticks;
80106059:	a1 a0 26 11 80       	mov    0x801126a0,%eax
8010605e:	89 45 f4             	mov    %eax,-0xc(%ebp)
  release(&tickslock);
80106061:	c7 04 24 60 1e 11 80 	movl   $0x80111e60,(%esp)
80106068:	e8 8d ea ff ff       	call   80104afa <release>
  return xticks;
8010606d:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80106070:	c9                   	leave  
80106071:	c3                   	ret    

80106072 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80106072:	55                   	push   %ebp
80106073:	89 e5                	mov    %esp,%ebp
80106075:	83 ec 08             	sub    $0x8,%esp
80106078:	8b 55 08             	mov    0x8(%ebp),%edx
8010607b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010607e:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80106082:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80106085:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80106089:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
8010608d:	ee                   	out    %al,(%dx)
}
8010608e:	c9                   	leave  
8010608f:	c3                   	ret    

80106090 <timerinit>:
#define TIMER_RATEGEN   0x04    // mode 2, rate generator
#define TIMER_16BIT     0x30    // r/w counter 16 bits, LSB first

void
timerinit(void)
{
80106090:	55                   	push   %ebp
80106091:	89 e5                	mov    %esp,%ebp
80106093:	83 ec 18             	sub    $0x18,%esp
  // Interrupt 100 times/sec.
  outb(TIMER_MODE, TIMER_SEL0 | TIMER_RATEGEN | TIMER_16BIT);
80106096:	c7 44 24 04 34 00 00 	movl   $0x34,0x4(%esp)
8010609d:	00 
8010609e:	c7 04 24 43 00 00 00 	movl   $0x43,(%esp)
801060a5:	e8 c8 ff ff ff       	call   80106072 <outb>
  outb(IO_TIMER1, TIMER_DIV(100) % 256);
801060aa:	c7 44 24 04 9c 00 00 	movl   $0x9c,0x4(%esp)
801060b1:	00 
801060b2:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
801060b9:	e8 b4 ff ff ff       	call   80106072 <outb>
  outb(IO_TIMER1, TIMER_DIV(100) / 256);
801060be:	c7 44 24 04 2e 00 00 	movl   $0x2e,0x4(%esp)
801060c5:	00 
801060c6:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
801060cd:	e8 a0 ff ff ff       	call   80106072 <outb>
  picenable(IRQ_TIMER);
801060d2:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801060d9:	e8 84 d9 ff ff       	call   80103a62 <picenable>
}
801060de:	c9                   	leave  
801060df:	c3                   	ret    

801060e0 <alltraps>:

  # vectors.S sends all traps here.
.globl alltraps
alltraps:
  # Build trap frame.
  pushl %ds
801060e0:	1e                   	push   %ds
  pushl %es
801060e1:	06                   	push   %es
  pushl %fs
801060e2:	0f a0                	push   %fs
  pushl %gs
801060e4:	0f a8                	push   %gs
  pushal
801060e6:	60                   	pusha  
  
  # Set up data and per-cpu segments.
  movw $(SEG_KDATA<<3), %ax
801060e7:	66 b8 10 00          	mov    $0x10,%ax
  movw %ax, %ds
801060eb:	8e d8                	mov    %eax,%ds
  movw %ax, %es
801060ed:	8e c0                	mov    %eax,%es
  movw $(SEG_KCPU<<3), %ax
801060ef:	66 b8 18 00          	mov    $0x18,%ax
  movw %ax, %fs
801060f3:	8e e0                	mov    %eax,%fs
  movw %ax, %gs
801060f5:	8e e8                	mov    %eax,%gs

  # Call trap(tf), where tf=%esp
  pushl %esp
801060f7:	54                   	push   %esp
  call trap
801060f8:	e8 d8 01 00 00       	call   801062d5 <trap>
  addl $4, %esp
801060fd:	83 c4 04             	add    $0x4,%esp

80106100 <trapret>:

  # Return falls through to trapret...
.globl trapret
trapret:
  popal
80106100:	61                   	popa   
  popl %gs
80106101:	0f a9                	pop    %gs
  popl %fs
80106103:	0f a1                	pop    %fs
  popl %es
80106105:	07                   	pop    %es
  popl %ds
80106106:	1f                   	pop    %ds
  addl $0x8, %esp  # trapno and errcode
80106107:	83 c4 08             	add    $0x8,%esp
  iret
8010610a:	cf                   	iret   

8010610b <lidt>:

struct gatedesc;

static inline void
lidt(struct gatedesc *p, int size)
{
8010610b:	55                   	push   %ebp
8010610c:	89 e5                	mov    %esp,%ebp
8010610e:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
80106111:	8b 45 0c             	mov    0xc(%ebp),%eax
80106114:	83 e8 01             	sub    $0x1,%eax
80106117:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
8010611b:	8b 45 08             	mov    0x8(%ebp),%eax
8010611e:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
80106122:	8b 45 08             	mov    0x8(%ebp),%eax
80106125:	c1 e8 10             	shr    $0x10,%eax
80106128:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lidt (%0)" : : "r" (pd));
8010612c:	8d 45 fa             	lea    -0x6(%ebp),%eax
8010612f:	0f 01 18             	lidtl  (%eax)
}
80106132:	c9                   	leave  
80106133:	c3                   	ret    

80106134 <rcr2>:
  return result;
}

static inline uint
rcr2(void)
{
80106134:	55                   	push   %ebp
80106135:	89 e5                	mov    %esp,%ebp
80106137:	83 ec 10             	sub    $0x10,%esp
  uint val;
  asm volatile("movl %%cr2,%0" : "=r" (val));
8010613a:	0f 20 d0             	mov    %cr2,%eax
8010613d:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return val;
80106140:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80106143:	c9                   	leave  
80106144:	c3                   	ret    

80106145 <tvinit>:
struct spinlock tickslock;
uint ticks;

void
tvinit(void)
{
80106145:	55                   	push   %ebp
80106146:	89 e5                	mov    %esp,%ebp
80106148:	83 ec 28             	sub    $0x28,%esp
  int i;

  for(i = 0; i < 256; i++)
8010614b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80106152:	e9 c3 00 00 00       	jmp    8010621a <tvinit+0xd5>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
80106157:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010615a:	8b 04 85 98 b0 10 80 	mov    -0x7fef4f68(,%eax,4),%eax
80106161:	89 c2                	mov    %eax,%edx
80106163:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106166:	66 89 14 c5 a0 1e 11 	mov    %dx,-0x7feee160(,%eax,8)
8010616d:	80 
8010616e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106171:	66 c7 04 c5 a2 1e 11 	movw   $0x8,-0x7feee15e(,%eax,8)
80106178:	80 08 00 
8010617b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010617e:	0f b6 14 c5 a4 1e 11 	movzbl -0x7feee15c(,%eax,8),%edx
80106185:	80 
80106186:	83 e2 e0             	and    $0xffffffe0,%edx
80106189:	88 14 c5 a4 1e 11 80 	mov    %dl,-0x7feee15c(,%eax,8)
80106190:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106193:	0f b6 14 c5 a4 1e 11 	movzbl -0x7feee15c(,%eax,8),%edx
8010619a:	80 
8010619b:	83 e2 1f             	and    $0x1f,%edx
8010619e:	88 14 c5 a4 1e 11 80 	mov    %dl,-0x7feee15c(,%eax,8)
801061a5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801061a8:	0f b6 14 c5 a5 1e 11 	movzbl -0x7feee15b(,%eax,8),%edx
801061af:	80 
801061b0:	83 e2 f0             	and    $0xfffffff0,%edx
801061b3:	83 ca 0e             	or     $0xe,%edx
801061b6:	88 14 c5 a5 1e 11 80 	mov    %dl,-0x7feee15b(,%eax,8)
801061bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801061c0:	0f b6 14 c5 a5 1e 11 	movzbl -0x7feee15b(,%eax,8),%edx
801061c7:	80 
801061c8:	83 e2 ef             	and    $0xffffffef,%edx
801061cb:	88 14 c5 a5 1e 11 80 	mov    %dl,-0x7feee15b(,%eax,8)
801061d2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801061d5:	0f b6 14 c5 a5 1e 11 	movzbl -0x7feee15b(,%eax,8),%edx
801061dc:	80 
801061dd:	83 e2 9f             	and    $0xffffff9f,%edx
801061e0:	88 14 c5 a5 1e 11 80 	mov    %dl,-0x7feee15b(,%eax,8)
801061e7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801061ea:	0f b6 14 c5 a5 1e 11 	movzbl -0x7feee15b(,%eax,8),%edx
801061f1:	80 
801061f2:	83 ca 80             	or     $0xffffff80,%edx
801061f5:	88 14 c5 a5 1e 11 80 	mov    %dl,-0x7feee15b(,%eax,8)
801061fc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801061ff:	8b 04 85 98 b0 10 80 	mov    -0x7fef4f68(,%eax,4),%eax
80106206:	c1 e8 10             	shr    $0x10,%eax
80106209:	89 c2                	mov    %eax,%edx
8010620b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010620e:	66 89 14 c5 a6 1e 11 	mov    %dx,-0x7feee15a(,%eax,8)
80106215:	80 
void
tvinit(void)
{
  int i;

  for(i = 0; i < 256; i++)
80106216:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
8010621a:	81 7d f4 ff 00 00 00 	cmpl   $0xff,-0xc(%ebp)
80106221:	0f 8e 30 ff ff ff    	jle    80106157 <tvinit+0x12>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
80106227:	a1 98 b1 10 80       	mov    0x8010b198,%eax
8010622c:	66 a3 a0 20 11 80    	mov    %ax,0x801120a0
80106232:	66 c7 05 a2 20 11 80 	movw   $0x8,0x801120a2
80106239:	08 00 
8010623b:	0f b6 05 a4 20 11 80 	movzbl 0x801120a4,%eax
80106242:	83 e0 e0             	and    $0xffffffe0,%eax
80106245:	a2 a4 20 11 80       	mov    %al,0x801120a4
8010624a:	0f b6 05 a4 20 11 80 	movzbl 0x801120a4,%eax
80106251:	83 e0 1f             	and    $0x1f,%eax
80106254:	a2 a4 20 11 80       	mov    %al,0x801120a4
80106259:	0f b6 05 a5 20 11 80 	movzbl 0x801120a5,%eax
80106260:	83 c8 0f             	or     $0xf,%eax
80106263:	a2 a5 20 11 80       	mov    %al,0x801120a5
80106268:	0f b6 05 a5 20 11 80 	movzbl 0x801120a5,%eax
8010626f:	83 e0 ef             	and    $0xffffffef,%eax
80106272:	a2 a5 20 11 80       	mov    %al,0x801120a5
80106277:	0f b6 05 a5 20 11 80 	movzbl 0x801120a5,%eax
8010627e:	83 c8 60             	or     $0x60,%eax
80106281:	a2 a5 20 11 80       	mov    %al,0x801120a5
80106286:	0f b6 05 a5 20 11 80 	movzbl 0x801120a5,%eax
8010628d:	83 c8 80             	or     $0xffffff80,%eax
80106290:	a2 a5 20 11 80       	mov    %al,0x801120a5
80106295:	a1 98 b1 10 80       	mov    0x8010b198,%eax
8010629a:	c1 e8 10             	shr    $0x10,%eax
8010629d:	66 a3 a6 20 11 80    	mov    %ax,0x801120a6
  
  initlock(&tickslock, "time");
801062a3:	c7 44 24 04 ac 84 10 	movl   $0x801084ac,0x4(%esp)
801062aa:	80 
801062ab:	c7 04 24 60 1e 11 80 	movl   $0x80111e60,(%esp)
801062b2:	e8 c0 e7 ff ff       	call   80104a77 <initlock>
}
801062b7:	c9                   	leave  
801062b8:	c3                   	ret    

801062b9 <idtinit>:

void
idtinit(void)
{
801062b9:	55                   	push   %ebp
801062ba:	89 e5                	mov    %esp,%ebp
801062bc:	83 ec 08             	sub    $0x8,%esp
  lidt(idt, sizeof(idt));
801062bf:	c7 44 24 04 00 08 00 	movl   $0x800,0x4(%esp)
801062c6:	00 
801062c7:	c7 04 24 a0 1e 11 80 	movl   $0x80111ea0,(%esp)
801062ce:	e8 38 fe ff ff       	call   8010610b <lidt>
}
801062d3:	c9                   	leave  
801062d4:	c3                   	ret    

801062d5 <trap>:

//PAGEBREAK: 41
void
trap(struct trapframe *tf)
{
801062d5:	55                   	push   %ebp
801062d6:	89 e5                	mov    %esp,%ebp
801062d8:	57                   	push   %edi
801062d9:	56                   	push   %esi
801062da:	53                   	push   %ebx
801062db:	83 ec 3c             	sub    $0x3c,%esp
  if(tf->trapno == T_SYSCALL){
801062de:	8b 45 08             	mov    0x8(%ebp),%eax
801062e1:	8b 40 30             	mov    0x30(%eax),%eax
801062e4:	83 f8 40             	cmp    $0x40,%eax
801062e7:	75 3f                	jne    80106328 <trap+0x53>
    if(proc->killed)
801062e9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801062ef:	8b 40 24             	mov    0x24(%eax),%eax
801062f2:	85 c0                	test   %eax,%eax
801062f4:	74 05                	je     801062fb <trap+0x26>
      exit();
801062f6:	e8 fa e0 ff ff       	call   801043f5 <exit>
    proc->tf = tf;
801062fb:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106301:	8b 55 08             	mov    0x8(%ebp),%edx
80106304:	89 50 18             	mov    %edx,0x18(%eax)
    syscall();
80106307:	e8 e9 ed ff ff       	call   801050f5 <syscall>
    if(proc->killed)
8010630c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106312:	8b 40 24             	mov    0x24(%eax),%eax
80106315:	85 c0                	test   %eax,%eax
80106317:	74 0a                	je     80106323 <trap+0x4e>
      exit();
80106319:	e8 d7 e0 ff ff       	call   801043f5 <exit>
    return;
8010631e:	e9 2d 02 00 00       	jmp    80106550 <trap+0x27b>
80106323:	e9 28 02 00 00       	jmp    80106550 <trap+0x27b>
  }

  switch(tf->trapno){
80106328:	8b 45 08             	mov    0x8(%ebp),%eax
8010632b:	8b 40 30             	mov    0x30(%eax),%eax
8010632e:	83 e8 20             	sub    $0x20,%eax
80106331:	83 f8 1f             	cmp    $0x1f,%eax
80106334:	0f 87 bc 00 00 00    	ja     801063f6 <trap+0x121>
8010633a:	8b 04 85 54 85 10 80 	mov    -0x7fef7aac(,%eax,4),%eax
80106341:	ff e0                	jmp    *%eax
  case T_IRQ0 + IRQ_TIMER:
    if(cpu->id == 0){
80106343:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106349:	0f b6 00             	movzbl (%eax),%eax
8010634c:	84 c0                	test   %al,%al
8010634e:	75 31                	jne    80106381 <trap+0xac>
      acquire(&tickslock);
80106350:	c7 04 24 60 1e 11 80 	movl   $0x80111e60,(%esp)
80106357:	e8 3c e7 ff ff       	call   80104a98 <acquire>
      ticks++;
8010635c:	a1 a0 26 11 80       	mov    0x801126a0,%eax
80106361:	83 c0 01             	add    $0x1,%eax
80106364:	a3 a0 26 11 80       	mov    %eax,0x801126a0
      wakeup(&ticks);
80106369:	c7 04 24 a0 26 11 80 	movl   $0x801126a0,(%esp)
80106370:	e8 32 e5 ff ff       	call   801048a7 <wakeup>
      release(&tickslock);
80106375:	c7 04 24 60 1e 11 80 	movl   $0x80111e60,(%esp)
8010637c:	e8 79 e7 ff ff       	call   80104afa <release>
    }
    lapiceoi();
80106381:	e8 2d cb ff ff       	call   80102eb3 <lapiceoi>
    break;
80106386:	e9 41 01 00 00       	jmp    801064cc <trap+0x1f7>
  case T_IRQ0 + IRQ_IDE:
    ideintr();
8010638b:	e8 4e c3 ff ff       	call   801026de <ideintr>
    lapiceoi();
80106390:	e8 1e cb ff ff       	call   80102eb3 <lapiceoi>
    break;
80106395:	e9 32 01 00 00       	jmp    801064cc <trap+0x1f7>
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
  case T_IRQ0 + IRQ_KBD:
    kbdintr();
8010639a:	e8 00 c9 ff ff       	call   80102c9f <kbdintr>
    lapiceoi();
8010639f:	e8 0f cb ff ff       	call   80102eb3 <lapiceoi>
    break;
801063a4:	e9 23 01 00 00       	jmp    801064cc <trap+0x1f7>
  case T_IRQ0 + IRQ_COM1:
    uartintr();
801063a9:	e8 97 03 00 00       	call   80106745 <uartintr>
    lapiceoi();
801063ae:	e8 00 cb ff ff       	call   80102eb3 <lapiceoi>
    break;
801063b3:	e9 14 01 00 00       	jmp    801064cc <trap+0x1f7>
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
801063b8:	8b 45 08             	mov    0x8(%ebp),%eax
801063bb:	8b 48 38             	mov    0x38(%eax),%ecx
            cpu->id, tf->cs, tf->eip);
801063be:	8b 45 08             	mov    0x8(%ebp),%eax
801063c1:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
801063c5:	0f b7 d0             	movzwl %ax,%edx
            cpu->id, tf->cs, tf->eip);
801063c8:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801063ce:	0f b6 00             	movzbl (%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
801063d1:	0f b6 c0             	movzbl %al,%eax
801063d4:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
801063d8:	89 54 24 08          	mov    %edx,0x8(%esp)
801063dc:	89 44 24 04          	mov    %eax,0x4(%esp)
801063e0:	c7 04 24 b4 84 10 80 	movl   $0x801084b4,(%esp)
801063e7:	e8 b4 9f ff ff       	call   801003a0 <cprintf>
            cpu->id, tf->cs, tf->eip);
    lapiceoi();
801063ec:	e8 c2 ca ff ff       	call   80102eb3 <lapiceoi>
    break;
801063f1:	e9 d6 00 00 00       	jmp    801064cc <trap+0x1f7>
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
801063f6:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801063fc:	85 c0                	test   %eax,%eax
801063fe:	74 11                	je     80106411 <trap+0x13c>
80106400:	8b 45 08             	mov    0x8(%ebp),%eax
80106403:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106407:	0f b7 c0             	movzwl %ax,%eax
8010640a:	83 e0 03             	and    $0x3,%eax
8010640d:	85 c0                	test   %eax,%eax
8010640f:	75 46                	jne    80106457 <trap+0x182>
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106411:	e8 1e fd ff ff       	call   80106134 <rcr2>
80106416:	8b 55 08             	mov    0x8(%ebp),%edx
80106419:	8b 5a 38             	mov    0x38(%edx),%ebx
              tf->trapno, cpu->id, tf->eip, rcr2());
8010641c:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80106423:	0f b6 12             	movzbl (%edx),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106426:	0f b6 ca             	movzbl %dl,%ecx
80106429:	8b 55 08             	mov    0x8(%ebp),%edx
8010642c:	8b 52 30             	mov    0x30(%edx),%edx
8010642f:	89 44 24 10          	mov    %eax,0x10(%esp)
80106433:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
80106437:	89 4c 24 08          	mov    %ecx,0x8(%esp)
8010643b:	89 54 24 04          	mov    %edx,0x4(%esp)
8010643f:	c7 04 24 d8 84 10 80 	movl   $0x801084d8,(%esp)
80106446:	e8 55 9f ff ff       	call   801003a0 <cprintf>
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
8010644b:	c7 04 24 0a 85 10 80 	movl   $0x8010850a,(%esp)
80106452:	e8 e3 a0 ff ff       	call   8010053a <panic>
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106457:	e8 d8 fc ff ff       	call   80106134 <rcr2>
8010645c:	89 c2                	mov    %eax,%edx
8010645e:	8b 45 08             	mov    0x8(%ebp),%eax
80106461:	8b 78 38             	mov    0x38(%eax),%edi
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106464:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010646a:	0f b6 00             	movzbl (%eax),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
8010646d:	0f b6 f0             	movzbl %al,%esi
80106470:	8b 45 08             	mov    0x8(%ebp),%eax
80106473:	8b 58 34             	mov    0x34(%eax),%ebx
80106476:	8b 45 08             	mov    0x8(%ebp),%eax
80106479:	8b 48 30             	mov    0x30(%eax),%ecx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
8010647c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106482:	83 c0 6c             	add    $0x6c,%eax
80106485:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80106488:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
8010648e:	8b 40 10             	mov    0x10(%eax),%eax
80106491:	89 54 24 1c          	mov    %edx,0x1c(%esp)
80106495:	89 7c 24 18          	mov    %edi,0x18(%esp)
80106499:	89 74 24 14          	mov    %esi,0x14(%esp)
8010649d:	89 5c 24 10          	mov    %ebx,0x10(%esp)
801064a1:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
801064a5:	8b 75 e4             	mov    -0x1c(%ebp),%esi
801064a8:	89 74 24 08          	mov    %esi,0x8(%esp)
801064ac:	89 44 24 04          	mov    %eax,0x4(%esp)
801064b0:	c7 04 24 10 85 10 80 	movl   $0x80108510,(%esp)
801064b7:	e8 e4 9e ff ff       	call   801003a0 <cprintf>
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
            rcr2());
    proc->killed = 1;
801064bc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801064c2:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
801064c9:	eb 01                	jmp    801064cc <trap+0x1f7>
    ideintr();
    lapiceoi();
    break;
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
801064cb:	90                   	nop
  }

  // Force process exit if it has been killed and is in user space.
  // (If it is still executing in the kernel, let it keep running 
  // until it gets to the regular system call return.)
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
801064cc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801064d2:	85 c0                	test   %eax,%eax
801064d4:	74 24                	je     801064fa <trap+0x225>
801064d6:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801064dc:	8b 40 24             	mov    0x24(%eax),%eax
801064df:	85 c0                	test   %eax,%eax
801064e1:	74 17                	je     801064fa <trap+0x225>
801064e3:	8b 45 08             	mov    0x8(%ebp),%eax
801064e6:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
801064ea:	0f b7 c0             	movzwl %ax,%eax
801064ed:	83 e0 03             	and    $0x3,%eax
801064f0:	83 f8 03             	cmp    $0x3,%eax
801064f3:	75 05                	jne    801064fa <trap+0x225>
    exit();
801064f5:	e8 fb de ff ff       	call   801043f5 <exit>

  // Force process to give up CPU on clock tick.
  // If interrupts were on while locks held, would need to check nlock.
  if(proc && proc->state == RUNNING && tf->trapno == T_IRQ0+IRQ_TIMER)
801064fa:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106500:	85 c0                	test   %eax,%eax
80106502:	74 1e                	je     80106522 <trap+0x24d>
80106504:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010650a:	8b 40 0c             	mov    0xc(%eax),%eax
8010650d:	83 f8 04             	cmp    $0x4,%eax
80106510:	75 10                	jne    80106522 <trap+0x24d>
80106512:	8b 45 08             	mov    0x8(%ebp),%eax
80106515:	8b 40 30             	mov    0x30(%eax),%eax
80106518:	83 f8 20             	cmp    $0x20,%eax
8010651b:	75 05                	jne    80106522 <trap+0x24d>
    yield();
8010651d:	e8 4e e2 ff ff       	call   80104770 <yield>

  // Check if the process has been killed since we yielded
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
80106522:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106528:	85 c0                	test   %eax,%eax
8010652a:	74 24                	je     80106550 <trap+0x27b>
8010652c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106532:	8b 40 24             	mov    0x24(%eax),%eax
80106535:	85 c0                	test   %eax,%eax
80106537:	74 17                	je     80106550 <trap+0x27b>
80106539:	8b 45 08             	mov    0x8(%ebp),%eax
8010653c:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106540:	0f b7 c0             	movzwl %ax,%eax
80106543:	83 e0 03             	and    $0x3,%eax
80106546:	83 f8 03             	cmp    $0x3,%eax
80106549:	75 05                	jne    80106550 <trap+0x27b>
    exit();
8010654b:	e8 a5 de ff ff       	call   801043f5 <exit>
}
80106550:	83 c4 3c             	add    $0x3c,%esp
80106553:	5b                   	pop    %ebx
80106554:	5e                   	pop    %esi
80106555:	5f                   	pop    %edi
80106556:	5d                   	pop    %ebp
80106557:	c3                   	ret    

80106558 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80106558:	55                   	push   %ebp
80106559:	89 e5                	mov    %esp,%ebp
8010655b:	83 ec 14             	sub    $0x14,%esp
8010655e:	8b 45 08             	mov    0x8(%ebp),%eax
80106561:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80106565:	0f b7 45 ec          	movzwl -0x14(%ebp),%eax
80106569:	89 c2                	mov    %eax,%edx
8010656b:	ec                   	in     (%dx),%al
8010656c:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
8010656f:	0f b6 45 ff          	movzbl -0x1(%ebp),%eax
}
80106573:	c9                   	leave  
80106574:	c3                   	ret    

80106575 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80106575:	55                   	push   %ebp
80106576:	89 e5                	mov    %esp,%ebp
80106578:	83 ec 08             	sub    $0x8,%esp
8010657b:	8b 55 08             	mov    0x8(%ebp),%edx
8010657e:	8b 45 0c             	mov    0xc(%ebp),%eax
80106581:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80106585:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80106588:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
8010658c:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80106590:	ee                   	out    %al,(%dx)
}
80106591:	c9                   	leave  
80106592:	c3                   	ret    

80106593 <uartinit>:

static int uart;    // is there a uart?

void
uartinit(void)
{
80106593:	55                   	push   %ebp
80106594:	89 e5                	mov    %esp,%ebp
80106596:	83 ec 28             	sub    $0x28,%esp
  char *p;

  // Turn off the FIFO
  outb(COM1+2, 0);
80106599:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801065a0:	00 
801065a1:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
801065a8:	e8 c8 ff ff ff       	call   80106575 <outb>
  
  // 9600 baud, 8 data bits, 1 stop bit, parity off.
  outb(COM1+3, 0x80);    // Unlock divisor
801065ad:	c7 44 24 04 80 00 00 	movl   $0x80,0x4(%esp)
801065b4:	00 
801065b5:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
801065bc:	e8 b4 ff ff ff       	call   80106575 <outb>
  outb(COM1+0, 115200/9600);
801065c1:	c7 44 24 04 0c 00 00 	movl   $0xc,0x4(%esp)
801065c8:	00 
801065c9:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
801065d0:	e8 a0 ff ff ff       	call   80106575 <outb>
  outb(COM1+1, 0);
801065d5:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801065dc:	00 
801065dd:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
801065e4:	e8 8c ff ff ff       	call   80106575 <outb>
  outb(COM1+3, 0x03);    // Lock divisor, 8 data bits.
801065e9:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
801065f0:	00 
801065f1:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
801065f8:	e8 78 ff ff ff       	call   80106575 <outb>
  outb(COM1+4, 0);
801065fd:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106604:	00 
80106605:	c7 04 24 fc 03 00 00 	movl   $0x3fc,(%esp)
8010660c:	e8 64 ff ff ff       	call   80106575 <outb>
  outb(COM1+1, 0x01);    // Enable receive interrupts.
80106611:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80106618:	00 
80106619:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
80106620:	e8 50 ff ff ff       	call   80106575 <outb>

  // If status is 0xFF, no serial port.
  if(inb(COM1+5) == 0xFF)
80106625:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
8010662c:	e8 27 ff ff ff       	call   80106558 <inb>
80106631:	3c ff                	cmp    $0xff,%al
80106633:	75 02                	jne    80106637 <uartinit+0xa4>
    return;
80106635:	eb 6a                	jmp    801066a1 <uartinit+0x10e>
  uart = 1;
80106637:	c7 05 4c b6 10 80 01 	movl   $0x1,0x8010b64c
8010663e:	00 00 00 

  // Acknowledge pre-existing interrupt conditions;
  // enable interrupts.
  inb(COM1+2);
80106641:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80106648:	e8 0b ff ff ff       	call   80106558 <inb>
  inb(COM1+0);
8010664d:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106654:	e8 ff fe ff ff       	call   80106558 <inb>
  picenable(IRQ_COM1);
80106659:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80106660:	e8 fd d3 ff ff       	call   80103a62 <picenable>
  ioapicenable(IRQ_COM1, 0);
80106665:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010666c:	00 
8010666d:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80106674:	e8 e4 c2 ff ff       	call   8010295d <ioapicenable>
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80106679:	c7 45 f4 d4 85 10 80 	movl   $0x801085d4,-0xc(%ebp)
80106680:	eb 15                	jmp    80106697 <uartinit+0x104>
    uartputc(*p);
80106682:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106685:	0f b6 00             	movzbl (%eax),%eax
80106688:	0f be c0             	movsbl %al,%eax
8010668b:	89 04 24             	mov    %eax,(%esp)
8010668e:	e8 10 00 00 00       	call   801066a3 <uartputc>
  inb(COM1+0);
  picenable(IRQ_COM1);
  ioapicenable(IRQ_COM1, 0);
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80106693:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106697:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010669a:	0f b6 00             	movzbl (%eax),%eax
8010669d:	84 c0                	test   %al,%al
8010669f:	75 e1                	jne    80106682 <uartinit+0xef>
    uartputc(*p);
}
801066a1:	c9                   	leave  
801066a2:	c3                   	ret    

801066a3 <uartputc>:

void
uartputc(int c)
{
801066a3:	55                   	push   %ebp
801066a4:	89 e5                	mov    %esp,%ebp
801066a6:	83 ec 28             	sub    $0x28,%esp
  int i;

  if(!uart)
801066a9:	a1 4c b6 10 80       	mov    0x8010b64c,%eax
801066ae:	85 c0                	test   %eax,%eax
801066b0:	75 02                	jne    801066b4 <uartputc+0x11>
    return;
801066b2:	eb 4b                	jmp    801066ff <uartputc+0x5c>
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
801066b4:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801066bb:	eb 10                	jmp    801066cd <uartputc+0x2a>
    microdelay(10);
801066bd:	c7 04 24 0a 00 00 00 	movl   $0xa,(%esp)
801066c4:	e8 0f c8 ff ff       	call   80102ed8 <microdelay>
{
  int i;

  if(!uart)
    return;
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
801066c9:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801066cd:	83 7d f4 7f          	cmpl   $0x7f,-0xc(%ebp)
801066d1:	7f 16                	jg     801066e9 <uartputc+0x46>
801066d3:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
801066da:	e8 79 fe ff ff       	call   80106558 <inb>
801066df:	0f b6 c0             	movzbl %al,%eax
801066e2:	83 e0 20             	and    $0x20,%eax
801066e5:	85 c0                	test   %eax,%eax
801066e7:	74 d4                	je     801066bd <uartputc+0x1a>
    microdelay(10);
  outb(COM1+0, c);
801066e9:	8b 45 08             	mov    0x8(%ebp),%eax
801066ec:	0f b6 c0             	movzbl %al,%eax
801066ef:	89 44 24 04          	mov    %eax,0x4(%esp)
801066f3:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
801066fa:	e8 76 fe ff ff       	call   80106575 <outb>
}
801066ff:	c9                   	leave  
80106700:	c3                   	ret    

80106701 <uartgetc>:

static int
uartgetc(void)
{
80106701:	55                   	push   %ebp
80106702:	89 e5                	mov    %esp,%ebp
80106704:	83 ec 04             	sub    $0x4,%esp
  if(!uart)
80106707:	a1 4c b6 10 80       	mov    0x8010b64c,%eax
8010670c:	85 c0                	test   %eax,%eax
8010670e:	75 07                	jne    80106717 <uartgetc+0x16>
    return -1;
80106710:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106715:	eb 2c                	jmp    80106743 <uartgetc+0x42>
  if(!(inb(COM1+5) & 0x01))
80106717:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
8010671e:	e8 35 fe ff ff       	call   80106558 <inb>
80106723:	0f b6 c0             	movzbl %al,%eax
80106726:	83 e0 01             	and    $0x1,%eax
80106729:	85 c0                	test   %eax,%eax
8010672b:	75 07                	jne    80106734 <uartgetc+0x33>
    return -1;
8010672d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106732:	eb 0f                	jmp    80106743 <uartgetc+0x42>
  return inb(COM1+0);
80106734:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
8010673b:	e8 18 fe ff ff       	call   80106558 <inb>
80106740:	0f b6 c0             	movzbl %al,%eax
}
80106743:	c9                   	leave  
80106744:	c3                   	ret    

80106745 <uartintr>:

void
uartintr(void)
{
80106745:	55                   	push   %ebp
80106746:	89 e5                	mov    %esp,%ebp
80106748:	83 ec 18             	sub    $0x18,%esp
  consoleintr(uartgetc);
8010674b:	c7 04 24 01 67 10 80 	movl   $0x80106701,(%esp)
80106752:	e8 56 a0 ff ff       	call   801007ad <consoleintr>
}
80106757:	c9                   	leave  
80106758:	c3                   	ret    

80106759 <vector0>:
# generated by vectors.pl - do not edit
# handlers
.globl alltraps
.globl vector0
vector0:
  pushl $0
80106759:	6a 00                	push   $0x0
  pushl $0
8010675b:	6a 00                	push   $0x0
  jmp alltraps
8010675d:	e9 7e f9 ff ff       	jmp    801060e0 <alltraps>

80106762 <vector1>:
.globl vector1
vector1:
  pushl $0
80106762:	6a 00                	push   $0x0
  pushl $1
80106764:	6a 01                	push   $0x1
  jmp alltraps
80106766:	e9 75 f9 ff ff       	jmp    801060e0 <alltraps>

8010676b <vector2>:
.globl vector2
vector2:
  pushl $0
8010676b:	6a 00                	push   $0x0
  pushl $2
8010676d:	6a 02                	push   $0x2
  jmp alltraps
8010676f:	e9 6c f9 ff ff       	jmp    801060e0 <alltraps>

80106774 <vector3>:
.globl vector3
vector3:
  pushl $0
80106774:	6a 00                	push   $0x0
  pushl $3
80106776:	6a 03                	push   $0x3
  jmp alltraps
80106778:	e9 63 f9 ff ff       	jmp    801060e0 <alltraps>

8010677d <vector4>:
.globl vector4
vector4:
  pushl $0
8010677d:	6a 00                	push   $0x0
  pushl $4
8010677f:	6a 04                	push   $0x4
  jmp alltraps
80106781:	e9 5a f9 ff ff       	jmp    801060e0 <alltraps>

80106786 <vector5>:
.globl vector5
vector5:
  pushl $0
80106786:	6a 00                	push   $0x0
  pushl $5
80106788:	6a 05                	push   $0x5
  jmp alltraps
8010678a:	e9 51 f9 ff ff       	jmp    801060e0 <alltraps>

8010678f <vector6>:
.globl vector6
vector6:
  pushl $0
8010678f:	6a 00                	push   $0x0
  pushl $6
80106791:	6a 06                	push   $0x6
  jmp alltraps
80106793:	e9 48 f9 ff ff       	jmp    801060e0 <alltraps>

80106798 <vector7>:
.globl vector7
vector7:
  pushl $0
80106798:	6a 00                	push   $0x0
  pushl $7
8010679a:	6a 07                	push   $0x7
  jmp alltraps
8010679c:	e9 3f f9 ff ff       	jmp    801060e0 <alltraps>

801067a1 <vector8>:
.globl vector8
vector8:
  pushl $8
801067a1:	6a 08                	push   $0x8
  jmp alltraps
801067a3:	e9 38 f9 ff ff       	jmp    801060e0 <alltraps>

801067a8 <vector9>:
.globl vector9
vector9:
  pushl $0
801067a8:	6a 00                	push   $0x0
  pushl $9
801067aa:	6a 09                	push   $0x9
  jmp alltraps
801067ac:	e9 2f f9 ff ff       	jmp    801060e0 <alltraps>

801067b1 <vector10>:
.globl vector10
vector10:
  pushl $10
801067b1:	6a 0a                	push   $0xa
  jmp alltraps
801067b3:	e9 28 f9 ff ff       	jmp    801060e0 <alltraps>

801067b8 <vector11>:
.globl vector11
vector11:
  pushl $11
801067b8:	6a 0b                	push   $0xb
  jmp alltraps
801067ba:	e9 21 f9 ff ff       	jmp    801060e0 <alltraps>

801067bf <vector12>:
.globl vector12
vector12:
  pushl $12
801067bf:	6a 0c                	push   $0xc
  jmp alltraps
801067c1:	e9 1a f9 ff ff       	jmp    801060e0 <alltraps>

801067c6 <vector13>:
.globl vector13
vector13:
  pushl $13
801067c6:	6a 0d                	push   $0xd
  jmp alltraps
801067c8:	e9 13 f9 ff ff       	jmp    801060e0 <alltraps>

801067cd <vector14>:
.globl vector14
vector14:
  pushl $14
801067cd:	6a 0e                	push   $0xe
  jmp alltraps
801067cf:	e9 0c f9 ff ff       	jmp    801060e0 <alltraps>

801067d4 <vector15>:
.globl vector15
vector15:
  pushl $0
801067d4:	6a 00                	push   $0x0
  pushl $15
801067d6:	6a 0f                	push   $0xf
  jmp alltraps
801067d8:	e9 03 f9 ff ff       	jmp    801060e0 <alltraps>

801067dd <vector16>:
.globl vector16
vector16:
  pushl $0
801067dd:	6a 00                	push   $0x0
  pushl $16
801067df:	6a 10                	push   $0x10
  jmp alltraps
801067e1:	e9 fa f8 ff ff       	jmp    801060e0 <alltraps>

801067e6 <vector17>:
.globl vector17
vector17:
  pushl $17
801067e6:	6a 11                	push   $0x11
  jmp alltraps
801067e8:	e9 f3 f8 ff ff       	jmp    801060e0 <alltraps>

801067ed <vector18>:
.globl vector18
vector18:
  pushl $0
801067ed:	6a 00                	push   $0x0
  pushl $18
801067ef:	6a 12                	push   $0x12
  jmp alltraps
801067f1:	e9 ea f8 ff ff       	jmp    801060e0 <alltraps>

801067f6 <vector19>:
.globl vector19
vector19:
  pushl $0
801067f6:	6a 00                	push   $0x0
  pushl $19
801067f8:	6a 13                	push   $0x13
  jmp alltraps
801067fa:	e9 e1 f8 ff ff       	jmp    801060e0 <alltraps>

801067ff <vector20>:
.globl vector20
vector20:
  pushl $0
801067ff:	6a 00                	push   $0x0
  pushl $20
80106801:	6a 14                	push   $0x14
  jmp alltraps
80106803:	e9 d8 f8 ff ff       	jmp    801060e0 <alltraps>

80106808 <vector21>:
.globl vector21
vector21:
  pushl $0
80106808:	6a 00                	push   $0x0
  pushl $21
8010680a:	6a 15                	push   $0x15
  jmp alltraps
8010680c:	e9 cf f8 ff ff       	jmp    801060e0 <alltraps>

80106811 <vector22>:
.globl vector22
vector22:
  pushl $0
80106811:	6a 00                	push   $0x0
  pushl $22
80106813:	6a 16                	push   $0x16
  jmp alltraps
80106815:	e9 c6 f8 ff ff       	jmp    801060e0 <alltraps>

8010681a <vector23>:
.globl vector23
vector23:
  pushl $0
8010681a:	6a 00                	push   $0x0
  pushl $23
8010681c:	6a 17                	push   $0x17
  jmp alltraps
8010681e:	e9 bd f8 ff ff       	jmp    801060e0 <alltraps>

80106823 <vector24>:
.globl vector24
vector24:
  pushl $0
80106823:	6a 00                	push   $0x0
  pushl $24
80106825:	6a 18                	push   $0x18
  jmp alltraps
80106827:	e9 b4 f8 ff ff       	jmp    801060e0 <alltraps>

8010682c <vector25>:
.globl vector25
vector25:
  pushl $0
8010682c:	6a 00                	push   $0x0
  pushl $25
8010682e:	6a 19                	push   $0x19
  jmp alltraps
80106830:	e9 ab f8 ff ff       	jmp    801060e0 <alltraps>

80106835 <vector26>:
.globl vector26
vector26:
  pushl $0
80106835:	6a 00                	push   $0x0
  pushl $26
80106837:	6a 1a                	push   $0x1a
  jmp alltraps
80106839:	e9 a2 f8 ff ff       	jmp    801060e0 <alltraps>

8010683e <vector27>:
.globl vector27
vector27:
  pushl $0
8010683e:	6a 00                	push   $0x0
  pushl $27
80106840:	6a 1b                	push   $0x1b
  jmp alltraps
80106842:	e9 99 f8 ff ff       	jmp    801060e0 <alltraps>

80106847 <vector28>:
.globl vector28
vector28:
  pushl $0
80106847:	6a 00                	push   $0x0
  pushl $28
80106849:	6a 1c                	push   $0x1c
  jmp alltraps
8010684b:	e9 90 f8 ff ff       	jmp    801060e0 <alltraps>

80106850 <vector29>:
.globl vector29
vector29:
  pushl $0
80106850:	6a 00                	push   $0x0
  pushl $29
80106852:	6a 1d                	push   $0x1d
  jmp alltraps
80106854:	e9 87 f8 ff ff       	jmp    801060e0 <alltraps>

80106859 <vector30>:
.globl vector30
vector30:
  pushl $0
80106859:	6a 00                	push   $0x0
  pushl $30
8010685b:	6a 1e                	push   $0x1e
  jmp alltraps
8010685d:	e9 7e f8 ff ff       	jmp    801060e0 <alltraps>

80106862 <vector31>:
.globl vector31
vector31:
  pushl $0
80106862:	6a 00                	push   $0x0
  pushl $31
80106864:	6a 1f                	push   $0x1f
  jmp alltraps
80106866:	e9 75 f8 ff ff       	jmp    801060e0 <alltraps>

8010686b <vector32>:
.globl vector32
vector32:
  pushl $0
8010686b:	6a 00                	push   $0x0
  pushl $32
8010686d:	6a 20                	push   $0x20
  jmp alltraps
8010686f:	e9 6c f8 ff ff       	jmp    801060e0 <alltraps>

80106874 <vector33>:
.globl vector33
vector33:
  pushl $0
80106874:	6a 00                	push   $0x0
  pushl $33
80106876:	6a 21                	push   $0x21
  jmp alltraps
80106878:	e9 63 f8 ff ff       	jmp    801060e0 <alltraps>

8010687d <vector34>:
.globl vector34
vector34:
  pushl $0
8010687d:	6a 00                	push   $0x0
  pushl $34
8010687f:	6a 22                	push   $0x22
  jmp alltraps
80106881:	e9 5a f8 ff ff       	jmp    801060e0 <alltraps>

80106886 <vector35>:
.globl vector35
vector35:
  pushl $0
80106886:	6a 00                	push   $0x0
  pushl $35
80106888:	6a 23                	push   $0x23
  jmp alltraps
8010688a:	e9 51 f8 ff ff       	jmp    801060e0 <alltraps>

8010688f <vector36>:
.globl vector36
vector36:
  pushl $0
8010688f:	6a 00                	push   $0x0
  pushl $36
80106891:	6a 24                	push   $0x24
  jmp alltraps
80106893:	e9 48 f8 ff ff       	jmp    801060e0 <alltraps>

80106898 <vector37>:
.globl vector37
vector37:
  pushl $0
80106898:	6a 00                	push   $0x0
  pushl $37
8010689a:	6a 25                	push   $0x25
  jmp alltraps
8010689c:	e9 3f f8 ff ff       	jmp    801060e0 <alltraps>

801068a1 <vector38>:
.globl vector38
vector38:
  pushl $0
801068a1:	6a 00                	push   $0x0
  pushl $38
801068a3:	6a 26                	push   $0x26
  jmp alltraps
801068a5:	e9 36 f8 ff ff       	jmp    801060e0 <alltraps>

801068aa <vector39>:
.globl vector39
vector39:
  pushl $0
801068aa:	6a 00                	push   $0x0
  pushl $39
801068ac:	6a 27                	push   $0x27
  jmp alltraps
801068ae:	e9 2d f8 ff ff       	jmp    801060e0 <alltraps>

801068b3 <vector40>:
.globl vector40
vector40:
  pushl $0
801068b3:	6a 00                	push   $0x0
  pushl $40
801068b5:	6a 28                	push   $0x28
  jmp alltraps
801068b7:	e9 24 f8 ff ff       	jmp    801060e0 <alltraps>

801068bc <vector41>:
.globl vector41
vector41:
  pushl $0
801068bc:	6a 00                	push   $0x0
  pushl $41
801068be:	6a 29                	push   $0x29
  jmp alltraps
801068c0:	e9 1b f8 ff ff       	jmp    801060e0 <alltraps>

801068c5 <vector42>:
.globl vector42
vector42:
  pushl $0
801068c5:	6a 00                	push   $0x0
  pushl $42
801068c7:	6a 2a                	push   $0x2a
  jmp alltraps
801068c9:	e9 12 f8 ff ff       	jmp    801060e0 <alltraps>

801068ce <vector43>:
.globl vector43
vector43:
  pushl $0
801068ce:	6a 00                	push   $0x0
  pushl $43
801068d0:	6a 2b                	push   $0x2b
  jmp alltraps
801068d2:	e9 09 f8 ff ff       	jmp    801060e0 <alltraps>

801068d7 <vector44>:
.globl vector44
vector44:
  pushl $0
801068d7:	6a 00                	push   $0x0
  pushl $44
801068d9:	6a 2c                	push   $0x2c
  jmp alltraps
801068db:	e9 00 f8 ff ff       	jmp    801060e0 <alltraps>

801068e0 <vector45>:
.globl vector45
vector45:
  pushl $0
801068e0:	6a 00                	push   $0x0
  pushl $45
801068e2:	6a 2d                	push   $0x2d
  jmp alltraps
801068e4:	e9 f7 f7 ff ff       	jmp    801060e0 <alltraps>

801068e9 <vector46>:
.globl vector46
vector46:
  pushl $0
801068e9:	6a 00                	push   $0x0
  pushl $46
801068eb:	6a 2e                	push   $0x2e
  jmp alltraps
801068ed:	e9 ee f7 ff ff       	jmp    801060e0 <alltraps>

801068f2 <vector47>:
.globl vector47
vector47:
  pushl $0
801068f2:	6a 00                	push   $0x0
  pushl $47
801068f4:	6a 2f                	push   $0x2f
  jmp alltraps
801068f6:	e9 e5 f7 ff ff       	jmp    801060e0 <alltraps>

801068fb <vector48>:
.globl vector48
vector48:
  pushl $0
801068fb:	6a 00                	push   $0x0
  pushl $48
801068fd:	6a 30                	push   $0x30
  jmp alltraps
801068ff:	e9 dc f7 ff ff       	jmp    801060e0 <alltraps>

80106904 <vector49>:
.globl vector49
vector49:
  pushl $0
80106904:	6a 00                	push   $0x0
  pushl $49
80106906:	6a 31                	push   $0x31
  jmp alltraps
80106908:	e9 d3 f7 ff ff       	jmp    801060e0 <alltraps>

8010690d <vector50>:
.globl vector50
vector50:
  pushl $0
8010690d:	6a 00                	push   $0x0
  pushl $50
8010690f:	6a 32                	push   $0x32
  jmp alltraps
80106911:	e9 ca f7 ff ff       	jmp    801060e0 <alltraps>

80106916 <vector51>:
.globl vector51
vector51:
  pushl $0
80106916:	6a 00                	push   $0x0
  pushl $51
80106918:	6a 33                	push   $0x33
  jmp alltraps
8010691a:	e9 c1 f7 ff ff       	jmp    801060e0 <alltraps>

8010691f <vector52>:
.globl vector52
vector52:
  pushl $0
8010691f:	6a 00                	push   $0x0
  pushl $52
80106921:	6a 34                	push   $0x34
  jmp alltraps
80106923:	e9 b8 f7 ff ff       	jmp    801060e0 <alltraps>

80106928 <vector53>:
.globl vector53
vector53:
  pushl $0
80106928:	6a 00                	push   $0x0
  pushl $53
8010692a:	6a 35                	push   $0x35
  jmp alltraps
8010692c:	e9 af f7 ff ff       	jmp    801060e0 <alltraps>

80106931 <vector54>:
.globl vector54
vector54:
  pushl $0
80106931:	6a 00                	push   $0x0
  pushl $54
80106933:	6a 36                	push   $0x36
  jmp alltraps
80106935:	e9 a6 f7 ff ff       	jmp    801060e0 <alltraps>

8010693a <vector55>:
.globl vector55
vector55:
  pushl $0
8010693a:	6a 00                	push   $0x0
  pushl $55
8010693c:	6a 37                	push   $0x37
  jmp alltraps
8010693e:	e9 9d f7 ff ff       	jmp    801060e0 <alltraps>

80106943 <vector56>:
.globl vector56
vector56:
  pushl $0
80106943:	6a 00                	push   $0x0
  pushl $56
80106945:	6a 38                	push   $0x38
  jmp alltraps
80106947:	e9 94 f7 ff ff       	jmp    801060e0 <alltraps>

8010694c <vector57>:
.globl vector57
vector57:
  pushl $0
8010694c:	6a 00                	push   $0x0
  pushl $57
8010694e:	6a 39                	push   $0x39
  jmp alltraps
80106950:	e9 8b f7 ff ff       	jmp    801060e0 <alltraps>

80106955 <vector58>:
.globl vector58
vector58:
  pushl $0
80106955:	6a 00                	push   $0x0
  pushl $58
80106957:	6a 3a                	push   $0x3a
  jmp alltraps
80106959:	e9 82 f7 ff ff       	jmp    801060e0 <alltraps>

8010695e <vector59>:
.globl vector59
vector59:
  pushl $0
8010695e:	6a 00                	push   $0x0
  pushl $59
80106960:	6a 3b                	push   $0x3b
  jmp alltraps
80106962:	e9 79 f7 ff ff       	jmp    801060e0 <alltraps>

80106967 <vector60>:
.globl vector60
vector60:
  pushl $0
80106967:	6a 00                	push   $0x0
  pushl $60
80106969:	6a 3c                	push   $0x3c
  jmp alltraps
8010696b:	e9 70 f7 ff ff       	jmp    801060e0 <alltraps>

80106970 <vector61>:
.globl vector61
vector61:
  pushl $0
80106970:	6a 00                	push   $0x0
  pushl $61
80106972:	6a 3d                	push   $0x3d
  jmp alltraps
80106974:	e9 67 f7 ff ff       	jmp    801060e0 <alltraps>

80106979 <vector62>:
.globl vector62
vector62:
  pushl $0
80106979:	6a 00                	push   $0x0
  pushl $62
8010697b:	6a 3e                	push   $0x3e
  jmp alltraps
8010697d:	e9 5e f7 ff ff       	jmp    801060e0 <alltraps>

80106982 <vector63>:
.globl vector63
vector63:
  pushl $0
80106982:	6a 00                	push   $0x0
  pushl $63
80106984:	6a 3f                	push   $0x3f
  jmp alltraps
80106986:	e9 55 f7 ff ff       	jmp    801060e0 <alltraps>

8010698b <vector64>:
.globl vector64
vector64:
  pushl $0
8010698b:	6a 00                	push   $0x0
  pushl $64
8010698d:	6a 40                	push   $0x40
  jmp alltraps
8010698f:	e9 4c f7 ff ff       	jmp    801060e0 <alltraps>

80106994 <vector65>:
.globl vector65
vector65:
  pushl $0
80106994:	6a 00                	push   $0x0
  pushl $65
80106996:	6a 41                	push   $0x41
  jmp alltraps
80106998:	e9 43 f7 ff ff       	jmp    801060e0 <alltraps>

8010699d <vector66>:
.globl vector66
vector66:
  pushl $0
8010699d:	6a 00                	push   $0x0
  pushl $66
8010699f:	6a 42                	push   $0x42
  jmp alltraps
801069a1:	e9 3a f7 ff ff       	jmp    801060e0 <alltraps>

801069a6 <vector67>:
.globl vector67
vector67:
  pushl $0
801069a6:	6a 00                	push   $0x0
  pushl $67
801069a8:	6a 43                	push   $0x43
  jmp alltraps
801069aa:	e9 31 f7 ff ff       	jmp    801060e0 <alltraps>

801069af <vector68>:
.globl vector68
vector68:
  pushl $0
801069af:	6a 00                	push   $0x0
  pushl $68
801069b1:	6a 44                	push   $0x44
  jmp alltraps
801069b3:	e9 28 f7 ff ff       	jmp    801060e0 <alltraps>

801069b8 <vector69>:
.globl vector69
vector69:
  pushl $0
801069b8:	6a 00                	push   $0x0
  pushl $69
801069ba:	6a 45                	push   $0x45
  jmp alltraps
801069bc:	e9 1f f7 ff ff       	jmp    801060e0 <alltraps>

801069c1 <vector70>:
.globl vector70
vector70:
  pushl $0
801069c1:	6a 00                	push   $0x0
  pushl $70
801069c3:	6a 46                	push   $0x46
  jmp alltraps
801069c5:	e9 16 f7 ff ff       	jmp    801060e0 <alltraps>

801069ca <vector71>:
.globl vector71
vector71:
  pushl $0
801069ca:	6a 00                	push   $0x0
  pushl $71
801069cc:	6a 47                	push   $0x47
  jmp alltraps
801069ce:	e9 0d f7 ff ff       	jmp    801060e0 <alltraps>

801069d3 <vector72>:
.globl vector72
vector72:
  pushl $0
801069d3:	6a 00                	push   $0x0
  pushl $72
801069d5:	6a 48                	push   $0x48
  jmp alltraps
801069d7:	e9 04 f7 ff ff       	jmp    801060e0 <alltraps>

801069dc <vector73>:
.globl vector73
vector73:
  pushl $0
801069dc:	6a 00                	push   $0x0
  pushl $73
801069de:	6a 49                	push   $0x49
  jmp alltraps
801069e0:	e9 fb f6 ff ff       	jmp    801060e0 <alltraps>

801069e5 <vector74>:
.globl vector74
vector74:
  pushl $0
801069e5:	6a 00                	push   $0x0
  pushl $74
801069e7:	6a 4a                	push   $0x4a
  jmp alltraps
801069e9:	e9 f2 f6 ff ff       	jmp    801060e0 <alltraps>

801069ee <vector75>:
.globl vector75
vector75:
  pushl $0
801069ee:	6a 00                	push   $0x0
  pushl $75
801069f0:	6a 4b                	push   $0x4b
  jmp alltraps
801069f2:	e9 e9 f6 ff ff       	jmp    801060e0 <alltraps>

801069f7 <vector76>:
.globl vector76
vector76:
  pushl $0
801069f7:	6a 00                	push   $0x0
  pushl $76
801069f9:	6a 4c                	push   $0x4c
  jmp alltraps
801069fb:	e9 e0 f6 ff ff       	jmp    801060e0 <alltraps>

80106a00 <vector77>:
.globl vector77
vector77:
  pushl $0
80106a00:	6a 00                	push   $0x0
  pushl $77
80106a02:	6a 4d                	push   $0x4d
  jmp alltraps
80106a04:	e9 d7 f6 ff ff       	jmp    801060e0 <alltraps>

80106a09 <vector78>:
.globl vector78
vector78:
  pushl $0
80106a09:	6a 00                	push   $0x0
  pushl $78
80106a0b:	6a 4e                	push   $0x4e
  jmp alltraps
80106a0d:	e9 ce f6 ff ff       	jmp    801060e0 <alltraps>

80106a12 <vector79>:
.globl vector79
vector79:
  pushl $0
80106a12:	6a 00                	push   $0x0
  pushl $79
80106a14:	6a 4f                	push   $0x4f
  jmp alltraps
80106a16:	e9 c5 f6 ff ff       	jmp    801060e0 <alltraps>

80106a1b <vector80>:
.globl vector80
vector80:
  pushl $0
80106a1b:	6a 00                	push   $0x0
  pushl $80
80106a1d:	6a 50                	push   $0x50
  jmp alltraps
80106a1f:	e9 bc f6 ff ff       	jmp    801060e0 <alltraps>

80106a24 <vector81>:
.globl vector81
vector81:
  pushl $0
80106a24:	6a 00                	push   $0x0
  pushl $81
80106a26:	6a 51                	push   $0x51
  jmp alltraps
80106a28:	e9 b3 f6 ff ff       	jmp    801060e0 <alltraps>

80106a2d <vector82>:
.globl vector82
vector82:
  pushl $0
80106a2d:	6a 00                	push   $0x0
  pushl $82
80106a2f:	6a 52                	push   $0x52
  jmp alltraps
80106a31:	e9 aa f6 ff ff       	jmp    801060e0 <alltraps>

80106a36 <vector83>:
.globl vector83
vector83:
  pushl $0
80106a36:	6a 00                	push   $0x0
  pushl $83
80106a38:	6a 53                	push   $0x53
  jmp alltraps
80106a3a:	e9 a1 f6 ff ff       	jmp    801060e0 <alltraps>

80106a3f <vector84>:
.globl vector84
vector84:
  pushl $0
80106a3f:	6a 00                	push   $0x0
  pushl $84
80106a41:	6a 54                	push   $0x54
  jmp alltraps
80106a43:	e9 98 f6 ff ff       	jmp    801060e0 <alltraps>

80106a48 <vector85>:
.globl vector85
vector85:
  pushl $0
80106a48:	6a 00                	push   $0x0
  pushl $85
80106a4a:	6a 55                	push   $0x55
  jmp alltraps
80106a4c:	e9 8f f6 ff ff       	jmp    801060e0 <alltraps>

80106a51 <vector86>:
.globl vector86
vector86:
  pushl $0
80106a51:	6a 00                	push   $0x0
  pushl $86
80106a53:	6a 56                	push   $0x56
  jmp alltraps
80106a55:	e9 86 f6 ff ff       	jmp    801060e0 <alltraps>

80106a5a <vector87>:
.globl vector87
vector87:
  pushl $0
80106a5a:	6a 00                	push   $0x0
  pushl $87
80106a5c:	6a 57                	push   $0x57
  jmp alltraps
80106a5e:	e9 7d f6 ff ff       	jmp    801060e0 <alltraps>

80106a63 <vector88>:
.globl vector88
vector88:
  pushl $0
80106a63:	6a 00                	push   $0x0
  pushl $88
80106a65:	6a 58                	push   $0x58
  jmp alltraps
80106a67:	e9 74 f6 ff ff       	jmp    801060e0 <alltraps>

80106a6c <vector89>:
.globl vector89
vector89:
  pushl $0
80106a6c:	6a 00                	push   $0x0
  pushl $89
80106a6e:	6a 59                	push   $0x59
  jmp alltraps
80106a70:	e9 6b f6 ff ff       	jmp    801060e0 <alltraps>

80106a75 <vector90>:
.globl vector90
vector90:
  pushl $0
80106a75:	6a 00                	push   $0x0
  pushl $90
80106a77:	6a 5a                	push   $0x5a
  jmp alltraps
80106a79:	e9 62 f6 ff ff       	jmp    801060e0 <alltraps>

80106a7e <vector91>:
.globl vector91
vector91:
  pushl $0
80106a7e:	6a 00                	push   $0x0
  pushl $91
80106a80:	6a 5b                	push   $0x5b
  jmp alltraps
80106a82:	e9 59 f6 ff ff       	jmp    801060e0 <alltraps>

80106a87 <vector92>:
.globl vector92
vector92:
  pushl $0
80106a87:	6a 00                	push   $0x0
  pushl $92
80106a89:	6a 5c                	push   $0x5c
  jmp alltraps
80106a8b:	e9 50 f6 ff ff       	jmp    801060e0 <alltraps>

80106a90 <vector93>:
.globl vector93
vector93:
  pushl $0
80106a90:	6a 00                	push   $0x0
  pushl $93
80106a92:	6a 5d                	push   $0x5d
  jmp alltraps
80106a94:	e9 47 f6 ff ff       	jmp    801060e0 <alltraps>

80106a99 <vector94>:
.globl vector94
vector94:
  pushl $0
80106a99:	6a 00                	push   $0x0
  pushl $94
80106a9b:	6a 5e                	push   $0x5e
  jmp alltraps
80106a9d:	e9 3e f6 ff ff       	jmp    801060e0 <alltraps>

80106aa2 <vector95>:
.globl vector95
vector95:
  pushl $0
80106aa2:	6a 00                	push   $0x0
  pushl $95
80106aa4:	6a 5f                	push   $0x5f
  jmp alltraps
80106aa6:	e9 35 f6 ff ff       	jmp    801060e0 <alltraps>

80106aab <vector96>:
.globl vector96
vector96:
  pushl $0
80106aab:	6a 00                	push   $0x0
  pushl $96
80106aad:	6a 60                	push   $0x60
  jmp alltraps
80106aaf:	e9 2c f6 ff ff       	jmp    801060e0 <alltraps>

80106ab4 <vector97>:
.globl vector97
vector97:
  pushl $0
80106ab4:	6a 00                	push   $0x0
  pushl $97
80106ab6:	6a 61                	push   $0x61
  jmp alltraps
80106ab8:	e9 23 f6 ff ff       	jmp    801060e0 <alltraps>

80106abd <vector98>:
.globl vector98
vector98:
  pushl $0
80106abd:	6a 00                	push   $0x0
  pushl $98
80106abf:	6a 62                	push   $0x62
  jmp alltraps
80106ac1:	e9 1a f6 ff ff       	jmp    801060e0 <alltraps>

80106ac6 <vector99>:
.globl vector99
vector99:
  pushl $0
80106ac6:	6a 00                	push   $0x0
  pushl $99
80106ac8:	6a 63                	push   $0x63
  jmp alltraps
80106aca:	e9 11 f6 ff ff       	jmp    801060e0 <alltraps>

80106acf <vector100>:
.globl vector100
vector100:
  pushl $0
80106acf:	6a 00                	push   $0x0
  pushl $100
80106ad1:	6a 64                	push   $0x64
  jmp alltraps
80106ad3:	e9 08 f6 ff ff       	jmp    801060e0 <alltraps>

80106ad8 <vector101>:
.globl vector101
vector101:
  pushl $0
80106ad8:	6a 00                	push   $0x0
  pushl $101
80106ada:	6a 65                	push   $0x65
  jmp alltraps
80106adc:	e9 ff f5 ff ff       	jmp    801060e0 <alltraps>

80106ae1 <vector102>:
.globl vector102
vector102:
  pushl $0
80106ae1:	6a 00                	push   $0x0
  pushl $102
80106ae3:	6a 66                	push   $0x66
  jmp alltraps
80106ae5:	e9 f6 f5 ff ff       	jmp    801060e0 <alltraps>

80106aea <vector103>:
.globl vector103
vector103:
  pushl $0
80106aea:	6a 00                	push   $0x0
  pushl $103
80106aec:	6a 67                	push   $0x67
  jmp alltraps
80106aee:	e9 ed f5 ff ff       	jmp    801060e0 <alltraps>

80106af3 <vector104>:
.globl vector104
vector104:
  pushl $0
80106af3:	6a 00                	push   $0x0
  pushl $104
80106af5:	6a 68                	push   $0x68
  jmp alltraps
80106af7:	e9 e4 f5 ff ff       	jmp    801060e0 <alltraps>

80106afc <vector105>:
.globl vector105
vector105:
  pushl $0
80106afc:	6a 00                	push   $0x0
  pushl $105
80106afe:	6a 69                	push   $0x69
  jmp alltraps
80106b00:	e9 db f5 ff ff       	jmp    801060e0 <alltraps>

80106b05 <vector106>:
.globl vector106
vector106:
  pushl $0
80106b05:	6a 00                	push   $0x0
  pushl $106
80106b07:	6a 6a                	push   $0x6a
  jmp alltraps
80106b09:	e9 d2 f5 ff ff       	jmp    801060e0 <alltraps>

80106b0e <vector107>:
.globl vector107
vector107:
  pushl $0
80106b0e:	6a 00                	push   $0x0
  pushl $107
80106b10:	6a 6b                	push   $0x6b
  jmp alltraps
80106b12:	e9 c9 f5 ff ff       	jmp    801060e0 <alltraps>

80106b17 <vector108>:
.globl vector108
vector108:
  pushl $0
80106b17:	6a 00                	push   $0x0
  pushl $108
80106b19:	6a 6c                	push   $0x6c
  jmp alltraps
80106b1b:	e9 c0 f5 ff ff       	jmp    801060e0 <alltraps>

80106b20 <vector109>:
.globl vector109
vector109:
  pushl $0
80106b20:	6a 00                	push   $0x0
  pushl $109
80106b22:	6a 6d                	push   $0x6d
  jmp alltraps
80106b24:	e9 b7 f5 ff ff       	jmp    801060e0 <alltraps>

80106b29 <vector110>:
.globl vector110
vector110:
  pushl $0
80106b29:	6a 00                	push   $0x0
  pushl $110
80106b2b:	6a 6e                	push   $0x6e
  jmp alltraps
80106b2d:	e9 ae f5 ff ff       	jmp    801060e0 <alltraps>

80106b32 <vector111>:
.globl vector111
vector111:
  pushl $0
80106b32:	6a 00                	push   $0x0
  pushl $111
80106b34:	6a 6f                	push   $0x6f
  jmp alltraps
80106b36:	e9 a5 f5 ff ff       	jmp    801060e0 <alltraps>

80106b3b <vector112>:
.globl vector112
vector112:
  pushl $0
80106b3b:	6a 00                	push   $0x0
  pushl $112
80106b3d:	6a 70                	push   $0x70
  jmp alltraps
80106b3f:	e9 9c f5 ff ff       	jmp    801060e0 <alltraps>

80106b44 <vector113>:
.globl vector113
vector113:
  pushl $0
80106b44:	6a 00                	push   $0x0
  pushl $113
80106b46:	6a 71                	push   $0x71
  jmp alltraps
80106b48:	e9 93 f5 ff ff       	jmp    801060e0 <alltraps>

80106b4d <vector114>:
.globl vector114
vector114:
  pushl $0
80106b4d:	6a 00                	push   $0x0
  pushl $114
80106b4f:	6a 72                	push   $0x72
  jmp alltraps
80106b51:	e9 8a f5 ff ff       	jmp    801060e0 <alltraps>

80106b56 <vector115>:
.globl vector115
vector115:
  pushl $0
80106b56:	6a 00                	push   $0x0
  pushl $115
80106b58:	6a 73                	push   $0x73
  jmp alltraps
80106b5a:	e9 81 f5 ff ff       	jmp    801060e0 <alltraps>

80106b5f <vector116>:
.globl vector116
vector116:
  pushl $0
80106b5f:	6a 00                	push   $0x0
  pushl $116
80106b61:	6a 74                	push   $0x74
  jmp alltraps
80106b63:	e9 78 f5 ff ff       	jmp    801060e0 <alltraps>

80106b68 <vector117>:
.globl vector117
vector117:
  pushl $0
80106b68:	6a 00                	push   $0x0
  pushl $117
80106b6a:	6a 75                	push   $0x75
  jmp alltraps
80106b6c:	e9 6f f5 ff ff       	jmp    801060e0 <alltraps>

80106b71 <vector118>:
.globl vector118
vector118:
  pushl $0
80106b71:	6a 00                	push   $0x0
  pushl $118
80106b73:	6a 76                	push   $0x76
  jmp alltraps
80106b75:	e9 66 f5 ff ff       	jmp    801060e0 <alltraps>

80106b7a <vector119>:
.globl vector119
vector119:
  pushl $0
80106b7a:	6a 00                	push   $0x0
  pushl $119
80106b7c:	6a 77                	push   $0x77
  jmp alltraps
80106b7e:	e9 5d f5 ff ff       	jmp    801060e0 <alltraps>

80106b83 <vector120>:
.globl vector120
vector120:
  pushl $0
80106b83:	6a 00                	push   $0x0
  pushl $120
80106b85:	6a 78                	push   $0x78
  jmp alltraps
80106b87:	e9 54 f5 ff ff       	jmp    801060e0 <alltraps>

80106b8c <vector121>:
.globl vector121
vector121:
  pushl $0
80106b8c:	6a 00                	push   $0x0
  pushl $121
80106b8e:	6a 79                	push   $0x79
  jmp alltraps
80106b90:	e9 4b f5 ff ff       	jmp    801060e0 <alltraps>

80106b95 <vector122>:
.globl vector122
vector122:
  pushl $0
80106b95:	6a 00                	push   $0x0
  pushl $122
80106b97:	6a 7a                	push   $0x7a
  jmp alltraps
80106b99:	e9 42 f5 ff ff       	jmp    801060e0 <alltraps>

80106b9e <vector123>:
.globl vector123
vector123:
  pushl $0
80106b9e:	6a 00                	push   $0x0
  pushl $123
80106ba0:	6a 7b                	push   $0x7b
  jmp alltraps
80106ba2:	e9 39 f5 ff ff       	jmp    801060e0 <alltraps>

80106ba7 <vector124>:
.globl vector124
vector124:
  pushl $0
80106ba7:	6a 00                	push   $0x0
  pushl $124
80106ba9:	6a 7c                	push   $0x7c
  jmp alltraps
80106bab:	e9 30 f5 ff ff       	jmp    801060e0 <alltraps>

80106bb0 <vector125>:
.globl vector125
vector125:
  pushl $0
80106bb0:	6a 00                	push   $0x0
  pushl $125
80106bb2:	6a 7d                	push   $0x7d
  jmp alltraps
80106bb4:	e9 27 f5 ff ff       	jmp    801060e0 <alltraps>

80106bb9 <vector126>:
.globl vector126
vector126:
  pushl $0
80106bb9:	6a 00                	push   $0x0
  pushl $126
80106bbb:	6a 7e                	push   $0x7e
  jmp alltraps
80106bbd:	e9 1e f5 ff ff       	jmp    801060e0 <alltraps>

80106bc2 <vector127>:
.globl vector127
vector127:
  pushl $0
80106bc2:	6a 00                	push   $0x0
  pushl $127
80106bc4:	6a 7f                	push   $0x7f
  jmp alltraps
80106bc6:	e9 15 f5 ff ff       	jmp    801060e0 <alltraps>

80106bcb <vector128>:
.globl vector128
vector128:
  pushl $0
80106bcb:	6a 00                	push   $0x0
  pushl $128
80106bcd:	68 80 00 00 00       	push   $0x80
  jmp alltraps
80106bd2:	e9 09 f5 ff ff       	jmp    801060e0 <alltraps>

80106bd7 <vector129>:
.globl vector129
vector129:
  pushl $0
80106bd7:	6a 00                	push   $0x0
  pushl $129
80106bd9:	68 81 00 00 00       	push   $0x81
  jmp alltraps
80106bde:	e9 fd f4 ff ff       	jmp    801060e0 <alltraps>

80106be3 <vector130>:
.globl vector130
vector130:
  pushl $0
80106be3:	6a 00                	push   $0x0
  pushl $130
80106be5:	68 82 00 00 00       	push   $0x82
  jmp alltraps
80106bea:	e9 f1 f4 ff ff       	jmp    801060e0 <alltraps>

80106bef <vector131>:
.globl vector131
vector131:
  pushl $0
80106bef:	6a 00                	push   $0x0
  pushl $131
80106bf1:	68 83 00 00 00       	push   $0x83
  jmp alltraps
80106bf6:	e9 e5 f4 ff ff       	jmp    801060e0 <alltraps>

80106bfb <vector132>:
.globl vector132
vector132:
  pushl $0
80106bfb:	6a 00                	push   $0x0
  pushl $132
80106bfd:	68 84 00 00 00       	push   $0x84
  jmp alltraps
80106c02:	e9 d9 f4 ff ff       	jmp    801060e0 <alltraps>

80106c07 <vector133>:
.globl vector133
vector133:
  pushl $0
80106c07:	6a 00                	push   $0x0
  pushl $133
80106c09:	68 85 00 00 00       	push   $0x85
  jmp alltraps
80106c0e:	e9 cd f4 ff ff       	jmp    801060e0 <alltraps>

80106c13 <vector134>:
.globl vector134
vector134:
  pushl $0
80106c13:	6a 00                	push   $0x0
  pushl $134
80106c15:	68 86 00 00 00       	push   $0x86
  jmp alltraps
80106c1a:	e9 c1 f4 ff ff       	jmp    801060e0 <alltraps>

80106c1f <vector135>:
.globl vector135
vector135:
  pushl $0
80106c1f:	6a 00                	push   $0x0
  pushl $135
80106c21:	68 87 00 00 00       	push   $0x87
  jmp alltraps
80106c26:	e9 b5 f4 ff ff       	jmp    801060e0 <alltraps>

80106c2b <vector136>:
.globl vector136
vector136:
  pushl $0
80106c2b:	6a 00                	push   $0x0
  pushl $136
80106c2d:	68 88 00 00 00       	push   $0x88
  jmp alltraps
80106c32:	e9 a9 f4 ff ff       	jmp    801060e0 <alltraps>

80106c37 <vector137>:
.globl vector137
vector137:
  pushl $0
80106c37:	6a 00                	push   $0x0
  pushl $137
80106c39:	68 89 00 00 00       	push   $0x89
  jmp alltraps
80106c3e:	e9 9d f4 ff ff       	jmp    801060e0 <alltraps>

80106c43 <vector138>:
.globl vector138
vector138:
  pushl $0
80106c43:	6a 00                	push   $0x0
  pushl $138
80106c45:	68 8a 00 00 00       	push   $0x8a
  jmp alltraps
80106c4a:	e9 91 f4 ff ff       	jmp    801060e0 <alltraps>

80106c4f <vector139>:
.globl vector139
vector139:
  pushl $0
80106c4f:	6a 00                	push   $0x0
  pushl $139
80106c51:	68 8b 00 00 00       	push   $0x8b
  jmp alltraps
80106c56:	e9 85 f4 ff ff       	jmp    801060e0 <alltraps>

80106c5b <vector140>:
.globl vector140
vector140:
  pushl $0
80106c5b:	6a 00                	push   $0x0
  pushl $140
80106c5d:	68 8c 00 00 00       	push   $0x8c
  jmp alltraps
80106c62:	e9 79 f4 ff ff       	jmp    801060e0 <alltraps>

80106c67 <vector141>:
.globl vector141
vector141:
  pushl $0
80106c67:	6a 00                	push   $0x0
  pushl $141
80106c69:	68 8d 00 00 00       	push   $0x8d
  jmp alltraps
80106c6e:	e9 6d f4 ff ff       	jmp    801060e0 <alltraps>

80106c73 <vector142>:
.globl vector142
vector142:
  pushl $0
80106c73:	6a 00                	push   $0x0
  pushl $142
80106c75:	68 8e 00 00 00       	push   $0x8e
  jmp alltraps
80106c7a:	e9 61 f4 ff ff       	jmp    801060e0 <alltraps>

80106c7f <vector143>:
.globl vector143
vector143:
  pushl $0
80106c7f:	6a 00                	push   $0x0
  pushl $143
80106c81:	68 8f 00 00 00       	push   $0x8f
  jmp alltraps
80106c86:	e9 55 f4 ff ff       	jmp    801060e0 <alltraps>

80106c8b <vector144>:
.globl vector144
vector144:
  pushl $0
80106c8b:	6a 00                	push   $0x0
  pushl $144
80106c8d:	68 90 00 00 00       	push   $0x90
  jmp alltraps
80106c92:	e9 49 f4 ff ff       	jmp    801060e0 <alltraps>

80106c97 <vector145>:
.globl vector145
vector145:
  pushl $0
80106c97:	6a 00                	push   $0x0
  pushl $145
80106c99:	68 91 00 00 00       	push   $0x91
  jmp alltraps
80106c9e:	e9 3d f4 ff ff       	jmp    801060e0 <alltraps>

80106ca3 <vector146>:
.globl vector146
vector146:
  pushl $0
80106ca3:	6a 00                	push   $0x0
  pushl $146
80106ca5:	68 92 00 00 00       	push   $0x92
  jmp alltraps
80106caa:	e9 31 f4 ff ff       	jmp    801060e0 <alltraps>

80106caf <vector147>:
.globl vector147
vector147:
  pushl $0
80106caf:	6a 00                	push   $0x0
  pushl $147
80106cb1:	68 93 00 00 00       	push   $0x93
  jmp alltraps
80106cb6:	e9 25 f4 ff ff       	jmp    801060e0 <alltraps>

80106cbb <vector148>:
.globl vector148
vector148:
  pushl $0
80106cbb:	6a 00                	push   $0x0
  pushl $148
80106cbd:	68 94 00 00 00       	push   $0x94
  jmp alltraps
80106cc2:	e9 19 f4 ff ff       	jmp    801060e0 <alltraps>

80106cc7 <vector149>:
.globl vector149
vector149:
  pushl $0
80106cc7:	6a 00                	push   $0x0
  pushl $149
80106cc9:	68 95 00 00 00       	push   $0x95
  jmp alltraps
80106cce:	e9 0d f4 ff ff       	jmp    801060e0 <alltraps>

80106cd3 <vector150>:
.globl vector150
vector150:
  pushl $0
80106cd3:	6a 00                	push   $0x0
  pushl $150
80106cd5:	68 96 00 00 00       	push   $0x96
  jmp alltraps
80106cda:	e9 01 f4 ff ff       	jmp    801060e0 <alltraps>

80106cdf <vector151>:
.globl vector151
vector151:
  pushl $0
80106cdf:	6a 00                	push   $0x0
  pushl $151
80106ce1:	68 97 00 00 00       	push   $0x97
  jmp alltraps
80106ce6:	e9 f5 f3 ff ff       	jmp    801060e0 <alltraps>

80106ceb <vector152>:
.globl vector152
vector152:
  pushl $0
80106ceb:	6a 00                	push   $0x0
  pushl $152
80106ced:	68 98 00 00 00       	push   $0x98
  jmp alltraps
80106cf2:	e9 e9 f3 ff ff       	jmp    801060e0 <alltraps>

80106cf7 <vector153>:
.globl vector153
vector153:
  pushl $0
80106cf7:	6a 00                	push   $0x0
  pushl $153
80106cf9:	68 99 00 00 00       	push   $0x99
  jmp alltraps
80106cfe:	e9 dd f3 ff ff       	jmp    801060e0 <alltraps>

80106d03 <vector154>:
.globl vector154
vector154:
  pushl $0
80106d03:	6a 00                	push   $0x0
  pushl $154
80106d05:	68 9a 00 00 00       	push   $0x9a
  jmp alltraps
80106d0a:	e9 d1 f3 ff ff       	jmp    801060e0 <alltraps>

80106d0f <vector155>:
.globl vector155
vector155:
  pushl $0
80106d0f:	6a 00                	push   $0x0
  pushl $155
80106d11:	68 9b 00 00 00       	push   $0x9b
  jmp alltraps
80106d16:	e9 c5 f3 ff ff       	jmp    801060e0 <alltraps>

80106d1b <vector156>:
.globl vector156
vector156:
  pushl $0
80106d1b:	6a 00                	push   $0x0
  pushl $156
80106d1d:	68 9c 00 00 00       	push   $0x9c
  jmp alltraps
80106d22:	e9 b9 f3 ff ff       	jmp    801060e0 <alltraps>

80106d27 <vector157>:
.globl vector157
vector157:
  pushl $0
80106d27:	6a 00                	push   $0x0
  pushl $157
80106d29:	68 9d 00 00 00       	push   $0x9d
  jmp alltraps
80106d2e:	e9 ad f3 ff ff       	jmp    801060e0 <alltraps>

80106d33 <vector158>:
.globl vector158
vector158:
  pushl $0
80106d33:	6a 00                	push   $0x0
  pushl $158
80106d35:	68 9e 00 00 00       	push   $0x9e
  jmp alltraps
80106d3a:	e9 a1 f3 ff ff       	jmp    801060e0 <alltraps>

80106d3f <vector159>:
.globl vector159
vector159:
  pushl $0
80106d3f:	6a 00                	push   $0x0
  pushl $159
80106d41:	68 9f 00 00 00       	push   $0x9f
  jmp alltraps
80106d46:	e9 95 f3 ff ff       	jmp    801060e0 <alltraps>

80106d4b <vector160>:
.globl vector160
vector160:
  pushl $0
80106d4b:	6a 00                	push   $0x0
  pushl $160
80106d4d:	68 a0 00 00 00       	push   $0xa0
  jmp alltraps
80106d52:	e9 89 f3 ff ff       	jmp    801060e0 <alltraps>

80106d57 <vector161>:
.globl vector161
vector161:
  pushl $0
80106d57:	6a 00                	push   $0x0
  pushl $161
80106d59:	68 a1 00 00 00       	push   $0xa1
  jmp alltraps
80106d5e:	e9 7d f3 ff ff       	jmp    801060e0 <alltraps>

80106d63 <vector162>:
.globl vector162
vector162:
  pushl $0
80106d63:	6a 00                	push   $0x0
  pushl $162
80106d65:	68 a2 00 00 00       	push   $0xa2
  jmp alltraps
80106d6a:	e9 71 f3 ff ff       	jmp    801060e0 <alltraps>

80106d6f <vector163>:
.globl vector163
vector163:
  pushl $0
80106d6f:	6a 00                	push   $0x0
  pushl $163
80106d71:	68 a3 00 00 00       	push   $0xa3
  jmp alltraps
80106d76:	e9 65 f3 ff ff       	jmp    801060e0 <alltraps>

80106d7b <vector164>:
.globl vector164
vector164:
  pushl $0
80106d7b:	6a 00                	push   $0x0
  pushl $164
80106d7d:	68 a4 00 00 00       	push   $0xa4
  jmp alltraps
80106d82:	e9 59 f3 ff ff       	jmp    801060e0 <alltraps>

80106d87 <vector165>:
.globl vector165
vector165:
  pushl $0
80106d87:	6a 00                	push   $0x0
  pushl $165
80106d89:	68 a5 00 00 00       	push   $0xa5
  jmp alltraps
80106d8e:	e9 4d f3 ff ff       	jmp    801060e0 <alltraps>

80106d93 <vector166>:
.globl vector166
vector166:
  pushl $0
80106d93:	6a 00                	push   $0x0
  pushl $166
80106d95:	68 a6 00 00 00       	push   $0xa6
  jmp alltraps
80106d9a:	e9 41 f3 ff ff       	jmp    801060e0 <alltraps>

80106d9f <vector167>:
.globl vector167
vector167:
  pushl $0
80106d9f:	6a 00                	push   $0x0
  pushl $167
80106da1:	68 a7 00 00 00       	push   $0xa7
  jmp alltraps
80106da6:	e9 35 f3 ff ff       	jmp    801060e0 <alltraps>

80106dab <vector168>:
.globl vector168
vector168:
  pushl $0
80106dab:	6a 00                	push   $0x0
  pushl $168
80106dad:	68 a8 00 00 00       	push   $0xa8
  jmp alltraps
80106db2:	e9 29 f3 ff ff       	jmp    801060e0 <alltraps>

80106db7 <vector169>:
.globl vector169
vector169:
  pushl $0
80106db7:	6a 00                	push   $0x0
  pushl $169
80106db9:	68 a9 00 00 00       	push   $0xa9
  jmp alltraps
80106dbe:	e9 1d f3 ff ff       	jmp    801060e0 <alltraps>

80106dc3 <vector170>:
.globl vector170
vector170:
  pushl $0
80106dc3:	6a 00                	push   $0x0
  pushl $170
80106dc5:	68 aa 00 00 00       	push   $0xaa
  jmp alltraps
80106dca:	e9 11 f3 ff ff       	jmp    801060e0 <alltraps>

80106dcf <vector171>:
.globl vector171
vector171:
  pushl $0
80106dcf:	6a 00                	push   $0x0
  pushl $171
80106dd1:	68 ab 00 00 00       	push   $0xab
  jmp alltraps
80106dd6:	e9 05 f3 ff ff       	jmp    801060e0 <alltraps>

80106ddb <vector172>:
.globl vector172
vector172:
  pushl $0
80106ddb:	6a 00                	push   $0x0
  pushl $172
80106ddd:	68 ac 00 00 00       	push   $0xac
  jmp alltraps
80106de2:	e9 f9 f2 ff ff       	jmp    801060e0 <alltraps>

80106de7 <vector173>:
.globl vector173
vector173:
  pushl $0
80106de7:	6a 00                	push   $0x0
  pushl $173
80106de9:	68 ad 00 00 00       	push   $0xad
  jmp alltraps
80106dee:	e9 ed f2 ff ff       	jmp    801060e0 <alltraps>

80106df3 <vector174>:
.globl vector174
vector174:
  pushl $0
80106df3:	6a 00                	push   $0x0
  pushl $174
80106df5:	68 ae 00 00 00       	push   $0xae
  jmp alltraps
80106dfa:	e9 e1 f2 ff ff       	jmp    801060e0 <alltraps>

80106dff <vector175>:
.globl vector175
vector175:
  pushl $0
80106dff:	6a 00                	push   $0x0
  pushl $175
80106e01:	68 af 00 00 00       	push   $0xaf
  jmp alltraps
80106e06:	e9 d5 f2 ff ff       	jmp    801060e0 <alltraps>

80106e0b <vector176>:
.globl vector176
vector176:
  pushl $0
80106e0b:	6a 00                	push   $0x0
  pushl $176
80106e0d:	68 b0 00 00 00       	push   $0xb0
  jmp alltraps
80106e12:	e9 c9 f2 ff ff       	jmp    801060e0 <alltraps>

80106e17 <vector177>:
.globl vector177
vector177:
  pushl $0
80106e17:	6a 00                	push   $0x0
  pushl $177
80106e19:	68 b1 00 00 00       	push   $0xb1
  jmp alltraps
80106e1e:	e9 bd f2 ff ff       	jmp    801060e0 <alltraps>

80106e23 <vector178>:
.globl vector178
vector178:
  pushl $0
80106e23:	6a 00                	push   $0x0
  pushl $178
80106e25:	68 b2 00 00 00       	push   $0xb2
  jmp alltraps
80106e2a:	e9 b1 f2 ff ff       	jmp    801060e0 <alltraps>

80106e2f <vector179>:
.globl vector179
vector179:
  pushl $0
80106e2f:	6a 00                	push   $0x0
  pushl $179
80106e31:	68 b3 00 00 00       	push   $0xb3
  jmp alltraps
80106e36:	e9 a5 f2 ff ff       	jmp    801060e0 <alltraps>

80106e3b <vector180>:
.globl vector180
vector180:
  pushl $0
80106e3b:	6a 00                	push   $0x0
  pushl $180
80106e3d:	68 b4 00 00 00       	push   $0xb4
  jmp alltraps
80106e42:	e9 99 f2 ff ff       	jmp    801060e0 <alltraps>

80106e47 <vector181>:
.globl vector181
vector181:
  pushl $0
80106e47:	6a 00                	push   $0x0
  pushl $181
80106e49:	68 b5 00 00 00       	push   $0xb5
  jmp alltraps
80106e4e:	e9 8d f2 ff ff       	jmp    801060e0 <alltraps>

80106e53 <vector182>:
.globl vector182
vector182:
  pushl $0
80106e53:	6a 00                	push   $0x0
  pushl $182
80106e55:	68 b6 00 00 00       	push   $0xb6
  jmp alltraps
80106e5a:	e9 81 f2 ff ff       	jmp    801060e0 <alltraps>

80106e5f <vector183>:
.globl vector183
vector183:
  pushl $0
80106e5f:	6a 00                	push   $0x0
  pushl $183
80106e61:	68 b7 00 00 00       	push   $0xb7
  jmp alltraps
80106e66:	e9 75 f2 ff ff       	jmp    801060e0 <alltraps>

80106e6b <vector184>:
.globl vector184
vector184:
  pushl $0
80106e6b:	6a 00                	push   $0x0
  pushl $184
80106e6d:	68 b8 00 00 00       	push   $0xb8
  jmp alltraps
80106e72:	e9 69 f2 ff ff       	jmp    801060e0 <alltraps>

80106e77 <vector185>:
.globl vector185
vector185:
  pushl $0
80106e77:	6a 00                	push   $0x0
  pushl $185
80106e79:	68 b9 00 00 00       	push   $0xb9
  jmp alltraps
80106e7e:	e9 5d f2 ff ff       	jmp    801060e0 <alltraps>

80106e83 <vector186>:
.globl vector186
vector186:
  pushl $0
80106e83:	6a 00                	push   $0x0
  pushl $186
80106e85:	68 ba 00 00 00       	push   $0xba
  jmp alltraps
80106e8a:	e9 51 f2 ff ff       	jmp    801060e0 <alltraps>

80106e8f <vector187>:
.globl vector187
vector187:
  pushl $0
80106e8f:	6a 00                	push   $0x0
  pushl $187
80106e91:	68 bb 00 00 00       	push   $0xbb
  jmp alltraps
80106e96:	e9 45 f2 ff ff       	jmp    801060e0 <alltraps>

80106e9b <vector188>:
.globl vector188
vector188:
  pushl $0
80106e9b:	6a 00                	push   $0x0
  pushl $188
80106e9d:	68 bc 00 00 00       	push   $0xbc
  jmp alltraps
80106ea2:	e9 39 f2 ff ff       	jmp    801060e0 <alltraps>

80106ea7 <vector189>:
.globl vector189
vector189:
  pushl $0
80106ea7:	6a 00                	push   $0x0
  pushl $189
80106ea9:	68 bd 00 00 00       	push   $0xbd
  jmp alltraps
80106eae:	e9 2d f2 ff ff       	jmp    801060e0 <alltraps>

80106eb3 <vector190>:
.globl vector190
vector190:
  pushl $0
80106eb3:	6a 00                	push   $0x0
  pushl $190
80106eb5:	68 be 00 00 00       	push   $0xbe
  jmp alltraps
80106eba:	e9 21 f2 ff ff       	jmp    801060e0 <alltraps>

80106ebf <vector191>:
.globl vector191
vector191:
  pushl $0
80106ebf:	6a 00                	push   $0x0
  pushl $191
80106ec1:	68 bf 00 00 00       	push   $0xbf
  jmp alltraps
80106ec6:	e9 15 f2 ff ff       	jmp    801060e0 <alltraps>

80106ecb <vector192>:
.globl vector192
vector192:
  pushl $0
80106ecb:	6a 00                	push   $0x0
  pushl $192
80106ecd:	68 c0 00 00 00       	push   $0xc0
  jmp alltraps
80106ed2:	e9 09 f2 ff ff       	jmp    801060e0 <alltraps>

80106ed7 <vector193>:
.globl vector193
vector193:
  pushl $0
80106ed7:	6a 00                	push   $0x0
  pushl $193
80106ed9:	68 c1 00 00 00       	push   $0xc1
  jmp alltraps
80106ede:	e9 fd f1 ff ff       	jmp    801060e0 <alltraps>

80106ee3 <vector194>:
.globl vector194
vector194:
  pushl $0
80106ee3:	6a 00                	push   $0x0
  pushl $194
80106ee5:	68 c2 00 00 00       	push   $0xc2
  jmp alltraps
80106eea:	e9 f1 f1 ff ff       	jmp    801060e0 <alltraps>

80106eef <vector195>:
.globl vector195
vector195:
  pushl $0
80106eef:	6a 00                	push   $0x0
  pushl $195
80106ef1:	68 c3 00 00 00       	push   $0xc3
  jmp alltraps
80106ef6:	e9 e5 f1 ff ff       	jmp    801060e0 <alltraps>

80106efb <vector196>:
.globl vector196
vector196:
  pushl $0
80106efb:	6a 00                	push   $0x0
  pushl $196
80106efd:	68 c4 00 00 00       	push   $0xc4
  jmp alltraps
80106f02:	e9 d9 f1 ff ff       	jmp    801060e0 <alltraps>

80106f07 <vector197>:
.globl vector197
vector197:
  pushl $0
80106f07:	6a 00                	push   $0x0
  pushl $197
80106f09:	68 c5 00 00 00       	push   $0xc5
  jmp alltraps
80106f0e:	e9 cd f1 ff ff       	jmp    801060e0 <alltraps>

80106f13 <vector198>:
.globl vector198
vector198:
  pushl $0
80106f13:	6a 00                	push   $0x0
  pushl $198
80106f15:	68 c6 00 00 00       	push   $0xc6
  jmp alltraps
80106f1a:	e9 c1 f1 ff ff       	jmp    801060e0 <alltraps>

80106f1f <vector199>:
.globl vector199
vector199:
  pushl $0
80106f1f:	6a 00                	push   $0x0
  pushl $199
80106f21:	68 c7 00 00 00       	push   $0xc7
  jmp alltraps
80106f26:	e9 b5 f1 ff ff       	jmp    801060e0 <alltraps>

80106f2b <vector200>:
.globl vector200
vector200:
  pushl $0
80106f2b:	6a 00                	push   $0x0
  pushl $200
80106f2d:	68 c8 00 00 00       	push   $0xc8
  jmp alltraps
80106f32:	e9 a9 f1 ff ff       	jmp    801060e0 <alltraps>

80106f37 <vector201>:
.globl vector201
vector201:
  pushl $0
80106f37:	6a 00                	push   $0x0
  pushl $201
80106f39:	68 c9 00 00 00       	push   $0xc9
  jmp alltraps
80106f3e:	e9 9d f1 ff ff       	jmp    801060e0 <alltraps>

80106f43 <vector202>:
.globl vector202
vector202:
  pushl $0
80106f43:	6a 00                	push   $0x0
  pushl $202
80106f45:	68 ca 00 00 00       	push   $0xca
  jmp alltraps
80106f4a:	e9 91 f1 ff ff       	jmp    801060e0 <alltraps>

80106f4f <vector203>:
.globl vector203
vector203:
  pushl $0
80106f4f:	6a 00                	push   $0x0
  pushl $203
80106f51:	68 cb 00 00 00       	push   $0xcb
  jmp alltraps
80106f56:	e9 85 f1 ff ff       	jmp    801060e0 <alltraps>

80106f5b <vector204>:
.globl vector204
vector204:
  pushl $0
80106f5b:	6a 00                	push   $0x0
  pushl $204
80106f5d:	68 cc 00 00 00       	push   $0xcc
  jmp alltraps
80106f62:	e9 79 f1 ff ff       	jmp    801060e0 <alltraps>

80106f67 <vector205>:
.globl vector205
vector205:
  pushl $0
80106f67:	6a 00                	push   $0x0
  pushl $205
80106f69:	68 cd 00 00 00       	push   $0xcd
  jmp alltraps
80106f6e:	e9 6d f1 ff ff       	jmp    801060e0 <alltraps>

80106f73 <vector206>:
.globl vector206
vector206:
  pushl $0
80106f73:	6a 00                	push   $0x0
  pushl $206
80106f75:	68 ce 00 00 00       	push   $0xce
  jmp alltraps
80106f7a:	e9 61 f1 ff ff       	jmp    801060e0 <alltraps>

80106f7f <vector207>:
.globl vector207
vector207:
  pushl $0
80106f7f:	6a 00                	push   $0x0
  pushl $207
80106f81:	68 cf 00 00 00       	push   $0xcf
  jmp alltraps
80106f86:	e9 55 f1 ff ff       	jmp    801060e0 <alltraps>

80106f8b <vector208>:
.globl vector208
vector208:
  pushl $0
80106f8b:	6a 00                	push   $0x0
  pushl $208
80106f8d:	68 d0 00 00 00       	push   $0xd0
  jmp alltraps
80106f92:	e9 49 f1 ff ff       	jmp    801060e0 <alltraps>

80106f97 <vector209>:
.globl vector209
vector209:
  pushl $0
80106f97:	6a 00                	push   $0x0
  pushl $209
80106f99:	68 d1 00 00 00       	push   $0xd1
  jmp alltraps
80106f9e:	e9 3d f1 ff ff       	jmp    801060e0 <alltraps>

80106fa3 <vector210>:
.globl vector210
vector210:
  pushl $0
80106fa3:	6a 00                	push   $0x0
  pushl $210
80106fa5:	68 d2 00 00 00       	push   $0xd2
  jmp alltraps
80106faa:	e9 31 f1 ff ff       	jmp    801060e0 <alltraps>

80106faf <vector211>:
.globl vector211
vector211:
  pushl $0
80106faf:	6a 00                	push   $0x0
  pushl $211
80106fb1:	68 d3 00 00 00       	push   $0xd3
  jmp alltraps
80106fb6:	e9 25 f1 ff ff       	jmp    801060e0 <alltraps>

80106fbb <vector212>:
.globl vector212
vector212:
  pushl $0
80106fbb:	6a 00                	push   $0x0
  pushl $212
80106fbd:	68 d4 00 00 00       	push   $0xd4
  jmp alltraps
80106fc2:	e9 19 f1 ff ff       	jmp    801060e0 <alltraps>

80106fc7 <vector213>:
.globl vector213
vector213:
  pushl $0
80106fc7:	6a 00                	push   $0x0
  pushl $213
80106fc9:	68 d5 00 00 00       	push   $0xd5
  jmp alltraps
80106fce:	e9 0d f1 ff ff       	jmp    801060e0 <alltraps>

80106fd3 <vector214>:
.globl vector214
vector214:
  pushl $0
80106fd3:	6a 00                	push   $0x0
  pushl $214
80106fd5:	68 d6 00 00 00       	push   $0xd6
  jmp alltraps
80106fda:	e9 01 f1 ff ff       	jmp    801060e0 <alltraps>

80106fdf <vector215>:
.globl vector215
vector215:
  pushl $0
80106fdf:	6a 00                	push   $0x0
  pushl $215
80106fe1:	68 d7 00 00 00       	push   $0xd7
  jmp alltraps
80106fe6:	e9 f5 f0 ff ff       	jmp    801060e0 <alltraps>

80106feb <vector216>:
.globl vector216
vector216:
  pushl $0
80106feb:	6a 00                	push   $0x0
  pushl $216
80106fed:	68 d8 00 00 00       	push   $0xd8
  jmp alltraps
80106ff2:	e9 e9 f0 ff ff       	jmp    801060e0 <alltraps>

80106ff7 <vector217>:
.globl vector217
vector217:
  pushl $0
80106ff7:	6a 00                	push   $0x0
  pushl $217
80106ff9:	68 d9 00 00 00       	push   $0xd9
  jmp alltraps
80106ffe:	e9 dd f0 ff ff       	jmp    801060e0 <alltraps>

80107003 <vector218>:
.globl vector218
vector218:
  pushl $0
80107003:	6a 00                	push   $0x0
  pushl $218
80107005:	68 da 00 00 00       	push   $0xda
  jmp alltraps
8010700a:	e9 d1 f0 ff ff       	jmp    801060e0 <alltraps>

8010700f <vector219>:
.globl vector219
vector219:
  pushl $0
8010700f:	6a 00                	push   $0x0
  pushl $219
80107011:	68 db 00 00 00       	push   $0xdb
  jmp alltraps
80107016:	e9 c5 f0 ff ff       	jmp    801060e0 <alltraps>

8010701b <vector220>:
.globl vector220
vector220:
  pushl $0
8010701b:	6a 00                	push   $0x0
  pushl $220
8010701d:	68 dc 00 00 00       	push   $0xdc
  jmp alltraps
80107022:	e9 b9 f0 ff ff       	jmp    801060e0 <alltraps>

80107027 <vector221>:
.globl vector221
vector221:
  pushl $0
80107027:	6a 00                	push   $0x0
  pushl $221
80107029:	68 dd 00 00 00       	push   $0xdd
  jmp alltraps
8010702e:	e9 ad f0 ff ff       	jmp    801060e0 <alltraps>

80107033 <vector222>:
.globl vector222
vector222:
  pushl $0
80107033:	6a 00                	push   $0x0
  pushl $222
80107035:	68 de 00 00 00       	push   $0xde
  jmp alltraps
8010703a:	e9 a1 f0 ff ff       	jmp    801060e0 <alltraps>

8010703f <vector223>:
.globl vector223
vector223:
  pushl $0
8010703f:	6a 00                	push   $0x0
  pushl $223
80107041:	68 df 00 00 00       	push   $0xdf
  jmp alltraps
80107046:	e9 95 f0 ff ff       	jmp    801060e0 <alltraps>

8010704b <vector224>:
.globl vector224
vector224:
  pushl $0
8010704b:	6a 00                	push   $0x0
  pushl $224
8010704d:	68 e0 00 00 00       	push   $0xe0
  jmp alltraps
80107052:	e9 89 f0 ff ff       	jmp    801060e0 <alltraps>

80107057 <vector225>:
.globl vector225
vector225:
  pushl $0
80107057:	6a 00                	push   $0x0
  pushl $225
80107059:	68 e1 00 00 00       	push   $0xe1
  jmp alltraps
8010705e:	e9 7d f0 ff ff       	jmp    801060e0 <alltraps>

80107063 <vector226>:
.globl vector226
vector226:
  pushl $0
80107063:	6a 00                	push   $0x0
  pushl $226
80107065:	68 e2 00 00 00       	push   $0xe2
  jmp alltraps
8010706a:	e9 71 f0 ff ff       	jmp    801060e0 <alltraps>

8010706f <vector227>:
.globl vector227
vector227:
  pushl $0
8010706f:	6a 00                	push   $0x0
  pushl $227
80107071:	68 e3 00 00 00       	push   $0xe3
  jmp alltraps
80107076:	e9 65 f0 ff ff       	jmp    801060e0 <alltraps>

8010707b <vector228>:
.globl vector228
vector228:
  pushl $0
8010707b:	6a 00                	push   $0x0
  pushl $228
8010707d:	68 e4 00 00 00       	push   $0xe4
  jmp alltraps
80107082:	e9 59 f0 ff ff       	jmp    801060e0 <alltraps>

80107087 <vector229>:
.globl vector229
vector229:
  pushl $0
80107087:	6a 00                	push   $0x0
  pushl $229
80107089:	68 e5 00 00 00       	push   $0xe5
  jmp alltraps
8010708e:	e9 4d f0 ff ff       	jmp    801060e0 <alltraps>

80107093 <vector230>:
.globl vector230
vector230:
  pushl $0
80107093:	6a 00                	push   $0x0
  pushl $230
80107095:	68 e6 00 00 00       	push   $0xe6
  jmp alltraps
8010709a:	e9 41 f0 ff ff       	jmp    801060e0 <alltraps>

8010709f <vector231>:
.globl vector231
vector231:
  pushl $0
8010709f:	6a 00                	push   $0x0
  pushl $231
801070a1:	68 e7 00 00 00       	push   $0xe7
  jmp alltraps
801070a6:	e9 35 f0 ff ff       	jmp    801060e0 <alltraps>

801070ab <vector232>:
.globl vector232
vector232:
  pushl $0
801070ab:	6a 00                	push   $0x0
  pushl $232
801070ad:	68 e8 00 00 00       	push   $0xe8
  jmp alltraps
801070b2:	e9 29 f0 ff ff       	jmp    801060e0 <alltraps>

801070b7 <vector233>:
.globl vector233
vector233:
  pushl $0
801070b7:	6a 00                	push   $0x0
  pushl $233
801070b9:	68 e9 00 00 00       	push   $0xe9
  jmp alltraps
801070be:	e9 1d f0 ff ff       	jmp    801060e0 <alltraps>

801070c3 <vector234>:
.globl vector234
vector234:
  pushl $0
801070c3:	6a 00                	push   $0x0
  pushl $234
801070c5:	68 ea 00 00 00       	push   $0xea
  jmp alltraps
801070ca:	e9 11 f0 ff ff       	jmp    801060e0 <alltraps>

801070cf <vector235>:
.globl vector235
vector235:
  pushl $0
801070cf:	6a 00                	push   $0x0
  pushl $235
801070d1:	68 eb 00 00 00       	push   $0xeb
  jmp alltraps
801070d6:	e9 05 f0 ff ff       	jmp    801060e0 <alltraps>

801070db <vector236>:
.globl vector236
vector236:
  pushl $0
801070db:	6a 00                	push   $0x0
  pushl $236
801070dd:	68 ec 00 00 00       	push   $0xec
  jmp alltraps
801070e2:	e9 f9 ef ff ff       	jmp    801060e0 <alltraps>

801070e7 <vector237>:
.globl vector237
vector237:
  pushl $0
801070e7:	6a 00                	push   $0x0
  pushl $237
801070e9:	68 ed 00 00 00       	push   $0xed
  jmp alltraps
801070ee:	e9 ed ef ff ff       	jmp    801060e0 <alltraps>

801070f3 <vector238>:
.globl vector238
vector238:
  pushl $0
801070f3:	6a 00                	push   $0x0
  pushl $238
801070f5:	68 ee 00 00 00       	push   $0xee
  jmp alltraps
801070fa:	e9 e1 ef ff ff       	jmp    801060e0 <alltraps>

801070ff <vector239>:
.globl vector239
vector239:
  pushl $0
801070ff:	6a 00                	push   $0x0
  pushl $239
80107101:	68 ef 00 00 00       	push   $0xef
  jmp alltraps
80107106:	e9 d5 ef ff ff       	jmp    801060e0 <alltraps>

8010710b <vector240>:
.globl vector240
vector240:
  pushl $0
8010710b:	6a 00                	push   $0x0
  pushl $240
8010710d:	68 f0 00 00 00       	push   $0xf0
  jmp alltraps
80107112:	e9 c9 ef ff ff       	jmp    801060e0 <alltraps>

80107117 <vector241>:
.globl vector241
vector241:
  pushl $0
80107117:	6a 00                	push   $0x0
  pushl $241
80107119:	68 f1 00 00 00       	push   $0xf1
  jmp alltraps
8010711e:	e9 bd ef ff ff       	jmp    801060e0 <alltraps>

80107123 <vector242>:
.globl vector242
vector242:
  pushl $0
80107123:	6a 00                	push   $0x0
  pushl $242
80107125:	68 f2 00 00 00       	push   $0xf2
  jmp alltraps
8010712a:	e9 b1 ef ff ff       	jmp    801060e0 <alltraps>

8010712f <vector243>:
.globl vector243
vector243:
  pushl $0
8010712f:	6a 00                	push   $0x0
  pushl $243
80107131:	68 f3 00 00 00       	push   $0xf3
  jmp alltraps
80107136:	e9 a5 ef ff ff       	jmp    801060e0 <alltraps>

8010713b <vector244>:
.globl vector244
vector244:
  pushl $0
8010713b:	6a 00                	push   $0x0
  pushl $244
8010713d:	68 f4 00 00 00       	push   $0xf4
  jmp alltraps
80107142:	e9 99 ef ff ff       	jmp    801060e0 <alltraps>

80107147 <vector245>:
.globl vector245
vector245:
  pushl $0
80107147:	6a 00                	push   $0x0
  pushl $245
80107149:	68 f5 00 00 00       	push   $0xf5
  jmp alltraps
8010714e:	e9 8d ef ff ff       	jmp    801060e0 <alltraps>

80107153 <vector246>:
.globl vector246
vector246:
  pushl $0
80107153:	6a 00                	push   $0x0
  pushl $246
80107155:	68 f6 00 00 00       	push   $0xf6
  jmp alltraps
8010715a:	e9 81 ef ff ff       	jmp    801060e0 <alltraps>

8010715f <vector247>:
.globl vector247
vector247:
  pushl $0
8010715f:	6a 00                	push   $0x0
  pushl $247
80107161:	68 f7 00 00 00       	push   $0xf7
  jmp alltraps
80107166:	e9 75 ef ff ff       	jmp    801060e0 <alltraps>

8010716b <vector248>:
.globl vector248
vector248:
  pushl $0
8010716b:	6a 00                	push   $0x0
  pushl $248
8010716d:	68 f8 00 00 00       	push   $0xf8
  jmp alltraps
80107172:	e9 69 ef ff ff       	jmp    801060e0 <alltraps>

80107177 <vector249>:
.globl vector249
vector249:
  pushl $0
80107177:	6a 00                	push   $0x0
  pushl $249
80107179:	68 f9 00 00 00       	push   $0xf9
  jmp alltraps
8010717e:	e9 5d ef ff ff       	jmp    801060e0 <alltraps>

80107183 <vector250>:
.globl vector250
vector250:
  pushl $0
80107183:	6a 00                	push   $0x0
  pushl $250
80107185:	68 fa 00 00 00       	push   $0xfa
  jmp alltraps
8010718a:	e9 51 ef ff ff       	jmp    801060e0 <alltraps>

8010718f <vector251>:
.globl vector251
vector251:
  pushl $0
8010718f:	6a 00                	push   $0x0
  pushl $251
80107191:	68 fb 00 00 00       	push   $0xfb
  jmp alltraps
80107196:	e9 45 ef ff ff       	jmp    801060e0 <alltraps>

8010719b <vector252>:
.globl vector252
vector252:
  pushl $0
8010719b:	6a 00                	push   $0x0
  pushl $252
8010719d:	68 fc 00 00 00       	push   $0xfc
  jmp alltraps
801071a2:	e9 39 ef ff ff       	jmp    801060e0 <alltraps>

801071a7 <vector253>:
.globl vector253
vector253:
  pushl $0
801071a7:	6a 00                	push   $0x0
  pushl $253
801071a9:	68 fd 00 00 00       	push   $0xfd
  jmp alltraps
801071ae:	e9 2d ef ff ff       	jmp    801060e0 <alltraps>

801071b3 <vector254>:
.globl vector254
vector254:
  pushl $0
801071b3:	6a 00                	push   $0x0
  pushl $254
801071b5:	68 fe 00 00 00       	push   $0xfe
  jmp alltraps
801071ba:	e9 21 ef ff ff       	jmp    801060e0 <alltraps>

801071bf <vector255>:
.globl vector255
vector255:
  pushl $0
801071bf:	6a 00                	push   $0x0
  pushl $255
801071c1:	68 ff 00 00 00       	push   $0xff
  jmp alltraps
801071c6:	e9 15 ef ff ff       	jmp    801060e0 <alltraps>

801071cb <lgdt>:

struct segdesc;

static inline void
lgdt(struct segdesc *p, int size)
{
801071cb:	55                   	push   %ebp
801071cc:	89 e5                	mov    %esp,%ebp
801071ce:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
801071d1:	8b 45 0c             	mov    0xc(%ebp),%eax
801071d4:	83 e8 01             	sub    $0x1,%eax
801071d7:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
801071db:	8b 45 08             	mov    0x8(%ebp),%eax
801071de:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
801071e2:	8b 45 08             	mov    0x8(%ebp),%eax
801071e5:	c1 e8 10             	shr    $0x10,%eax
801071e8:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lgdt (%0)" : : "r" (pd));
801071ec:	8d 45 fa             	lea    -0x6(%ebp),%eax
801071ef:	0f 01 10             	lgdtl  (%eax)
}
801071f2:	c9                   	leave  
801071f3:	c3                   	ret    

801071f4 <ltr>:
  asm volatile("lidt (%0)" : : "r" (pd));
}

static inline void
ltr(ushort sel)
{
801071f4:	55                   	push   %ebp
801071f5:	89 e5                	mov    %esp,%ebp
801071f7:	83 ec 04             	sub    $0x4,%esp
801071fa:	8b 45 08             	mov    0x8(%ebp),%eax
801071fd:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("ltr %0" : : "r" (sel));
80107201:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80107205:	0f 00 d8             	ltr    %ax
}
80107208:	c9                   	leave  
80107209:	c3                   	ret    

8010720a <loadgs>:
  return eflags;
}

static inline void
loadgs(ushort v)
{
8010720a:	55                   	push   %ebp
8010720b:	89 e5                	mov    %esp,%ebp
8010720d:	83 ec 04             	sub    $0x4,%esp
80107210:	8b 45 08             	mov    0x8(%ebp),%eax
80107213:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("movw %0, %%gs" : : "r" (v));
80107217:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
8010721b:	8e e8                	mov    %eax,%gs
}
8010721d:	c9                   	leave  
8010721e:	c3                   	ret    

8010721f <lcr3>:
  return val;
}

static inline void
lcr3(uint val) 
{
8010721f:	55                   	push   %ebp
80107220:	89 e5                	mov    %esp,%ebp
  asm volatile("movl %0,%%cr3" : : "r" (val));
80107222:	8b 45 08             	mov    0x8(%ebp),%eax
80107225:	0f 22 d8             	mov    %eax,%cr3
}
80107228:	5d                   	pop    %ebp
80107229:	c3                   	ret    

8010722a <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
8010722a:	55                   	push   %ebp
8010722b:	89 e5                	mov    %esp,%ebp
8010722d:	8b 45 08             	mov    0x8(%ebp),%eax
80107230:	05 00 00 00 80       	add    $0x80000000,%eax
80107235:	5d                   	pop    %ebp
80107236:	c3                   	ret    

80107237 <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
80107237:	55                   	push   %ebp
80107238:	89 e5                	mov    %esp,%ebp
8010723a:	8b 45 08             	mov    0x8(%ebp),%eax
8010723d:	05 00 00 00 80       	add    $0x80000000,%eax
80107242:	5d                   	pop    %ebp
80107243:	c3                   	ret    

80107244 <seginit>:

// Set up CPU's kernel segment descriptors.
// Run once on entry on each CPU.
void
seginit(void)
{
80107244:	55                   	push   %ebp
80107245:	89 e5                	mov    %esp,%ebp
80107247:	53                   	push   %ebx
80107248:	83 ec 24             	sub    $0x24,%esp

  // Map "logical" addresses to virtual addresses using identity map.
  // Cannot share a CODE descriptor for both kernel and user
  // because it would have to have DPL_USR, but the CPU forbids
  // an interrupt from CPL=0 to DPL=3.
  c = &cpus[cpunum()];
8010724b:	e8 0b bc ff ff       	call   80102e5b <cpunum>
80107250:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80107256:	05 20 f9 10 80       	add    $0x8010f920,%eax
8010725b:	89 45 f4             	mov    %eax,-0xc(%ebp)
  c->gdt[SEG_KCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, 0);
8010725e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107261:	66 c7 40 78 ff ff    	movw   $0xffff,0x78(%eax)
80107267:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010726a:	66 c7 40 7a 00 00    	movw   $0x0,0x7a(%eax)
80107270:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107273:	c6 40 7c 00          	movb   $0x0,0x7c(%eax)
80107277:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010727a:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
8010727e:	83 e2 f0             	and    $0xfffffff0,%edx
80107281:	83 ca 0a             	or     $0xa,%edx
80107284:	88 50 7d             	mov    %dl,0x7d(%eax)
80107287:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010728a:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
8010728e:	83 ca 10             	or     $0x10,%edx
80107291:	88 50 7d             	mov    %dl,0x7d(%eax)
80107294:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107297:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
8010729b:	83 e2 9f             	and    $0xffffff9f,%edx
8010729e:	88 50 7d             	mov    %dl,0x7d(%eax)
801072a1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801072a4:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
801072a8:	83 ca 80             	or     $0xffffff80,%edx
801072ab:	88 50 7d             	mov    %dl,0x7d(%eax)
801072ae:	8b 45 f4             	mov    -0xc(%ebp),%eax
801072b1:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
801072b5:	83 ca 0f             	or     $0xf,%edx
801072b8:	88 50 7e             	mov    %dl,0x7e(%eax)
801072bb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801072be:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
801072c2:	83 e2 ef             	and    $0xffffffef,%edx
801072c5:	88 50 7e             	mov    %dl,0x7e(%eax)
801072c8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801072cb:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
801072cf:	83 e2 df             	and    $0xffffffdf,%edx
801072d2:	88 50 7e             	mov    %dl,0x7e(%eax)
801072d5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801072d8:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
801072dc:	83 ca 40             	or     $0x40,%edx
801072df:	88 50 7e             	mov    %dl,0x7e(%eax)
801072e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801072e5:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
801072e9:	83 ca 80             	or     $0xffffff80,%edx
801072ec:	88 50 7e             	mov    %dl,0x7e(%eax)
801072ef:	8b 45 f4             	mov    -0xc(%ebp),%eax
801072f2:	c6 40 7f 00          	movb   $0x0,0x7f(%eax)
  c->gdt[SEG_KDATA] = SEG(STA_W, 0, 0xffffffff, 0);
801072f6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801072f9:	66 c7 80 80 00 00 00 	movw   $0xffff,0x80(%eax)
80107300:	ff ff 
80107302:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107305:	66 c7 80 82 00 00 00 	movw   $0x0,0x82(%eax)
8010730c:	00 00 
8010730e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107311:	c6 80 84 00 00 00 00 	movb   $0x0,0x84(%eax)
80107318:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010731b:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107322:	83 e2 f0             	and    $0xfffffff0,%edx
80107325:	83 ca 02             	or     $0x2,%edx
80107328:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
8010732e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107331:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107338:	83 ca 10             	or     $0x10,%edx
8010733b:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107341:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107344:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
8010734b:	83 e2 9f             	and    $0xffffff9f,%edx
8010734e:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107354:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107357:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
8010735e:	83 ca 80             	or     $0xffffff80,%edx
80107361:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
80107367:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010736a:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107371:	83 ca 0f             	or     $0xf,%edx
80107374:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
8010737a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010737d:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107384:	83 e2 ef             	and    $0xffffffef,%edx
80107387:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
8010738d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107390:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107397:	83 e2 df             	and    $0xffffffdf,%edx
8010739a:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
801073a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801073a3:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
801073aa:	83 ca 40             	or     $0x40,%edx
801073ad:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
801073b3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801073b6:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
801073bd:	83 ca 80             	or     $0xffffff80,%edx
801073c0:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
801073c6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801073c9:	c6 80 87 00 00 00 00 	movb   $0x0,0x87(%eax)
  c->gdt[SEG_UCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, DPL_USER);
801073d0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801073d3:	66 c7 80 90 00 00 00 	movw   $0xffff,0x90(%eax)
801073da:	ff ff 
801073dc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801073df:	66 c7 80 92 00 00 00 	movw   $0x0,0x92(%eax)
801073e6:	00 00 
801073e8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801073eb:	c6 80 94 00 00 00 00 	movb   $0x0,0x94(%eax)
801073f2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801073f5:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
801073fc:	83 e2 f0             	and    $0xfffffff0,%edx
801073ff:	83 ca 0a             	or     $0xa,%edx
80107402:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107408:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010740b:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107412:	83 ca 10             	or     $0x10,%edx
80107415:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
8010741b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010741e:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107425:	83 ca 60             	or     $0x60,%edx
80107428:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
8010742e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107431:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107438:	83 ca 80             	or     $0xffffff80,%edx
8010743b:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107441:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107444:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
8010744b:	83 ca 0f             	or     $0xf,%edx
8010744e:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107454:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107457:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
8010745e:	83 e2 ef             	and    $0xffffffef,%edx
80107461:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107467:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010746a:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107471:	83 e2 df             	and    $0xffffffdf,%edx
80107474:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
8010747a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010747d:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107484:	83 ca 40             	or     $0x40,%edx
80107487:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
8010748d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107490:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107497:	83 ca 80             	or     $0xffffff80,%edx
8010749a:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
801074a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801074a3:	c6 80 97 00 00 00 00 	movb   $0x0,0x97(%eax)
  c->gdt[SEG_UDATA] = SEG(STA_W, 0, 0xffffffff, DPL_USER);
801074aa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801074ad:	66 c7 80 98 00 00 00 	movw   $0xffff,0x98(%eax)
801074b4:	ff ff 
801074b6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801074b9:	66 c7 80 9a 00 00 00 	movw   $0x0,0x9a(%eax)
801074c0:	00 00 
801074c2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801074c5:	c6 80 9c 00 00 00 00 	movb   $0x0,0x9c(%eax)
801074cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801074cf:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
801074d6:	83 e2 f0             	and    $0xfffffff0,%edx
801074d9:	83 ca 02             	or     $0x2,%edx
801074dc:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
801074e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801074e5:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
801074ec:	83 ca 10             	or     $0x10,%edx
801074ef:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
801074f5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801074f8:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
801074ff:	83 ca 60             	or     $0x60,%edx
80107502:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107508:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010750b:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107512:	83 ca 80             	or     $0xffffff80,%edx
80107515:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
8010751b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010751e:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107525:	83 ca 0f             	or     $0xf,%edx
80107528:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
8010752e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107531:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107538:	83 e2 ef             	and    $0xffffffef,%edx
8010753b:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107541:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107544:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
8010754b:	83 e2 df             	and    $0xffffffdf,%edx
8010754e:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107554:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107557:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
8010755e:	83 ca 40             	or     $0x40,%edx
80107561:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107567:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010756a:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107571:	83 ca 80             	or     $0xffffff80,%edx
80107574:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
8010757a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010757d:	c6 80 9f 00 00 00 00 	movb   $0x0,0x9f(%eax)

  // Map cpu, and curproc
  c->gdt[SEG_KCPU] = SEG(STA_W, &c->cpu, 8, 0);
80107584:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107587:	05 b4 00 00 00       	add    $0xb4,%eax
8010758c:	89 c3                	mov    %eax,%ebx
8010758e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107591:	05 b4 00 00 00       	add    $0xb4,%eax
80107596:	c1 e8 10             	shr    $0x10,%eax
80107599:	89 c1                	mov    %eax,%ecx
8010759b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010759e:	05 b4 00 00 00       	add    $0xb4,%eax
801075a3:	c1 e8 18             	shr    $0x18,%eax
801075a6:	89 c2                	mov    %eax,%edx
801075a8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801075ab:	66 c7 80 88 00 00 00 	movw   $0x0,0x88(%eax)
801075b2:	00 00 
801075b4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801075b7:	66 89 98 8a 00 00 00 	mov    %bx,0x8a(%eax)
801075be:	8b 45 f4             	mov    -0xc(%ebp),%eax
801075c1:	88 88 8c 00 00 00    	mov    %cl,0x8c(%eax)
801075c7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801075ca:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
801075d1:	83 e1 f0             	and    $0xfffffff0,%ecx
801075d4:	83 c9 02             	or     $0x2,%ecx
801075d7:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
801075dd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801075e0:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
801075e7:	83 c9 10             	or     $0x10,%ecx
801075ea:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
801075f0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801075f3:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
801075fa:	83 e1 9f             	and    $0xffffff9f,%ecx
801075fd:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107603:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107606:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
8010760d:	83 c9 80             	or     $0xffffff80,%ecx
80107610:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107616:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107619:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107620:	83 e1 f0             	and    $0xfffffff0,%ecx
80107623:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107629:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010762c:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107633:	83 e1 ef             	and    $0xffffffef,%ecx
80107636:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
8010763c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010763f:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107646:	83 e1 df             	and    $0xffffffdf,%ecx
80107649:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
8010764f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107652:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107659:	83 c9 40             	or     $0x40,%ecx
8010765c:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107662:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107665:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
8010766c:	83 c9 80             	or     $0xffffff80,%ecx
8010766f:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107675:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107678:	88 90 8f 00 00 00    	mov    %dl,0x8f(%eax)

  lgdt(c->gdt, sizeof(c->gdt));
8010767e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107681:	83 c0 70             	add    $0x70,%eax
80107684:	c7 44 24 04 38 00 00 	movl   $0x38,0x4(%esp)
8010768b:	00 
8010768c:	89 04 24             	mov    %eax,(%esp)
8010768f:	e8 37 fb ff ff       	call   801071cb <lgdt>
  loadgs(SEG_KCPU << 3);
80107694:	c7 04 24 18 00 00 00 	movl   $0x18,(%esp)
8010769b:	e8 6a fb ff ff       	call   8010720a <loadgs>
  
  // Initialize cpu-local storage.
  cpu = c;
801076a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801076a3:	65 a3 00 00 00 00    	mov    %eax,%gs:0x0
  proc = 0;
801076a9:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
801076b0:	00 00 00 00 
}
801076b4:	83 c4 24             	add    $0x24,%esp
801076b7:	5b                   	pop    %ebx
801076b8:	5d                   	pop    %ebp
801076b9:	c3                   	ret    

801076ba <walkpgdir>:
// Return the address of the PTE in page table pgdir
// that corresponds to virtual address va.  If alloc!=0,
// create any required page table pages.
static pte_t *
walkpgdir(pde_t *pgdir, const void *va, int alloc)
{
801076ba:	55                   	push   %ebp
801076bb:	89 e5                	mov    %esp,%ebp
801076bd:	83 ec 28             	sub    $0x28,%esp
  pde_t *pde;
  pte_t *pgtab;

  pde = &pgdir[PDX(va)];
801076c0:	8b 45 0c             	mov    0xc(%ebp),%eax
801076c3:	c1 e8 16             	shr    $0x16,%eax
801076c6:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
801076cd:	8b 45 08             	mov    0x8(%ebp),%eax
801076d0:	01 d0                	add    %edx,%eax
801076d2:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(*pde & PTE_P){
801076d5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801076d8:	8b 00                	mov    (%eax),%eax
801076da:	83 e0 01             	and    $0x1,%eax
801076dd:	85 c0                	test   %eax,%eax
801076df:	74 17                	je     801076f8 <walkpgdir+0x3e>
    pgtab = (pte_t*)p2v(PTE_ADDR(*pde));
801076e1:	8b 45 f0             	mov    -0x10(%ebp),%eax
801076e4:	8b 00                	mov    (%eax),%eax
801076e6:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801076eb:	89 04 24             	mov    %eax,(%esp)
801076ee:	e8 44 fb ff ff       	call   80107237 <p2v>
801076f3:	89 45 f4             	mov    %eax,-0xc(%ebp)
801076f6:	eb 4b                	jmp    80107743 <walkpgdir+0x89>
  } else {
    if(!alloc || (pgtab = (pte_t*)kalloc()) == 0)
801076f8:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801076fc:	74 0e                	je     8010770c <walkpgdir+0x52>
801076fe:	e8 df b3 ff ff       	call   80102ae2 <kalloc>
80107703:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107706:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010770a:	75 07                	jne    80107713 <walkpgdir+0x59>
      return 0;
8010770c:	b8 00 00 00 00       	mov    $0x0,%eax
80107711:	eb 47                	jmp    8010775a <walkpgdir+0xa0>
    // Make sure all those PTE_P bits are zero.
    memset(pgtab, 0, PGSIZE);
80107713:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
8010771a:	00 
8010771b:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107722:	00 
80107723:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107726:	89 04 24             	mov    %eax,(%esp)
80107729:	e8 be d5 ff ff       	call   80104cec <memset>
    // The permissions here are overly generous, but they can
    // be further restricted by the permissions in the page table 
    // entries, if necessary.
    *pde = v2p(pgtab) | PTE_P | PTE_W | PTE_U;
8010772e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107731:	89 04 24             	mov    %eax,(%esp)
80107734:	e8 f1 fa ff ff       	call   8010722a <v2p>
80107739:	83 c8 07             	or     $0x7,%eax
8010773c:	89 c2                	mov    %eax,%edx
8010773e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107741:	89 10                	mov    %edx,(%eax)
  }
  return &pgtab[PTX(va)];
80107743:	8b 45 0c             	mov    0xc(%ebp),%eax
80107746:	c1 e8 0c             	shr    $0xc,%eax
80107749:	25 ff 03 00 00       	and    $0x3ff,%eax
8010774e:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80107755:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107758:	01 d0                	add    %edx,%eax
}
8010775a:	c9                   	leave  
8010775b:	c3                   	ret    

8010775c <mappages>:
// Create PTEs for virtual addresses starting at va that refer to
// physical addresses starting at pa. va and size might not
// be page-aligned.
static int
mappages(pde_t *pgdir, void *va, uint size, uint pa, int perm)
{
8010775c:	55                   	push   %ebp
8010775d:	89 e5                	mov    %esp,%ebp
8010775f:	83 ec 28             	sub    $0x28,%esp
  char *a, *last;
  pte_t *pte;
  
  a = (char*)PGROUNDDOWN((uint)va);
80107762:	8b 45 0c             	mov    0xc(%ebp),%eax
80107765:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010776a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  last = (char*)PGROUNDDOWN(((uint)va) + size - 1);
8010776d:	8b 55 0c             	mov    0xc(%ebp),%edx
80107770:	8b 45 10             	mov    0x10(%ebp),%eax
80107773:	01 d0                	add    %edx,%eax
80107775:	83 e8 01             	sub    $0x1,%eax
80107778:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010777d:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(;;){
    if((pte = walkpgdir(pgdir, a, 1)) == 0)
80107780:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
80107787:	00 
80107788:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010778b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010778f:	8b 45 08             	mov    0x8(%ebp),%eax
80107792:	89 04 24             	mov    %eax,(%esp)
80107795:	e8 20 ff ff ff       	call   801076ba <walkpgdir>
8010779a:	89 45 ec             	mov    %eax,-0x14(%ebp)
8010779d:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801077a1:	75 07                	jne    801077aa <mappages+0x4e>
      return -1;
801077a3:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801077a8:	eb 48                	jmp    801077f2 <mappages+0x96>
    if(*pte & PTE_P)
801077aa:	8b 45 ec             	mov    -0x14(%ebp),%eax
801077ad:	8b 00                	mov    (%eax),%eax
801077af:	83 e0 01             	and    $0x1,%eax
801077b2:	85 c0                	test   %eax,%eax
801077b4:	74 0c                	je     801077c2 <mappages+0x66>
      panic("remap");
801077b6:	c7 04 24 dc 85 10 80 	movl   $0x801085dc,(%esp)
801077bd:	e8 78 8d ff ff       	call   8010053a <panic>
    *pte = pa | perm | PTE_P;
801077c2:	8b 45 18             	mov    0x18(%ebp),%eax
801077c5:	0b 45 14             	or     0x14(%ebp),%eax
801077c8:	83 c8 01             	or     $0x1,%eax
801077cb:	89 c2                	mov    %eax,%edx
801077cd:	8b 45 ec             	mov    -0x14(%ebp),%eax
801077d0:	89 10                	mov    %edx,(%eax)
    if(a == last)
801077d2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801077d5:	3b 45 f0             	cmp    -0x10(%ebp),%eax
801077d8:	75 08                	jne    801077e2 <mappages+0x86>
      break;
801077da:	90                   	nop
    a += PGSIZE;
    pa += PGSIZE;
  }
  return 0;
801077db:	b8 00 00 00 00       	mov    $0x0,%eax
801077e0:	eb 10                	jmp    801077f2 <mappages+0x96>
    if(*pte & PTE_P)
      panic("remap");
    *pte = pa | perm | PTE_P;
    if(a == last)
      break;
    a += PGSIZE;
801077e2:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
    pa += PGSIZE;
801077e9:	81 45 14 00 10 00 00 	addl   $0x1000,0x14(%ebp)
  }
801077f0:	eb 8e                	jmp    80107780 <mappages+0x24>
  return 0;
}
801077f2:	c9                   	leave  
801077f3:	c3                   	ret    

801077f4 <setupkvm>:
};

// Set up kernel part of a page table.
pde_t*
setupkvm(void)
{
801077f4:	55                   	push   %ebp
801077f5:	89 e5                	mov    %esp,%ebp
801077f7:	53                   	push   %ebx
801077f8:	83 ec 34             	sub    $0x34,%esp
  pde_t *pgdir;
  struct kmap *k;

  if((pgdir = (pde_t*)kalloc()) == 0)
801077fb:	e8 e2 b2 ff ff       	call   80102ae2 <kalloc>
80107800:	89 45 f0             	mov    %eax,-0x10(%ebp)
80107803:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80107807:	75 0a                	jne    80107813 <setupkvm+0x1f>
    return 0;
80107809:	b8 00 00 00 00       	mov    $0x0,%eax
8010780e:	e9 98 00 00 00       	jmp    801078ab <setupkvm+0xb7>
  memset(pgdir, 0, PGSIZE);
80107813:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
8010781a:	00 
8010781b:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107822:	00 
80107823:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107826:	89 04 24             	mov    %eax,(%esp)
80107829:	e8 be d4 ff ff       	call   80104cec <memset>
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
8010782e:	c7 04 24 00 00 00 0e 	movl   $0xe000000,(%esp)
80107835:	e8 fd f9 ff ff       	call   80107237 <p2v>
8010783a:	3d 00 00 00 fe       	cmp    $0xfe000000,%eax
8010783f:	76 0c                	jbe    8010784d <setupkvm+0x59>
    panic("PHYSTOP too high");
80107841:	c7 04 24 e2 85 10 80 	movl   $0x801085e2,(%esp)
80107848:	e8 ed 8c ff ff       	call   8010053a <panic>
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
8010784d:	c7 45 f4 a0 b4 10 80 	movl   $0x8010b4a0,-0xc(%ebp)
80107854:	eb 49                	jmp    8010789f <setupkvm+0xab>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
80107856:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107859:	8b 48 0c             	mov    0xc(%eax),%ecx
8010785c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010785f:	8b 50 04             	mov    0x4(%eax),%edx
80107862:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107865:	8b 58 08             	mov    0x8(%eax),%ebx
80107868:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010786b:	8b 40 04             	mov    0x4(%eax),%eax
8010786e:	29 c3                	sub    %eax,%ebx
80107870:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107873:	8b 00                	mov    (%eax),%eax
80107875:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80107879:	89 54 24 0c          	mov    %edx,0xc(%esp)
8010787d:	89 5c 24 08          	mov    %ebx,0x8(%esp)
80107881:	89 44 24 04          	mov    %eax,0x4(%esp)
80107885:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107888:	89 04 24             	mov    %eax,(%esp)
8010788b:	e8 cc fe ff ff       	call   8010775c <mappages>
80107890:	85 c0                	test   %eax,%eax
80107892:	79 07                	jns    8010789b <setupkvm+0xa7>
                (uint)k->phys_start, k->perm) < 0)
      return 0;
80107894:	b8 00 00 00 00       	mov    $0x0,%eax
80107899:	eb 10                	jmp    801078ab <setupkvm+0xb7>
  if((pgdir = (pde_t*)kalloc()) == 0)
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
8010789b:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
8010789f:	81 7d f4 e0 b4 10 80 	cmpl   $0x8010b4e0,-0xc(%ebp)
801078a6:	72 ae                	jb     80107856 <setupkvm+0x62>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
                (uint)k->phys_start, k->perm) < 0)
      return 0;
  return pgdir;
801078a8:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
801078ab:	83 c4 34             	add    $0x34,%esp
801078ae:	5b                   	pop    %ebx
801078af:	5d                   	pop    %ebp
801078b0:	c3                   	ret    

801078b1 <kvmalloc>:

// Allocate one page table for the machine for the kernel address
// space for scheduler processes.
void
kvmalloc(void)
{
801078b1:	55                   	push   %ebp
801078b2:	89 e5                	mov    %esp,%ebp
801078b4:	83 ec 08             	sub    $0x8,%esp
  kpgdir = setupkvm();
801078b7:	e8 38 ff ff ff       	call   801077f4 <setupkvm>
801078bc:	a3 f8 26 11 80       	mov    %eax,0x801126f8
  switchkvm();
801078c1:	e8 02 00 00 00       	call   801078c8 <switchkvm>
}
801078c6:	c9                   	leave  
801078c7:	c3                   	ret    

801078c8 <switchkvm>:

// Switch h/w page table register to the kernel-only page table,
// for when no process is running.
void
switchkvm(void)
{
801078c8:	55                   	push   %ebp
801078c9:	89 e5                	mov    %esp,%ebp
801078cb:	83 ec 04             	sub    $0x4,%esp
  lcr3(v2p(kpgdir));   // switch to the kernel page table
801078ce:	a1 f8 26 11 80       	mov    0x801126f8,%eax
801078d3:	89 04 24             	mov    %eax,(%esp)
801078d6:	e8 4f f9 ff ff       	call   8010722a <v2p>
801078db:	89 04 24             	mov    %eax,(%esp)
801078de:	e8 3c f9 ff ff       	call   8010721f <lcr3>
}
801078e3:	c9                   	leave  
801078e4:	c3                   	ret    

801078e5 <switchuvm>:

// Switch TSS and h/w page table to correspond to process p.
void
switchuvm(struct proc *p)
{
801078e5:	55                   	push   %ebp
801078e6:	89 e5                	mov    %esp,%ebp
801078e8:	53                   	push   %ebx
801078e9:	83 ec 14             	sub    $0x14,%esp
  pushcli();
801078ec:	e8 fb d2 ff ff       	call   80104bec <pushcli>
  cpu->gdt[SEG_TSS] = SEG16(STS_T32A, &cpu->ts, sizeof(cpu->ts)-1, 0);
801078f1:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801078f7:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
801078fe:	83 c2 08             	add    $0x8,%edx
80107901:	89 d3                	mov    %edx,%ebx
80107903:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
8010790a:	83 c2 08             	add    $0x8,%edx
8010790d:	c1 ea 10             	shr    $0x10,%edx
80107910:	89 d1                	mov    %edx,%ecx
80107912:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80107919:	83 c2 08             	add    $0x8,%edx
8010791c:	c1 ea 18             	shr    $0x18,%edx
8010791f:	66 c7 80 a0 00 00 00 	movw   $0x67,0xa0(%eax)
80107926:	67 00 
80107928:	66 89 98 a2 00 00 00 	mov    %bx,0xa2(%eax)
8010792f:	88 88 a4 00 00 00    	mov    %cl,0xa4(%eax)
80107935:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
8010793c:	83 e1 f0             	and    $0xfffffff0,%ecx
8010793f:	83 c9 09             	or     $0x9,%ecx
80107942:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80107948:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
8010794f:	83 c9 10             	or     $0x10,%ecx
80107952:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80107958:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
8010795f:	83 e1 9f             	and    $0xffffff9f,%ecx
80107962:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80107968:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
8010796f:	83 c9 80             	or     $0xffffff80,%ecx
80107972:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80107978:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
8010797f:	83 e1 f0             	and    $0xfffffff0,%ecx
80107982:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80107988:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
8010798f:	83 e1 ef             	and    $0xffffffef,%ecx
80107992:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80107998:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
8010799f:	83 e1 df             	and    $0xffffffdf,%ecx
801079a2:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
801079a8:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
801079af:	83 c9 40             	or     $0x40,%ecx
801079b2:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
801079b8:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
801079bf:	83 e1 7f             	and    $0x7f,%ecx
801079c2:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
801079c8:	88 90 a7 00 00 00    	mov    %dl,0xa7(%eax)
  cpu->gdt[SEG_TSS].s = 0;
801079ce:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801079d4:	0f b6 90 a5 00 00 00 	movzbl 0xa5(%eax),%edx
801079db:	83 e2 ef             	and    $0xffffffef,%edx
801079de:	88 90 a5 00 00 00    	mov    %dl,0xa5(%eax)
  cpu->ts.ss0 = SEG_KDATA << 3;
801079e4:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801079ea:	66 c7 40 10 10 00    	movw   $0x10,0x10(%eax)
  cpu->ts.esp0 = (uint)proc->kstack + KSTACKSIZE;
801079f0:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801079f6:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
801079fd:	8b 52 08             	mov    0x8(%edx),%edx
80107a00:	81 c2 00 10 00 00    	add    $0x1000,%edx
80107a06:	89 50 0c             	mov    %edx,0xc(%eax)
  ltr(SEG_TSS << 3);
80107a09:	c7 04 24 30 00 00 00 	movl   $0x30,(%esp)
80107a10:	e8 df f7 ff ff       	call   801071f4 <ltr>
  if(p->pgdir == 0)
80107a15:	8b 45 08             	mov    0x8(%ebp),%eax
80107a18:	8b 40 04             	mov    0x4(%eax),%eax
80107a1b:	85 c0                	test   %eax,%eax
80107a1d:	75 0c                	jne    80107a2b <switchuvm+0x146>
    panic("switchuvm: no pgdir");
80107a1f:	c7 04 24 f3 85 10 80 	movl   $0x801085f3,(%esp)
80107a26:	e8 0f 8b ff ff       	call   8010053a <panic>
  lcr3(v2p(p->pgdir));  // switch to new address space
80107a2b:	8b 45 08             	mov    0x8(%ebp),%eax
80107a2e:	8b 40 04             	mov    0x4(%eax),%eax
80107a31:	89 04 24             	mov    %eax,(%esp)
80107a34:	e8 f1 f7 ff ff       	call   8010722a <v2p>
80107a39:	89 04 24             	mov    %eax,(%esp)
80107a3c:	e8 de f7 ff ff       	call   8010721f <lcr3>
  popcli();
80107a41:	e8 ea d1 ff ff       	call   80104c30 <popcli>
}
80107a46:	83 c4 14             	add    $0x14,%esp
80107a49:	5b                   	pop    %ebx
80107a4a:	5d                   	pop    %ebp
80107a4b:	c3                   	ret    

80107a4c <inituvm>:

// Load the initcode into address 0 of pgdir.
// sz must be less than a page.
void
inituvm(pde_t *pgdir, char *init, uint sz)
{
80107a4c:	55                   	push   %ebp
80107a4d:	89 e5                	mov    %esp,%ebp
80107a4f:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  
  if(sz >= PGSIZE)
80107a52:	81 7d 10 ff 0f 00 00 	cmpl   $0xfff,0x10(%ebp)
80107a59:	76 0c                	jbe    80107a67 <inituvm+0x1b>
    panic("inituvm: more than a page");
80107a5b:	c7 04 24 07 86 10 80 	movl   $0x80108607,(%esp)
80107a62:	e8 d3 8a ff ff       	call   8010053a <panic>
  mem = kalloc();
80107a67:	e8 76 b0 ff ff       	call   80102ae2 <kalloc>
80107a6c:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(mem, 0, PGSIZE);
80107a6f:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107a76:	00 
80107a77:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107a7e:	00 
80107a7f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a82:	89 04 24             	mov    %eax,(%esp)
80107a85:	e8 62 d2 ff ff       	call   80104cec <memset>
  mappages(pgdir, 0, PGSIZE, v2p(mem), PTE_W|PTE_U);
80107a8a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a8d:	89 04 24             	mov    %eax,(%esp)
80107a90:	e8 95 f7 ff ff       	call   8010722a <v2p>
80107a95:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
80107a9c:	00 
80107a9d:	89 44 24 0c          	mov    %eax,0xc(%esp)
80107aa1:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107aa8:	00 
80107aa9:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107ab0:	00 
80107ab1:	8b 45 08             	mov    0x8(%ebp),%eax
80107ab4:	89 04 24             	mov    %eax,(%esp)
80107ab7:	e8 a0 fc ff ff       	call   8010775c <mappages>
  memmove(mem, init, sz);
80107abc:	8b 45 10             	mov    0x10(%ebp),%eax
80107abf:	89 44 24 08          	mov    %eax,0x8(%esp)
80107ac3:	8b 45 0c             	mov    0xc(%ebp),%eax
80107ac6:	89 44 24 04          	mov    %eax,0x4(%esp)
80107aca:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107acd:	89 04 24             	mov    %eax,(%esp)
80107ad0:	e8 e6 d2 ff ff       	call   80104dbb <memmove>
}
80107ad5:	c9                   	leave  
80107ad6:	c3                   	ret    

80107ad7 <loaduvm>:

// Load a program segment into pgdir.  addr must be page-aligned
// and the pages from addr to addr+sz must already be mapped.
int
loaduvm(pde_t *pgdir, char *addr, struct inode *ip, uint offset, uint sz)
{
80107ad7:	55                   	push   %ebp
80107ad8:	89 e5                	mov    %esp,%ebp
80107ada:	53                   	push   %ebx
80107adb:	83 ec 24             	sub    $0x24,%esp
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
80107ade:	8b 45 0c             	mov    0xc(%ebp),%eax
80107ae1:	25 ff 0f 00 00       	and    $0xfff,%eax
80107ae6:	85 c0                	test   %eax,%eax
80107ae8:	74 0c                	je     80107af6 <loaduvm+0x1f>
    panic("loaduvm: addr must be page aligned");
80107aea:	c7 04 24 24 86 10 80 	movl   $0x80108624,(%esp)
80107af1:	e8 44 8a ff ff       	call   8010053a <panic>
  for(i = 0; i < sz; i += PGSIZE){
80107af6:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80107afd:	e9 a9 00 00 00       	jmp    80107bab <loaduvm+0xd4>
    if((pte = walkpgdir(pgdir, addr+i, 0)) == 0)
80107b02:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b05:	8b 55 0c             	mov    0xc(%ebp),%edx
80107b08:	01 d0                	add    %edx,%eax
80107b0a:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80107b11:	00 
80107b12:	89 44 24 04          	mov    %eax,0x4(%esp)
80107b16:	8b 45 08             	mov    0x8(%ebp),%eax
80107b19:	89 04 24             	mov    %eax,(%esp)
80107b1c:	e8 99 fb ff ff       	call   801076ba <walkpgdir>
80107b21:	89 45 ec             	mov    %eax,-0x14(%ebp)
80107b24:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80107b28:	75 0c                	jne    80107b36 <loaduvm+0x5f>
      panic("loaduvm: address should exist");
80107b2a:	c7 04 24 47 86 10 80 	movl   $0x80108647,(%esp)
80107b31:	e8 04 8a ff ff       	call   8010053a <panic>
    pa = PTE_ADDR(*pte);
80107b36:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107b39:	8b 00                	mov    (%eax),%eax
80107b3b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107b40:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(sz - i < PGSIZE)
80107b43:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b46:	8b 55 18             	mov    0x18(%ebp),%edx
80107b49:	29 c2                	sub    %eax,%edx
80107b4b:	89 d0                	mov    %edx,%eax
80107b4d:	3d ff 0f 00 00       	cmp    $0xfff,%eax
80107b52:	77 0f                	ja     80107b63 <loaduvm+0x8c>
      n = sz - i;
80107b54:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b57:	8b 55 18             	mov    0x18(%ebp),%edx
80107b5a:	29 c2                	sub    %eax,%edx
80107b5c:	89 d0                	mov    %edx,%eax
80107b5e:	89 45 f0             	mov    %eax,-0x10(%ebp)
80107b61:	eb 07                	jmp    80107b6a <loaduvm+0x93>
    else
      n = PGSIZE;
80107b63:	c7 45 f0 00 10 00 00 	movl   $0x1000,-0x10(%ebp)
    if(readi(ip, p2v(pa), offset+i, n) != n)
80107b6a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b6d:	8b 55 14             	mov    0x14(%ebp),%edx
80107b70:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
80107b73:	8b 45 e8             	mov    -0x18(%ebp),%eax
80107b76:	89 04 24             	mov    %eax,(%esp)
80107b79:	e8 b9 f6 ff ff       	call   80107237 <p2v>
80107b7e:	8b 55 f0             	mov    -0x10(%ebp),%edx
80107b81:	89 54 24 0c          	mov    %edx,0xc(%esp)
80107b85:	89 5c 24 08          	mov    %ebx,0x8(%esp)
80107b89:	89 44 24 04          	mov    %eax,0x4(%esp)
80107b8d:	8b 45 10             	mov    0x10(%ebp),%eax
80107b90:	89 04 24             	mov    %eax,(%esp)
80107b93:	e8 d0 a1 ff ff       	call   80101d68 <readi>
80107b98:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80107b9b:	74 07                	je     80107ba4 <loaduvm+0xcd>
      return -1;
80107b9d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107ba2:	eb 18                	jmp    80107bbc <loaduvm+0xe5>
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
    panic("loaduvm: addr must be page aligned");
  for(i = 0; i < sz; i += PGSIZE){
80107ba4:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80107bab:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bae:	3b 45 18             	cmp    0x18(%ebp),%eax
80107bb1:	0f 82 4b ff ff ff    	jb     80107b02 <loaduvm+0x2b>
    else
      n = PGSIZE;
    if(readi(ip, p2v(pa), offset+i, n) != n)
      return -1;
  }
  return 0;
80107bb7:	b8 00 00 00 00       	mov    $0x0,%eax
}
80107bbc:	83 c4 24             	add    $0x24,%esp
80107bbf:	5b                   	pop    %ebx
80107bc0:	5d                   	pop    %ebp
80107bc1:	c3                   	ret    

80107bc2 <allocuvm>:

// Allocate page tables and physical memory to grow process from oldsz to
// newsz, which need not be page aligned.  Returns new size or 0 on error.
int
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
80107bc2:	55                   	push   %ebp
80107bc3:	89 e5                	mov    %esp,%ebp
80107bc5:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  uint a;

  if(newsz >= KERNBASE)
80107bc8:	8b 45 10             	mov    0x10(%ebp),%eax
80107bcb:	85 c0                	test   %eax,%eax
80107bcd:	79 0a                	jns    80107bd9 <allocuvm+0x17>
    return 0;
80107bcf:	b8 00 00 00 00       	mov    $0x0,%eax
80107bd4:	e9 c1 00 00 00       	jmp    80107c9a <allocuvm+0xd8>
  if(newsz < oldsz)
80107bd9:	8b 45 10             	mov    0x10(%ebp),%eax
80107bdc:	3b 45 0c             	cmp    0xc(%ebp),%eax
80107bdf:	73 08                	jae    80107be9 <allocuvm+0x27>
    return oldsz;
80107be1:	8b 45 0c             	mov    0xc(%ebp),%eax
80107be4:	e9 b1 00 00 00       	jmp    80107c9a <allocuvm+0xd8>

  a = PGROUNDUP(oldsz);
80107be9:	8b 45 0c             	mov    0xc(%ebp),%eax
80107bec:	05 ff 0f 00 00       	add    $0xfff,%eax
80107bf1:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107bf6:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a < newsz; a += PGSIZE){
80107bf9:	e9 8d 00 00 00       	jmp    80107c8b <allocuvm+0xc9>
    mem = kalloc();
80107bfe:	e8 df ae ff ff       	call   80102ae2 <kalloc>
80107c03:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(mem == 0){
80107c06:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80107c0a:	75 2c                	jne    80107c38 <allocuvm+0x76>
      cprintf("allocuvm out of memory\n");
80107c0c:	c7 04 24 65 86 10 80 	movl   $0x80108665,(%esp)
80107c13:	e8 88 87 ff ff       	call   801003a0 <cprintf>
      deallocuvm(pgdir, newsz, oldsz);
80107c18:	8b 45 0c             	mov    0xc(%ebp),%eax
80107c1b:	89 44 24 08          	mov    %eax,0x8(%esp)
80107c1f:	8b 45 10             	mov    0x10(%ebp),%eax
80107c22:	89 44 24 04          	mov    %eax,0x4(%esp)
80107c26:	8b 45 08             	mov    0x8(%ebp),%eax
80107c29:	89 04 24             	mov    %eax,(%esp)
80107c2c:	e8 6b 00 00 00       	call   80107c9c <deallocuvm>
      return 0;
80107c31:	b8 00 00 00 00       	mov    $0x0,%eax
80107c36:	eb 62                	jmp    80107c9a <allocuvm+0xd8>
    }
    memset(mem, 0, PGSIZE);
80107c38:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107c3f:	00 
80107c40:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107c47:	00 
80107c48:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107c4b:	89 04 24             	mov    %eax,(%esp)
80107c4e:	e8 99 d0 ff ff       	call   80104cec <memset>
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
80107c53:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107c56:	89 04 24             	mov    %eax,(%esp)
80107c59:	e8 cc f5 ff ff       	call   8010722a <v2p>
80107c5e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80107c61:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
80107c68:	00 
80107c69:	89 44 24 0c          	mov    %eax,0xc(%esp)
80107c6d:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107c74:	00 
80107c75:	89 54 24 04          	mov    %edx,0x4(%esp)
80107c79:	8b 45 08             	mov    0x8(%ebp),%eax
80107c7c:	89 04 24             	mov    %eax,(%esp)
80107c7f:	e8 d8 fa ff ff       	call   8010775c <mappages>
    return 0;
  if(newsz < oldsz)
    return oldsz;

  a = PGROUNDUP(oldsz);
  for(; a < newsz; a += PGSIZE){
80107c84:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80107c8b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c8e:	3b 45 10             	cmp    0x10(%ebp),%eax
80107c91:	0f 82 67 ff ff ff    	jb     80107bfe <allocuvm+0x3c>
      return 0;
    }
    memset(mem, 0, PGSIZE);
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
  }
  return newsz;
80107c97:	8b 45 10             	mov    0x10(%ebp),%eax
}
80107c9a:	c9                   	leave  
80107c9b:	c3                   	ret    

80107c9c <deallocuvm>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
int
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
80107c9c:	55                   	push   %ebp
80107c9d:	89 e5                	mov    %esp,%ebp
80107c9f:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;
  uint a, pa;

  if(newsz >= oldsz)
80107ca2:	8b 45 10             	mov    0x10(%ebp),%eax
80107ca5:	3b 45 0c             	cmp    0xc(%ebp),%eax
80107ca8:	72 08                	jb     80107cb2 <deallocuvm+0x16>
    return oldsz;
80107caa:	8b 45 0c             	mov    0xc(%ebp),%eax
80107cad:	e9 a4 00 00 00       	jmp    80107d56 <deallocuvm+0xba>

  a = PGROUNDUP(newsz);
80107cb2:	8b 45 10             	mov    0x10(%ebp),%eax
80107cb5:	05 ff 0f 00 00       	add    $0xfff,%eax
80107cba:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107cbf:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a  < oldsz; a += PGSIZE){
80107cc2:	e9 80 00 00 00       	jmp    80107d47 <deallocuvm+0xab>
    pte = walkpgdir(pgdir, (char*)a, 0);
80107cc7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cca:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80107cd1:	00 
80107cd2:	89 44 24 04          	mov    %eax,0x4(%esp)
80107cd6:	8b 45 08             	mov    0x8(%ebp),%eax
80107cd9:	89 04 24             	mov    %eax,(%esp)
80107cdc:	e8 d9 f9 ff ff       	call   801076ba <walkpgdir>
80107ce1:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(!pte)
80107ce4:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80107ce8:	75 09                	jne    80107cf3 <deallocuvm+0x57>
      a += (NPTENTRIES - 1) * PGSIZE;
80107cea:	81 45 f4 00 f0 3f 00 	addl   $0x3ff000,-0xc(%ebp)
80107cf1:	eb 4d                	jmp    80107d40 <deallocuvm+0xa4>
    else if((*pte & PTE_P) != 0){
80107cf3:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107cf6:	8b 00                	mov    (%eax),%eax
80107cf8:	83 e0 01             	and    $0x1,%eax
80107cfb:	85 c0                	test   %eax,%eax
80107cfd:	74 41                	je     80107d40 <deallocuvm+0xa4>
      pa = PTE_ADDR(*pte);
80107cff:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107d02:	8b 00                	mov    (%eax),%eax
80107d04:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107d09:	89 45 ec             	mov    %eax,-0x14(%ebp)
      if(pa == 0)
80107d0c:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80107d10:	75 0c                	jne    80107d1e <deallocuvm+0x82>
        panic("kfree");
80107d12:	c7 04 24 7d 86 10 80 	movl   $0x8010867d,(%esp)
80107d19:	e8 1c 88 ff ff       	call   8010053a <panic>
      char *v = p2v(pa);
80107d1e:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107d21:	89 04 24             	mov    %eax,(%esp)
80107d24:	e8 0e f5 ff ff       	call   80107237 <p2v>
80107d29:	89 45 e8             	mov    %eax,-0x18(%ebp)
      kfree(v);
80107d2c:	8b 45 e8             	mov    -0x18(%ebp),%eax
80107d2f:	89 04 24             	mov    %eax,(%esp)
80107d32:	e8 12 ad ff ff       	call   80102a49 <kfree>
      *pte = 0;
80107d37:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107d3a:	c7 00 00 00 00 00    	movl   $0x0,(%eax)

  if(newsz >= oldsz)
    return oldsz;

  a = PGROUNDUP(newsz);
  for(; a  < oldsz; a += PGSIZE){
80107d40:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80107d47:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d4a:	3b 45 0c             	cmp    0xc(%ebp),%eax
80107d4d:	0f 82 74 ff ff ff    	jb     80107cc7 <deallocuvm+0x2b>
      char *v = p2v(pa);
      kfree(v);
      *pte = 0;
    }
  }
  return newsz;
80107d53:	8b 45 10             	mov    0x10(%ebp),%eax
}
80107d56:	c9                   	leave  
80107d57:	c3                   	ret    

80107d58 <freevm>:

// Free a page table and all the physical memory pages
// in the user part.
void
freevm(pde_t *pgdir)
{
80107d58:	55                   	push   %ebp
80107d59:	89 e5                	mov    %esp,%ebp
80107d5b:	83 ec 28             	sub    $0x28,%esp
  uint i;

  if(pgdir == 0)
80107d5e:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80107d62:	75 0c                	jne    80107d70 <freevm+0x18>
    panic("freevm: no pgdir");
80107d64:	c7 04 24 83 86 10 80 	movl   $0x80108683,(%esp)
80107d6b:	e8 ca 87 ff ff       	call   8010053a <panic>
  deallocuvm(pgdir, KERNBASE, 0);
80107d70:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80107d77:	00 
80107d78:	c7 44 24 04 00 00 00 	movl   $0x80000000,0x4(%esp)
80107d7f:	80 
80107d80:	8b 45 08             	mov    0x8(%ebp),%eax
80107d83:	89 04 24             	mov    %eax,(%esp)
80107d86:	e8 11 ff ff ff       	call   80107c9c <deallocuvm>
  for(i = 0; i < NPDENTRIES; i++){
80107d8b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80107d92:	eb 48                	jmp    80107ddc <freevm+0x84>
    if(pgdir[i] & PTE_P){
80107d94:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d97:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80107d9e:	8b 45 08             	mov    0x8(%ebp),%eax
80107da1:	01 d0                	add    %edx,%eax
80107da3:	8b 00                	mov    (%eax),%eax
80107da5:	83 e0 01             	and    $0x1,%eax
80107da8:	85 c0                	test   %eax,%eax
80107daa:	74 2c                	je     80107dd8 <freevm+0x80>
      char * v = p2v(PTE_ADDR(pgdir[i]));
80107dac:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107daf:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80107db6:	8b 45 08             	mov    0x8(%ebp),%eax
80107db9:	01 d0                	add    %edx,%eax
80107dbb:	8b 00                	mov    (%eax),%eax
80107dbd:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107dc2:	89 04 24             	mov    %eax,(%esp)
80107dc5:	e8 6d f4 ff ff       	call   80107237 <p2v>
80107dca:	89 45 f0             	mov    %eax,-0x10(%ebp)
      kfree(v);
80107dcd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107dd0:	89 04 24             	mov    %eax,(%esp)
80107dd3:	e8 71 ac ff ff       	call   80102a49 <kfree>
  uint i;

  if(pgdir == 0)
    panic("freevm: no pgdir");
  deallocuvm(pgdir, KERNBASE, 0);
  for(i = 0; i < NPDENTRIES; i++){
80107dd8:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80107ddc:	81 7d f4 ff 03 00 00 	cmpl   $0x3ff,-0xc(%ebp)
80107de3:	76 af                	jbe    80107d94 <freevm+0x3c>
    if(pgdir[i] & PTE_P){
      char * v = p2v(PTE_ADDR(pgdir[i]));
      kfree(v);
    }
  }
  kfree((char*)pgdir);
80107de5:	8b 45 08             	mov    0x8(%ebp),%eax
80107de8:	89 04 24             	mov    %eax,(%esp)
80107deb:	e8 59 ac ff ff       	call   80102a49 <kfree>
}
80107df0:	c9                   	leave  
80107df1:	c3                   	ret    

80107df2 <clearpteu>:

// Clear PTE_U on a page. Used to create an inaccessible
// page beneath the user stack.
void
clearpteu(pde_t *pgdir, char *uva)
{
80107df2:	55                   	push   %ebp
80107df3:	89 e5                	mov    %esp,%ebp
80107df5:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
80107df8:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80107dff:	00 
80107e00:	8b 45 0c             	mov    0xc(%ebp),%eax
80107e03:	89 44 24 04          	mov    %eax,0x4(%esp)
80107e07:	8b 45 08             	mov    0x8(%ebp),%eax
80107e0a:	89 04 24             	mov    %eax,(%esp)
80107e0d:	e8 a8 f8 ff ff       	call   801076ba <walkpgdir>
80107e12:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(pte == 0)
80107e15:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80107e19:	75 0c                	jne    80107e27 <clearpteu+0x35>
    panic("clearpteu");
80107e1b:	c7 04 24 94 86 10 80 	movl   $0x80108694,(%esp)
80107e22:	e8 13 87 ff ff       	call   8010053a <panic>
  *pte &= ~PTE_U;
80107e27:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e2a:	8b 00                	mov    (%eax),%eax
80107e2c:	83 e0 fb             	and    $0xfffffffb,%eax
80107e2f:	89 c2                	mov    %eax,%edx
80107e31:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e34:	89 10                	mov    %edx,(%eax)
}
80107e36:	c9                   	leave  
80107e37:	c3                   	ret    

80107e38 <copyuvm>:

// Given a parent process's page table, create a copy
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint sz)
{
80107e38:	55                   	push   %ebp
80107e39:	89 e5                	mov    %esp,%ebp
80107e3b:	53                   	push   %ebx
80107e3c:	83 ec 44             	sub    $0x44,%esp
  pde_t *d;
  pte_t *pte;
  uint pa, i, flags;
  char *mem;

  if((d = setupkvm()) == 0)
80107e3f:	e8 b0 f9 ff ff       	call   801077f4 <setupkvm>
80107e44:	89 45 f0             	mov    %eax,-0x10(%ebp)
80107e47:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80107e4b:	75 0a                	jne    80107e57 <copyuvm+0x1f>
    return 0;
80107e4d:	b8 00 00 00 00       	mov    $0x0,%eax
80107e52:	e9 fd 00 00 00       	jmp    80107f54 <copyuvm+0x11c>
  for(i = 0; i < sz; i += PGSIZE){
80107e57:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80107e5e:	e9 d0 00 00 00       	jmp    80107f33 <copyuvm+0xfb>
    if((pte = walkpgdir(pgdir, (void *) i, 0)) == 0)
80107e63:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e66:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80107e6d:	00 
80107e6e:	89 44 24 04          	mov    %eax,0x4(%esp)
80107e72:	8b 45 08             	mov    0x8(%ebp),%eax
80107e75:	89 04 24             	mov    %eax,(%esp)
80107e78:	e8 3d f8 ff ff       	call   801076ba <walkpgdir>
80107e7d:	89 45 ec             	mov    %eax,-0x14(%ebp)
80107e80:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80107e84:	75 0c                	jne    80107e92 <copyuvm+0x5a>
      panic("copyuvm: pte should exist");
80107e86:	c7 04 24 9e 86 10 80 	movl   $0x8010869e,(%esp)
80107e8d:	e8 a8 86 ff ff       	call   8010053a <panic>
    if(!(*pte & PTE_P))
80107e92:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107e95:	8b 00                	mov    (%eax),%eax
80107e97:	83 e0 01             	and    $0x1,%eax
80107e9a:	85 c0                	test   %eax,%eax
80107e9c:	75 0c                	jne    80107eaa <copyuvm+0x72>
      panic("copyuvm: page not present");
80107e9e:	c7 04 24 b8 86 10 80 	movl   $0x801086b8,(%esp)
80107ea5:	e8 90 86 ff ff       	call   8010053a <panic>
    pa = PTE_ADDR(*pte);
80107eaa:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107ead:	8b 00                	mov    (%eax),%eax
80107eaf:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107eb4:	89 45 e8             	mov    %eax,-0x18(%ebp)
    flags = PTE_FLAGS(*pte);
80107eb7:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107eba:	8b 00                	mov    (%eax),%eax
80107ebc:	25 ff 0f 00 00       	and    $0xfff,%eax
80107ec1:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if((mem = kalloc()) == 0)
80107ec4:	e8 19 ac ff ff       	call   80102ae2 <kalloc>
80107ec9:	89 45 e0             	mov    %eax,-0x20(%ebp)
80107ecc:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80107ed0:	75 02                	jne    80107ed4 <copyuvm+0x9c>
      goto bad;
80107ed2:	eb 70                	jmp    80107f44 <copyuvm+0x10c>
    memmove(mem, (char*)p2v(pa), PGSIZE);
80107ed4:	8b 45 e8             	mov    -0x18(%ebp),%eax
80107ed7:	89 04 24             	mov    %eax,(%esp)
80107eda:	e8 58 f3 ff ff       	call   80107237 <p2v>
80107edf:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107ee6:	00 
80107ee7:	89 44 24 04          	mov    %eax,0x4(%esp)
80107eeb:	8b 45 e0             	mov    -0x20(%ebp),%eax
80107eee:	89 04 24             	mov    %eax,(%esp)
80107ef1:	e8 c5 ce ff ff       	call   80104dbb <memmove>
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), flags) < 0)
80107ef6:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
80107ef9:	8b 45 e0             	mov    -0x20(%ebp),%eax
80107efc:	89 04 24             	mov    %eax,(%esp)
80107eff:	e8 26 f3 ff ff       	call   8010722a <v2p>
80107f04:	8b 55 f4             	mov    -0xc(%ebp),%edx
80107f07:	89 5c 24 10          	mov    %ebx,0x10(%esp)
80107f0b:	89 44 24 0c          	mov    %eax,0xc(%esp)
80107f0f:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107f16:	00 
80107f17:	89 54 24 04          	mov    %edx,0x4(%esp)
80107f1b:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107f1e:	89 04 24             	mov    %eax,(%esp)
80107f21:	e8 36 f8 ff ff       	call   8010775c <mappages>
80107f26:	85 c0                	test   %eax,%eax
80107f28:	79 02                	jns    80107f2c <copyuvm+0xf4>
      goto bad;
80107f2a:	eb 18                	jmp    80107f44 <copyuvm+0x10c>
  uint pa, i, flags;
  char *mem;

  if((d = setupkvm()) == 0)
    return 0;
  for(i = 0; i < sz; i += PGSIZE){
80107f2c:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80107f33:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f36:	3b 45 0c             	cmp    0xc(%ebp),%eax
80107f39:	0f 82 24 ff ff ff    	jb     80107e63 <copyuvm+0x2b>
      goto bad;
    memmove(mem, (char*)p2v(pa), PGSIZE);
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), flags) < 0)
      goto bad;
  }
  return d;
80107f3f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107f42:	eb 10                	jmp    80107f54 <copyuvm+0x11c>

bad:
  freevm(d);
80107f44:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107f47:	89 04 24             	mov    %eax,(%esp)
80107f4a:	e8 09 fe ff ff       	call   80107d58 <freevm>
  return 0;
80107f4f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80107f54:	83 c4 44             	add    $0x44,%esp
80107f57:	5b                   	pop    %ebx
80107f58:	5d                   	pop    %ebp
80107f59:	c3                   	ret    

80107f5a <uva2ka>:

//PAGEBREAK!
// Map user virtual address to kernel address.
char*
uva2ka(pde_t *pgdir, char *uva)
{
80107f5a:	55                   	push   %ebp
80107f5b:	89 e5                	mov    %esp,%ebp
80107f5d:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
80107f60:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80107f67:	00 
80107f68:	8b 45 0c             	mov    0xc(%ebp),%eax
80107f6b:	89 44 24 04          	mov    %eax,0x4(%esp)
80107f6f:	8b 45 08             	mov    0x8(%ebp),%eax
80107f72:	89 04 24             	mov    %eax,(%esp)
80107f75:	e8 40 f7 ff ff       	call   801076ba <walkpgdir>
80107f7a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((*pte & PTE_P) == 0)
80107f7d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f80:	8b 00                	mov    (%eax),%eax
80107f82:	83 e0 01             	and    $0x1,%eax
80107f85:	85 c0                	test   %eax,%eax
80107f87:	75 07                	jne    80107f90 <uva2ka+0x36>
    return 0;
80107f89:	b8 00 00 00 00       	mov    $0x0,%eax
80107f8e:	eb 25                	jmp    80107fb5 <uva2ka+0x5b>
  if((*pte & PTE_U) == 0)
80107f90:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f93:	8b 00                	mov    (%eax),%eax
80107f95:	83 e0 04             	and    $0x4,%eax
80107f98:	85 c0                	test   %eax,%eax
80107f9a:	75 07                	jne    80107fa3 <uva2ka+0x49>
    return 0;
80107f9c:	b8 00 00 00 00       	mov    $0x0,%eax
80107fa1:	eb 12                	jmp    80107fb5 <uva2ka+0x5b>
  return (char*)p2v(PTE_ADDR(*pte));
80107fa3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107fa6:	8b 00                	mov    (%eax),%eax
80107fa8:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107fad:	89 04 24             	mov    %eax,(%esp)
80107fb0:	e8 82 f2 ff ff       	call   80107237 <p2v>
}
80107fb5:	c9                   	leave  
80107fb6:	c3                   	ret    

80107fb7 <copyout>:
// Copy len bytes from p to user address va in page table pgdir.
// Most useful when pgdir is not the current page table.
// uva2ka ensures this only works for PTE_U pages.
int
copyout(pde_t *pgdir, uint va, void *p, uint len)
{
80107fb7:	55                   	push   %ebp
80107fb8:	89 e5                	mov    %esp,%ebp
80107fba:	83 ec 28             	sub    $0x28,%esp
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
80107fbd:	8b 45 10             	mov    0x10(%ebp),%eax
80107fc0:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(len > 0){
80107fc3:	e9 87 00 00 00       	jmp    8010804f <copyout+0x98>
    va0 = (uint)PGROUNDDOWN(va);
80107fc8:	8b 45 0c             	mov    0xc(%ebp),%eax
80107fcb:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107fd0:	89 45 ec             	mov    %eax,-0x14(%ebp)
    pa0 = uva2ka(pgdir, (char*)va0);
80107fd3:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107fd6:	89 44 24 04          	mov    %eax,0x4(%esp)
80107fda:	8b 45 08             	mov    0x8(%ebp),%eax
80107fdd:	89 04 24             	mov    %eax,(%esp)
80107fe0:	e8 75 ff ff ff       	call   80107f5a <uva2ka>
80107fe5:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(pa0 == 0)
80107fe8:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80107fec:	75 07                	jne    80107ff5 <copyout+0x3e>
      return -1;
80107fee:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107ff3:	eb 69                	jmp    8010805e <copyout+0xa7>
    n = PGSIZE - (va - va0);
80107ff5:	8b 45 0c             	mov    0xc(%ebp),%eax
80107ff8:	8b 55 ec             	mov    -0x14(%ebp),%edx
80107ffb:	29 c2                	sub    %eax,%edx
80107ffd:	89 d0                	mov    %edx,%eax
80107fff:	05 00 10 00 00       	add    $0x1000,%eax
80108004:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(n > len)
80108007:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010800a:	3b 45 14             	cmp    0x14(%ebp),%eax
8010800d:	76 06                	jbe    80108015 <copyout+0x5e>
      n = len;
8010800f:	8b 45 14             	mov    0x14(%ebp),%eax
80108012:	89 45 f0             	mov    %eax,-0x10(%ebp)
    memmove(pa0 + (va - va0), buf, n);
80108015:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108018:	8b 55 0c             	mov    0xc(%ebp),%edx
8010801b:	29 c2                	sub    %eax,%edx
8010801d:	8b 45 e8             	mov    -0x18(%ebp),%eax
80108020:	01 c2                	add    %eax,%edx
80108022:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108025:	89 44 24 08          	mov    %eax,0x8(%esp)
80108029:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010802c:	89 44 24 04          	mov    %eax,0x4(%esp)
80108030:	89 14 24             	mov    %edx,(%esp)
80108033:	e8 83 cd ff ff       	call   80104dbb <memmove>
    len -= n;
80108038:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010803b:	29 45 14             	sub    %eax,0x14(%ebp)
    buf += n;
8010803e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108041:	01 45 f4             	add    %eax,-0xc(%ebp)
    va = va0 + PGSIZE;
80108044:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108047:	05 00 10 00 00       	add    $0x1000,%eax
8010804c:	89 45 0c             	mov    %eax,0xc(%ebp)
{
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
  while(len > 0){
8010804f:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
80108053:	0f 85 6f ff ff ff    	jne    80107fc8 <copyout+0x11>
    memmove(pa0 + (va - va0), buf, n);
    len -= n;
    buf += n;
    va = va0 + PGSIZE;
  }
  return 0;
80108059:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010805e:	c9                   	leave  
8010805f:	c3                   	ret    
